# The lean4lean-model bridge: Carneiro's hypothesis implies Lech's
...
Lech's `SetTheory` interface (`Lech/SetTheory/Core.lean`).

It is a **separate package on purpose**: the Lech libraries never
...
pinned to the matching tag (`v4.33.0`).  `Lech.SetTheory.Core` is built
from `../..` by the path `require`.  Everything lands in this
directory's `.lake` (gitignored).

## Contents

`LechBridge/Carneiro.lean`:

* `LechBridge.OmegaInaccessibles` — **Carneiro's hypothesis, verbatim**
...
* `isTGUniverse_vonNeumann : κ.IsInaccessible → Lech.IsTGUniverse (· ∈ ·) (V_ κ.ord)`
...
* `setTheoryOfChain κ hmono hinacc : Lech.SetTheory ZFSet.{u}` — the
...
* `carneiro_implies_lech : OmegaInaccessibles.{u} → Nonempty (Σ V : Type (u + 1), Lech.SetTheory V)`.
...
`U = V_κ`.  Applied to Lech's chain, `univChain (n + 2)` is `V_{κ_n}` for
