...
#
# WHAT IT SCANS.  Every `*.lean` in `Setlec/`, `tests/`, `scripts/` and
# the four top-level roots (`Main`, `Setlec`, `SetlecPreprocess`,
...
# are never part of setlec's own build.
#
...
#
#   Setlec/Kernel/Expr.lean          unsafe, ptrAddrUnsafe,
...
#       and the census that argues them is `Setlec/Cached/ExprC.lean`
...
#       above, which is why `Setlec/Kernel/Level.lean` needs no entry.
...
# `Setlec/SetTheory/Derive/*`.  Twenty `@[implemented_by …] … unsafeCast
...
ROOTS = ('Main.lean', 'Setlec.lean', 'SetlecPreprocess.lean', 'PinDump.lean')

def sources():
    out = []
    for top in ('Setlec', 'tests', 'scripts'):
