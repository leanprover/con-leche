...
|---|---|
| `Setlec.no_proof_of_False` | **THE MAIN THEOREM**: an accepted stream yields no constant of type `False` |
...
two-loop ruling): anything about the `SETLEC_PROGRESS` lane.  That lane
...
/-! ## The main theorem (`Setlec/MainTheorem.lean`)
