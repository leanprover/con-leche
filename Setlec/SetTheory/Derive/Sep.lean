import Setlec.SetTheory.Derive.Empty
...
of it).  cf. nanodatg `derived/src/separation.rs`, which needs the same
detour because replacement's antecedent demands totality.
-/
...
noncomputable def sep (a : V) (p : V → Prop) : V :=
  if h : ∃ x, x ∈ᵗ a ∧ p x then
    image (fun x => if p x then x else Classical.choose h) a
  else empty

theorem mem_sep {a z : V} {p : V → Prop} : z ∈ᵗ sep a p ↔ z ∈ᵗ a ∧ p z := by
  unfold sep
  split
  · next h =>
...
theorem sep_subset {a : V} {p : V → Prop} : sep a p ⊆ᵗ a :=
  fun _ hz => (mem_sep.mp hz).1

theorem sep_congr {a : V} {p q : V → Prop} (h : ∀ x, x ∈ᵗ a → (p x ↔ q x)) :
    sep a p = sep a q :=
  ext fun z => by
    rw [mem_sep, mem_sep]
...
theorem image_congr {a : V} {f g : V → V} (h : ∀ x, x ∈ᵗ a → f x = g x) :
    image f a = image g a :=
  ext fun z => by
