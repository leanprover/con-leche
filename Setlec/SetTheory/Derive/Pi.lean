...
`Instance.lean`.  Laws whose target mentions `univ v` only in the
`v = 0` case take that case's premise as `v = 0 → … ∈ᵗ univZero`.
-/
...
noncomputable def pi (v : Nat) (A : V) (B : V → V) : V :=
  if v = 0 then truthVal (∀ x, x ∈ᵗ A → ∃ y, y ∈ᵗ B x) else piSet A B
...
theorem pi_zero {A : V} {B : V → V} :
    pi 0 A B = truthVal (∀ x, x ∈ᵗ A → ∃ y, y ∈ᵗ B x) := by
...
theorem pi_congr {v : Nat} {A : V} {B B' : V → V}
    (h : ∀ x, x ∈ᵗ A → B x = B' x) : pi v A B = pi v A B' := by
...
theorem lam_congr {v : Nat} {A : V} {F F' : V → V}
    (h : ∀ x, x ∈ᵗ A → F x = F' x) : lam v A F = lam v A F' := by
  rcases Nat.eq_zero_or_pos v with rfl | hv
...
theorem lam_mem {v : Nat} {A : V} {F B : V → V}
    (hF : ∀ x, x ∈ᵗ A → F x ∈ᵗ B x) : lam v A F ∈ᵗ pi v A B := by
  rcases Nat.eq_zero_or_pos v with rfl | hv
  · rw [lam_zero, pi_zero]
...
theorem mem_pi_zero {A f : V} {B : V → V} (hf : f ∈ᵗ pi 0 A B) : f = pt := by
  rw [pi_zero] at hf
...
theorem app_mem {v : Nat} {A f a : V} {B : V → V}
    (hf : f ∈ᵗ pi v A B) (ha : a ∈ᵗ A)
    (hB0 : v = 0 → ∀ x, x ∈ᵗ A → B x ∈ᵗ (univZero : V)) : app f a ∈ᵗ B a := by
  rcases Nat.eq_zero_or_pos v with rfl | hv
...
theorem app_lam {v : Nat} {A a : V} {F B : V → V}
    (ha : a ∈ᵗ A) (hF : ∀ x, x ∈ᵗ A → F x ∈ᵗ B x)
    (hB0 : v = 0 → ∀ x, x ∈ᵗ A → B x ∈ᵗ (univZero : V)) :
...
theorem lam_dom {v' v : Nat} {A A' : V} {F : V → V} {B : V → V}
    (hf : lam v' A F ∈ᵗ pi v A' B) (hv : v ≠ 0) (hv' : v' ≠ 0) :
    ∀ x, x ∈ᵗ A' → x ∈ᵗ A := by
...
theorem lam_eta {v : Nat} {A f : V} {B : V → V} (hf : f ∈ᵗ pi v A B) :
...
theorem _root_.Setlec.IsTGUniverse.pi_mem {U A : V} {B : V → V} {v : Nat}
    (hU : IsTGUniverse (Mem (V := V)) U) (hv : v ≠ 0) (hA : A ∈ᵗ U)
    (hB : ∀ x, x ∈ᵗ A → B x ∈ᵗ U) : pi v A B ∈ᵗ U := by
  rw [pi_pos hv]
