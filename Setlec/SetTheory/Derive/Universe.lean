...
theorem _root_.Setlec.IsTGUniverse.transitive : ∀ {y z : V}, y ∈ˢ U → z ∈ˢ y → z ∈ˢ U :=
  fun {y z} hy hz => hU.1 y z hy hz

theorem _root_.Setlec.IsTGUniverse.mem_of_subset_mem : ∀ {y z : V}, y ∈ˢ U → z ⊆ˢ y → z ∈ˢ U :=
  fun {y z} hy hz => hU.2.1 y z hy hz

theorem _root_.Setlec.IsTGUniverse.subset_of_mem {y : V} (hy : y ∈ˢ U) : y ⊆ˢ U :=
  fun _ hz => hU.transitive hy hz

theorem _root_.Setlec.IsTGUniverse.power_mem {y : V} (hy : y ∈ˢ U) : power y ∈ˢ U := by
...
theorem _root_.Setlec.IsTGUniverse.empty_mem {y : V} (hy : y ∈ˢ U) : (empty : V) ∈ˢ U :=
  hU.mem_of_subset_mem hy (empty_subset y)

open Classical in
/-- Pairing closure, via `covered_mem` from the two-element member
`power (power ∅) = {∅, {∅}}`. -/
theorem _root_.Setlec.IsTGUniverse.upair_mem {a b y : V} (hy : y ∈ˢ U)
...
theorem _root_.Setlec.IsTGUniverse.sing_mem {a y : V} (hy : y ∈ˢ U) (ha : a ∈ˢ U) :
...
2-cycle. -/
theorem _root_.Setlec.IsTGUniverse.sUnion_mem {s : V} (hs : s ∈ˢ U) : sUnion s ∈ˢ U := by
...
theorem _root_.Setlec.IsTGUniverse.binUnion_mem {a b y : V} (hy : y ∈ˢ U)
    (ha : a ∈ˢ U) (hb : b ∈ˢ U) : binUnion a b ∈ˢ U :=
  hU.sUnion_mem (hU.upair_mem hy ha hb)

theorem _root_.Setlec.IsTGUniverse.kpair_mem {a b y : V} (hy : y ∈ˢ U)
    (ha : a ∈ˢ U) (hb : b ∈ˢ U) : kpair a b ∈ˢ U :=
  hU.upair_mem hy (hU.sing_mem hy ha) (hU.upair_mem hy ha hb)

theorem _root_.Setlec.IsTGUniverse.sep_mem {a : V} {p : V → Prop} (ha : a ∈ˢ U) :
    sep a p ∈ˢ U :=
