...
minimal `SetTheory` class** (`Setlec/SetTheory/Core.lean`) on the
...
* **Replacement** (`image`) is validated for **arbitrary Lean
  ...
  (see the `Setlec/SetTheory/Core.lean` module doc).
...
it; it is only reachable from the root `Setlec.lean` so that it builds
...
/--
info: 'Setlec.SetTheory.ofAczelChain' depends on axioms: [propext, Classical.choice, Quot.sound]
-/
#guard_msgs in
