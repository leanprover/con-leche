...
/-!
# `Setlec.Semantics` — the Expr-facing semantic tier

**Split from `Setlec/SetBase/*` on 2026-09-06** (cleanup pass A, user
...
the unit-terminated tuple tower — is `Setlec/SetModel/*`, the
namespace `Setlec.SetModel`, and mentions neither `Expr` nor the
...
(`Tower/*`).  The namespace is `Setlec.Semantics` (formerly
`Setlec.SetR.Interp2` and `Setlec.SetR`, both retired with the
collapsed lane they were named for).  The layering rule is unchanged
in substance: `Setlec/{Kernel,Cached,Frontend}/*` and `Main.lean`
never import this tier, `Setlec/SetP/*` stands on it.
