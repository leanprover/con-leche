...
`Setlec/TTVerify/IndBottom.lean`), which is why the nested iota rules'
...
def _root_.Setlec.TT.VExpr.instRevChain : List Setlec.TT.VExpr →
    Setlec.TT.VExpr → Setlec.TT.VExpr
  | [], X => X
  | v :: vs, X =>
    Setlec.TT.VExpr.instRevChain vs (X.inst (v.liftN vs.length) 0)
