...
theorem checkDecl_sound {env env' : Env} {d : Declaration}
  ...
  | thmDecl cv value =>
    ...
    simp only [htr] at h
    -- inferType of the declared type (from checkConstantVal)
    cases hst0 : inferType env 0 cv.type with
    | error e => rw [hst0] at h; exact nomatch h
    | ok stype0 =>
    rw [hst0] at h; dsimp only at h
    cases hsort0 : ensureSort env stype0 with
    | error e => rw [hsort0] at h; exact nomatch h
    | ok u0 =>
    rw [hsort0] at h; dsimp only at h
    -- the proposition check
    cases hpz : Level.isEquiv u0 Level.zero with
    ...
    have htrans : ∀ (e : Expr), e.constsResolve env = true → ∀ ψ : Name → Nat,
        interpClosed V val' (⟨ConstantInfo.thmInfo cv value :: env.consts⟩ : Env) ψ e =
          interpClosed V m.val env ψ e := by
      intro e hres ψ
      rw [interpClosed_mono (cval := val') (c₀ := ConstantInfo.thmInfo cv value) m.wf hfind' hres]
      exact interp_cval_ext hagree e 0 (rho0 V)
    have hwf' : EnvWF ⟨ConstantInfo.thmInfo cv value :: env.consts⟩ := by
    ...
    have htsound := fun (ψ : Name → Nat) =>
      inferType_sound (φ := ψ) (ρ := rho0 V) m cv.type hst0 hwt
