...
theorem AnnotOk.ext : ∀ (e : Expr) {d : Nat} {ρ ρ' : Nat → V},
  ...
  | .forallE n ty body m, d, ρ, ρ', h, hb, ha => by
    ...
    have hupd : ∀ (x : V) i, i < d + 1 → updV V ρ d x i = updV V ρ' d x i := by
      intro x i hi
      simp only [updV]
      split
      · rfl
      · exact h i (by omega)
    refine ⟨AnnotOk.ext ty h hb.1 haty, ⟨vE, ?_⟩, ?_⟩
    · intro x A hA hx
      rw [← interp_ext ty h hb.1] at hA
      obtain ⟨hbody, hw⟩ := hcond x A hA hx
      refine ⟨AnnotOk.ext _ (hupd x) (fvarsBelow_instantiate1 0 hb.2) hbody, ?_⟩
      obtain ⟨w, hwi, hmem⟩ := hw
      exact ⟨w, by rw [← interp_ext _ (hupd x) (fvarsBelow_instantiate1 0 hb.2)]; exact hwi, hmem⟩
...
theorem AnnotOk.shift : ∀ (e : Expr) {d p : Nat} {ρ : Nat → V} {x0 : V},
  ...
  | .forallE n ty body m, d, p, ρ, x0, hpd, hw, ha => by
    ...
    refine ⟨AnnotOk.shift ty hpd hw'.1 haty, ⟨vE, ?_⟩, ?_⟩
    · intro x A hA hx
...
theorem AnnotOk.unshift : ∀ (e : Expr) {d p : Nat} {ρ : Nat → V} {x0 : V},
  ...
  | .forallE n ty body m, d, p, ρ, x0, hpd, hw, ha => by
    ...
    obtain ⟨haty, ⟨vE, hcond⟩, htie⟩ := ha
    simp only [AnnotOk]
    refine ⟨AnnotOk.unshift ty hpd hw'.1 haty, ⟨vE, ?_⟩, ?_⟩
    · intro x A hA hx
    ...
    · intro v hv x A hA hx
      rw [← interp_shift ty hpd hw'.1 (x := x0)] at hA
      obtain ⟨w, hwi, hmem⟩ := htie v hv x A hA hx
      rw [← shiftFrom_instantiate1 hpd, ← insV_updV hpd] at hwi
      refine ⟨w, ?_, hmem⟩
      rw [← interp_shift _ (Nat.le_succ_of_le hpd)
        (hw'.1.instantiate1 0 hw'.2) (x := x0)]
      exact hwi
...
theorem AnnotOk_renameConsts {f : Name → Name}
  ...
  | .forallE n ty body m, d, ρ, ha => by
    ...
    obtain ⟨haty, ⟨vE, hcond⟩, htie⟩ := ha
    simp only [AnnotOk]
    refine ⟨AnnotOk_renameConsts hro ty d ρ haty, ⟨vE, ?_⟩, ?_⟩
    · intro x A hA hx
    ...
    · intro v hv x A hA hx
      rw [← interp_renameConsts hro ty d ρ] at hA
      obtain ⟨w, hwi, hmem⟩ := htie v hv x A hA hx
      rw [← renameConsts_instantiate1] at hwi
      refine ⟨w, ?_, hmem⟩
      rw [← interp_renameConsts hro _ (d + 1) (updV V ρ d x)]
      exact hwi
  | .lam n ty body m, d, ρ, ha => by
...
theorem AnnotOk.mono {c₀ : ConstantInfo} (hfresh : env.find? c₀.name = none) :
  ...
  | .forallE n ty body m, d, ρ, hres, ha => by
    ...
    simp only [AnnotOk] at ha ⊢
    obtain ⟨haty, ⟨vE, hcond⟩, htie⟩ := ha
    refine ⟨AnnotOk.mono hfresh ty d ρ hres.1 haty, ⟨vE, ?_⟩, ?_⟩
    · intro x A hA hx
...
theorem AnnotOk.cval_ext {cval₁ cval₂ : ConstVal V}
  ...
  | .forallE n ty body m, d, ρ, ha => by
    simp only [AnnotOk] at ha ⊢
    obtain ⟨haty, ⟨vE, hcond⟩, htie⟩ := ha
    refine ⟨AnnotOk.cval_ext hagree ty d ρ haty, ⟨vE, ?_⟩, ?_⟩
    · intro x A hA hx
...
theorem AnnotOk.env_ext {env₁ env₂ : Env}
  ...
  | .forallE n ty body m, d, ρ, ha => by
    simp only [AnnotOk] at ha ⊢
    obtain ⟨haty, ⟨vE, hcond⟩, htie⟩ := ha
    refine ⟨AnnotOk.env_ext henv hnat hstr ty d ρ haty, ⟨vE, ?_⟩, ?_⟩
    · intro x A hA hx
