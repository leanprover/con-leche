...
blobs used to be *computed* while `Setlec/Kernel/NatOpPins.lean`
elaborated, by loading `Setlec/PinGen/Certs.olean` into a full-view
environment (`importModules` at `OLeanLevel.private`).  That is not an
import edge, so Lake never ordered the two — on a cold tree
`lake build setlec` failed with

    object file '…/Setlec/PinGen/Certs.olean' of module
...
* the **loader** (`Setlec/Kernel/NatOpPins.lean`) `include_str`s the
...
Not the `Setlec.Expr` tree, but the **share table** the emitter builds
from it (`Setlec.PinGen.ShareSt`): pins share subterms heavily, and
...
`Setlec.Frontend.*` imports `Setlec.Kernel.*`, which imports
`Setlec.Kernel.NatOpPins` — a cycle.  A bespoke line format would have
...
inductive PinRef where
  ...
  | idx (i : Nat)
  /-- The inline `Setlec.Name.anonymous`. -/
  | anon
  /-- The inline `Setlec.Level.zero`. -/
  | lzero
...
inductive PinEntry where
  ...
  | exApp (f a : PinRef)
  | exLam (n ty b : PinRef) (bi : Setlec.BinderInfo) (pw : Option (List Setlec.Name))
  | exForall (n ty b : PinRef) (bi : Setlec.BinderInfo) (pw : Option (List Setlec.Name))
...
def dumpFormatTag : String := "setlec-natop-pins/1"
...
/-- Quote a `Setlec.Name` structurally (used for the `pw` parameter
lists, which the share table does not cover — they are tiny). -/
def quoteName : Setlec.Name → Lean.Expr
  | .anonymous => .const ``Setlec.Name.anonymous []
  | .str p s => mkApp2 (.const ``Setlec.Name.str []) (quoteName p) (mkStrLit s)
  | .num p n => mkApp2 (.const ``Setlec.Name.num []) (quoteName p) (mkRawNatLit n)

def quoteBinderInfo : Setlec.BinderInfo → Lean.Expr
...
def quoteNameList (ns : List Setlec.Name) : Lean.Expr :=
  ns.foldr
...
def quotePropWhen : Option (List Setlec.Name) → Lean.Expr
  | none => .const ``Setlec.PropWhen.never []
  | some ps => .app (.const ``Setlec.PropWhen.ifAllZero []) (quoteNameList ps)

def quoteBinderMeta (bi : Setlec.BinderInfo)
    (pw : Option (List Setlec.Name)) : Lean.Expr :=
  mkApp2 (.const ``Setlec.BinderMeta.mk []) (quoteBinderInfo bi)
    (quotePropWhen pw)
...
def refExpr : PinRef → Lean.Expr
  | .idx i => .bvar i
  | .anon => .const ``Setlec.Name.anonymous []
  | .lzero => .const ``Setlec.Level.zero []
...
def PinEntry.value : PinEntry → Lean.Expr
  | .nameStr p s =>
    mkApp2 (.const ``Setlec.Name.str []) (refExpr p) (mkStrLit s)
  | .nameNum p i =>
    mkApp2 (.const ``Setlec.Name.num []) (refExpr p) (mkRawNatLit i)
  | .levSucc u => .app (.const ``Setlec.Level.succ []) (refExpr u)
  | .levMax u v =>
    mkApp2 (.const ``Setlec.Level.max []) (refExpr u) (refExpr v)
  | .levImax u v =>
    mkApp2 (.const ``Setlec.Level.imax []) (refExpr u) (refExpr v)
  | .levParam n => .app (.const ``Setlec.Level.param []) (refExpr n)
  | .exBVar i => .app (.const ``Setlec.Expr.bvar []) (mkRawNatLit i)
  | .exFVar idx n ty =>
    mkApp3 (.const ``Setlec.Expr.fvar []) (mkRawNatLit idx) (refExpr n)
      (refExpr ty)
  | .exSort u => .app (.const ``Setlec.Expr.sort []) (refExpr u)
  | .exConst n us =>
    mkApp2 (.const ``Setlec.Expr.const []) (refExpr n)
      (levelListE (us.map refExpr))
  | .exApp f a =>
    mkApp2 (.const ``Setlec.Expr.app []) (refExpr f) (refExpr a)
  | .exLam n ty b bi pw =>
    mkApp4 (.const ``Setlec.Expr.lam []) (refExpr n) (refExpr ty) (refExpr b)
      (quoteBinderMeta bi pw)
  | .exForall n ty b bi pw =>
    mkApp4 (.const ``Setlec.Expr.forallE []) (refExpr n) (refExpr ty)
      (refExpr b) (quoteBinderMeta bi pw)
  | .exLet n ty v b =>
    mkApp4 (.const ``Setlec.Expr.letE []) (refExpr n) (refExpr ty) (refExpr v)
      (refExpr b)
  | .exLitNat v =>
    .app (.const ``Setlec.Expr.lit [])
      (.app (.const ``Setlec.Literal.natVal []) (mkRawNatLit v))
  | .exLitStr s =>
    .app (.const ``Setlec.Expr.lit [])
      (.app (.const ``Setlec.Literal.strVal []) (mkStrLit s))
  | .exProj s i e =>
    mkApp3 (.const ``Setlec.Expr.proj []) (refExpr s) (mkRawNatLit i)
      (refExpr e)
...
becomes one `PinEntry` (`Setlec/PinGen/Dump.lean`), bound once in the
...
structure ShareSt where
  ...
  entries : Array PinEntry := #[]
  nameMap : Std.HashMap Setlec.Name PinRef := {}
  levelMap : Std.HashMap Setlec.Level PinRef := {}
  exprMap : Std.HashMap Setlec.Expr PinRef := {}
...
partial def shareName (n : Setlec.Name) : ShareM PinRef := do
...
partial def shareLevel (l : Setlec.Level) : ShareM PinRef := do
...
partial def shareExpr (e : Setlec.Expr) : ShareM PinRef := do
...
def blobOf (e : Setlec.Expr) : PinBlob :=
...
shared `let`-chain. -/
def buildExprValue (e : Setlec.Expr) : Lean.Expr := (blobOf e).value
...
def biToJson : Setlec.BinderInfo → Json
  | .default => natJ 0
  | .implicit => natJ 1
  | .strictImplicit => natJ 2
  | .instImplicit => natJ 3

def biOfNat : Nat → Except String Setlec.BinderInfo
  | 0 => return .default
  | 1 => return .implicit
...
/-- A `Setlec.Name` as the array of its components, outermost last. -/
def snameComps : Setlec.Name → Array Json → Array Json
  | .anonymous, acc => acc
  | .str p s, acc => (snameComps p acc).push (Json.str s)
  | .num p i, acc => (snameComps p acc).push (natJ i)

def snameToJson (n : Setlec.Name) : Json := Json.arr (snameComps n #[])

def snameOfJson (j : Json) : Except String Setlec.Name := do
  let arr ← j.getArr?
  let mut n : Setlec.Name := .anonymous
  for c in arr do
    match c with
...
def pwToJson : Option (List Setlec.Name) → Json
  | none => Json.null
  | some ps => Json.arr (ps.map snameToJson).toArray

def pwOfJson : Json → Except String (Option (List Setlec.Name))
...
`Setlec/Kernel/NatOpPins.lean` `include_str`s the committed dump and
...
def loadPinsFromText (text : String) : Elab.Command.CommandElabM Unit := do
  ...
  unless d.leanVersion == Lean.versionString do
    throwError "the committed pin dump was generated by Lean \
      ...
      Setlec/Kernel/NatOpPins.lean for the toolchain-named file)."
...
def dumpLines (d : PinDumpFile) : Array String := Id.run do
  ...
  out := out.push ("\"_README\":" ++ (Json.str
    ("GENERATED FILE — do not edit.  The pinned Nat-operation defining \
     ...
     (see Setlec/PinGen/Dump.lean for the encoding).  Spliced into \
     Setlec/Kernel/NatOpPins.lean by #load_natop_pins.")).compress ++ ",")
