...
(**Resolved 2026-08-28 in favour of A** — see "A5 RESOLVED" at the end
of this file.)

## A5 RESOLVED — repair **A** granted and landed (task #152, 2026-08-28)

The fork A5 escalated (reinstate a kernel check, or bet on B5″) was
ruled by the user in favour of **A**, with an explicit exception to the
goal's no-new-checks clause and its rationale on record: *front door
checks are cheap — they do not affect reduction, and they benefit from
the infer caches.*  Landed as task #152; see DESIGN.md, "The λ-rule
computes its codomain sort".

**What the checker now computes.**  `inferBody`'s `.lam` clause runs
the ∀ clause's own move on the codomain — infer the body's type, then
`ensureSort` it — at `CheckMode.verified` (`.setModel` and `.ttModel`;
**off** at `.noModel`, which is the official-parity lane, since the
reference kernel's `infer_lambda` does not run it).  So the λ codomain
sort exists at the *default* mode, which is the mode this campaign's
relation family is stated at.

**What tier A gets, exactly.**  `inferTypeCore_lam_inv`
(`Setlec/Verify/InferLemmas.lean`) carries

```
    mode.verified = true → body.isLam = false →
      ∃ btt v, inferTypeCore … (d+1) bt = .ok btt ∧
               whnf … (d+1) btt = .ok (.sort v)
```

i.e. `HasSort (A :: Δ) B v` at the **innermost binder of a λ-chain**,
on the chain's body type.  The checker cannot deliver it per node: the
interned twin peels a λ-chain in one loop and never materializes the
intermediate opened body types, so a per-node check is not reproducible
against it without either de-telescoping (the pre-#72 cost on
`perf/app-lam`'s 4 000-binder towers) or an `O(k · size)` rebuild per
chain.  The obstacle is recorded in full in DESIGN.md's finding.

**The induction tier A/B owes, and why it is small.**  At an outer
binder of a chain the codomain is the inner λ's own `.pi`, and

* the inner **domain**'s sort is checked at that binder and is already
  in the inversion (`whnf tty = .sort u`), and
* the inner **codomain**'s sort is the induction hypothesis, bottoming
  out at the innermost binder, where the new conjunct supplies it,

so `hasSort_pi_of` — proved, and listed in A5's own case map as I7's
free clause — closes each step.  The premise the I7 amendment wants is
therefore available at *every* λ node; it is derived along the chain
rather than read off one inversion.  Note that this is precisely the
shape A5's table predicted: I7 was never the failing clause (I8 was),
and I7's derivation only ever needed the numeral at the bottom of the
chain.

**Unchanged by this.**  `Rel.lean`'s I7 has no new premise *yet* (the
amendment is tier A's to land, now unblocked); `checkStepR` and
`infer_lam_claimR` compile untouched — the new conjunct is dropped as
`-` at all four inversion consumers, including
`Bridge/InferStruct.lean`.  B5″ stays unattempted and unrefuted; it is
