...
def WhnfCoreClaims2D (μ : CheckMode) {env : Env} (m : EnvS2UM V μ env)
...
def WhnfClaims2D (μ : CheckMode) {env : Env} (m : EnvS2UM V μ env)
    (φ : Name → Nat) (fuel : Nat) : Prop :=
  ∀ {d : Nat} {e e' : Expr} {Δa : List AVExpr},
...
def DefEqClaims2D (μ : CheckMode) {env : Env} (m : EnvS2UM V μ env)
    (φ : Name → Nat) (fuel : Nat) : Prop :=
...
def InferClaims2D (μ : CheckMode) {env : Env} (m : EnvS2UM V μ env)
    (φ : Name → Nat) (fuel : Nat) : Prop :=
...
theorem checkSound2D {μ : CheckMode} {env : Env}
    (hstep : CheckStep2D μ V) (m : EnvS2UM V μ env) (φ : Name → Nat) :
    ∀ fuel : Nat,
      WhnfCoreClaims2D μ m φ fuel ∧ WhnfClaims2D μ m φ fuel ∧
