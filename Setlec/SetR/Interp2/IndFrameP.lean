...
`AVExpr` bookkeeping — so they transpose as a rename, and they are
...
def ctxInstAtP (v : AVExpr) (j : Nat) : List AVExpr → List AVExpr
  | [] => []
  | B :: Γ => B.inst v (j + Γ.length) :: ctxInstAtP v j Γ

@[simp] theorem ctxInstAtP_nil (v : AVExpr) (j : Nat) :
    ctxInstAtP v j [] = [] := rfl

theorem ctxInstAtP_cons (v : AVExpr) (j : Nat) (B : AVExpr)
    (Γ : List AVExpr) :
    ctxInstAtP v j (B :: Γ) = B.inst v (j + Γ.length) :: ctxInstAtP v j Γ :=
  rfl

theorem ctxInstAtP_append (v : AVExpr) (j : Nat) :
    ∀ (Γ₁ Γ₂ : List AVExpr),
      ctxInstAtP v j (Γ₁ ++ Γ₂) =
        ctxInstAtP v (j + Γ₂.length) Γ₁ ++ ctxInstAtP v j Γ₂
  | [], _ => rfl
  | B :: Γ₁, Γ₂ => by
    simp only [List.cons_append, ctxInstAtP_cons,
      ctxInstAtP_append v j Γ₁ Γ₂, List.length_append, List.cons.injEq]
    exact ⟨by congr 1; omega, trivial⟩

theorem ctxInstAtP_snoc (v : AVExpr) (j : Nat) (Γ : List AVExpr)
    (A : AVExpr) :
    ctxInstAtP v j (Γ ++ [A]) = ctxInstAtP v (j + 1) Γ ++ [A.inst v j] := by
  rw [ctxInstAtP_append]
  rfl

/-- `ctxInstAtP`, per entry: the entry at index `i` is instantiated at
its own residual depth (`ctxInstAt_getD`). -/
theorem ctxInstAtP_getD (v : AVExpr) (j : Nat) :
    ∀ (Γ : List AVExpr) (i : Nat), i < Γ.length →
      (ctxInstAtP v j Γ).getD i default =
        (Γ.getD i default).inst v (j + Γ.length - 1 - i)
  | [], _, h => absurd h (by simp)
  | B :: Γ, 0, _ => by
    simp only [ctxInstAtP_cons, List.getD_cons_zero, List.length_cons,
      Nat.sub_zero]
    congr 1
  | B :: Γ, i + 1, h => by
    simp only [ctxInstAtP_cons, List.getD_cons_succ, List.length_cons]
    rw [ctxInstAtP_getD v j Γ i (by simpa using h)]
    congr 1
    omega

/-- A substituted tower is a tower over the substituted context
(`PiTele.inst`). -/
theorem PiTeleP.inst : ∀ {k : Nat} {T : AVExpr} {Γ : List AVExpr}
    {R : AVExpr}, PiTeleP k T Γ R → ∀ (v : AVExpr) (j : Nat),
      PiTeleP k (T.inst v j) (ctxInstAtP v j Γ) (R.inst v (j + k)) := by
  intro k T Γ R h
  induction h with
  | nil => intro v j; simpa using PiTeleP.nil
  | @cons k u v' A B R Γ _ ih =>
    intro v j
    rw [AVExpr.inst_pi, ctxInstAtP_snoc]
    have h1 := ih v (j + 1)
    rw [show j + 1 + k = j + (k + 1) from by omega] at h1
    exact PiTeleP.cons h1
...
step for step and `PiTeleP.inst` plays exactly the role `PiTele.inst`
plays in `teleFitV_of_tower`.  What changes is only the residual's
spelling — `AVExpr.instSeq` in place of `VExpr.instSeq`. -/

/-- **`Sat2` of a tower's context at the chain** (`sat_of_tower`),
from per-step chain memberships. -/
theorem sat2_of_tower {k : Nat} {T : AVExpr} {Γ : List AVExpr}
    {R : AVExpr} (h : PiTeleP k T Γ R) {ws : List AVExpr} {ρ : Nat → V}
    (hlen : ws.length = k)
    (hmem : ∀ n, n < k →
      interp2 V ρ (ws.getD n default)
        ∈ˢ interp2 V (chainP V ρ (ws.take n))
          (Γ.getD (k - 1 - n) default)) :
    Sat2 V Γ (chainP V ρ ws) := by
  intro i Aa hi
  ...
  rw [chainP_lt (by omega),
    show ws.length - 1 - i = k - 1 - i from by omega,
    show (fun j => chainP V ρ ws (j + i + 1))
        = chainP V ρ (ws.take (k - 1 - i)) from by
      rw [← show ws.length - 1 - i = k - 1 - i from by omega]
      exact chainP_tail (by omega)]
  exact h1
...
theorem teleFitPA_of_tower :
    ∀ (k : Nat) {T : AVExpr} {Γ : List AVExpr} {R : AVExpr},
      ...
      (∀ n, n < k →
        interp2 V ρ (ws.getD n default)
          ∈ˢ interp2 V (chainP V ρ (ws.take n))
            (Γ.getD (k - 1 - n) default)) →
      TeleFitPA V ρ T ws (Setlec.SetR.AVExpr.instSeq ws (k - 1) R) := by
  intro k
  induction k with
  | zero =>
  ...
  | succ k ihk =>
    ...
    rw [show (Γ' ++ [A]).getD (k + 1 - 1 - 0) default = A from by
        ...
        simp] at h0
    simp only [List.getD_cons_zero, List.take_zero, chainP_nil] at h0
    ...
    · rw [show Setlec.SetR.AVExpr.instSeq (w :: ws') (k + 1 - 1) R
          = Setlec.SetR.AVExpr.instSeq ws' (k - 1) (R.inst w k) from by
    ...
    · intro n hn
      ...
      have htklen : (ws'.take n).length = n := by
        ...
        omega
      rw [interp2_inst,
        show shiftE n 0 (chainP V ρ (ws'.take n)) = ρ from by
          funext j
        ...
        show instE n (interp2 V ρ w) (chainP V ρ (ws'.take n))
          ...
          rw [chainP_cons_eq_instE, htklen]]
      exact h1

/-! ## `CtxOkP` at the opened frame
...
onto `DefEq.refl`), which carries its own justification; `CtxOkP`'s is
a **semantic equation plus a grading**, and while the equation is free
(it is `interp2_liftN` against a `shiftE` that computes), the grading
...
`Sat2` at all.  Lifting the entry to the leaf's depth shifts the
...
hypothesis is consulted, and `ctxOkP_of_openers` never inspects `ρ`. -/

/-- **`CtxOkP` at the opened frame** (`ctxOkR_of_openers`'s
...
theorem ctxOkP_of_openers {env : Env} {m : EnvS2Core V env}
    {φ : Name → Nat}
    ...
    (hokA : ∀ i, i < n → ∀ ρ : Nat → V, Sat2 V Δa ρ →
      AnnotOkP V (fun j => ρ (j + (k - 1 - i) + 1)) (Aa i)) :
  ...
  · have hd1 := hdoms l.1 _ hpos
    ...
    rfl
  · -- the equation: `interp2_liftN`'s `shiftE` against the entry's own
  ...
  · -- the grading: the entry's, transported across the same lift
    intro ρ hρ
    refine (AnnotOkP_liftN V (k - l.1) (Aa l.1) 0 ρ).mpr ?_
