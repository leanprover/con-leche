...
Part 5 raised a **risk flag**, unmechanized, on `RecRuleLawP`'s ∃-form
nested-pin conjunct (`Annot/EnvS2P.lean`):
...
— the grading `∀ ρ : Nat → V, AnnotOkP V ρ vpa` is demanded
...
2. **The reading of such a pin is an open application**
   (`openRev_pin_shape` / `denoteP_pin_shape` below, both by `rfl`):
   `openRev 0 rP` turns the pin's loose bvars into `fvar`s and
   `denoteP … rP` turns those into `.bvar`s, so `vpa` is
...
3. **No such reading is graded uniformly in `ρ`**
   (`not_uniform_annotOk2_open_app`).  `AnnotOk2` of `.app f a` carries
   ...
   `mem_typeP` — positive kind, so link 3's hypotheses are exactly the
   environment invariant `EnvS2PM` already carries.
...
form `∀ ρ, Sat2 V Δa ρ → AnnotOkP V ρ …` — guarded by a context, never
...
`AVExpr.instRevChain (xs.take rP) vpa`, under the ambient
`TeleFitPA V ρ TVa (xs ++ …) restR` premise, and that is the term
`DefEqClaims2P` needs graded.
...
`.app f (.bvar j)` cannot be `AnnotOk2` at every `ρ`: the app clause
pins the domain, and no set contains every value. -/
theorem not_uniform_annotOk2_open_app {f : AVExpr} {j v : Nat}
    {A : V} {B : V → V} (hv : v ≠ 0)
    (hcl : ∀ ρ ρ' : Nat → V, interp2 V ρ f = interp2 V ρ' f)
    {ρ₀ : Nat → V} (hf : interp2 V ρ₀ f ∈ˢ piR v A B) :
    ¬ (∀ ρ : Nat → V, AnnotOk2 V ρ (.app f (.bvar j))) := by
  intro h
  have hA := h (fun _ => A)
  ...
  rw [interp2_bvar] at ha'
  exact not_mem_self A (hAA ▸ ha')

/-- The same at the P tier's currency (`AnnotOkP = AnnotOk2 ∧
AnnotValidV`). -/
theorem not_uniform_annotOkP_open_app {f : AVExpr} {j v : Nat}
    {A : V} {B : V → V} (hv : v ≠ 0)
    (hcl : ∀ ρ ρ' : Nat → V, interp2 V ρ f = interp2 V ρ' f)
    {ρ₀ : Nat → V} (hf : interp2 V ρ₀ f ∈ˢ piR v A B) :
    ¬ (∀ ρ : Nat → V, AnnotOkP V ρ (.app f (.bvar j))) :=
  fun h => not_uniform_annotOk2_open_app hv hcl hf fun ρ => (h ρ).1

/-- **The refutation at an environment leaf** — the shape the pin's
reading actually has.  `hf`'s hypothesis is what `EnvS2PM.mem_typeP`
gives for a type former at a positive sort (`Tree : Type → Type`
lands in `piR 1 (univ 1) (fun _ => univ 1)`). -/
theorem not_uniform_annotOkP_acval_open_app {env : Env}
    (m : EnvS2Core V env) (n : Name) (ψ : Name → Nat) {j v : Nat}
    {A : V} {B : V → V} (hv : v ≠ 0) {ρ₀ : Nat → V}
    (hf : interp2 V ρ₀ (m.acval n ψ) ∈ˢ piR v A B) :
    ¬ (∀ ρ : Nat → V, AnnotOkP V ρ (.app (m.acval n ψ) (.bvar j))) :=
  not_uniform_annotOkP_open_app hv
    (fun ρ ρ' => acval_interp2_closedC m n ψ ρ ρ') hf
...
`not_uniform_annotOk2_open_app` is not vacuous. -/
theorem open_app_shape_realizable :
    ∃ (f : AVExpr) (v : Nat) (A : V) (B : V → V),
      v ≠ 0 ∧ (∀ ρ ρ' : Nat → V, interp2 V ρ f = interp2 V ρ' f) ∧
      (∀ ρ : Nat → V, interp2 V ρ f ∈ˢ piR v A B) := by
  refine ⟨.lam 1 (.sort 0) (.bvar 0), 1, univ 0, fun _ => univ 0,
    Nat.one_ne_zero, ?_, ?_⟩
  · intro ρ ρ'
    simp [interp2_lam, interp2_sort, interp2_bvar, cons_zero]
  · intro ρ
    simp only [interp2_lam, interp2_sort, interp2_bvar, cons_zero]
...
theorem exists_ungradable_open_app :
    ∃ (f : AVExpr) (j : Nat),
      ¬ (∀ ρ : Nat → V, AnnotOkP V ρ (.app f (.bvar j))) := by
  obtain ⟨f, v, A, B, hv, hcl, hf⟩ := open_app_shape_realizable (V := V)
  exact ⟨f, 0, not_uniform_annotOkP_open_app hv hcl (hf (fun _ => A))⟩
...
theorem denoteP_pin_shape {env : Env} (m : EnvS2Core V env)
    (φ : Name → Nat) (c : Name) (cv : Setlec.ConstantInfo)
