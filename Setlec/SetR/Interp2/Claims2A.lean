...
def WhnfCoreClaims2A (μ : CheckMode) {env : Env} (m : EnvS2UM V μ env)
...
def WhnfClaims2A (μ : CheckMode) {env : Env} (m : EnvS2UM V μ env)
    (φ : Name → Nat) (fuel : Nat) : Prop :=
  ∀ {d : Nat} {e e' : Expr} {Δa : List AVExpr},
...
def DefEqClaims2A (μ : CheckMode) {env : Env} (m : EnvS2UM V μ env)
    (φ : Name → Nat) (fuel : Nat) : Prop :=
...
def InferClaims2A (μ : CheckMode) {env : Env} (m : EnvS2UM V μ env)
    (φ : Name → Nat) (fuel : Nat) : Prop :=
...
theorem checkSound2A {μ : CheckMode} {env : Env}
    (hstep : CheckStep2A μ V) (m : EnvS2UM V μ env) (φ : Name → Nat) :
    ∀ fuel : Nat,
      WhnfCoreClaims2A μ m φ fuel ∧ WhnfClaims2A μ m φ fuel ∧
