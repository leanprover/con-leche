...
`capsOkP_cons_fresh`'s transport verbatim (the same
...
* `EtaLawP` for a block former at `etaFields = 0`, and
* `UnitLawP` for the cons's own former,
    ...
    (hpins : ∀ caps, c₀ = .indInfo cvA caps →
      Setlec.EtaPins μ env cvA.name cvA.levelParams caps ∧
        ...
        (caps.eta = true → 0 < caps.etaFields →
          env.find? (Setlec.projFnName cvA.name 0) = none))
...
theorem capsOkP_cons_member (mp : EnvS2PM V μ env)
