...
theorem ctxOk2Open_of {env : Env} (m : EnvS2UM V μ env)
...
conjunct adds. -/
def CtxOk2OpenD {env : Env} (m : EnvS2UM V μ env)
    (φ : Name → Nat) : Prop :=
...
scoping. -/
theorem ctxOk2OpenD_of {env : Env} (m : EnvS2UM V μ env)
    (φ : Name → Nat) : CtxOk2OpenD m φ :=
