...
`m : EnvS2 V env` — no environment shape, no fuel, no mode, no level
...
def CtxOk2RShape {env : Env} (m : EnvS2 V env) (μ : CheckMode)
    (φ : Name → Nat) : Prop :=
...
theorem CtxOk2RShape.of {env : Env} {m : EnvS2 V env} {μ : CheckMode}
...
No premise beyond an arbitrary `m : EnvS2 V env`. -/
theorem ctxOk2R_refuted {env : Env} (m : EnvS2 V env) (μ : CheckMode)
...
seal 3 did not test. -/
theorem ctxOk2R_refuted_nonvacuous {env : Env} (m : EnvS2 V env)
...
theorem not_ctxOk2R {env : Env} (m : EnvS2 V env) (μ : CheckMode)
