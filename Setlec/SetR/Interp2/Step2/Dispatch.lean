...
/-! `Sat2_tail` moved to `Setlec/SetBase/Sat2.lean` at THE SEPARATION's
S2, beside `Sat2` itself: it is the definition's own inverse and both
lanes' weakening steps consume it.  Name and statement unchanged. -/
...
/-! `AnnotOk2.hoist_pi` stood here, and the rest of the hoist kit below
the example that follows.  THE SEPARATION's S2 moved the whole kit to
`Setlec/SetBase/Hoist.lean`: every lemma in it is an implication between
`∀ ρ, Sat2 → …` shapes — the trap-check note in the moved file says so
itself — and BOTH lanes' definitional-equality and inference quarters
split and reassemble their nodes with it.  Names and statements
unchanged; this module imports them back.  The satisfiability example
just below stays, because it is about `CtxOk2`, which is 2U. -/
...
/-! The rest of the hoist kit — `hoist_lam`, the non-binder splitters,
the converses, the head transfer and the trap-check examples — moved to
`Setlec/SetBase/Hoist.lean` at the same S2 sever. -/
