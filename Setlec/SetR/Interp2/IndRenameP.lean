...
(`Verify/Denote/Rename.lean`) at `denoteP`/`AVExpr`, and the
...
`RenameOkP`'s third conjunct is exactly what `BlockAcvalInstalled`
stores — an installed member's leaf *is* its model's — so
`blockRenameOkP` is `blockRenameOkT`'s twin with the valuation clause
...
renaming. -/
def RenameOkP (acval : Name → (Name → Nat) → AVExpr) (env : Env)
    (f : Name → Name) : Prop :=
...
theorem denoteP_renameConsts {f : Name → Name}
    (hro : RenameOkP acval env f) :
    ∀ (e : Expr) (d : Nat),
  ...
  | .const n ws, d => by
    ...
    | none =>
      rw [denoteP, denoteP, hf, hro.2.1 n hf]
    | some ci =>
...
theorem RenEqT.denoteP {f : Name → Name} (hro : RenameOkP acval env f)
    {e₁ e₂ : Expr} (h : RenEqT f e₁ e₂) (d : Nat) :
    denoteP acval env φ d e₂ = denoteP acval env φ d e₁ := by
  rw [← denoteP_erasedEq h d]
  exact denoteP_renameConsts hro e₁ d
...
theorem blockRenameOkP {blockNames : List Name} {cval : TConstVal}
