...
that was not previously stored, and `capsOkP_cons_fresh` (which needs
...
* `T ≠ c₀.name` — if they were equal the family's own lookup would
  return the cons, i.e. a `recInfo`, where `CapsOkP`'s premise says
  `indInfo`;
...
is `capsOkP_cons_fresh`'s transport verbatim — the same
`denoteP_cons_fresh_mono` forward crossing and the same three
`acvalWith_ne` leaf moves — so what is left as a premise is exactly
the one live law: `EtaLawP` for the family the cons *completes*.

The unit half needs no split at all.  A `recInfo` cons is not an
`indInfo`, so `CapsOkP`'s unit premise `find? T = some (.indInfo …)`
descends by kind at once, and — post-repair — the unit half carries no
family premise to re-establish. -/
theorem capsOkP_cons_proj (mp : EnvS2PM V μ env)
  ...
  · -- the unit-like half: the premise descends by kind
    ...
    have hnT : T ≠ c₀.name := by
      intro hh
      rw [hh, Setlec.Env.find?_cons_self] at hf
