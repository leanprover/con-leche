...
`capsOkP_cons_proj` leaves exactly one law open — `EtaLawP` for the
family the projection-function cons *completes* — and this file is it.
It is `etaLawKeyS`'s transpose with the half `memberEtaLawP` was
...
is smaller than the phrase suggests: `memberEtaLawP`'s skeleton is
...
* each of those applications is evaluated by the *same*
  `interp2_bvarSpine` the parameter spine uses.
...
`instSeq_openSpine` at `nP+1` reads it, and one `interp2_bvarSpine` at
...
/-- `interp2` of an application spine, as a `map`-then-`foldl`.  The
member key inlined this; the mixed spine needs it as a rewrite so the
two halves can be split with `List.map_append`. -/
theorem interp2_mkAppN_map (σ : Nat → V) (K : AVExpr) :
    ∀ as : List AVExpr,
      interp2 V σ (AVExpr.mkAppN K as)
        = (as.map (interp2 V σ)).foldl SetTheory.app (interp2 V σ K) := by
  intro as
  rw [interp2_mkAppN]
  generalize interp2 V σ K = b
  induction as generalizing b with
  | nil => rfl
  | cons a asr ih => simpa using ih (SetTheory.app b (interp2 V σ a))

/-! ## The key -/

/-- **The projection cons's live η law**: the family a projection
function completes carries `EtaLawP`.  `capsOkP_cons_proj`'s
`hcomplete`.
...
def ProjEtaLawP (V : Type w) [SetTheory V] : Prop :=
    ...
    ∀ (cvT : ConstantVal) (caps : IndCaps),
      ...
      ∀ m₂ : EnvS2Core V ⟨c₀ :: env.consts⟩,
        ...
        (∀ j, j < caps.etaFields → ∀ ψ : Name → Nat,
          m₂.acval (Setlec.projFnName T j) ψ
            = mp.base2.acval (Setlec.projModelName T j) ψ) →
        ∀ φ' : Name → Nat, EtaLawP m₂ φ' T cvT caps

set_option maxHeartbeats 3200000 in
theorem projEtaLawP : ProjEtaLawP V := by
  ...
  have hT0 : T ≠ c₀.name := by
    intro hh
    rw [hh, Setlec.Env.find?_cons_self, hc₀eq] at hfT
    exact nomatch hfT
  ...
  have hEqTy : ∀ ψ : Name → Nat,
      denoteP mp.base2.acval env ψ 0 cvT.type
        = denoteP mp.base2.acval env ψ 0 cvmT.type := by
    intro ψ
  ...
  -- its model
  rw [show etaFabArgs2 (fun n => interp2 V ρ (m₂.acval n ψ)) T ts x
        caps.etaFields
      = ts ++ (List.range caps.etaFields).map (fun j =>
          (ts ++ [x]).foldl SetTheory.app
            (interp2 V ρ (mp.base2.acval (Setlec.projModelName T j) ψ)))
      from by
  ...
  have hKle : ∀ (K : Name) (ci : ConstantInfo),
      ...
      = some (AVExpr.mkAppN (mp.base2.acval K ψ)
          ((List.range caps.etaParams).map fun q =>
            AVExpr.bvar (d - 1 - (0 + q)))) :=
    fun K ci hf hlps d hd => denoteP_openSpine hf hlps _ d hd
  have hspineVal : ∀ (K : Name) (d : Nat) (σ : Nat → V),
      (∀ q, q < ts.length →
        σ (d - 1 - (0 + q)) = consN ts ρ (ts.length - 1 - q)) →
      interp2 V σ (AVExpr.mkAppN (mp.base2.acval K ψ)
          ((List.range caps.etaParams).map fun q =>
            AVExpr.bvar (d - 1 - (0 + q))))
        = ts.foldl SetTheory.app (interp2 V ρ (mp.base2.acval K ψ)) := by
    intro K d σ hσ
    rw [← hlents]
    exact interp2_bvarSpine (V := V) ts (ρ := ρ) (σ := σ)
  ...
  have hx' : x ∈ˢ interp2 V (consN ts ρ) Ax := by
    rw [hAx, hspineVal _ caps.etaParams (consN ts ρ)
      (fun q hq => congrArg (consN ts ρ) (by omega))]
    exact hmx
  -- ===== the fit, moved and continued =====
  have hteleM' : PiTeleP ts.length ta Γm Cm := by rw [hlents]; exact hteleM
  have hteleS2' : PiTeleP ts.length ua Γ₂ (.pi ux vx Ax Cs) := by
    rw [hlents]; exact hteleS2
  have hfitFull : TeleFitP V ρ ua (ts ++ [x])
      (interp2 V (cons x (consN ts ρ)) Cs) :=
    teleFitP_congr_ext ts hteleM' hteleS2' hdomEq hfit
      (TeleFitP.cons hx' TeleFitP.nil)
  have hteleFull : PiTeleP (ts ++ [x]).length ua Γs Cs := by
  ...
  have hprojRead : ∀ j ∈ List.range caps.etaFields,
      denoteP mp.base2.acval env ψ (caps.etaParams + 1)
        ((fun j => Expr.mkAppN
          (.const (Setlec.projModelName T j)
  ...
  have hprojList : ((List.range caps.etaFields).map (fun j =>
        Expr.mkAppN (.const (Setlec.projModelName T j)
          (cvT.levelParams.map .param))
  ...
  have hCs : Cs = .app (.app (.app
      ...
      (AVExpr.mkAppN (mp.base2.acval (T.str "_model") ψ)
        ((List.range caps.etaParams).map fun q =>
          AVExpr.bvar (caps.etaParams + 1 - 1 - (0 + q)))))
      (.bvar 0))
      (AVExpr.mkAppN (mp.base2.acval (caps.etaCtor.str "_model") ψ)
        (((List.range caps.etaParams).map fun q =>
            AVExpr.bvar (caps.etaParams + 1 - 1 - (0 + q)))
          ++ (List.range caps.etaFields).map fun j =>
              AVExpr.mkAppN
      ...
      (DenoteSpineP.cons
        ...
        (DenoteSpineP.cons
          ...
          (DenoteSpineP.cons
            (denoteP_mkAppN
              (DenoteSpineP.append
                (denoteSpineP_openFvars caps.etaParams 0
                  (caps.etaParams + 1) (by omega))
                (DenoteSpineP.map_list (List.range caps.etaFields)
                  hprojRead))
              hctorHead)
            DenoteSpineP.nil)))
    ...
    exact (Option.some.inj h).symm
  -- ===== fire =====
  have hokCs : AnnotOkP V (cons x (consN ts ρ)) Cs := by
    have h := teleFitP_okP_residual (ts ++ [x]) hteleFull (hokua ρ)
      hfitFull
    rwa [hconsApp] at h
  have hSval : ∀ K : Name, interp2 V (cons x (consN ts ρ))
      (AVExpr.mkAppN (mp.base2.acval K ψ)
        ((List.range caps.etaParams).map fun q =>
          AVExpr.bvar (caps.etaParams + 1 - 1 - (0 + q))))
      = ts.foldl SetTheory.app (interp2 V ρ (mp.base2.acval K ψ)) :=
  ...
  have hPval : ∀ K : Name, interp2 V (cons x (consN ts ρ))
      (AVExpr.mkAppN (mp.base2.acval K ψ)
        ((List.range (caps.etaParams + 1)).map fun q =>
          AVExpr.bvar (caps.etaParams + 1 - 1 - (0 + q))))
      = (ts ++ [x]).foldl SetTheory.app
          (interp2 V ρ (mp.base2.acval K ψ)) := by
    intro K
    have hlen1 : (ts ++ [x]).length = caps.etaParams + 1 := by
      rw [List.length_append, hlents]; rfl
    have h := interp2_bvarSpine (V := V) (ts ++ [x]) (ρ := ρ)
  ...
  have hmapP : ((List.range caps.etaFields).map fun j =>
        AVExpr.mkAppN (mp.base2.acval (Setlec.projModelName T j) ψ)
          ((List.range (caps.etaParams + 1)).map fun q =>
            AVExpr.bvar (caps.etaParams + 1 - 1 - (0 + q)))).map
        (interp2 V (cons x (consN ts ρ)))
      = (List.range caps.etaFields).map fun j =>
          (ts ++ [x]).foldl SetTheory.app
            (interp2 V ρ (mp.base2.acval (Setlec.projModelName T j) ψ)) := by
    rw [List.map_map]
    exact List.map_congr_left fun j _ => hPval _
  rw [List.map_map] at hmapS hmapP
  rw [hmapS, hmapP,
    acval_interp2_closedC mp.base2 (caps.etaCtor.str "_model") ψ
      (cons x (consN ts ρ)) ρ] at hRS
  have hlanded := memFoldl_of_teleFitP (ts ++ [x]) (hokua ρ)
    (hmemua ρ) hfitFull
  rw [hCs] at hlanded
  have hc0 : (cons x (consN ts ρ)) 0 = x := rfl
  simp only [interp2_app, interp2_bvar, hSval, hc0] at hlanded
  rw [interp2_mkAppN_map, List.map_append, List.map_map, List.map_map]
    at hlanded
  rw [hmapS, hmapP,
    acval_interp2_closedC mp.base2 (caps.etaCtor.str "_model") ψ
      (cons x (consN ts ρ)) ρ] at hlanded
  rw [(mp.eq_lawP heqfE _).1 (cons x (consN ts ρ)) _ x _
    hSuniv hxS hRS] at hlanded
  exact eq_of_mem_eqv hlanded

/-! ## The row, closed

`capsOkP_cons_proj` and `projEtaLawP` compose with **no residue**: the
...
theorem capsOkP_cons_proj_of (mp : EnvS2PM V μ env)
