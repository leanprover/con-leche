...
theorem no_proof_of_Empty_R (V : Type w) [SetTheory V]
    ...
    (c : ConstantInfo) (hc : c ∈ env'.consts)
    (hty : c.toConstantVal.type = .const emptyName []) : False := by
  obtain ⟨m⟩ := checkDecls_sound_R (hg := hg) (V := V) h
...
run; everything after that is `foldlM_R (hg := hg)`'s step. -/
theorem foldlM_RC
    {μ : CheckMode} (hg : μ.betaGate = false) :
  ...
  | d :: ds, env, _, hm, h => by
    ...
    | ok env1 =>
      ...
      exact foldlM_RC (hg := hg) ds env1
        (declStepS (hg := hg) divModPinS reducePinS stdAxiomKeyS declBasisS
          (declIndS memberKeyS) m hE (checkDeclR_sound (hg := hg) m hE hF)) h

/-- **The acceptance theorem for the cached executable checker.** -/
theorem checkDeclsC_sound_R
    {μ : CheckMode} (hg : μ.betaGate = false) {ds : List Declaration} {env' : Env}
    (h : checkDecls μ (cachedOps μ) ds = .ok env') :
    Nonempty (EnvS V env') :=
  (foldlM_RC (hg := hg) ds Env.empty
    ⟨⟨EnvS.empty V⟩, EtaFamiliesClosed.empty⟩ h).1

/-- **No proof of `Empty`** is accepted by the cached executable. -/
theorem no_proof_of_Empty_C_R (V : Type w) [SetTheory V] {μ : CheckMode} (hg : μ.betaGate = false)
    {ds : List Declaration} {env' : Env}
    (h : checkDecls μ (cachedOps μ) ds = .ok env')
    (c : ConstantInfo) (hc : c ∈ env'.consts)
    (hty : c.toConstantVal.type = .const emptyName []) : False := by
  obtain ⟨m⟩ := checkDeclsC_sound_R (hg := hg) (V := V) h
  exact no_constant_of_Empty_R m c hc hty
...
theorem foldlM_RS
  ...
  | d :: ds, fe, fe', hfe, hm, h => by
    ...
    | ok fe1 =>
      ...
      exact foldlM_RS (hg := hg) ds fe1 hfe1
        (declStepS (hg := hg) divModPinS reducePinS stdAxiomKeyS declBasisS
          (declIndS memberKeyS) m hE (checkDeclR_sound (hg := hg) m hE hF)) h
...
theorem checkDeclsS_sound_R
  ...
  | ok fe =>
    ...
    exact (foldlM_RS (hg := hg) ds (mkFEnv Env.empty) rfl
      ⟨⟨EnvS.empty V⟩, EtaFamiliesClosed.empty⟩ hf).1

/-- **No proof of `Empty`** is accepted by the shared-state
executable. -/
theorem no_proof_of_Empty_S_R (V : Type w) [SetTheory V] {μ : CheckMode} (hg : μ.betaGate = false)
    {ds : List Declaration} {env' : Env}
    (h : checkDeclsShared μ ds = .ok env')
    (c : ConstantInfo) (hc : c ∈ env'.consts)
    (hty : c.toConstantVal.type = .const emptyName []) : False := by
  obtain ⟨m⟩ := checkDeclsS_sound_R (hg := hg) (V := V) h
  exact no_constant_of_Empty_R m c hc hty
...
theorem foldSP_R
    {μ : CheckMode} (hg : μ.betaGate = false) {st0 : EStore} (hwfst : st0.WF) :
  ...
  | pd :: pds, fe, fe', s₀, s', hfe, hm, hres, hext0, h => by
    ...
    exact foldSP_R (hg := hg) hwfst pds fe₁ hfe₁
      (declStepS (hg := hg) divModPinS reducePinS stdAxiomKeyS declBasisS
        (declIndS memberKeyS) m hE (checkDeclR_sound (hg := hg) m hE hF)) hres₁
      (hext0.trans hext₁) h

/-- **The acceptance theorem for the parsed-index executable.** -/
theorem checkDeclsSP_sound_R {μ : CheckMode} (hg : μ.betaGate = false)
...
theorem no_proof_of_Empty_SP_R (V : Type w) [SetTheory V]
    ...
    (c : ConstantInfo) (hc : c ∈ env'.consts)
    (hty : c.toConstantVal.type = .const emptyName []) : False := by
  obtain ⟨m⟩ := checkDeclsSP_sound_R (hg := hg) (V := V) h
  exact no_constant_of_Empty_R m c hc hty
...
theorem foldlM_no_Empty_R
  ...
  | d :: ds, env, env', hm, h, hd => by
    ...
    · obtain ⟨type, hann, c, hc, hcv⟩ := checkDecl_stores hdd hdis
      ...
      obtain ⟨m1⟩ := (checkDecl_sound_R (hg := hg) (d := d)
        m hE hdd).1
      exact no_constant_of_Empty_R m1 c hc (by rw [hcv])
    · refine foldlM_no_Empty_R (hg := hg) (value := value) (hint := hint)
        hty ds env1
        (checkDecl_sound_R (hg := hg) (d := d) m hE hdd)
        h ?_
  ...
  | d :: ds, env, env', hm, h, hd => by
    ...
    · obtain ⟨type, hann, c, hc, hcv⟩ := checkDecl_stores hF hdis
      ...
      obtain ⟨m1⟩ := (checkDecl_sound_R (hg := hg) (d := d)
        m hE hF).1
      exact no_constant_of_Empty_R m1 c hc (by rw [hcv])
    · refine foldlM_no_Empty_RC (hg := hg) (value := value) (hint := hint)
        hty ds env1
        (checkDecl_sound_R (hg := hg) (d := d) m hE hF) h ?_
      rcases hd with hd | hd
      · rcases List.mem_cons.mp hd with rfl | hmem
        · exact absurd (Or.inl rfl) hdis
        · exact Or.inl hmem
...
theorem no_proof_of_Empty_input_C_R (V : Type w) [SetTheory V]
    ...
    (hd : Declaration.defnDecl cv value hint ∈ ds ∨
      Declaration.thmDecl cv value ∈ ds)
    (hty : cv.type = .const emptyName []) : False :=
  foldlM_no_Empty_RC (hg := hg) hty ds Env.empty
    ⟨⟨EnvS.empty V⟩, EtaFamiliesClosed.empty⟩ h hd
  ...
  | d :: ds, fe, fe', hfe, hm, h, hd => by
    ...
    · obtain ⟨type, hann, c, hc, hcv⟩ := checkDecl_stores hF hdis
      ...
      obtain ⟨m1⟩ := (checkDecl_sound_R (hg := hg) (d := d)
        m hE hF).1
      exact no_constant_of_Empty_R m1 c hc (by rw [hcv])
    · refine foldlM_no_Empty_RS (hg := hg) (value := value) (hint := hint)
        hty ds fe1 hfe1
        (checkDecl_sound_R (hg := hg) (d := d) m hE hF) h ?_
      rcases hd with hd | hd
      · rcases List.mem_cons.mp hd with rfl | hmem
...
theorem no_proof_of_Empty_input_S_R (V : Type w) [SetTheory V]
  ...
  | error e => rw [hf] at h; exact nomatch h
  | ok fe =>
    exact foldlM_no_Empty_RS (hg := hg) hty ds
      (mkFEnv Env.empty) rfl ⟨⟨EnvS.empty V⟩, EtaFamiliesClosed.empty⟩
      hf hd
    ...
    | cons pd pds ih =>
      ...
      · have hd' : (∃ hint, DeclP.defnDecl cvp value hint ∈ pds) ∨
          ...
          · rcases List.mem_cons.mp hdm2 with heq | hmem
            ...
            · exact .inr hmem
        exact ih fe₁ hfe₁
          (checkDecl_sound_R (hg := hg) (d := d) m hE hF)
