...
/-- Pure mirror of `inferLamsOutI` (task #100 stage 3: the former
λ-annotation re-check died with the level-free collapse model; the
rebuild is a pure fold). -/
def inferLamsOut (d : Nat) :
  ...
  | (n, tyo, mb) :: rest, j, cur =>
    ...
    match rest with
    | [] => pure node
    | e' :: rest' => inferLamsOut d (e' :: rest') (j - 1) node
...
(innermost first, `j` the head entry's binder level).  A pure fold
since the λ-annotation re-check died (task #100 stage 3). -/
...
/-- The wrap's value: the pure ∀-telescope fold. -/
def inferLamsFold (d : Nat) : List InferLamEntryX → Nat → Expr → Expr
  | [], _j, bt => bt
  | (n, tyo, mb) :: rest, j, bt =>
    inferLamsFold d rest (j - 1) (.forallE n tyo (bt.abstract1 (d + j)) mb)
      ...
      = (do
        ...
        | .sort _ =>
          inferLams r d fuel body (k + 1)
            ...
            ((n, ty.instantiateList fvs, ⟨bi, some v⟩) :: stk)
...
theorem inferLamsOut_cons (n : Name) (tyo : Expr) (mb : BinderMeta)
    (e' : InferLamEntryX) (rest : List InferLamEntryX)
    (j : Nat) (cur : Expr) :
    inferLamsOut (m := m) d ((n, tyo, mb) :: e' :: rest) j cur
      = inferLamsOut d (e' :: rest) (j - 1)
          (Expr.forallE n (tyo.abstractRange d j) cur mb) := rfl

omit [MonadExceptOf CheckError m] in
theorem inferLamsOut_cons' (n : Name) (tyo : Expr) (mb : BinderMeta)
    (rest : List InferLamEntryX) (j : Nat) (cur : Expr) :
    inferLamsOut (m := m) d ((n, tyo, mb) :: rest) j cur
      = (let node := Expr.forallE n (tyo.abstractRange d j) cur mb
        match rest with
        | [] => pure node
        | e' :: rest' => inferLamsOut d (e' :: rest') (j - 1) node) := by
  cases rest <;> rfl

omit [MonadExceptOf CheckError m] in
theorem inferLamsOut_single (n : Name) (tyo : Expr) (mb : BinderMeta)
    (j : Nat) (cur : Expr) :
    inferLamsOut (m := m) d [(n, tyo, mb)] j cur
...
theorem inferLamsOut_atF (d : Nat) :
  ...
  | [], _, _, _ => rfl
  | (n, tyo, mb) :: rest, j, cur, F => by
    cases rest with
    | nil => rfl
    | cons e' rest' =>
      rw [inferLamsOut_cons, inferLamsOut_cons]
      exact inferLamsOut_atF d (e' :: rest') (j - 1) _ F
...
/-- The pure wrap fold, written as a function (the wrap never fails). -/
theorem inferLamsWrap_eq_pure {d : Nat} :
    ∀ (stk : List InferLamEntryX) (j : Nat) (bt : Expr),
      inferLamsWrap (m := CheckM) d stk j bt
        = .ok (inferLamsFold d stk j bt)
  | [], _, _ => rfl
  | (n, tyo, mb) :: rest, j, bt =>
    inferLamsWrap_eq_pure rest (j - 1)
      (.forallE n tyo (bt.abstract1 (d + j)) mb)
...
/-- A successful out-phase run is reproduced by the chained tails: both
are the same pure ∀-telescope fold (task #100 stage 3: no re-checks
remain). -/
theorem inferLamsOut_wrap {d : Nat} :
    ∀ (stk : List InferLamEntryX) (j : Nat) (bt : Expr) (res : Expr),
      stk.length = j + 1 →
      inferLamsOut (m := CheckM) d stk j (bt.abstractRange d (j + 1))
          = .ok res →
      inferLamsWrap (m := CheckM) d stk j bt = .ok res := by
  intro stk
  induction stk with
  | nil => intro j bt res hlen; exact nomatch hlen
  | cons e rest ih =>
    obtain ⟨n, tyo, mb⟩ := e
    intro j bt res hlen hrun
    cases rest with
    | nil =>
      ...
      rw [h1, h2] at hres
      show Except.ok (Expr.forallE n tyo (bt.abstract1 (d + 0)) mb)
        = Except.ok res
      rw [hres]
    | cons e' rest' =>
      have hj1 : 1 ≤ j := by
      ...
      rw [hnode] at hrun
      exact ih (j - 1) (Expr.forallE n tyo (bt.abstract1 (d + j)) mb) res
...
theorem inferLamsLeaf_sound {d : Nat} {t : Expr}
  ...
  | cons e rest =>
    ...
    have hrun' : inferLamsOut (m := CheckM) d (e :: rest) (k - 1)
        (bt.abstractRange d ((k - 1) + 1)) = .ok res := by
      rw [hkeq]
      exact hrun
    have hwrap := inferLamsOut_wrap (e :: rest) (k - 1) bt res
      (by simp only [List.length_cons] at hlen ⊢; omega) hrun'
    refine ⟨F, ?_⟩
    rw [hbt, okB_bind]
    exact hwrap
  ...
  | succ fuel ihf =>
    ...
    · obtain ⟨n, ty, body, ⟨bi, cod⟩, rfl⟩ := hlam
      ...
      | some v =>
        ...
        -- the wrap's head step is the lam tail (definitional)
