...
**Relocated from `Setlec/Model/BridgeS.lean` by task #148's T1.**  None
of this file mentions `V`: it is a simulation argument between two
*executable* checkers, and it was in `Setlec/Model/*` only because its
last few theorems reach the direct simple-structure path, whose
inversions (`Setlec/Model/DirectDecl.lean`) still live there.  Those
theorems — `checkDirectProjsS_run` onwards, through
`checkDeclSharedF_bridge` — stayed behind in `Setlec/Model/BridgeS.lean`,
which imports this module.  The remainder can be severed the moment the
direct path is (§4 of the #148 design has it scope-cut).
-/
