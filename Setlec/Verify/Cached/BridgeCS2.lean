...
Port of `Setlec/Verify/BridgeS2.lean` for the cached tier.  The
...
`Setlec/Verify/BridgeWfImp.lean`'s `_wfimp` walks; the operation
...
theorem checkTypedListS_sim (henv : EnvWF env) {depth : Nat} :
  ...
  | a :: as, b :: bs, ha, hb, s₀, hs => by
    ...
    refine SimC.bind (opE_infer_sim henv hs (ha a List.mem_cons_self))
      (fun s₁ ty ty' hs₁ hP => ?_)
    obtain ⟨rfl, htyW⟩ := hP
    refine SimC.bind (opB_sim henv hs₁ htyW (hb b List.mem_cons_self))
      (fun s₂ c c' hs₂ hC => ?_)
    obtain rfl : c = c' := hC
    cases c with
    | false =>
...
theorem checkDefEqListS_sim (henv : EnvWF env) {depth : Nat} :
  ...
  | a :: as, b :: bs, ha, hb, s₀, hs => by
    ...
    dsimp only [sharedOpsC]
    refine SimC.bind (opB_sim henv hs (ha a List.mem_cons_self)
...
theorem checkIotaSidesTyS_sim {depth : Nat} {alphaS lhsS rhsS : Expr}
  ...
  dsimp only [sharedOpsC]
  refine SimC.bind (opE_infer_sim henv hs hl)
    (fun s₁ tl tl' hs₁ hTl => ?_)
  obtain ⟨rfl, htlW⟩ := hTl
  refine SimC.bind (opB_sim henv hs₁ htlW hα)
  ...
  | true =>
  simp only [↓reduceIte]
  refine SimC.bind (opE_infer_sim henv hs₂ hr)
    (fun s₃ tr tr' hs₃ hTr => ?_)
  obtain ⟨rfl, htrW⟩ := hTr
  refine SimC.bind (opB_sim henv hs₃ htrW hα)
  ...
  | true =>
  simp only [↓reduceIte]
  refine SimC.bind (opE_infer_sim henv hs₄ hα)
    (fun s₅ tα tα' hs₅ hTα => ?_)
  obtain ⟨rfl, htαW⟩ := hTα
  refine SimC.bind (opB_sim henv hs₅ htαW (by simp [Expr.WScoped]))
...
theorem checkIotaThmS_sim {env' : Env} (henv' : EnvWF env')
  ...
  refine SimC.bind (opB_sim henv hs12 hrhsSW
      (Expr.WScoped.mkAppN
        (Expr.WScoped.of_not_hasFvar (by
          rw [hasFvar_renameConsts]
          exact hrhsA))
...
theorem checkIotaThmNS_sim {env' : Env} (henv' : EnvWF env')
  ...
  rw [Nat.zero_add] at hopenPW
  obtain ⟨hfvsPW, -⟩ := hopenPW
  refine SimC.bind (checkAnnotListS_sim henv
      (fun a ha => by
        obtain ⟨x, hx, rfl⟩ := List.mem_map.mp ha
        exact (instSpine_WScoped (rP - 1)
  ...
  have hcinstPW := instPisAt_WScoped (d := rP) _ _ hcinstP
    ...
    (fun a ha => by
      ...
      exact instSpine_WScoped (rP - 1)
        ...
        (fun a' ha' => hfvsPW a' (List.mem_of_mem_take ha')))
  obtain ⟨hcdomsPW, hcrestPW⟩ := hcinstPW
  refine SimC.bind (checkTypedListS_sim henv
      (fun a ha => by
        obtain ⟨x, hx, rfl⟩ := List.mem_map.mp ha
        exact (instSpine_WScoped (rP - 1)
          (Expr.WScoped.of_not_hasFvar (hpinsF x hx))
  ...
  | true =>
    simp only [↓reduceIte]
    refine SimC.bind (checkIotaSidesTyS_sim henv
...
theorem checkIotaRuleS_sim {env' : Env} (henv' : EnvWF env')
  ...
  case neg => simp only [if_neg h7]; exact SimC.throw_bind
  simp only [if_pos h7]
  refine SimC.bind (opE_infer_sim henv hs₁ hwrhsA)
  ...
  case neg =>
    simp only [if_neg h8]
    refine SimC.bind (checkIotaThmNS_sim henv' henv htyA hctorF
        hrhsAF hs₂)
      (fun s₃ fire fire' hs₃ hP₃ => ?_)
    obtain rfl : fire = fire' := hP₃
    exact SimC.pure hs₃ rfl
  simp only [if_pos h8]
  refine SimC.bind (checkIotaThmS_sim henv' henv htyA hctorF hrhsAF hs₂)
    (fun s₃ u u' hs₃ hP₃ => ?_)
  first
  | exact SimC.pure hs₃ rfl
...
theorem checkIotaRulesS_sim {env' : Env} (henv' : EnvWF env')
  ...
  | j, r :: rest, s₀, hs => by
    ...
    refine SimC.bind (checkIotaRuleS_sim henv' henv htyA hs)
      (fun s₁ r' r'' hs₁ hP => ?_)
    obtain rfl : r' = r'' := hP
    refine SimC.bind (checkIotaRulesS_sim henv' henv htyA hs₁)
...
theorem checkProjRuleS_sim (henv : EnvWF env) {pty : Expr}
  ...
  refine SimC.bind (checkDefEqListS_sim henv
    ...
    (fun s₃ u2 u2' hs₃ hU2 => ?_)
  refine SimC.bind (opE_infer_sim henv hs₃
