...
protected theorem withStore {β α γ : Type} {P : β → α → Prop}
    ...
    SimC mode env s₀ P (Setlec.Cached.withStore f >>= k) p := by
  intro v' s' hr
  apply h v' s'
  simpa only [Setlec.Cached.withStore, Bind.bind, StateT.bind, pure,
...
protected theorem withStore {β γ : Type} {Q : β → Prop}
    ...
    CEff mode env s₀ Q (Setlec.Cached.withStore f >>= k) := by
  intro v' s' hr
  apply h v' s'
  simpa only [Setlec.Cached.withStore, Bind.bind, StateT.bind, pure,
    StateT.pure, Except.pure, Except.bind] using hr
