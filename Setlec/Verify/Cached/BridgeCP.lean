...
Port of the SP layer of `Setlec/Verify/BridgeP.lean` (and, at the end,
of `Setlec/Verify/BridgePDecl.lean`) for the cached tier: each lemma
relates a `ParsedC` driver function (`Setlec/Cached/ParsedC.lean`,
...
* the DAG-memoized syntactic guards are pure `ExprC` walks, not
  ...
  with the `Expr`-side guards is `Setlec/Verify/Cached/GuardsC.lean`'s
...
* `recordCConst`'s effect (`recordCConst_eff`,
  `Setlec/Verify/Cached/SimCEff.lean`) takes `RelC` facts where
...
(`WFc` + `eraseC`) on every `ExprC` slot, equality on the rest.  The
...
`checkDeclSPC` simulate the generic `checkDecl` at the fueled families
