...
Port of `Setlec/Verify/DiscI5.lean` under the recipe (DESIGN.md,
task #163): the simulation walks for `defeqStepI`, `defeqLoopI` and
`defeqBodyI` (`Setlec/Cached/CoreC.lean`), whose bodies are
character-identical to their `Setlec/Kernel/CoreI.lean` originals up to
...
`ExprC.beq_iff` — decided equality on `WFc` nodes *is* equality of the
...
private theorem beq_transferC {i j : ExprC} {a b : Expr}
  ...
  obtain ⟨hwa, rfl⟩ := ha
  obtain ⟨hwb, rfl⟩ := hb
  rw [Bool.eq_iff_iff, beq_iff hwa hwb, beq_iff_eq]
...
private theorem defeqC_etaR_arm (ih : SSimC mode env f) (henv : EnvWF env)
  ...
  | false =>
    ...
    exact stuckIrrelC_sim ih henv hs₁ haS hbS hwa' hwb'

/-- The one-sided-λ (left) stuck arm. -/
private theorem defeqC_etaL_arm (ih : SSimC mode env f) (henv : EnvWF env)
...
theorem defeqStepC_sim (ih : SSimC mode env f) (henv : EnvWF env)
  ...
  · rw [if_neg hab, if_neg hab]
    ...
    · rw [if_neg hab', if_neg hab']
      ...
      refine SimC.bind (proofIrrelC_sim ih hs₂ ha'd hb'd hwa' hwb')
      ...
      rw [hasFvarI_spec ha'd.1 ha'd.2, hasFvarI_spec hb'd.1 hb'd.2]
        ...
        | none =>
          ...
          | none =>
            ...
            | none =>
              ...
              refine SimC.withStore ?_
              refine SimC.withStore ?_
              rw [unfoldableHeadI_spec ha'd.1 ha'd.2,
                unfoldableHeadI_spec hb'd.1 hb'd.2]
              ...
              | true =>
                ...
                | true =>
                  ...
                  refine SimC.withStore ?_
                  refine SimC.withStore ?_
                  rw [headHintI_spec ha'd.1 ha'd.2,
                    headHintI_spec hb'd.1 hb'd.2]
                  ...
                  · rw [if_neg hlt₁, if_neg hlt₁]
                    ...
                    · rw [if_neg hlt₂, if_neg hlt₂]
                      refine SimC.withStore ?_
                      rw [sameConstHeadsI_spec ha'd.1 hb'd.1 ha'd.2 hb'd.2]
              ...
              | false =>
                ...
                have hbS := hb'd
                have hs₆ := hs₄
                obtain ⟨hwca, rfl⟩ := ha'd
                obtain ⟨hwcb, rfl⟩ := hb'd
                refine SimC.view ?_
                refine SimC.view ?_
                cases a' with
                | sort u₁ hh₁ bb₁ fb₁ lp₁ =>
                  ...
                  | lam nm₂ t₂ b₂ m₂ hh₂ bb₂ fb₂ lp₂ =>
                    dsimp only [eraseC, ExprC.view]
                    obtain ⟨hwt₂, hwb₂, -⟩ := hwcb.lam_inv
                    exact defeqC_etaR_arm ih henv hs₆ haS ⟨hwt₂, rfl⟩
                      ⟨hwb₂, rfl⟩ hbS hwa' hwb'
                  | _ =>
                    dsimp only [eraseC, ExprC.view]
                    exact stuckIrrelC_sim ih henv hs₆ haS hbS hwa' hwb'
                  ...
                  | natVal n₁ =>
                    ...
                    | const c₂ us₂ hh₂ bb₂ fb₂ lp₂ =>
                      ...
                      · rw [if_neg hz, if_neg hz]
                        exact stuckIrrelC_sim ih henv hs₆b haS hbS hwa' hwb'
                    | app f₂ x₂ hh₂ bb₂ fb₂ lp₂ =>
                      obtain ⟨hwf₂, hwx₂, -⟩ := hwcb.app_inv
                      have hwb'' : Expr.WScoped d
                        (Expr.app (eraseC f₂) (eraseC x₂)) := hwb'
                      have h2 : Expr.WScoped d (eraseC f₂) ∧
                          Expr.WScoped d (eraseC x₂) := by
                        simpa only [Expr.WScoped] using hwb''
                      dsimp only [eraseC, ExprC.view]
                      refine SimC.view ?_
                      cases n₁ with
                          ...
                          | nil =>
                            ...
                            · rw [if_pos hsc, if_pos hsc]
                              refine SimC.bind_left (internI_eff hs₆b
                                (n := ExprView.lit (.natVal k)) trivial)
                                (fun s₇ kl hs₇ hQk => ?_)
                              have hQk' : RelC kl
                                (Expr.lit (.natVal k)) := hQk
                              exact ih.defeq hs₇ hQk' ⟨hwx₂, rfl⟩
                                (by simp [Expr.WScoped]) h2.2
                            · rw [if_neg hsc, if_neg hsc]
                              exact stuckIrrelC_sim ih henv hs₆b haS hbS
                                hwa' hwb'
                        | _ =>
                          dsimp only [eraseC, ExprC.view]
                          exact stuckIrrelC_sim ih henv hs₆ haS hbS hwa' hwb'
                ...
                | forallE nm₁ t₁ b₁ m₁ hh₁ bb₁ fb₁ lp₁ =>
                  obtain ⟨hwt₁, hwb₁, -⟩ := hwca.forallE_inv
                  have hwa'' : Expr.WScoped d
                    (Expr.forallE nm₁ (eraseC t₁) (eraseC b₁) m₁) := hwa'
                  have h1 : Expr.WScoped d (eraseC t₁) ∧
                      Expr.WScoped d (eraseC b₁) := by
                    simpa only [Expr.WScoped] using hwa''
                  cases b' with
                  | forallE nm₂ t₂ b₂ m₂ hh₂ bb₂ fb₂ lp₂ =>
                    dsimp only [eraseC, ExprC.view]
                    obtain ⟨hwt₂, hwb₂, -⟩ := hwcb.forallE_inv
                    have hwb'' : Expr.WScoped d
                      (Expr.forallE nm₂ (eraseC t₂) (eraseC b₂) m₂) := hwb'
                    have h2 : Expr.WScoped d (eraseC t₂) ∧
                        Expr.WScoped d (eraseC b₂) := by
                      simpa only [Expr.WScoped] using hwb''
                    refine SimC.bind
                      (ih.defeq hs₆ ⟨hwt₁, rfl⟩ ⟨hwt₂, rfl⟩ h1.1 h2.1)
                      (fun s₇ r₁ r₁' hs₇ hP₁ => ?_)
                    obtain rfl : r₁ = r₁' := hP₁
                    cases r₁ with
                    | false =>
                    ...
                    | true =>
                      simp only [↓reduceIte]
                      refine SimC.bind_left
                        (internI_eff hs₇ (n := ExprView.fvar d nm₁ t₁) hwt₁)
                        (fun s₈ fv₁ hs₈ hQf₁ => ?_)
                      have hQf₁' : RelC fv₁
                        (Expr.fvar d nm₁ (eraseC t₁)) := hQf₁
                      refine SimC.bind_left
                        (inst1M_eff hs₈ ⟨hwb₁, rfl⟩ hQf₁')
                        (fun s₉ ob₁ hs₉ hQo₁ => ?_)
                      refine SimC.bind_left
                        (internI_eff hs₉ (n := ExprView.fvar d nm₂ t₂) hwt₂)
                        (fun s₁₀ fv₂ hs₁₀ hQf₂ => ?_)
                  ...
                  | _ =>
                    ...
                    exact stuckIrrelC_sim ih henv hs₆ haS hbS hwa' hwb'
                | lam nm₁ t₁ b₁ m₁ hh₁ bb₁ fb₁ lp₁ =>
                  obtain ⟨hwt₁, hwb₁, -⟩ := hwca.lam_inv
                  have hwa'' : Expr.WScoped d
                    (Expr.lam nm₁ (eraseC t₁) (eraseC b₁) m₁) := hwa'
                  have h1 : Expr.WScoped d (eraseC t₁) ∧
                      Expr.WScoped d (eraseC b₁) := by
                    simpa only [Expr.WScoped] using hwa''
                  cases b' with
                  | lam nm₂ t₂ b₂ m₂ hh₂ bb₂ fb₂ lp₂ =>
                    ...
                    obtain ⟨hwt₂, hwb₂, -⟩ := hwcb.lam_inv
                    have hwb'' : Expr.WScoped d
                      (Expr.lam nm₂ (eraseC t₂) (eraseC b₂) m₂) := hwb'
                    have h2 : Expr.WScoped d (eraseC t₂) ∧
                        Expr.WScoped d (eraseC b₂) := by
                      simpa only [Expr.WScoped] using hwb''
                    refine SimC.bind
                  ...
                  | _ =>
                    dsimp only [eraseC, ExprC.view]
                    exact defeqC_etaL_arm ih henv hs₆ haS ⟨hwt₁, rfl⟩
                      ⟨hwb₁, rfl⟩ hbS hwa' hwb'
                | app f₁ x₁ hh₁ bb₁ fb₁ lp₁ =>
                  obtain ⟨hwf₁, hwx₁, -⟩ := hwca.app_inv
                  have hwa'' : Expr.WScoped d
                    (Expr.app (eraseC f₁) (eraseC x₁)) := hwa'
                  have h1 : Expr.WScoped d (eraseC f₁) ∧
                      Expr.WScoped d (eraseC x₁) := by
                    simpa only [Expr.WScoped] using hwa''
                  cases b' with
                  | app f₂ x₂ hh₂ bb₂ fb₂ lp₂ =>
                    ...
                    refine SimC.withStore ?_
                    have hAA : RelCL
                        (ExprC.getAppArgs (ExprC.app f₁ x₁ hh₁ bb₁ fb₁ lp₁))
                        (Expr.app (eraseC f₁) (eraseC x₁)).getAppArgs :=
                      ExprC.getAppArgs_spec hwca
                    have hBB : RelCL
                        (ExprC.getAppArgs (ExprC.app f₂ x₂ hh₂ bb₂ fb₂ lp₂))
                        (Expr.app (eraseC f₂) (eraseC x₂)).getAppArgs :=
                      ExprC.getAppArgs_spec hwcb
                    have hlena :
                        (ExprC.getAppArgs
                          (ExprC.app f₁ x₁ hh₁ bb₁ fb₁ lp₁)).length
                        = (Expr.app (eraseC f₁) (eraseC x₁)).getAppArgs.length :=
                      RelCL.length hAA
                    have hlenb :
                        (ExprC.getAppArgs
                          (ExprC.app f₂ x₂ hh₂ bb₂ fb₂ lp₂)).length
                    ...
                    · rw [if_pos hlen, if_pos hlen]
                      ...
                      refine SimC.bind (ih.defeq hs₆ ⟨hwfa, hfa⟩ ⟨hwfb, hfb⟩
                        ...
                        | const cf usf hhf bbf fbf lpf =>
                          ...
                          | nil =>
                            ...
                            · rw [if_pos hsc, if_pos hsc]
                              ...
                              have hQk' : RelC kl
                                (Expr.lit (.natVal k)) := hQk
                              exact ih.defeq hs₇ ⟨hwx₁, rfl⟩ hQk' h1.2
                ...
                | letE nm₁ t₁ v₁ b₁ hh₁ bb₁ fb₁ lp₁ =>
                  ...
                  | _ =>
                    dsimp only [eraseC, ExprC.view]
                    exact stuckIrrelC_sim ih henv hs₆ haS hbS hwa' hwb'
                | proj s₁' j₁ e₁ hh₁ bb₁ fb₁ lp₁ =>
                  obtain ⟨hwe₁, -⟩ := hwca.proj_inv
                  have hwa'' : Expr.WScoped d
                    (Expr.proj s₁' j₁ (eraseC e₁)) := hwa'
                  have h1 : Expr.WScoped d (eraseC e₁) := by
                    simpa only [Expr.WScoped] using hwa''
                  cases b' with
                  | proj s₂' j₂ e₂ hh₂ bb₂ fb₂ lp₂ =>
                    dsimp only [eraseC, ExprC.view]
                    obtain ⟨hwe₂, -⟩ := hwcb.proj_inv
                    have hwb'' : Expr.WScoped d
                      (Expr.proj s₂' j₂ (eraseC e₂)) := hwb'
                    have h2 : Expr.WScoped d (eraseC e₂) := by
                      simpa only [Expr.WScoped] using hwb''
                    by_cases hjj : (j₁ == j₂) = true
                    · rw [if_pos hjj, if_pos hjj]
                      refine SimC.bind
                        (ih.defeq hs₆ ⟨hwe₁, rfl⟩ ⟨hwe₂, rfl⟩ h1 h2)
                        (fun s₇ r₁ r₁' hs₇ hP₁ => ?_)
                      obtain rfl : r₁ = r₁' := hP₁
                      cases r₁ with
                      | true =>
