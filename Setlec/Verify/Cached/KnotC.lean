...
corresponding fueled family on `WFc`, well-scoped inputs.  This module
...
theorem CSOK.insertWhnfCoreC {s : CState} (hs : CSOK mode env s)
    {i j : ExprC} (hj : WFc j)
    (hrun : ∃ F, ∀ d, (eraseC i).wscopedB d = true →
      whnfCore mode env F d (eraseC i) = .ok (eraseC j)) :
...
theorem CSOK.insertWhnfC {s : CState} (hs : CSOK mode env s)
    {i j : ExprC} (hj : WFc j)
    (hrun : ∃ F, ∀ d, (eraseC i).wscopedB d = true →
      whnf mode env F d (eraseC i) = .ok (eraseC j)) :
...
theorem CSOK.insertInferC {s : CState} (hs : CSOK mode env s)
    {i j : ExprC} (hj : WFc j)
    (hrun : ∃ F, ∀ d, (eraseC i).wscopedB d = true →
      inferTypeCore mode env F d (eraseC i) = .ok (eraseC j)) :
...
theorem CSOK.insertAnnotC {s : CState} (hs : CSOK mode env s)
    {i j : ExprC} (hj : WFc j)
    (hrun : ∃ F, ∀ d, (eraseC i).wscopedB d = true →
      annotateCore mode env F d (eraseC i) = .ok (eraseC j)) :
...
theorem CSOK.insertDefeqC {s : CState} (hs : CSOK mode env s)
    {i j : ExprC} {r : Bool}
    (hrun : ∃ F, ∀ d, (eraseC i).wscopedB d = true →
      (eraseC j).wscopedB d = true →
      isDefEqCore mode env F d (eraseC i) (eraseC j) = .ok r) :
...
theorem memoEI_whnfCore_sim (henv : EnvWF env)
  ...
  | some j =>
    ...
    obtain ⟨hwfj, F, hall⟩ := hs.whnfCoreC i _ hl
    have hrun := hall d hw.to_wscopedB
    exact ⟨hs, eraseC j, ⟨⟨hwfj, rfl⟩, whnfCore_WScoped henv F hrun hw⟩,
      F, hrun⟩
  | none =>
    rw [hl] at hr
    ...
    | ok pr =>
      ...
      obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. ▸ hr
      obtain ⟨hs₁, v, ⟨⟨hwfr, rfl⟩, hwv⟩, F, hF⟩ := hbody hs hden hw r s₁ hb
      rw [whnfCoreBody_atF] at hF
      rw [← whnfCore_succ] at hF
      have hins := hs₁.insertWhnfCoreC hwfr
        ⟨F + 1, fun d' hd' => by
          rw [whnfCore_depth_inv henv (F + 1) hd' hw.to_wscopedB]
          exact hF⟩
      exact ⟨hins, eraseC r, ⟨⟨hwfr, rfl⟩, hwv⟩, F + 1, hF⟩
...
theorem memoEI_whnf_sim (henv : EnvWF env)
  ...
  | some j =>
    ...
    simp only [pure, StateT.pure, Except.pure, Except.ok.injEq] at hr
    obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. ▸ hr
    obtain ⟨hwfj, F, hall⟩ := hs.whnfC i _ hl
    have hrun := hall d hw.to_wscopedB
    exact ⟨hs, eraseC j, ⟨⟨hwfj, rfl⟩, whnf_WScoped henv F hrun hw⟩, F, hrun⟩
  | none =>
    rw [hl] at hr
    try dsimp only at hr
    try simp only [StateT.bind] at hr
...
theorem memoEI_infer_sim (henv : EnvWF env)
  ...
  | some j =>
    ...
    simp only [pure, StateT.pure, Except.pure, Except.ok.injEq] at hr
    obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. ▸ hr
    obtain ⟨hwfj, F, hall⟩ := hs.inferC i _ hl
    have hrun := hall d hw.to_wscopedB
    exact ⟨hs, eraseC j,
      ⟨⟨hwfj, rfl⟩, inferTypeCore_WScoped henv F hrun hw⟩, F, hrun⟩
  | none =>
    rw [hl] at hr
    try dsimp only at hr
    try simp only [StateT.bind] at hr
    ...
    | ok pr =>
      ...
      have hins := hs₁.insertInferC hwfr
        ⟨F + 1, fun d' hd' => by
...
theorem memoEI_annotate_sim (henv : EnvWF env)
  ...
  | some j =>
    ...
    simp only [pure, StateT.pure, Except.pure, Except.ok.injEq] at hr
    obtain ⟨rfl, rfl⟩ := Prod.mk.injEq .. ▸ hr
    obtain ⟨hwfj, F, hall⟩ := hs.annotC i _ hl
    have hrun := hall d hw.to_wscopedB
    exact ⟨hs, eraseC j,
      ⟨⟨hwfj, rfl⟩, annotateCore_WScoped F _ hrun hw⟩, F, hrun⟩
  | none =>
    rw [hl] at hr
    try dsimp only at hr
    try simp only [StateT.bind] at hr
    ...
    | ok pr =>
      ...
      have hins := hs₁.insertAnnotC hwfr
        ⟨F + 1, fun d' hd' => by
...
theorem memoBI_defeq_sim (henv : EnvWF env)
  ...
  have hdena : RelC i (eraseC i) := ⟨hwca, rfl⟩
