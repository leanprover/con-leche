...
theorem memoEI_whnfCore_sim (henv : EnvWF env)
  ...
  | some j =>
    ...
    exact ⟨hs, j, ⟨⟨hwfj, rfl⟩, whnfCore_WScoped henv F hrun hw⟩,
      F, hrun⟩
  | none =>
    rw [hl] at hr
    ...
    | ok pr =>
      ...
      have hins := hs₁.insertWhnfCoreC hwfr
        ⟨F + 1, fun d' hd' => by
          ...
          exact hF⟩
      exact ⟨hins, r, ⟨⟨hwfr, rfl⟩, hwv⟩, F + 1, hF⟩
...
theorem memoEI_whnf_sim (henv : EnvWF env)
  ...
  | some j =>
    ...
    have hrun := hall d hw.to_wscopedB
    exact ⟨hs, j, ⟨⟨hwfj, rfl⟩, whnf_WScoped henv F hrun hw⟩, F, hrun⟩
  | none =>
    rw [hl] at hr
    try dsimp only at hr
    try simp only [StateT.bind] at hr
