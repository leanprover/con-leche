...
Port of `Setlec/Verify/SimS.lean` for the cached tier.  The cached core
simulation (`CSOK`/`SimC`, `Setlec/Verify/Cached/SimC.lean`, knotted in
`Setlec/Verify/Cached/KnotC.lean`) is stated for *arbitrary* initial
...
* `mkFEnv_push` (`Setlec/Verify/SimS.lean`) is `Expr`-level — the index
  of a cons-extended environment is one insert, definitionally, with no
  reference to any state representation;
* `flushC_csok` (`Setlec/Verify/Cached/SimC.lean`) is the `flushS_isok`
...
`Setlec/Verify/Cached/BridgeCS*.lean`.
-/
