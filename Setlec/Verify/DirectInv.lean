...
/-! ## Stage 5: a projection entry -/

theorem checkDirectProjEntry_shape {env envOut : Env} {T C : Name}
    ...
    ∃ ptyA : Expr,
      ...
      (∃ sty u, inferTypeCore mode env F 0 ptyA = .ok sty ∧
  ...
  case neg => rw [if_neg h3] at h; close_throw
  rw [if_pos h3] at h
  obtain ⟨u0, -, h⟩ := exceptBind_ok h
  obtain ⟨q1, hop, h⟩ := exceptBind_ok h
  obtain ⟨fvsP, prest⟩ := q1
  have hop' := unwrapOr_ok hop
  try simp only at h
  obtain ⟨q2, hsb, h⟩ := exceptBind_ok h
  obtain ⟨sbs, sbody⟩ := q2
  have hsb' := unwrapOr_ok hsb
  try simp only at h
  obtain ⟨sdom, hsd, h⟩ := exceptBind_ok h
  have hsd' := unwrapOr_ok hsd
  obtain ⟨b1, hb1, h⟩ := exceptBind_ok h
  cases b1 with
  | false =>
    simp only [Bool.false_eq_true, ↓reduceIte] at h
  ...
  rw [if_pos rfl] at h
  try simp only at h
  obtain ⟨q3, hot, h⟩ := exceptBind_ok h
  obtain ⟨tFvs, resid⟩ := q3
  have hot' := unwrapOr_ok hot
  try simp only at h
  ...
  have hci' := unwrapOr_ok hci
  try simp only at h
  obtain ⟨fdom, hfd, h⟩ := exceptBind_ok h
  have hfd' := unwrapOr_ok hfd
