...
theorem iotaCertsGIAux_sim (ih : SSimI env f) {d : Nat} :
    ∀ {args : List EIdx} {xs : List Expr} {acc : List EIdx}
      ...
      DenL s₀.store args xs → (∀ x ∈ xs, WScoped d x) →
      SimAt env s₀ RelV
        (iotaCertsGIAux (coreKnotI (mkFEnv env) f) (mkFEnv env) d ty acc args)
        (iotaCertsG (fueledFns env) env d (tyx.instantiateList ws) xs)
  | [], xs, acc, ws, ty, tyx, s₀, hs, hty, hacc, hwty, hargs, hwargs => by
    match xs, hargs with
    | [], _ =>
    ...
    | x :: xs, ⟨hax, hasxs⟩ =>
      ...
      | forallE nm t b m =>
        ...
        have hwtb : WScoped d (et.instantiateList ws)
          ...
          simpa only [WScoped] using hwty
        show SimAt env s₀ RelV
          (codNonZeroIM m >>= fun g =>
            if g then
              iotaCertsGIAux (coreKnotI (mkFEnv env) f) (mkFEnv env) d b
                (a :: acc) as
            else
              instListM t acc >>= fun dom' =>
              (coreKnotI (mkFEnv env) f).infer d a >>= fun ta =>
              (coreKnotI (mkFEnv env) f).defeq d ta dom' >>= fun r =>
              if r then iotaCertsGIAux (coreKnotI (mkFEnv env) f)
                (mkFEnv env) d b (a :: acc) as
              else pure false)
          (if codNonZero bm then
            iotaCertsG (fueledFns env) env d
              ((eb.instantiateList ws 1).instantiate1 x) xs
          else
            (fueledFns env).infer d x >>= fun ta =>
            (fueledFns env).defeq d ta (et.instantiateList ws) >>= fun r =>
            if r then iotaCertsG (fueledFns env) env d
              ((eb.instantiateList ws 1).instantiate1 x) xs
            else pure false)
        have hwx : WScoped d x := hwargs x (List.mem_cons_self ..)
        have hwsub : WScoped d
            ((eb.instantiateList ws 1).instantiate1 x) :=
          WScoped.instantiate1_gen hwx 0 hwtb.2
      ...
      | bvar k =>
        ...
        | a' :: acc', w :: ws', hacc =>
          show SimAt env s₀ RelV
            (instListM ty (a' :: acc') >>= fun ty' =>
              iotaCertsGIAux (coreKnotI (mkFEnv env) f) (mkFEnv env)
                d ty' [] (a :: as))
            (iotaCertsG (fueledFns env) env d
              ((Expr.bvar k).instantiateList (w :: ws')) (x :: xs))
          refine SimAt.bind_left (instListM_eff (d := 0) hs hty hacc)
...
decreasing_by
  all_goals first
    ...
    | (apply Prod.Lex.right' <;> first | (rw [hacceq]; simp) | simp)

/-- Twin sim for the gated bulk loop at the empty accumulator. -/
theorem iotaCertsGI_sim (ih : SSimI env f) {d : Nat} :
  ...
  intro args xs ty tyx s₀ hs hty hwty hargs hwargs
  have := iotaCertsGIAux_sim ih (acc := []) (ws := []) hs hty DenL.nil
    (by rw [Expr.instantiateList_nil]; exact hwty) hargs hwargs
  rw [Expr.instantiateList_nil] at this
  exact this
