...
Relocated verbatim from `Setlec/TTVerify/Extend.lean` (task #148, T5;
the same move T3 made for `Setlec/Verify/Denote/Levels.lean`): the
...
`Setlec.TTVerify` so no call site moves.
...
(Relocated from `Setlec/TTVerify/DeclValue.lean`; `EnvTT.defn_eq` — and
