...
(`Setlec/TTVerify/Extend.lean`, at `ValParams`, `natLitT_params`,
...
`RenEqT`/`PiDomsRenEqT` restate `Setlec/Model/TeleElim.lean`'s `RenEq`
and `PiDomsRenEq`.  They are `V`-free and belong in `Setlec/Verify/*`
