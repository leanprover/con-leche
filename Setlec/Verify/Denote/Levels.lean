...
Relocated out of `Setlec/TTVerify/Extend.lean` (task #148, T3): this
...
`hparams`, the delta step) and the `Setlec/SetR/*` bridge need them, so
...
Transpose of `interp_params_ext` (`Setlec/Model/InterpLemmas.lean`),
