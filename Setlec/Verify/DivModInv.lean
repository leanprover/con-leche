...
valuation.  All of it was written in `Setlec/Model/DivModCert.lean`
...
inversion belongs in `Setlec/Verify`**, so both verification paths can
consume it instead of restating it.

`Setlec/Model/DivModCert.lean` imports this file; the valuation-carrying
