...
theorem eqFormerKeyP {env : Env} (mp : EnvS2PM V μ env)
  ...
  have hmem := Setlec.SetR.Env.find?_mem heqfE
  have hname := Setlec.SetR.Env.find?_name heqfE
...
theorem fireP {m : EnvS2Core V env} {ψ' : Name → Nat}
    ...
    (hfit : TeleFitPA V ρ Tstmt zs
      (Setlec.SetR.AVExpr.instSeq zs (K - 1) Rbody)) :
    ∃ vα vL vR : AVExpr,
      denoteP m.acval env ψ' K αS = some vα ∧
      denoteP m.acval env ψ' K lhsS = some vL ∧
      denoteP m.acval env ψ' K rhsS = some vR ∧
  ...
  have hbit : pwBit ψ' Setlec.PropWhen.never = 1 := rfl
    ...
    have hinst : eqA.toConstantVal.type.instantiateLevelParams
        ...
        = .forallE (.str .anonymous "α") (.sort ℓA)
            (.forallE (.str .anonymous "a") (.bvar 0)
              ...
              ⟨.default, .never⟩) ⟨.implicit, .never⟩ := rfl
  ...
  have hresid : interp2 V ρ
      (Setlec.SetR.AVExpr.instSeq zs (K - 1)
        (AVExpr.mkAppN vEq [vα, vL, vR]))
    ...
    show interp2 V ρ (AVExpr.mkAppN vEq
        [Setlec.SetR.AVExpr.instSeq zs (zs.length - 1) vα,
         Setlec.SetR.AVExpr.instSeq zs (zs.length - 1) vL,
         Setlec.SetR.AVExpr.instSeq zs (zs.length - 1) vR]) = _
    show SetTheory.app (SetTheory.app (SetTheory.app
        (interp2 V ρ vEq)
        (interp2 V ρ (Setlec.SetR.AVExpr.instSeq zs (zs.length - 1) vα)))
        (interp2 V ρ (Setlec.SetR.AVExpr.instSeq zs (zs.length - 1) vL)))
        (interp2 V ρ (Setlec.SetR.AVExpr.instSeq zs (zs.length - 1) vR))
      = _
