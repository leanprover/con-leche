...
theorem ne_of_notReserved {n m : Name}
    (h : Setlec.reservedBasisNames.contains n = false)
    (hm : Setlec.reservedBasisNames.contains m = true) : n ≠ m := by
  intro hh
  rw [hh, hm] at h
  exact nomatch h

theorem reserved_natName : Setlec.reservedBasisNames.contains natName = true := by
  decide

theorem reserved_natZeroName :
    Setlec.reservedBasisNames.contains natZeroName = true := by decide

theorem reserved_natSuccName :
    Setlec.reservedBasisNames.contains natSuccName = true := by decide
