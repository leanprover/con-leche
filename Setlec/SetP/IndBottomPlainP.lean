...
`Install/IndBottomPlainS.lean` transposed to `denoteP`/`interp2`: the
checked canonical `iota_j` theorem, fired at an arbitrary fitting
spine, yields the stored `.plain` rule's `RecRuleLawP` law — the
interp-equality of redex and reduct together with the truthfulness
transport.

The seven stages compose exactly as in v1 (`zipperP` ∘ `fireP` ∘
`reductP` ∘ `pointP`, then `annotPFrameEqP` ∘ `annotMemP` ∘
`annotTransportP`), with the four position ladders
(`prefixGradeFireP`, `paramGradeFireP`, `plainParamSupplyP`,
`fieldGradeFireP`) feeding the zipper and the point stage the gradings
...
theorem indBottomPlainP {μ : CheckMode} {env : Env}
  ...
  have hTVj0 : denoteP mp.base2.acval env (Level.substFn φ lps us) 0
  ...
  have hTVjK : denoteP mp.base2.acval env (Level.substFn φ lps us)
    ...
    refine denoteP_depth_of_closed mp.base2.acval_closed hCwR hTVjcl
      ?_ (rP + cnF)
  ...
  have hmixPar : ∀ q, q < cnP →
      interp2 V ρ ((xs.take cnP ++ ys.drop cnP).getD q default)
  ...
  have hpadEnt : ∀ N, N ≤ rP + cnF → ∀ i, i < N →
    ...
    · rw [List.getElem?_eq_none_iff] at hg; omega
    · rfl
  have hTVjPd : denoteP mp.base2.acval env (Level.substFn φ lps us)
      (rP + cnF) cvj.type = some TVja :=
    denoteP_depth_of_closed mp.base2.acval_closed hCw hTVjcl hTVj0
      (rP + cnF)
  have hparZ : ∀ N, rP ≤ N → N ≤ rP + cnF → ∀ q, q < cnP →
    ...
    intro N hrPN hN
    exact plainParamSupplyP (hdeq _) hplainLe hrPN hfvslen hshapeS
  ...
  have hbα : αS.looseBVarsBounded 0 = true :=
    Setlec.looseBVarsBounded_getAppArgs hbBody αS hmemα
  have hbL : lhsS.looseBVarsBounded 0 = true :=
    Setlec.looseBVarsBounded_getAppArgs hbBody lhsS hmemL
  have hbR : rhsS.looseBVarsBounded 0 = true :=
    Setlec.looseBVarsBounded_getAppArgs hbBody rhsS hmemR
  ...
  have hCfb : Expr.fvarsBelow rP cvj.type :=
    Expr.fvarsBelow_of_fvarLeaves (fun l hl => by
      ...
      exact nomatch hl)
  have hTVjrP : denoteP mp.base2.acval env (Level.substFn φ lps us) rP
      cvj.type = some TVja :=
    denoteP_depth_of_closed mp.base2.acval_closed hCw hTVjcl hTVj0 rP
  ...
  have hΓsBnd : ∀ m, m < rP + cnF →
      VExpr.bvarsBelow m (Γs.getD (rP + cnF - 1 - m) default).erase := by
    intro m hm
  ...
  obtain ⟨bvs, hbvslen, hbvsel⟩ : ∃ bvs : List AVExpr,
      bvs.length = rP + cnF ∧
      ∀ k, k < rP + cnF →
        bvs[k]? = some (AVExpr.bvar (rP + cnF - 1 - k)) := by
    refine ⟨(List.range (rP + cnF)).map
        (fun j => AVExpr.bvar (rP + cnF - 1 - j)),
      by rw [List.length_map, List.length_range], fun k hk => ?_⟩
    rw [List.getElem?_map, List.getElem?_range hk]
    rfl
  have hbvsgetD : ∀ k, k < rP + cnF →
      bvs.getD k default = AVExpr.bvar (rP + cnF - 1 - k) := by
    intro k hk
    rw [List.getD, hbvsel k hk]
    rfl
  have hspBvs : DenoteSpineP mp.base2.acval env (Level.substFn φ lps us)
      (rP + cnF) fvs bvs := by
  ...
  have hRaK : denoteP mp.base2.acval env (Level.substFn φ lps us)
      (rP + cnF) (rhsA.renameConsts f) = some Ra := by
    refine denoteP_depth_of_closed mp.base2.acval_closed hrhsRw hRacl
      ?_ (rP + cnF)
    rw [denoteP_renameConsts hroT]
    exact hRaden
  have hbaEq : denoteP mp.base2.acval env (Level.substFn φ lps us)
      (rP + cnF) (Expr.mkAppN (rhsA.renameConsts f) fvs)
      = some (AVExpr.mkAppN Ra bvs) := denoteP_mkAppN_of fvs hRaK hspBvs
  have hokApp : ∀ σ : Nat → V, Sat2 V Γs σ → ∀ ba : AVExpr,
    ...
    have hsatB : Sat2 V Γs (chainP V σ bvs) := by
      ...
      obtain rfl : Aa = Γs.getD i default := by
        rw [List.getD, hi]
        rfl
      have hbnd : VExpr.bvarsBelow (rP + cnF - 1 - i)
      ...
      rw [hchainbvs σ i hiK,
        interp2_congr_below V (Γs.getD i default) (rP + cnF - 1 - i)
    ...
    have hmemZB : ∀ k, k < rP + cnF →
      ...
      rw [interp2_congr_below V (Γs.getD (rP + cnF - 1 - k) default) k
        ...
        (fun j hj => by
          rw [chainP_lt (by rw [htklen]; omega), htklen,
            show (bvs.take k).getD (k - 1 - j) default
              = AVExpr.bvar (rP + cnF - 1 - (k - 1 - j)) from by
    ...
    exact annotTransportP (hdeq (Level.substFn φ lps us)) hPlen hPshape
      ...
      (fun w hw => by
        ...
        obtain rfl : w = AVExpr.bvar (rP + cnF - 1 - q) :=
  ...
  have hconstDenC : denoteP mp.base2.acval env (Level.substFn φ lps us)
    ...
    rw [denoteP_const hfCmE (by rw [hCmlps, List.length_map]), hCmlps,
  ...
  have hspMem : ∀ σ : Nat → V, Sat2 V Γs σ →
    ...
    · have h := hparZ (rP + cnF) (by omega) (Nat.le_refl _) q hqc σ
      ...
      rw [hgq] at h
      exact h
    · -- a field position: the opener's own `Sat2` slot, crossed by the
