...
named the fourth's single blocker — the `ReduceOpsP` field, which
`Interp2/ReduceOpsP.lean` now supplies.  This file is the branch, and
...
* **the innermost fibre is where the new field pays.**  The hypothesis
  ...
  `eq_lawP` gives both values and `ReduceOpsP` collapses `op x` to `x`,
...
(`Interp2/FoldP.lean`, where `AxiomStepPB` is stated) assembles the
...
theorem ofReduce_shapeS {n : Name} {type' : Expr}
    (hn : n = Setlec.ofReduceNatName ∨ n = Setlec.ofReduceBoolName)
    (h : type'.erasePw.eraseNames
      = (Setlec.ofReducePinA n).type.erasePw.eraseNames) :
    ∃ n₁ n₂ n₃ m₁ m₂ m₃,
      type' = .forallE n₁
        (.const (Setlec.reduceElemName (Setlec.ofReduceOp n)) [])
        (.forallE n₂
          (.const (Setlec.reduceElemName (Setlec.ofReduceOp n)) [])
          (.forallE n₃
            (.app (.app (.app (.const eqName [.succ .zero])
                (.const (Setlec.reduceElemName
                  (Setlec.ofReduceOp n)) []))
              (.app (.const (Setlec.ofReduceOp n) []) (.bvar 1)))
              (.bvar 0))
            (.app (.app (.app (.const eqName [.succ .zero])
                (.const (Setlec.reduceElemName
                  (Setlec.ofReduceOp n)) []))
              (.bvar 2)) (.bvar 1)) m₃) m₂) m₁ := by
  rw [Setlec.TTVerify.ofReducePin_type hn] at h
...
type carries bit `0`, at every assignment.  `propext_bitsP` verbatim:
...
theorem ofReduce_bitsP (hμ : μ.verified = true)
  ...
  match F2, hbt2 with
  | 0, hbt2 => rw [Setlec.inferTypeCore_zero] at hbt2; exact nomatch hbt2
  | F3 + 1, hbt2 =>
  obtain ⟨tty3, u3, bt3, v3, -, -, hbt3, hens3, hpw3, rfl⟩ :=
    Setlec.inferTypeCore_forall_inv hbt2
...
theorem ofReduce_gatesS {cvA : ConstantVal}
    (hok : Setlec.ofReduceAxOk env cvA = true) :
    env.find? eqName = some eqA ∧
    Setlec.reduceElemOk env (Setlec.ofReduceOp cvA.name) = true ∧
    (∃ cvR, env.find? (Setlec.ofReduceOp cvA.name)
        = some (.axiomInfo cvR) ∧
      ConstantVal.matchesPin cvR
        (Setlec.reduceOpCvA (Setlec.ofReduceOp cvA.name)) = true) ∧
    cvA.type.erasePw.eraseNames
      = (Setlec.ofReducePinA cvA.name).type.erasePw.eraseNames := by
  simp only [Setlec.ofReduceAxOk, Bool.and_eq_true,
    decide_eq_true_eq] at hok
  obtain ⟨⟨⟨hEq, helem⟩, hstored⟩, hpin⟩ := hok
  refine ⟨hEq, helem, ?_, ?_⟩
  · rw [Setlec.reduceStoredOk] at hstored
    cases hf : env.find? (Setlec.ofReduceOp cvA.name) with
...
theorem ofReduce_memP (hμ : μ.verified = true) (mp : EnvS2PM V μ env)
  ...
  obtain ⟨ciE, hfE, hlpE, htyE⟩ := Setlec.TTVerify.reduceElem_sort helem
  obtain ⟨-, hlpR⟩ := Setlec.TTVerify.matchesPin_invT hmpR
  rw [show (Setlec.reduceOpCvA
      (Setlec.ofReduceOp cvA.name)).levelParams = [] from by
    unfold Setlec.reduceOpCvA; split <;> rfl] at hlpR
  have hmemOp : Setlec.ofReduceOp cvA.name ∈ Setlec.reduceOpNames := by
    unfold Setlec.ofReduceOp; split <;> decide
  ...
  have hEd : ∀ e, denoteP mp.base2.acval env ψ e
      (.const (Setlec.reduceElemName (Setlec.ofReduceOp cvA.name)) [])
      = some (mp.base2.acval
  ...
  have hOd : ∀ e, denoteP mp.base2.acval env ψ e
      (.const (Setlec.ofReduceOp cvA.name) [])
      = some (mp.base2.acval (Setlec.ofReduceOp cvA.name) ψ) :=
    fun e => denoteP_levelless_const hfR
      (show (ConstantInfo.axiomInfo cvR).toConstantVal.levelParams = []
        from hlpR)
  have hQd : ∀ e, denoteP mp.base2.acval env ψ e
  ...
  have hden : denoteP mp.base2.acval env ψ 0
      (.forallE n₁
        ...
        (.forallE n₂
          (.const (Setlec.reduceElemName
            (Setlec.ofReduceOp cvA.name)) [])
          (.forallE n₃
            (.app (.app (.app (.const eqName [.succ .zero])
                (.const (Setlec.reduceElemName
                  (Setlec.ofReduceOp cvA.name)) []))
              (.app (.const (Setlec.ofReduceOp cvA.name) []) (.bvar 1)))
              (.bvar 0))
            (.app (.app (.app (.const eqName [.succ .zero])
                (.const (Setlec.reduceElemName
                  (Setlec.ofReduceOp cvA.name)) []))
              (.bvar 2)) (.bvar 1)) m₃) m₂) m₁)
      = some (.pi 0 0
          (mp.base2.acval (Setlec.reduceElemName
            (Setlec.ofReduceOp cvA.name)) ψ)
          (.pi 0 0
            (mp.base2.acval (Setlec.reduceElemName
              (Setlec.ofReduceOp cvA.name)) ψ)
            (.pi 0 0
              (.app (.app (.app (mp.base2.acval eqName
                  ...
                  (mp.base2.acval (Setlec.reduceElemName
                    (Setlec.ofReduceOp cvA.name)) ψ))
                (.app (mp.base2.acval (Setlec.ofReduceOp cvA.name) ψ)
                  (.bvar 1))) (.bvar 0))
              (.app (.app (.app (mp.base2.acval eqName
                    (Level.substFn ψ eqA.toConstantVal.levelParams
                      [Level.zero.succ]))
  ...
  have hclO : ∀ ρ' : Nat → V,
      interp2 V ρ' (mp.base2.acval (Setlec.ofReduceOp cvA.name) ψ)
        = interp2 V ρ (mp.base2.acval (Setlec.ofReduceOp cvA.name) ψ) :=
    fun ρ' => acval_interp2_closedC mp.base2 _ ψ ρ' ρ
  have hclQ : ∀ ρ' : Nat → V,
  ...
  have hopx : SetTheory.app (interp2 V ρ
      (mp.base2.acval (Setlec.ofReduceOp cvA.name) ψ)) x = x :=
...
theorem axiomOfReduceP (hμ : μ.verified = true)
  ...
  intro ψ ta hta ρ
  exact ofReduce_memP hμ mp hok hor hst ψ ta hta ρ
