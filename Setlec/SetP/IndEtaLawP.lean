...
of the family, yields the stored family's `EtaLawP`.

The skeleton is `memberUnitLawP`'s — the same four moves, one member
binder instead of two — and the two deltas are exactly v1's:

* **`etaFields = 0`.** `capsOkP_cons_member` only ever leaves the η row
...
now carries, which is why those were added to `MemberEtaLawP`: the law's
...
theorem eqThird_mem (mp : EnvS2PM V μ env)
    ...
    (hS : interp2 V σ Sa ∈ˢ (univ (χ uN) : V))
    (hl : interp2 V σ la ∈ˢ interp2 V σ Sa) :
    interp2 V σ ra ∈ˢ interp2 V σ Sa := by
  ...
  have h3 : AnnotOk2 V σ
      (.app (.app (.app (mp.base2.acval eqName χ) Sa) la) ra) := hok.1
  rw [AnnotOk2_app] at h3
  obtain ⟨-, -, v, A, B, hIn, hrIn, -⟩ := h3
  have hIn' : SetTheory.app
      (SetTheory.app (interp2 V σ (mp.base2.acval eqName χ))
        (interp2 V σ Sa)) (interp2 V σ la) ∈ˢ piR v A B := by
    rw [← interp2_app, ← interp2_app]; exact hIn
  have hv : v ≠ 0 := by
    intro hv0
    subst hv0
...
theorem memberEtaLawP : MemberEtaLawP V := by
  ...
  have hvC : ∀ ψ : Name → Nat,
    ...
    · rw [hac, acvalWith_ne hC0]
      ...
      have hfCe : env.find? caps.etaCtor
        ...
        rw [Setlec.Env.find?_cons, if_neg (fun hh => hC0 hh.symm)] at hfC
        exact hfC
  ...
  have hEqTy : ∀ ψ : Name → Nat,
    ...
    · have hc₀ : c₀ = .indInfo cvT caps := by
        rw [hT0, Setlec.Env.find?_cons_self] at hfT
  ...
  rw [← hψ, hvT] at hmx
  rw [← hψ, h0, hvC,
    show etaFabArgs2 (fun n => interp2 V ρ (m₂.acval n ψ)) T ts x 0
