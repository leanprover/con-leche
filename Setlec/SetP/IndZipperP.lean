...
introducing the padded satisfaction at each step (`sat2_pad_of_mems`)
...
* the prefix positions are `prefixGradeFireP` (part 4), whose equality
  identifies the statement tower's slot with the recursor tower's, so
  the prefix fit's own chain membership crosses;
* the field positions are `fieldGradeFireP` (part 5), whose equality
  ...
  `zipFieldTermEqP`'s term identity.
...
else does (`plainParamSupplyP` is the canonical rule's supplier).  The
...
theorem zipperP {m : EnvS2Core V env} {F : Nat}
    (hclaims : DefEqClaims2P μ m φ F)
    {f : Name → Name} (hroT : RenameOkP m.acval env f)
    {rP cnP cnF mI : Nat} {xs ys : List AVExpr} {ρ : Nat → V}
    ...
    {TVj : AVExpr}
    (hTVjK : denoteP m.acval env φ (rP + cnF) ctyR = some TVj)
    ...
    (hmixsp : ∀ (q : Nat) (x : Expr), sp[q]? = some x →
      ∃ w0, denoteP m.acval env φ (rP + cnF) x = some w0 ∧
        mix[q]? = some (AVExpr.instSeq (xs.take rP ++ ys.drop cnP)
          (rP + cnF - 1) w0))
    (hmixFldEq : ∀ j, j < cnF →
    ...
    Sat2 V Γs (chainP V ρ (xs.take rP ++ ys.drop cnP)) ∧
  ...
  have hmixVal : ∀ q, q < cnP + cnF →
      interp2 V ρ (mix.getD q default)
  ...
  have hchainEnvEq : ∀ q, q ≤ cnP + cnF →
      chainP V ρ (mix.take q) = chainP V ρ (ys.take q) := by
    intro q hq
    funext i
  ...
  have hmixMem : ∀ q, q < cnP + cnF →
      interp2 V ρ (mix.getD q default)
        ∈ˢ interp2 V (chainP V ρ (mix.take q))
  ...
  have hzipAll : ∀ j, j < cnF →
      ...
      ∃ vdomLow,
        denoteP m.acval env φ (rP + j) (cdoms.getD (cnP + j) default)
          = some vdomLow ∧
        AVExpr.instSeq ((xs.take rP ++ ys.drop cnP).take (rP + j))
            (rP + j - 1) vdomLow
          = AVExpr.instSeq (mix.take (cnP + j)) (cnP + j - 1)
              (Γj.getD (cnP + cnF - 1 - (cnP + j)) default) := fun j hj =>
    zipFieldTermEqP m.acval_closed
  ...
  have hall : ∀ n, n ≤ rP + cnF → ∀ p, p < n →
      interp2 V ρ ((xs.take rP ++ ys.drop cnP).getD p default)
        ∈ˢ interp2 V (chainP V ρ ((xs.take rP ++ ys.drop cnP).take p))
    ...
    | succ n ihn =>
      ...
      · obtain rfl : n = p := by omega
        ...
        have hsat := sat2_pad_of_mems htowerS hzslen
          (n := n) (by omega) (fun p' hp' => ihn (by omega) p' hp')
        have hstrip : (fun j => padE2 V (rP + cnF - n)
              (chainP V ρ ((xs.take rP ++ ys.drop cnP).take n))
              (j + (rP + cnF - n)))
            = chainP V ρ ((xs.take rP ++ ys.drop cnP).take n) := by
        ...
        · -- the prefix step
          have hfire := prefixGradeFireP (m := m) hclaims hfvslen
        ...
        · -- the field step
          ...
          have hdw : denoteP m.acval env φ (rP + cnF)
              (cdoms.getD (cnP + (n - rP)) default)
              = some (vdomLow.liftN ((rP + cnF) - n) 0) := by
            have hd := denoteP_lift (env := env) (φ := φ)
              m.acval_closed
            ...
            rw [hnj] at hd
            exact hd
          have hfire := fieldGradeFireP (m := m) hclaims hfvslen hshapeS
          ...
          rw [hstrip, interp2_liftN,
            show shiftE ((rP + cnF) - n) 0 (padE2 V (rP + cnF - n)
                (chainP V ρ ((xs.take rP ++ ys.drop cnP).take n)))
              = chainP V ρ ((xs.take rP ++ ys.drop cnP).take n) from
          ...
          have htake1 : (mix.take (cnP + (n - rP))).length
            ...
            rw [List.length_take, hmixlen]
            omega
          rw [show interp2 V (chainP V ρ (mix.take (cnP + (n - rP))))
              (Γj.getD (cnP + cnF - 1 - (cnP + (n - rP))) default)
            = interp2 V ρ (AVExpr.instSeq (mix.take (cnP + (n - rP)))
                (cnP + (n - rP) - 1)
                (Γj.getD (cnP + cnF - 1 - (cnP + (n - rP))) default))
            from by rw [← interp2_instSeq, htake1]] at h1
          rw [hnj] at hterm
          rw [← hterm] at h1
          have htake2 : ((xs.take rP ++ ys.drop cnP).take n).length
            ...
            omega
          rw [show interp2 V ρ (AVExpr.instSeq
              ((xs.take rP ++ ys.drop cnP).take n) (n - 1) vdomLow)
            = interp2 V (chainP V ρ
                ((xs.take rP ++ ys.drop cnP).take n)) vdomLow from by
              rw [← interp2_instSeq, htake2]] at h1
          have hval : interp2 V ρ (mix.getD (cnP + (n - rP)) default)
              = interp2 V ρ ((xs.take rP ++ ys.drop cnP).getD n
                default) := by
