...
inherits all of it through `declStepPM_of_ind_rec_cons`; what is left
...
* **the stored type's three rows**, read through the *pruned*
  ...
  the checker's roundtrip pin, `denoteP_renameConsts` turns it into an
  equality of readings, and `EnvS2PM.acval_memTypeP` at the model
  definition supplies the reading, its grading and the leaf's
  membership in one call — the same three-field read every P cons
  makes;
* **`caps_ok`**, through `capsOkP_cons_proj` (part 9's row) —
  called directly rather than through `capsOkP_cons_proj_of` because
...
* **`rec_rules`**, a premise here exactly as v1's `hheadRec` is: the
  projection bottom fires **below** this cons (`projFnP`), and
...
theorem blockAcvalInstalled_fresh_cons {blockNames : List Name}
    {env : Env} {acval : Name → (Name → Nat) → AVExpr}
...
theorem projConsP {env' : Env} (mp : EnvS2PM V μ env')
  ...
  have hstrNe₀ : ∀ n : Name, c₀.name ≠ n.str "_model" := by
    rw [hname]; exact fun n => Setlec.Name.num_ne_str _ _ _ _
  ...
  have hrenP : pty.renameConsts (fun n =>
    ...
    rw [Setlec.Expr.renameConsts_congr_resolve
      (g := projFwd T ctorName nF)
      (fun n hn => by simp only [hn, if_true]) _ hptyres]
    exact eq_of_beq hround
  have htyPre : ∀ ψ : Name → Nat, ∃ ta : AVExpr,
      denoteP mp.base2.acval env' ψ 0 pty = some ta ∧
  ...
  have hcapsH : ∀ m₂ : EnvS2Core V ⟨c₀ :: env'.consts⟩,
    ...
    have hvP : ∀ j, j < caps.etaFields → ∀ ψ : Name → Nat,
      ...
      · have hjneP : projFnName T j ≠ projFnName T i := by
          intro hh
          have hh2 : Setlec.Name.num (T.str "proj") j
            = Setlec.Name.num (T.str "proj") i := hh
  ...
  have hrecH : ∀ m₂ : EnvS2Core V ⟨c₀ :: env'.consts⟩,
    ...
    intro m₂ hac φ
    refine recRulesP_cons_rec mp hfresh hc₀ m₂ hac φ ?_
    intro rl hrl hfire
    rw [hname]
  ...
  · rw [hmp']
    exact projPhaseAcvalP_cons hname hinvA hfreshP hTf hCf hilt A hAdef
  · rw [hmp']
