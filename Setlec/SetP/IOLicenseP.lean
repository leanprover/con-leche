...
consumer, the io app clause (`Step2/InferIOP.lean`).  These four
...
* **The license** (`io_domain_transfer`, `io_app_mem`) — the fact an
  ...
  function type's domain.  `AnnotOk2`'s app slot carries
  ...
  **no** freshness premise.  The #100 empty-domain countermodel
  ...
  truth, while the term itself can never carry the `AnnotOk2` app
...
* **The fence** (`io_squash_no_transfer`,
  ...
  a theorem, the way `gate_zero_kind_unreachable` (`Step2/GateP.lean`)
