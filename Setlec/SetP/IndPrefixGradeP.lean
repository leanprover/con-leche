...
* `AnnotOkP V (fun j => ρ' (j + (K - n))) (ΓP.getD (rP - 1 - n))` — the
  b-side grading at position `n`, walked out of the **recursor's own**
  tower by `annotOkP_tower_slot`, whose membership premises are the
  *earlier* positions' equalities;
* `interp2 … (Γs.getD (K - 1 - n)) = interp2 … (ΓP.getD (rP - 1 - n))`
  — position `n`'s equality, fired from the recorded run by
  `defEqAtP_of_run` against that grading and `hokA_padded`.
...
own slot for opener `m`.  That coincidence is what lets `Sat2` at the
...
theorem prefixGradeFireP {m : EnvS2Core V env} {F : Nat}
  ...
  have hΔlen : (List.replicate (rP + cnF - N) (AVExpr.sort 0) ++ Γs.drop (rP + cnF - N)).length = (rP + cnF) := by
    rw [List.length_append, List.length_replicate, List.length_drop,
      hΓslen]
    omega
  -- above the padding the padded context is the tower's
  have hent : ∀ i, i < N → (List.replicate (rP + cnF - N) (AVExpr.sort 0) ++ Γs.drop (rP + cnF - N))[(rP + cnF) - 1 - i]? = some
  ...
  have hokAll : ∀ i, i ≤ N → i < rP + cnF → ∀ ρ' : Nat → V, Sat2 V (List.replicate (rP + cnF - N) (AVExpr.sort 0) ++ Γs.drop (rP + cnF - N)) ρ' →
      AnnotOkP V (fun j => ρ' (j + ((rP + cnF) - 1 - i) + 1))
        (Γs.getD ((rP + cnF) - 1 - i) default) := by
    intro i hi hiK ρ' hsat
    refine annotOkP_tower_slot (rP + cnF) htowerS (hokTst _) ((rP + cnF) - 1 - i)
  ...
  have hfvsAt : ∀ n, n < (rP + cnF) → ∃ nm ty, fvs[n]? = some (.fvar n nm ty) := by
    intro n hn
  ...
  have hAv : denoteP m.acval env φ (rP + cnF) ty
      = some ((Γs.getD ((rP + cnF) - 1 - n) default).liftN ((rP + cnF) - n) 0) := by
    rw [denoteP_lift m.acval_closed hwsTy (rP + cnF) (by omega), hdomTy]
    rfl
  have hBv : denoteP m.acval env φ (rP + cnF) (rdoms.getD n default)
      = some ((ΓP.getD (rP - 1 - n) default).liftN ((rP + cnF) - n) 0) := by
    rw [denoteP_lift m.acval_closed hwsRd (rP + cnF) (by omega), hrdden]
    rfl
  -- **the b-side grading, at every satisfying environment**: the
  -- recursor's own tower, descended by `annotOkP_tower_slot`, whose
  -- membership premises are the earlier positions' equalities
  have hgrBall : ∀ ρ0 : Nat → V, Sat2 V (List.replicate (rP + cnF - N) (AVExpr.sort 0) ++ Γs.drop (rP + cnF - N)) ρ0 →
      AnnotOkP V (fun j => ρ0 (j + ((rP + cnF) - n)))
        (ΓP.getD (rP - 1 - n) default) := by
    intro ρ0 hρ0
  ...
  have hfire := defEqAtP_of_run (m := m) hclaims (k := (rP + cnF)) (fvs := fvs)
    (Aa := fun i => Γs.getD ((rP + cnF) - 1 - i) default) (Δa := (List.replicate (rP + cnF - N) (AVExpr.sort 0) ++ Γs.drop (rP + cnF - N))) hΔlen
    ...
    (fun ρ0 hρ0 => (AnnotOkP_liftN V ((rP + cnF) - n) _ 0 ρ0).mpr (by
      rw [hshiftEnv ρ0]
