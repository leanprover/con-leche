...
theorem sumCtorsLoop (hμ : μ.verifiedChecks = true)
    ...
    ∀ (rest : List (ConstantVal × Nat)) (k : Nat) (env : Env) (mp : EnvS2PM V μ env),
      ...
      env.find? p.cvT.name = some (.indInfo cvTa (Setlec.directSumCaps p)) →
      ...
      ∃ mp' : EnvS2PM V μ (Setlec.consSumCtors p.nP rest env),
        ...
        (Setlec.consSumCtors p.nP rest env).find? p.cvT.name
          = some (.indInfo cvTa (Setlec.directSumCaps p)) ∧
        FormerData mp'.base2 cvTa (p.nP + p.nIdx) p.resSort ppsAll ∧
        (∀ ψ, mp'.base2.acval p.cvT.name ψ
          = directSumTyAV (p.resSort.eval ψ) (ppsAll ψ)
              (rChains p.nIdx p.nIdx (fssOf p.nP (ctorDataList dsF esF ψ ctorsA 0))
  ...
  | cA :: rest, k, env, mp, hrest, hk, hE, hfT, hFD, hleafT, hcons, hpend => by
    ...
    have hfT' : (⟨.ctorInfo cA.1 p.nP cA.2 :: env.consts⟩ : Env).find? p.cvT.name
        = some (.indInfo cvTa (Setlec.directSumCaps p)) := Setlec.Env.find?_cons_of_fresh hfresh hfT
    ...
    have hcons' : ConsedAt mpC.base2 p.cvT.name p.cvT.levelParams p.nP p.nIdx p.resSort p.isProp
      ...
      · obtain ⟨⟨hfi, hlpsi, hCDi⟩, hresi, hleaf⟩ := hcons i cAi hlt hcAi
        ...
        refine ⟨⟨Setlec.Env.find?_cons_of_fresh hfresh hfi, hlpsi,
      ...
      · have hik : i = k := by omega
        ...
        refine ⟨⟨Setlec.Env.find?_cons_self (.ctorInfo cA.1 p.nP cA.2) env, hlpsC,
...
theorem declDirectSumP (hμ : μ.verifiedChecks = true) {F : Nat} {env env₂ : Env}
  ...
  obtain ⟨hProp, -, -, hClps, -, -, helimR, hRlps, -, -⟩ := Setlec.directSumParts?_inv hdp
  -- the former
  ...
  have hfT_I : (⟨.indInfo cvTa (Setlec.directSumCaps p) :: env.consts⟩ : Env).find? p.cvT.name
      = some (.indInfo cvTa (Setlec.directSumCaps p)) := by
  ...
  obtain ⟨tfvs, trest, hopT⟩ := openPisAtFvars_of_stripPis_isSome p.nP 0
    (Setlec.stripPis_isSome_of_le (Nat.le_add_right _ _) (by rw [hstripT]; rfl))
  have hE_I : Setlec.EtaFamiliesClosed ⟨.indInfo cvTa (Setlec.directSumCaps p) :: env.consts⟩ := by
    intro T'' cvT'' caps hf he hr
    rw [Setlec.Env.find?_cons] at hf
    split at hf
  ...
  have hrunOf : ∀ (j : Nat) (cA : ConstantVal × Nat), ctorsA[j]? = some cA →
      ...
      (⟨.indInfo cvTa (Setlec.directSumCaps p) :: env.consts⟩ : Env).find? cA.1.name = none ∧
      cA.1.type.constsResolve ⟨.indInfo cvTa (Setlec.directSumCaps p) :: env.consts⟩ = true ∧
      Setlec.checkDirectSumCtor (Setlec.fueledOps μ F) env
        ⟨.indInfo cvTa (Setlec.directSumCaps p) :: env.consts⟩
    ...
    rw [← hnF] at hCtor
    obtain ⟨hccvC, -, -⟩ := Setlec.checkDirectSumCtor_shape hCtor
  ...
  have hFD_I₀ : FormerData mpI₀.base2 cvTa (p.nP + p.nIdx) p.resSort ppsAll :=
    hFD.cross (c₀ := .indInfo cvTa (Setlec.directSumCaps p)) hTfresh
  ...
  have hFssFacts : ∀ (idxF : Nat → List Expr) (dsF : Nat → (Name → Nat) → List (Nat × Nat × AVExpr))
      (esF : Nat → (Name → Nat) → List AVExpr) (srcsF : Nat → List (Option Nat))
      (m : EnvS2Core V ⟨.indInfo cvTa (Setlec.directSumCaps p) :: env.consts⟩),
  ...
  have hidxRes_C : ∀ (j : Nat) (cA : ConstantVal × Nat), ctorsA[j]? = some cA →
      ∀ e ∈ idxF j, e.constsResolve (Setlec.consSumCtors p.nP ctorsA
        ⟨.indInfo cvTa (Setlec.directSumCaps p) :: env.consts⟩) = true :=
