...
reads them off `checkDirectSumCtor`'s run (the residual `T p⃗ e⃗` at
...
(`spineFit_of_ok2_lams`: an application spine graded against a
...
open Setlec (Env Expr Name Level ConstantInfo ConstantVal IndCaps BinderMeta)
...
theorem bvarsBelow_mkAppN_inv {k : Nat} :
    ∀ {as : List VExpr} {f : VExpr}, VExpr.bvarsBelow k (VExpr.mkAppN f as) →
      VExpr.bvarsBelow k f ∧ ∀ a ∈ as, VExpr.bvarsBelow k a
...
theorem AnnotOk2.mkAppN_inv {ρ : Nat → V} :
    ∀ {args : List AVExpr} {f : AVExpr}, AnnotOk2 V ρ (AVExpr.mkAppN f args) →
      AnnotOk2 V ρ f ∧ ∀ a ∈ args, AnnotOk2 V ρ a
  | [], _, h => ⟨h, fun _ ha => nomatch ha⟩
  | a :: args, f, h => by
    rw [AVExpr.mkAppN_cons] at h
    obtain ⟨hfa, hall⟩ := AnnotOk2.mkAppN_inv h
    rw [AnnotOk2_app] at hfa
...
theorem spineFit_of_ok2_lams {u : Nat} (hu : u ≠ 0) {b : AVExpr} :
    ∀ {args : List AVExpr} {ds : List (Nat × Nat × AVExpr)} {σ ρ : Nat → V} {f : AVExpr},
      args.length ≤ ds.length →
      AnnotOk2 V ρ (AVExpr.mkAppN f args) →
      interp2 V ρ f = interp2 V σ (mkLamsC u ds b) →
      SpineFit σ ((ds.take args.length).map (·.2.2)) (args.map (interp2 V ρ))
  | [], _, _, _, _, _, _, _ => trivial
  | _ :: _, [], _, _, _, hlen, _, _ => by simp at hlen
  | a :: args, d :: ds, σ, ρ, f, hlen, hok, hf => by
    rw [AVExpr.mkAppN_cons] at hok
    have hokfa : AnnotOk2 V ρ (.app f a) := (AnnotOk2.mkAppN_inv hok).1
    rw [AnnotOk2_app] at hokfa
    obtain ⟨-, -, v, A, B, hfm, ham, -⟩ := hokfa
    have hf' : interp2 V ρ f
        = lamR u (interp2 V σ d.2.2) fun x => interp2 V (cons x σ) (mkLamsC u ds b) := by
      rw [hf]; rfl
    rw [hf'] at hfm
    have hA : A = interp2 V σ d.2.2 := lamR_mem_piR_dom hu hfm
    rw [hA] at ham
    have happ : interp2 V ρ (.app f a) = interp2 V (cons (interp2 V ρ a) σ) (mkLamsC u ds b) := by
      rw [interp2_app, hf', app_lamR_pos hu ham]
    have ih := spineFit_of_ok2_lams hu (args := args) (ds := ds) (σ := cons (interp2 V ρ a) σ)
      (ρ := ρ) (f := .app f a) (by simpa using hlen) hok happ
    simp only [List.length_cons, List.take_succ_cons, List.map_cons, SpineFit]
    exact ⟨ham, ih⟩

theorem DenoteSpineP.getElem? {acval : Name → (Name → Nat) → AVExpr} {d : Nat} :
    ∀ {as : List Expr} {vs : List AVExpr}, DenoteSpineP acval env φ d as vs →
      ∀ {l : Nat} {a : Expr}, as[l]? = some a →
        ∃ v, vs[l]? = some v ∧ denoteP acval env φ d a = some v
  ...
  | _, _, .cons ha htl, l, a, h => by
    ...
    | succ l =>
      ...
      exact DenoteSpineP.getElem? htl h

/-- A read spine crosses a cons its terms do not mention. -/
theorem DenoteSpineP.cons_mono {acval : Name → (Name → Nat) → AVExpr}
    {c₀ : ConstantInfo} {A : (Name → Nat) → AVExpr} (hfresh : env.find? c₀.name = none)
    (hat : ∀ e : Expr, ConsCrossAt c₀ e) {ψ : Name → Nat} {d : Nat} :
    ∀ {as : List Expr} {vs : List AVExpr}, (∀ a ∈ as, ConstsBound env a) →
      DenoteSpineP acval env ψ d as vs →
      DenoteSpineP (acvalWith acval c₀.name A) ⟨c₀ :: env.consts⟩ ψ d as vs
  | _, _, _, .nil => .nil
  | a :: _, _, hcb, .cons ha htl =>
    .cons (denoteP_cons_mono hfresh (hat a) ψ d (hcb a List.mem_cons_self) ha)
      (DenoteSpineP.cons_mono hfresh hat (fun a' ha' => hcb a' (List.mem_cons_of_mem _ ha')) htl)
...
def FieldsBoundSrc (ρ : Nat → V) : List AVExpr → List (Option Nat) → Prop
  | [], _ => True
  | _ :: _, [] => True
  | F :: Fs, s :: ss => (s = none → interp2 V ρ F ∈ˢ (univ 0 : V)) ∧
      ∀ a, a ∈ˢ interp2 V ρ F → FieldsBoundSrc (cons a ρ) Fs ss

/-- `fieldsBound_of_frame` at the sourced fields. -/
theorem fieldsBoundSrc_of_frame {Γ : List AVExpr} {k nP nF : Nat} {srcs : List (Option Nat)}
...
def ctorBodyAVI {env : Env} (m : EnvS2Core V env) (T : Name) (nP nF : Nat)
    (ψ : Name → Nat) (Es : List AVExpr) : AVExpr :=
  AVExpr.mkAppN (m.acval T ψ) (paramBvars nP nF ++ Es)
...
propositional. -/
structure CtorDataI {env : Env} (m : EnvS2Core V env) (T : Name) (lps : List Name)
  ...
  resid : ∃ (cbs : List (Name × Expr × BinderMeta)) (es : List Expr),
    cvC.type.stripPis (nP + nF)
      = some (cbs, Expr.mkAppN (.const T (lps.map .param)) (Setlec.directPsAt nF nP ++ es)) ∧
    es.length = nIdx
  ...
  idxRead : ∀ ψ : Name → Nat, DenoteSpineP m.acval env ψ (nP + nF) idxArgs (Es ψ)
  bits : ∀ (ψ : Name → Nat) (d : Nat × Nat × AVExpr), d ∈ ds ψ →
    (resSort.eval ψ = 0 ↔ d.2.1 = 0)
  okTy : ∀ (ψ : Name → Nat) (ρ : Nat → V),
    AnnotOkP V ρ (mkPisAV (ds ψ) (ctorBodyAVI m T nP nF ψ (Es ψ)))
  below : ∀ ψ : Name → Nat, DomsBelow 0 (ds ψ)
  belowE : ∀ ψ : Name → Nat, ∀ E ∈ Es ψ, VExpr.bvarsBelow (nP + nF) E.erase
...
theorem CtorDataI.cross {m : EnvS2Core V env} {T : Name} {lps : List Name} {cvC : ConstantVal}
  ...
  · rw [hac, hbody]
    exact denoteP_cons_mono hfresh (hat _) ψ 0 hcb (h.read ψ)
  · rw [hac]
    exact DenoteSpineP.cons_mono hfresh hat hcbI (h.idxRead ψ)
  · rw [hbody]; exact h.okTy ψ ρ

/-- A read spine of terms not mentioning a constant reads alike at
either valuation of it. -/
theorem DenoteSpineP.acvalWith_congr {acval : Name → (Name → Nat) → AVExpr} {T' : Name}
    {A₁ A₂ : (Name → Nat) → AVExpr} {env₀ : Env} (hfresh : env₀.find? T' = none)
    {ψ : Name → Nat} {d : Nat} :
    ∀ {as : List Expr} {vs : List AVExpr}, (∀ e ∈ as, e.constsResolve env₀ = true) →
      DenoteSpineP (acvalWith acval T' A₁) env ψ d as vs →
      DenoteSpineP (acvalWith acval T' A₂) env ψ d as vs
  | _, _, _, .nil => .nil
  | a :: _, _, hres, .cons ha htl =>
    .cons (by
        rw [← denoteP_acvalWith_unmentioned₂ (A₁ := A₁) (A₂ := A₂) hfresh _ _
          (hres a List.mem_cons_self)]
        exact ha)
      (DenoteSpineP.acvalWith_congr hfresh (fun e he => hres e (List.mem_cons_of_mem _ he)) htl)

/-- The index readings of two data at the same residual agree across
carriers that differ only at an unmentioned constant. -/
theorem CtorDataI.Es_eq {env : Env} {acval : Name → (Name → Nat) → AVExpr} {T T' : Name}
  ...
  rw [hac₁] at hsp₁
  rw [hac₂] at hsp₂
  exact DenoteSpineP.unique (DenoteSpineP.acvalWith_congr hfresh hres hsp₁) hsp₂
...
theorem sumCtorData_of (hμ : μ.verifiedChecks = true) (mp : EnvS2PM V μ env)
    ...
    (hCtor : Setlec.checkDirectSumCtor (Setlec.fueledOps μ F) env₀ env T lps nP nIdx resSort
  ...
  have hper : ∀ ψ : Name → Nat, ∃ (ds : List (Nat × Nat × AVExpr)) (Es : List AVExpr),
      denoteP mp.base2.acval env ψ 0 type'
        = some (mkPisAV ds (ctorBodyAVI mp.base2 T nP nF ψ Es)) ∧
      ds.length = nP + nF ∧ Es.length = nIdx ∧
      DenoteSpineP mp.base2.acval env ψ (nP + nF) idxArgs Es ∧
      (∀ d ∈ ds, (resSort.eval ψ = 0 ↔ d.2.1 = 0)) ∧
      (∀ ρ : Nat → V, AnnotOkP V ρ (mkPisAV ds (ctorBodyAVI mp.base2 T nP nF ψ Es))) ∧
      DomsBelow 0 ds ∧ (∀ E ∈ Es, VExpr.bvarsBelow (nP + nF) E.erase) := by
    intro ψ
    ...
    obtain ⟨fa, vs, hfa, hsp, hR⟩ := denoteP_mkAppN_inv hop'.body
    ...
    have hvs₁ : vs₁ = paramBvars nP nF := by
      have := DenoteSpineP.unique hsp₁
        (denoteSpineP_indexed (acval := mp.base2.acval) (env := env) (φ := ψ) (d := nP + nF)
          fvsP 0 hidx)
      rw [this, hlenP]
      unfold paramBvars
    ...
    · intro E hE
      ...
      rw [AVExpr.erase_mkAppN] at hb
      exact (bvarsBelow_mkAppN_inv hb).2 _
        (List.mem_map.mpr ⟨E, List.mem_append_right _ hE, rfl⟩)
  -- the sources
  obtain ⟨hlenS, hfields⟩ := Setlec.checkDirectFieldSortsI_inv hsorts
  ...
  · -- the unsourced fields are propositional at a large-eliminating
    ...
    have hC : OpenedP mp.base2 ψ (nP + nF) type' (fvsP ++ xFvs)
      ...
      openedP_of_peel hopAll htf' hbt' (hspec ψ).1 (hspec ψ).2.1 (hspec ψ).2.2.2.2.2.1
    ...
    have hρ' : Sat2 V (((((Classical.choose (hper ψ)).map (·.2.2)).reverse)).drop
        (nP + nF - (nP + 0))) ρ := by
...
theorem sumCtorFrames (hμ : μ.verifiedChecks = true) (mp : EnvS2PM V μ env)
  ...
  have hframes : ∀ ψ : Name → Nat,
    ...
    obtain ⟨Γt, Rt, hteleT, hT⟩ := openedP_of hopT hTf hTb (hFD.read ψ) (hFD.okTy ψ)
    obtain ⟨pps', hst', hΓt⟩ := stripPisAV_of_piTeleP hteleT
    have hst'' := stripPisAV_mkPisAV_take nP (ppsAll ψ) (AVExpr.sort (resSort.eval ψ))
    ...
    have hiff : ∀ ρ : Nat → V, Sat2 V (((ppsAll ψ).take nP).map (·.2.2)).reverse ρ ↔
    ...
    · -- the index expressions at a fitting field spine
      ...
      have hsat : Sat2 V (((ds ψ).map (·.2.2)).reverse) (consList bs ρ) := by
        have := sat2_of_spineFit (Δ₀ := (((ds ψ).take nP).map (·.2.2)).reverse) hρ hsp
        rwa [← reverse_map_take_drop] at this
      have hokRP := hC.okR _ hsat
      unfold ctorBodyAVI at hokRP
      have hokR := hokRP.1
      obtain ⟨-, hargs⟩ := AnnotOk2.mkAppN_inv hokR
      obtain ⟨-, hargsV⟩ := AnnotValidV.mkAppN_inv hokRP.2
      ...
      have hps : (paramBvars nP nF).map (interp2 V (consList bs ρ))
      ...
      rw [hps] at h2
      show SpineFit ρ (((ppsAll ψ).drop nP).map (·.2.2)) ((Es ψ).map (interp2 V (consList bs ρ)))
