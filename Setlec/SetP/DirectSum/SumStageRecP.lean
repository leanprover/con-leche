...
theorem ctorFactsAt_cross {m : EnvS2Core V env} {T : Name} {lps : List Name} {nP nIdx : Nat}
  ...
  exact ⟨Setlec.Env.find?_cons_of_fresh hfresh hf, hlps,
...
theorem sumRecRuleLaw (mp : EnvS2PM V μ env)
  ...
  have hpin : ∀ i, i < p.nIdx →
    ...
    · omega
    have hrest : restC = Setlec.SetP.AVExpr.instSeq ys (p.nP + cA.2 - 1)
    ...
    have hrest2 : AVExpr.mkAppN Ha cargsa
        = AVExpr.mkAppN (Setlec.SetP.AVExpr.instSeq ys (p.nP + cA.2 - 1) (m₂.acval p.cvT.name ψC))
            ((paramBvars p.nP cA.2 ++ esF j ψR).map (Setlec.SetP.AVExpr.instSeq ys (p.nP + cA.2 - 1))) := by
      rw [← hrestEq, hrest]
      unfold ctorBodyAVI
      rw [instSeqP_mkAppN]
    ...
    have hcel : cargsa.getD (p.nP + i) default
        = Setlec.SetP.AVExpr.instSeq ys (p.nP + cA.2 - 1) ((esF j ψR).getD i default) := by
...
theorem sumRecOpenedAll (mp : EnvS2PM V μ env)
  ...
  have hsmin : ∀ (ctors : List (Name × Nat × Expr)) (o : Nat) (body mins : Expr),
    ...
    | nil => intro o body mins h; rw [Setlec.directMinorsPisI_nil h]; exact ⟨[], rfl⟩
    | cons c cs ih =>
      intro o body mins h
      obtain ⟨C, nF, cty⟩ := c
      obtain ⟨mty, rest, -, hrest, rfl⟩ := Setlec.directMinorsPisI_cons h
  ...
  have h23 := Setlec.stripPis_append _ hbsm hs34
  have hs2 := Setlec.stripPis_append 1 (e := Expr.forallE (.str .anonymous "motive") motiveTy minors
      ⟨.default, Level.zeronessOf (Setlec.directElimLevel p.elim p.large)⟩)
    (bs := [(.str .anonymous "motive", motiveTy,
      ⟨.default, Level.zeronessOf (Setlec.directElimLevel p.elim p.large)⟩)])
    (by simp [Expr.stripPis]) h23
  have hs1 := Setlec.replacePisPw_stripPis p.nP hrec hsT
...
theorem stageSumRec (hE : Setlec.EtaFamiliesClosed env)
    ...
    (hFssOk : ∀ (ψ : Name → Nat) (ρ : Nat → V),
      ...
      SumFieldsValid ρ
        (rChains p.nIdx p.nIdx (fssOf p.nP (ctorDataList dsF esF ψ ctorsA 0))
          (essOf (ctorDataList dsF esF ψ ctorsA 0))))
    (hwl : p.large = true → p.resSort.isNeverZero = true ∨ ctorsA.length < 2) :
    ∃ mp' : EnvS2PM V μ ⟨.recInfo cvRa mI rP (Setlec.directSumRules p.nP mI rP cvRa.type ctorsA rhss)
        :: env.consts⟩,
  ...
  have hwf := Setlec.direct_sum_rec_wf (mI := mI) (rP := rP) mp.base2.wf hRec
  ...
  have hleaf : ∀ (ψ : Name → Nat) (ρ : Nat → V),
    ...
    exact h
  -- the cons head
  let c₀ : ConstantInfo := .recInfo cvRa mI rP (Setlec.directSumRules p.nP mI rP cvRa.type ctorsA rhss)
