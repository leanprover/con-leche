...
`Setlec/SetP/Direct/DirectRecReadP.lean` at a constructor list over
...
theorem stripPis_instSeq :
  ...
  | a :: sp, t, e, n, bs, body, hlen, h => by
    obtain ⟨bs', h', -⟩ := Setlec.stripPis_instantiate1_full (v := a) n t h
...
theorem directPsAt_zero (n : Nat) :
    Setlec.directPsAt 0 n = (List.range n).map fun k => Expr.bvar (n - 1 - k) := by
  simp [Setlec.directPsAt]
...
theorem map_instSeq_directPsAt_one (ifvs : List Expr) (nIdx : Nat)
    ...
    (Setlec.directPsAt 1 nIdx).map (Expr.instSeq ifvs nIdx) = ifvs := by
  apply List.ext_getElem
  · simp [Setlec.directPsAt, hlen]
  · intro k h1 h2
    have hk : k < nIdx := by simpa [Setlec.directPsAt] using h1
    simp only [Setlec.directPsAt, List.getElem_map, List.getElem_range]
...
structure CtorRead {env : Env} (m : EnvS2Core V env) (ψ : Name → Nat) (T : Name)
  ...
  resid : ∃ (cbs : List (Name × Expr × BinderMeta)) (es : List Expr),
    c.2.2.stripPis (nP + c.2.1)
      = some (cbs, Expr.mkAppN (.const T (lps.map .param)) (Setlec.directPsAt c.2.1 nP ++ es)) ∧
    es.length = nIdx
...
theorem denoteSpineP_idxArgs_lift {m : EnvS2Core V env} {ψ : Name → Nat} {T : Name}
    ...
    (hsF : crest0.stripPis nF
      = some (fbs, Expr.mkAppN (.const T (lps.map .param)) (Setlec.directPsAt nF nP ++ es)))
  ...
  obtain ⟨ds', hci⟩ := Setlec.instPisAt_of_stripPis xFvs (by rw [hlenX]; exact hsF')
  ...
  have hres : Expr.instSeq xFvs (nF - 1) (Expr.instSeq tfvs (nP - 1 + nF)
      (Expr.mkAppN (.const T (lps.map .param)) (Setlec.directPsAt nF nP ++ es)))
    ...
    rw [instSeq_idx_congr (sp := tfvs) (t := nP - 1 + nF) (t' := nP + nF - 1) _ hnil,
      ...
      Setlec.map_instSeq_directPsAt tfvs nF nP hclT (by omega), List.take_of_length_le (by omega),
...
theorem denoteP_minorAt {m : EnvS2Core V env} {ψ : Name → Nat} {T C : Name} {lps : List Name}
    ...
    (hmin : Setlec.directMinorTyI C lps nP nF extras.length pw cty = some mty)
  ...
  rw [Setlec.instSeq_minorBodyI_at tfvs extras xFvs hlenT hlenX hclT hclE hclX hhead hes] at hminor
...
theorem denoteP_minorsPis {m : EnvS2Core V env} {ψ : Name → Nat} {T : Name} {lps : List Name}
    ...
    ∀ {ctors : List (Name × Nat × Expr)} {cds : List CtorDatum}
      ...
      CtorReads m ψ T lps nP nIdx ctors cds →
      Setlec.directMinorsPisI lps nP pw ctors extras.length body = some mins →
      0 < extras.length →
  ...
  | [], cds, body, mins, extras, hcr, hmin, _, hidxE => by
    ...
    rw [Setlec.directMinorsPisI_nil hmin]
  ...
  | (C, nF, cty) :: cs, cds, body, mins, extras, hcr, hmin, ho, hidxE => by
    ...
    obtain ⟨mty, rest, hmty, hrest, rfl⟩ := Setlec.directMinorsPisI_cons hmin
    ...
    have hidxE' : ∀ (k : Nat) (x : Expr), (extras ++ [mkfv])[k]? = some x →
      ...
      · rw [List.getElem?_append_right (by omega)] at hx
        ...
        congr 1
        omega
    have hrest' : Setlec.directMinorsPisI lps nP pw cs (extras ++ [mkfv]).length body
        = some rest := by
...
theorem denoteP_minorsLams {m : EnvS2Core V env} {ψ : Name → Nat} {T : Name} {lps : List Name}
    ...
    ∀ {ctors : List (Name × Nat × Expr)} {cds : List CtorDatum}
      {body mins : Expr} {extras : List Expr},
      CtorReads m ψ T lps nP nIdx ctors cds →
      Setlec.directMinorsLamsI lps nP pw ctors extras.length body = some mins →
      0 < extras.length →
      (∀ (k : Nat) (x : Expr), extras[k]? = some x → ∃ nm ty, x = Expr.fvar (nP + k) nm ty) →
      ∃ extras' : List Expr, extras'.length = ctors.length + extras.length ∧
        (∀ (k : Nat) (x : Expr), extras'[k]? = some x → ∃ nm ty, x = Expr.fvar (nP + k) nm ty) ∧
  ...
  | [], cds, body, mins, extras, hcr, hmin, _, hidxE => by
    ...
    | nil =>
    refine ⟨extras, by simp, hidxE, ?_⟩
    rw [Setlec.directMinorsLamsI_nil hmin]
  ...
  | (C, nF, cty) :: cs, cds, body, mins, extras, hcr, hmin, ho, hidxE => by
    ...
    subst hC' hnF'
    obtain ⟨ci, hfC, hlpsC⟩ := hc.find
    obtain ⟨mty, rest, hmty, hrest, rfl⟩ := Setlec.directMinorsLamsI_cons hmin
    have hlenTE : (tfvs ++ extras).length = nP + extras.length := by simp [hlenT]
    ...
    have hidxE' : ∀ (k : Nat) (x : Expr), (extras ++ [mkfv])[k]? = some x →
      ...
      · rw [List.getElem?_append_right (by omega)] at hx
        ...
        congr 1
        omega
    have hrest' : Setlec.directMinorsLamsI lps nP pw cs (extras ++ [mkfv]).length body
        = some rest := by
      rw [List.length_append, List.length_singleton]; exact hrest
    obtain ⟨extras', hlenE', hidxE'', hread⟩ :=
...
theorem denoteP_motiveI {m : EnvS2Core V env} {ψ : Name → Nat} {T : Name} {lps : List Name}
    ...
    (hmot : Setlec.directMotiveTyI T lps nP nIdx ℓ itele = some motiveTy)
  ...
  have hnil : tfvs = [] ∨ nP - 1 + nIdx = nIdx + nP - 1 := by
    ...
    · left; rw [h0] at hlenT; exact List.eq_nil_of_length_eq_zero hlenT
    · right; omega
  unfold Setlec.directMotiveTyI at hmot
  have hmot' := Setlec.replacePisPw_instSeq tfvs (nP - 1) (by omega) hmot
  ...
  have hdom1 : Expr.instSeq tfvs (nP - 1 + nIdx) (Setlec.directFamI T lps nP nIdx 0 0)
      = Expr.mkAppN (.const T (lps.map .param))
          (tfvs ++ (List.range nIdx).map fun k => Expr.bvar (nIdx - 1 - k)) := by
    unfold Setlec.directFamI
    rw [instSeq_idx_congr (sp := tfvs) (t := nP - 1 + nIdx) (t' := nIdx + nP - 1) _ hnil,
      ...
      Setlec.map_instSeq_directPsAt tfvs nIdx nP hclT (by omega), List.take_of_length_le (by omega),
      directPsAt_zero, Setlec.map_instSeq_fieldBvars_above tfvs (nIdx + nP - 1) nIdx
        (by rw [hlenT]; omega)]
  have hdom2 : Expr.instSeq ifvs (nIdx - 1) (Expr.mkAppN (.const T (lps.map .param))
    ...
    rw [Expr.instSeq_mkAppN, List.map_append,
      ...
      Setlec.map_instSeq_fieldBvars ifvs nIdx hclI hlenI]
  have hbody : Expr.instSeq ifvs (nIdx - 1) (Expr.instSeq tfvs (nP - 1 + nIdx)
      (.forallE (.str .anonymous "t") (Setlec.directFamI T lps nP nIdx 0 0) (.sort ℓ)
        ⟨.default, .never⟩))
...
theorem denoteP_directRecTyI {m : EnvS2Core V env} {ψ : Name → Nat} {T : Name}
    ...
    {tty recTy : Expr}
    (hgen : Setlec.directRecTyI T lps elim large nP nIdx tty ctors = some recTy)
    ...
    denoteP m.acval env ψ 0 recTy
      = some (mkPisAV (sumRecDataAV m T ψ nP nIdx (Setlec.directElimLevel elim large)
  ...
  obtain ⟨tbs, itele, motiveTy, major, minors, hsT, hmot, hmaj, hmin, hrec⟩ :=
    Setlec.directRecTyI_unfold hgen
  ...
  generalize hℓ : Setlec.directElimLevel elim large = ℓ at hrec hmaj hmin hmot ⊢
  ...
  have hidxE : ∀ (k : Nat) (x : Expr), [mfv][k]? = some x → ∃ nm ty, x = Expr.fvar (nP + k) nm ty := by
    ...
    | succ k => simp at hx
  have hmin' : Setlec.directMinorsPisI lps nP pw ctors [mfv].length major = some minors := by
  ...
  have hmaj' := Setlec.replacePisPw_instSeq (tfvs ++ extras') (nP + 1 + n - 1)
    (by rw [hlenTE]; omega) hmaj
  have hres := Setlec.instSeq_minorTele tfvs extras' hlenT hclT htb0
  ...
  have hdom1 : Expr.instSeq (tfvs ++ extras') (nP + 1 + n - 1 + nIdx)
      (Setlec.directFamI T lps nP nIdx (n + 1) 0)
      = Expr.mkAppN (.const T (lps.map .param))
          (tfvs ++ (List.range nIdx).map fun k => Expr.bvar (nIdx - 1 - k)) := by
    unfold Setlec.directFamI
    rw [Expr.instSeq_mkAppN, List.map_append,
      ...
      Setlec.map_instSeq_directPsAt (tfvs ++ extras') (0 + (n + 1) + nIdx) nP hclTE
        (by rw [hlenTE]; omega),
      List.take_append_of_le_length (by omega), List.take_of_length_le (by omega),
      directPsAt_zero, Setlec.map_instSeq_fieldBvars_above (tfvs ++ extras') _ nIdx
        (by rw [hlenTE]; omega)]
  have hcod1 : Expr.instSeq (tfvs ++ extras') (nP + 1 + n - 1 + nIdx + 1)
      (Expr.mkAppN (.bvar (nIdx + n + 1)) (Setlec.directPsAt 1 nIdx ++ [.bvar 0]))
      = Expr.mkAppN (.fvar nP nmM tyM) (Setlec.directPsAt 1 nIdx ++ [.bvar 0]) := by
    ...
    · refine (List.map_congr_left ?_).trans (List.map_id _)
      ...
      exact Setlec.instSeq_bvar_lt _ _ _ (by rw [hlenTE]; omega)
    · simp only [List.map_cons, List.map_nil]
      rw [Setlec.instSeq_bvar_lt _ _ _ (by rw [hlenTE]; omega)]
  have hdom2 : Expr.instSeq ifvs (nIdx - 1) (Expr.mkAppN (.const T (lps.map .param))
      (tfvs ++ (List.range nIdx).map fun k => Expr.bvar (nIdx - 1 - k)))
      = Expr.mkAppN (.const T (lps.map .param)) (tfvs ++ ifvs) := by
    rw [Expr.instSeq_mkAppN, List.map_append,
  ...
  have hcod2 : Expr.instSeq ifvs (nIdx - 1 + 1)
      (Expr.mkAppN (.fvar nP nmM tyM) (Setlec.directPsAt 1 nIdx ++ [.bvar 0]))
    ...
    simp only [List.map_cons, List.map_nil]
    rw [Setlec.instSeq_bvar_lt ifvs _ 0 (by omega)]
  have hbody : Expr.instSeq ifvs (nIdx - 1) (Expr.instSeq (tfvs ++ extras') (nP + 1 + n - 1 + nIdx)
      (.forallE (.str .anonymous "t") (Setlec.directFamI T lps nP nIdx (n + 1) 0)
        (Expr.mkAppN (.bvar (nIdx + n + 1)) (Setlec.directPsAt 1 nIdx ++ [.bvar 0]))
        ⟨.default, pw⟩))
      = .forallE (.str .anonymous "t") (Expr.mkAppN (.const T (lps.map .param)) (tfvs ++ ifvs))
...
theorem denoteP_directRecRhsI {m : EnvS2Core V env} {ψ : Name → Nat} {T : Name}
    ...
    (hcr : CtorReads m ψ T lps nP nIdx ctors cds)
    {tty rhs : Expr}
    (hgen : Setlec.directRecRhsI T lps elim large nP nIdx tty ctors j = some rhs)
    ...
    denoteP m.acval env ψ 0 rhs
      = some (mkLamsAV (sumRuleDataAV m T ψ nP nIdx (Setlec.directElimLevel elim large)
  ...
  obtain ⟨C₀, nF₀, cty, tbs, cbs, itele, motiveTy, crest0, inner, minors, hj, hsT, hmot, hsC,
    hinner, hmin, hr⟩ := Setlec.directRecRhsI_unfold hgen
  ...
  generalize hℓ : Setlec.directElimLevel elim large = ℓ at hr hmin hinner hmot ⊢
  ...
  have hidxE : ∀ (k : Nat) (x : Expr), [mfv][k]? = some x → ∃ nm ty, x = Expr.fvar (nP + k) nm ty := by
    ...
    | zero =>
      ...
      exact ⟨_, _, by rw [← hmfv, Nat.add_zero]⟩
    | succ k => simp at hx
  have hmin' : Setlec.directMinorsLamsI lps nP pw ctors [mfv].length inner = some minors := by
    rw [List.length_singleton]; exact hmin
  obtain ⟨extras', hlenE', hidxE', hread⟩ :=
