...
import Setlec.SetP.Direct.DirectStageCtorP

/-!
# A sum constructor's cons (task #175 sum-types)

`stageSumCtor`: the P step at constructor `j`'s cons — onto the
environment holding the former and the earlier constructors — with
the leaf `directSumMkAV (resSort.eval ψ) j (ds ψ) Fs_j (Fss ψ)`.  The
constructor's run was taken at the former's environment
(`checkDirectSumCtor`) and its data crossed to the cons's environment
(`CtorData.cross`); the family application at the bottom folds the
former's leaf along the parameters (`sumFormerFold`).  The
capability laws are vacuous (no capability is claimed).
-/
...
theorem ctorWalksS {m : EnvS2Core V env} {T : Name} {cvT cvC : ConstantVal}
    {nP nF j : Nat} {resSort : Level}
    ...
    (hleafT : ∀ ψ, m.acval T ψ = directSumTyAV (resSort.eval ψ) (pps ψ) (Fss ψ))
    ...
    (ψ : Name → Nat) (ρ : Nat → V) :
    MkPreS (resSort.eval ψ) j ρ (((ds ψ).drop nP).map (·.2.2)) (Fss ψ)
        (ctorBodyAV m T nP nF ψ) ((ds ψ).take nP) ∧
      UnderTowerValid ρ
        (sumInjAtAV (resSort.eval ψ) (Fss ψ) (((ds ψ).drop nP).map (·.2.2)).length
          (numeralAV j) (mkTowerGo (resSort.eval ψ) (((ds ψ).drop nP).map (·.2.2))))
  ...
  have hpok : ∀ ρ₀ : Nat → V, ParamsOkS (resSort.eval ψ) ρ₀ (Fss ψ) (pps ψ) := fun ρ₀ =>
    (formerWalksS hFD hFssOk ψ ρ₀).1
  constructor
  · have hw := hereditaryWalk (V := V)
      (Q := fun ρ pds => MkPreS (resSort.eval ψ) j ρ Fs (Fss ψ) (ctorBodyAV m T nP nF ψ) pds)
    ...
    refine ⟨(hFssOk ψ ρ hρt).1, hFsj ψ, fun bs hsp => ?_⟩
    ...
    have hbody : interp2 V (consList bs ρ) (ctorBodyAV m T nP nF ψ)
      ...
      rw [hlen'] at this
      exact this
    rw [hbody, hleafT, sumFormerFold (hpok _) hspP, consList_range_reverse]
  · have hw := hereditaryWalk (V := V)
      (Q := fun ρ ds' => UnderTowerValid ρ
        (sumInjAtAV (resSort.eval ψ) (Fss ψ) Fs.length (numeralAV j)
          (mkTowerGo (resSort.eval ψ) Fs)) ds')
    ...
    have hρp : Sat2 V (((ds ψ).take nP).map (·.2.2)).reverse (fun j => ρ' (j + nF)) := by
      ...
      exact this
    have hρt : Sat2 V ((pps ψ).map (·.2.2)).reverse (fun j => ρ' (j + nF)) :=
      (hiff ψ _).mpr hρp
    obtain ⟨hokAll, hvAll⟩ := hFssOk ψ _ hρt
    have hokF : FieldsOkB (resSort.eval ψ) (fun j => ρ' (j + nF)) Fs :=
      hokAll _ (List.mem_of_getElem? (hFsj ψ))
    have hvF : FieldsValid (fun j => ρ' (j + nF)) Fs :=
      hvAll _ (List.mem_of_getElem? (hFsj ψ))
    have := sumInj_validV_at_fields (w := resSort.eval ψ) (j := j) hvAll hvF
      (fun hw => hokF.toBound hw) hspF
...
theorem stageSumCtor
    ...
    {F : Nat} {T : Name} {lps : List Name} {nP nF j : Nat} {resSort : Level}
    ...
    (hfT : env.find? T = some (.indInfo cvTa {}))
    ...
    (hleafT : ∀ ψ, mp.base2.acval T ψ
      = directSumTyAV (resSort.eval ψ) (pps ψ) (Fss ψ))
    (hFsj : ∀ ψ, (Fss ψ)[j]? = some (((ds ψ).drop nP).map (·.2.2)))
    ...
    ∃ mp' : EnvS2PM V μ ⟨.ctorInfo cvCa nP nF :: env.consts⟩,
      mp'.base2.acval = acvalWith mp.base2.acval cvCa.name
        (fun ψ => directSumMkAV (resSort.eval ψ) j (ds ψ) (((ds ψ).drop nP).map (·.2.2))
          (Fss ψ)) := by
  obtain ⟨hccv, -, -⟩ := Setlec.checkDirectSumCtor_shape hCtor
  ...
  have hwfC : Setlec.EnvWF ⟨.ctorInfo cvCa nP nF :: env.consts⟩ := by
    refine Setlec.EnvWF.cons mp.base2.wf (Setlec.directConstWF ?_ ?_ (Expr.constsResolve_mono htr) ?_
  ...
  have hreadC : ∀ ψ : Name → Nat,
      denoteP (acvalWith mp.base2.acval cvCa.name A)
        ...
        = some (mkPisAV (ds ψ) (ctorBodyAV mp.base2 T nP nF ψ)) := fun ψ =>
  ...
  · intro ψ₁ ψ₂ hφ
    ...
    show directSumMkAV _ j (ds ψ₁) (((ds ψ₁).drop nP).map (·.2.2)) (Fss ψ₁)
      = directSumMkAV _ j (ds ψ₂) (((ds ψ₂).drop nP).map (·.2.2)) (Fss ψ₂)
    rw [hw, hCD.params ψ₁ ψ₂ hφ, hFssParams ψ₁ ψ₂ (by rw [← hlpsC]; exact hφ)]
  · intro ψ ρ
