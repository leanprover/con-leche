...
theorem frame_appFnP {m : EnvS2Core V env} {d : Nat} {Δa : List AVExpr}
  ...
  ⟨hws.getAppFn, Setlec.looseBVarsBounded_getAppFn hb,
    fun l hl => hLb l (Setlec.fvarLeaves_getAppFn l hl),
    hC.of_subset (fun l hl => Setlec.fvarLeaves_getAppFn l hl)⟩
...
theorem map_interp2_of_defEqListP {m : EnvS2Core V env}
  ...
  | nil =>
    ...
    | nil => cases hsb; rfl
    | cons _ _ =>
      simp [Setlec.defEqListP, Setlec.defEqList, pure, Except.pure] at h
  | cons x xs ih =>
    ...
    | nil =>
      simp [Setlec.defEqListP, Setlec.defEqList, pure, Except.pure] at h
    | cons y ys =>
      obtain ⟨hxy, htail⟩ := Setlec.defEqList_step_inv h
...
theorem spine_congrP {m : EnvS2Core V env}
  ...
  rw [show a = Expr.mkAppN a.getAppFn a.getAppArgs from
    (Setlec.Expr.mkAppN_getApp a).symm] at hda
  rw [show b = Expr.mkAppN b.getAppFn b.getAppArgs from
    (Setlec.Expr.mkAppN_getApp b).symm] at hdb
...
theorem stuckIrrelP_of_claims {m : EnvS2Core V env}
  ...
  | false =>
  cases h4 : Setlec.structEtaCertP μ env fuel d b a with
  ...
  | true =>
    exact (hseta h4 hwb hbb hLb hwa hba hLa hCb hCa hdb hda hokB hokA
      ρ hρ).symm
  | false =>
  cases h5 : Setlec.structUnitCertP μ env fuel d a b with
...
private theorem denoteP_const_nolevelsP
    ...
    {ci : Setlec.ConstantInfo} (hf : env.find? c = some ci)
...
private theorem denoteP_nilTermP {acval : Name → (Name → Nat) → AVExpr}
    (hg : Setlec.strLitSupported env = true) (d : Nat) :
    denoteP acval env φ d
        (.app (.const Setlec.listNilName [.zero])
          (.const Setlec.charName []))
      = some (.app (acval Setlec.listNilName
          (Level.substFn φ (levelParamsAt env Setlec.listNilName)
            [.zero]))
        (acval Setlec.charName (Level.substFn φ [] []))) := by
  obtain ⟨ciN, p, nm, mb, hfN, hlpN, -⟩ := listNil_shape hg
  obtain ⟨ciC, hfC, hlpC, -⟩ := char_shape hg
  have hlpa : ciN.toConstantVal.levelParams
      = levelParamsAt env Setlec.listNilName := by
...
private theorem denoteP_consTermP {acval : Name → (Name → Nat) → AVExpr}
    (hg : Setlec.strLitSupported env = true) (d : Nat) :
    denoteP acval env φ d
        (.app (.const Setlec.listConsName [.zero])
          (.const Setlec.charName []))
      = some (.app (acval Setlec.listConsName
          (Level.substFn φ (levelParamsAt env Setlec.listConsName)
            [.zero]))
        (acval Setlec.charName (Level.substFn φ [] []))) := by
  obtain ⟨ciC', p, -, -, -, -, -, -, hfC', hlpC', -⟩ := listCons_shape hg
  obtain ⟨ciC, hfC, hlpC, -⟩ := char_shape hg
  have hlpa : ciC'.toConstantVal.levelParams
      = levelParamsAt env Setlec.listConsName := by
...
private theorem denoteP_strLitListP
    ...
    (hg : Setlec.strLitSupported env = true) (d : Nat) :
    ∀ cs : List Char,
      denoteP acval env φ d (Setlec.strLitList cs)
        = some (charListT2
          (.app (acval Setlec.listNilName
            (Level.substFn φ (levelParamsAt env Setlec.listNilName)
              [.zero]))
            (acval Setlec.charName (Level.substFn φ [] [])))
          (.app (acval Setlec.listConsName
            (Level.substFn φ (levelParamsAt env Setlec.listConsName)
              [.zero]))
            (acval Setlec.charName (Level.substFn φ [] [])))
          (acval Setlec.charOfNatName (Level.substFn φ [] []))
          (acval Setlec.natZeroName (Level.substFn φ [] []))
          (acval Setlec.natSuccName (Level.substFn φ [] []))
          cs) := by
  have hnat : Setlec.natLitSupported env = true := by
  ...
  induction cs with
  | nil =>
    rw [Setlec.strLitList, charListT2]; exact denoteP_nilTermP hg d
  | cons c cs ih =>
    rw [Setlec.strLitList, charListT2, denoteP, denoteP,
...
theorem denotePStrLit_of_guard {m : EnvS2Core V env} :
  ...
  refine ⟨?_, Setlec.strLitToConstructor_WScoped st d,
    Setlec.strLitToConstructor_looseBVars st 0, fun l hl => ?_, ?_⟩
  · rw [Setlec.strLitToConstructor_eq, denoteP,
  ...
  · exact Setlec.strLitToConstructor_fvarLeaves st
...
theorem etaCertStepP_of_claims {m : EnvS2Core V env}
  ...
  have hbody : ∀ σ : Nat → V, Sat2 V (ta :: Δa) σ →
    ...
    ihd hdbody (Expr.WScoped.instantiate1 hwa.1 0 hwa.2)
      (Setlec.looseBVarsBounded_instantiate1 bd 0 hba.2)
      (fun l hl => by
        rcases Setlec.Expr.fvarLeaves_instantiate1 bd 0 hl with h2 | h2
        · exact hLbd l h2
        · rw [Setlec.Expr.fvarLeaves] at h2
