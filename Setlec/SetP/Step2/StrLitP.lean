...
open Setlec (CheckMode Env Expr Name Level ConstantInfo inferTypeCore)
...
theorem strLitFactsP {m : EnvS2Core V env} (hct : ConstTypeP m φ)
    ...
    (hg : Setlec.strLitSupported env = true) {d : Nat} {s : String}
    {ea : AVExpr}
    (hea : denoteP m.acval env φ d (.lit (.strVal s)) = some ea)
    (ρ : Nat → V) :
    AnnotOkP V ρ ea ∧
      interp2 V ρ ea ∈ˢ
        interp2 V ρ (m.acval Setlec.stringName (Level.substFn φ [] [])) := by
  ...
  have hlpAtN : levelParamsAt env Setlec.listNilName
      = ciN.toConstantVal.levelParams := by rw [levelParamsAt, hfN]
  have hlpAtC : levelParamsAt env Setlec.listConsName
  ...
  have hKL : ∀ D : Nat, denoteP m.acval env φ D
      (.const Setlec.listName [.zero])
      = some (m.acval Setlec.listName
  ...
  have hKS : ∀ D : Nat, denoteP m.acval env φ D
      ...
      = some (m.acval Setlec.stringName (Level.substFn φ [] [])) := by
    intro D
  ...
  have hRN : denoteP m.acval env φ 0
    ...
    rw [hIN, denoteP_forallE, denoteP_sort,
      show (Expr.app (.const Setlec.listName [Level.zero]) (.bvar 0)).instantiate1
          (.fvar 0 nmN (Expr.sort (Level.succ Level.zero)))
        = Expr.app (.const Setlec.listName [Level.zero])
  ...
  have hRF : denoteP m.acval env φ 0
    ...
    rw [hIF, denoteP_forallE, hKNat,
      show (Expr.const Setlec.charName ([] : List Level)).instantiate1
  ...
  have hRO : denoteP m.acval env φ 0
      ...
      = some ((.pi 0 (pwBit φ (Level.substPW [] [] mbO.pw))
          (.app (m.acval Setlec.listName
            (Level.substFn φ ciL.toConstantVal.levelParams [Level.zero]))
            (m.acval Setlec.charName (Level.substFn φ [] [])))
          (m.acval Setlec.stringName
            (Level.substFn φ [] []))) : AVExpr) := by
    rw [hIO, denoteP_forallE, denoteP_app, hKL, hKH,
      show (Expr.const Setlec.stringName ([] : List Level)).instantiate1
          (.fvar 0 nmO (Expr.app (.const Setlec.listName [Level.zero])
            (.const Setlec.charName [])))
  ...
  obtain ⟨hclA, hclm⟩ :=
    charList_factsP (V := V) (ρ := ρ)
      (KL := m.acval Setlec.listName
        (Level.substFn φ ciL.toConstantVal.levelParams [Level.zero]))
      (KH := m.acval Setlec.charName (Level.substFn φ [] []))
      (KN := m.acval Setlec.listNilName
        (Level.substFn φ (levelParamsAt env Setlec.listNilName) [Level.zero]))
      (KC := m.acval Setlec.listConsName
        (Level.substFn φ (levelParamsAt env Setlec.listConsName) [Level.zero]))
      (KF := m.acval Setlec.charOfNatName (Level.substFn φ [] []))
      (Kz := m.acval Setlec.natZeroName (Level.substFn φ [] []))
      (Ks := m.acval Setlec.natSuccName (Level.substFn φ [] []))
      (KNat := m.acval Setlec.natName (Level.substFn φ [] []))
  ...
  have h := annotOkP_mkAppN_of_fit (V := V) (ρ := ρ)
    [charListT2
      (.app (m.acval Setlec.listNilName
          (Level.substFn φ (levelParamsAt env Setlec.listNilName) [.zero]))
        (m.acval Setlec.charName (Level.substFn φ [] [])))
      (.app (m.acval Setlec.listConsName
          (Level.substFn φ (levelParamsAt env Setlec.listConsName) [.zero]))
        (m.acval Setlec.charName (Level.substFn φ [] [])))
      (m.acval Setlec.charOfNatName (Level.substFn φ [] []))
      (m.acval Setlec.natZeroName (Level.substFn φ [] []))
      (m.acval Setlec.natSuccName (Level.substFn φ [] []))
      s.toList]
  ...
  refine ⟨h.1, ?_⟩
  have h2 := h.2
  rwa [hleafC Setlec.stringName (Level.substFn φ [] []) _] at h2
...
theorem inferStrLitStepP_of_claims {m : EnvS2Core V env}
  ...
  · next hgb =>
    ...
    subst h
    have hg : Setlec.strLitSupported env = true := by simpa using hgb
    obtain ⟨hs, ciS, ciO, ciL, ciN, ciC, ciH, ciF, pL, pN, pC, hfS, -,
      -, -, -, -, -, hlpS, -, -, -, -, -, -, -, -, -, -, -, -, -⟩ :=
      Setlec.strLitSupported_inv hg
    have hta' : ta = m.acval Setlec.stringName (Level.substFn φ [] []) := by
