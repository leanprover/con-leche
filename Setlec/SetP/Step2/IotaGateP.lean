...
* `certs_teleLicP` — the walk (`certs_telePA`'s licensed twin): the
  ...
  (`AnnotOk2_app`), and the head's membership in the telescope's
  reading (`mem_typeP`, via `constTypeP_pkg`) is carried down the
  telescope by `annotOkP_mkAppN_of_fitA` one slot at a time.  At a
...
open Setlec (CheckMode Env Expr Name Level BinderMeta PropWhen)
...
slot `f ∈ piR v' A' B' ∧ a ∈ A'` is the redex's own `AnnotOk2` app
...
/-- The head of a graded spine is graded (`hoistP_spine`'s first
...
theorem AVExpr.mkAppN_append (f : AVExpr) :
    ∀ (as bs : List AVExpr),
      AVExpr.mkAppN f (as ++ bs) = AVExpr.mkAppN (AVExpr.mkAppN f as) bs
  | [], _ => rfl
  | _ :: as, bs => AVExpr.mkAppN_append _ as bs

/-- **An app's argument may be exchanged for an interpretation-equal
graded one**: the slot is about `⟦a⟧`, so it transfers. -/
theorem annotOkP_app_congr_arg {ρ : Nat → V} {f a a' : AVExpr}
    (h : AnnotOkP V ρ (.app f a)) (ha' : AnnotOkP V ρ a')
    (heq : interp2 V ρ a = interp2 V ρ a') :
    AnnotOkP V ρ (.app f a') := by
  obtain ⟨h1, h2⟩ := h
...
theorem annotOkP_mkAppN_snoc_congr {ρ : Nat → V} {f a a' : AVExpr}
    {as : List AVExpr}
    (h : AnnotOkP V ρ (AVExpr.mkAppN f (as ++ [a]))) (ha' : AnnotOkP V ρ a')
    (heq : interp2 V ρ a = interp2 V ρ a') :
    AnnotOkP V ρ (AVExpr.mkAppN f (as ++ [a'])) := by
  rw [AVExpr.mkAppN_append] at h ⊢
  exact annotOkP_app_congr_arg h ha' heq

/-! ## The licensed walk -/

/-- **A licensed-and-certified spine fits the type's reading**
(`certs_telePA`'s twin at `lic`).  Two inputs beyond `certs_telePA`'s:
the redex's own grading `AnnotOkP (mkAppN fa vs)` — the supplier of
...
theorem certs_teleLicP {m : EnvS2Core V env}
    ...
    ∀ {d : Nat} {Δa : List AVExpr} (ty : Expr) (args : List Expr)
      ...
      Setlec.iotaCertsP μ env fuel d lic ty args = .ok true →
      ...
      (∀ ρ : Nat → V, Sat2 V Δa ρ → AnnotOkP V ρ (AVExpr.mkAppN fa vs)) →
      (∀ ρ : Nat → V, Sat2 V Δa ρ → interp2 V ρ fa ∈ˢ interp2 V ρ Ta) →
      ∃ resta : AVExpr,
  ...
  | cons a as ih =>
    ...
    have hokApp : ∀ ρ : Nat → V, Sat2 V Δa ρ → AnnotOkP V ρ (.app fa aa) :=
      fun ρ hρ => annotOkP_mkAppN_head vs' (hokS ρ hρ)
    have hokFa : ∀ ρ : Nat → V, Sat2 V Δa ρ → AnnotOkP V ρ fa :=
      fun ρ hρ =>
        ⟨((AnnotOk2_app V ρ fa aa) ▸ (hokApp ρ hρ).1).1,
          ((AnnotValidV_app V ρ fa aa) ▸ (hokApp ρ hρ).2).1⟩
    -- **THE SLOT MEMBERSHIP**: licensed or certified
    obtain ⟨hmemA, hrestc⟩ : (∀ ρ : Nat → V, Sat2 V Δa ρ →
          interp2 V ρ aa ∈ˢ interp2 V ρ doma) ∧
        Setlec.iotaCertsP μ env fuel d lic (body.instantiate1 a) as
          = .ok true := by
      rcases Setlec.iotaCerts_step_inv_gate hc with ⟨hg, hrestc⟩ |
          ⟨ta, hta, hde, hrestc⟩
    ...
    have hokBody' : ∀ ρ : Nat → V, Sat2 V Δa ρ →
      ...
      (AnnotOkP_inst0 (hokA ρ hρ)).mpr (hokBody ρ hρ _ (hmemA ρ hρ))
    -- the applied prefix inhabits the peeled reading
    have hfa' : ∀ ρ : Nat → V, Sat2 V Δa ρ →
        interp2 V ρ (.app fa aa) ∈ˢ interp2 V ρ (bodya.inst aa) := by
      intro ρ hρ
      have hoks : ∀ x ∈ [aa], AnnotOkP V ρ x := by
        intro x hx
        rw [List.mem_singleton] at hx
        subst hx
        exact hokA ρ hρ
      exact (annotOkP_mkAppN_of_fitA [aa] (hokT ρ hρ) (hokFa ρ hρ) hoks
