...
theorem AnnotOk2_instRevChain (ρ : Nat → V) :
    ∀ (vs : List AVExpr), (∀ v ∈ vs, AnnotOk2 V ρ v) →
      ∀ X : AVExpr,
        (AnnotOk2 V ρ (Setlec.Semantics.AVExpr.instRevChain vs X) ↔
  ...
  | cons v vs ih =>
    intro hvs X
    show AnnotOk2 V ρ (Setlec.Semantics.AVExpr.instRevChain vs
...
theorem AnnotValidV_instRevChain (ρ : Nat → V) :
    ∀ (vs : List AVExpr), (∀ v ∈ vs, AnnotValidV V ρ v) →
      ∀ X : AVExpr,
        (AnnotValidV V ρ (Setlec.Semantics.AVExpr.instRevChain vs X) ↔
  ...
  | cons v vs ih =>
    intro hvs X
    show AnnotValidV V ρ (Setlec.Semantics.AVExpr.instRevChain vs
        (X.inst (v.liftN vs.length) 0)) ↔ _
    rw [ih (fun v' hv' => hvs v' (List.mem_cons_of_mem _ hv'))]
...
theorem annotOkP_instRevChain {ρ : Nat → V} {vs : List AVExpr}
    ...
    AnnotOkP V ρ (Setlec.Semantics.AVExpr.instRevChain vs X) :=
