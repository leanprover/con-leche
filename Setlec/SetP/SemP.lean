...
/-!
# `Sem` agrees with the two-stage reading, is functional, and the model it exposes

`Setlec/Semantics/Sem.lean` defines the interpretation of a checker
term as a syntax-directed relation, so that a statement about the model
can be read without `AVExpr`, `denoteP` or `interp2`.  This module is
the bridge: wherever `denoteP` reads a term, the reading's `interp2` is
a denotation under `Sem` at the interpreted leaves (`Sem_of_denoteP`,
by `denoteP`'s own induction principle, clause for clause); `Sem` is
functional (`Sem_functional`, by derivation induction); and from the
first the P carrier yields the model statement `EnvS2PM.model_exists`
— a set for every constant such that definitions and theorems denote
their bodies and every stored constant is a member of its type.
`Setlec/Verify/Cached/MainC.lean` states it for the shipped driver;
`ChallengeModel.lean` is the comparator challenge.
-/
...
/-! ### The constants of the two literal spines -/

/-- A constant stored without universe parameters, at the empty
instantiation. -/
theorem Sem_const_nil {cval : Name → (Name → Nat) → V} {env : Env} {φ : Name → Nat}
    {d : Nat} {ρ : Nat → V} {n : Name} {ci : ConstantInfo}
    (hf : env.find? n = some ci) (h0 : ci.toConstantVal.levelParams = []) :
    Sem cval env φ d ρ (.const n []) (cval n (Level.substFn φ [] [])) := by
  have h := Sem.const (cval := cval) (env := env) (φ := φ) (d := d) (ρ := ρ) hf
    (us := []) (by rw [h0]; rfl)
  rwa [h0] at h

/-- A constant stored with one universe parameter, at `[.zero]`, valued
at the stored parameter list as `denoteP` spells it. -/
theorem Sem_const_one {cval : Name → (Name → Nat) → V} {env : Env} {φ : Name → Nat}
    {d : Nat} {ρ : Nat → V} {n : Name} {ci : ConstantInfo} {p : Name}
    (hf : env.find? n = some ci) (h1 : ci.toConstantVal.levelParams = [p]) :
    Sem cval env φ d ρ (.const n [.zero])
...
denotes the annotated numeral `denoteP` reads. -/
theorem Sem_natLitToConstructor {acval : Name → (Name → Nat) → AVExpr}
  ...
  induction k with
  | zero =>
    rw [Setlec.natLitToConstructor, natLitT2, interp2_cvalOf hcl]
    exact Sem_const_nil hz hz0
  | succ k ih =>
    rw [Setlec.natLitToConstructor, natLitT2, interp2_app, interp2_cvalOf hcl]
    exact Sem.app (Sem_const_nil hs hs0) (Sem.natLit ih)

/-- Under the `String`-literal guard, the character-list spine of a
literal's constructor form denotes the annotated spine `denoteP`
reads. -/
theorem Sem_charList {acval : Name → (Name → Nat) → AVExpr}
  ...
  induction l with
  | nil =>
    simp only [List.foldr_nil, charListT2, interp2_app, interp2_cvalOf hcl]
    exact Sem.app (Sem_const_one hN hN1) (Sem_const_nil hH hH0)
  | cons c cs ih =>
    simp only [List.foldr_cons, charListT2, interp2_app, interp2_cvalOf hcl]
    exact Sem.app
      (Sem.app (Sem.app (Sem_const_one hC hC1) (Sem_const_nil hH hH0))
        (Sem.app (Sem_const_nil hF hF0)
          (Sem.natLit (Sem_natLitToConstructor hcl hnat d ρ _))))
      ih

/-- Under the `String`-literal guard, the constructor form of a literal
denotes what `denoteP` reads for the literal. -/
theorem Sem_strLitToConstructor {acval : Name → (Name → Nat) → AVExpr}
  ...
  rw [interp2_app, interp2_cvalOf hcl]
  exact Sem.app (Sem_const_nil hO hO0) (Sem_charList hcl hsup d ρ _)

/-- **The bridge**: wherever `denoteP` reads, `interp2` of the reading
is a denotation under `Sem` at the interpreted leaves. -/
theorem Sem_of_denoteP {acval : Name → (Name → Nat) → AVExpr}
  ...
  | case3 d n us ci hf hlen =>
    ...
    rw [if_pos hlen] at h
    cases h
    rw [interp2_cvalOf hcl]
    exact Sem.const hf hlen
  ...
  | case6 d n ty body mb ihty ihbody =>
    ...
    cases h
    rw [interp2_pi]
    refine Sem.pi (ihty hA ρ) fun x => ?_
    rw [push_eq_cons]
    exact ihbody hB _
  | case7 d n ty body mb ihty ihbody =>
    ...
    cases h
    rw [interp2_lam]
    refine Sem.lam (ihty hA ρ) fun x => ?_
    rw [push_eq_cons]
    exact ihbody hB _
  | case8 d fe a ihf iha =>
    ...
    cases h
    rw [interp2_app]
    exact Sem.app (ihf hF ρ) (iha hA ρ)
  ...
  | case10 d sn i e ihe =>
    ...
    | some entry =>
      ...
      cases h
      rw [interp2_projAV]
      exact Sem.projTower hfp (ihe hE ρ)
    | none =>
      by_cases hi : i < 2
      · have hd : denoteP acval env φ d (.proj sn i e) = some (AVExpr.proj i ea) := by
        ...
        rcases i with _ | _ | i
        · rw [if_pos rfl]
          exact Sem.projFst hfp (ihe hE ρ)
        · rw [if_neg (by decide)]
          exact Sem.projSnd hfp (ihe hE ρ)
  ...
  | case11 d k hsup =>
    ...
    cases h
    exact Sem.natLit (Sem_natLitToConstructor hcl hsup d ρ k)
  | case12 d k hsup =>
    intro ta h
  ...
  | case13 d s hsup =>
    ...
    rw [denoteP, if_pos hsup] at h
    cases h
    exact Sem.strLit (Sem_strLitToConstructor hcl hsup d ρ s)
  | case14 d s hsup =>
    intro ta h
    rw [denoteP, if_neg hsup] at h
    exact nomatch h
...
theorem Sem_functional {cval : Name → (Name → Nat) → V} {env : Env} {φ : Name → Nat} :
    ∀ {d : Nat} {ρ : Nat → V} {e : Expr} {v w : V},
      Sem cval env φ d ρ e v → Sem cval env φ d ρ e w → v = w := by
  intro d ρ e v w hv
  induction hv generalizing w with
  | sort => intro hw; cases hw; rfl
  | fvar => intro hw; cases hw; rfl
  | const hf hlen =>
    intro hw
    cases hw with
    | const hf' hlen' => rw [hf] at hf'; cases hf'; rfl
  | pi hA hB ihA ihB =>
    intro hw
    cases hw with
    | pi hA' hB' =>
  ...
  | projTower ht he ih =>
    intro hw
    cases hw with
    | projTower ht' he' => rw [ih he']
    | projFst ht' he' => rw [ht] at ht'; exact nomatch ht'
    | projSnd ht' he' => rw [ht] at ht'; exact nomatch ht'
  | projFst ht he ih =>
    intro hw
    cases hw with
    | projTower ht' he' => rw [ht] at ht'; exact nomatch ht'
    | projFst ht' he' => rw [ih he']
  | projSnd ht he ih =>
    intro hw
    cases hw with
    | projTower ht' he' => rw [ht] at ht'; exact nomatch ht'
    | projSnd ht' he' => rw [ih he']
  | natLit h ih =>
    intro hw
    cases hw with
    | natLit h' => exact ih h'
...
theorem EnvS2PM.model_exists {μ : CheckMode} {env : Env} (mp : EnvS2PM V μ env) :
  ...
  · intro cv value hmem φ ρ
    have h := Sem_of_denoteP hcl 0 value (mp.defn_reads φ cv value hmem) ρ
    rwa [interp2_cvalOf hcl] at h
  · intro c hc φ ρ
    obtain ⟨ta, hta⟩ := mp.type_reads c hc φ
    refine ⟨interp2 V ρ ta, Sem_of_denoteP hcl 0 _ hta ρ, ?_⟩
    rw [← interp2_cvalOf hcl c.name φ ρ]
