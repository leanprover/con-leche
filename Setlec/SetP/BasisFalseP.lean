...
`Setlec/SetP/BasisEmptyP.lean`'s recipe at the pinned `False` block
(`Setlec/Kernel/Basis/False.lean`): `False` is `Empty.{0}` in the
...
theorem extendFalseP (mp : EnvS2PM V μ env)
  ...
  refine nonempty_of_exists (declStepPM_of_basis_cons mp
    ...
    (ConsHeadP.ofBasis hwf (fun _ => trivial) (fun _ => rfl)
      (fun ψ t hp => by
        rw [show Setlec.TTVerify.pinnedDirectT falseA.name ψ
...
theorem denoteP_falseRecA_type {m : EnvS2Core V env}
  ...
  have hpd : Setlec.TTVerify.pinnedDirectT falseName ψ
...
theorem extendFalseRecP (mp : EnvS2PM V μ env)
  ...
  refine nonempty_of_exists (declStepPM_of_basis_cons mp
    ...
    (ConsHeadP.ofBasis hwf (fun _ => trivial) (fun _ => rfl)
      (fun ψ t hp => by
        rw [show Setlec.TTVerify.pinnedDirectT falseRecA.name ψ
...
theorem declBasisPB_falseK {env₂ : Env} (mp : EnvS2PM V μ env)
    (h : Setlec.Semantics.BasisInstallRun env Setlec.BasisKind.falseK.declsA env₂) :
    Nonempty (EnvS2PM V μ env₂) := by
  rw [show Setlec.BasisKind.falseK.declsA = [falseA, falseRecA] from rfl]
    at h
  obtain ⟨h1, h2, hnil⟩ := h
  subst hnil
