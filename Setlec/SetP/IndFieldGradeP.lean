...
* `.plain` — `paramGradeFireP` at the recursor-frame context, carried
  across by the prefix positions' equalities and the block renaming;
* `.nested` — the pins' recorded `TypedListOk` run (the second
  widening's other row) against `RecRuleLawP`'s carried graded pins.

Everything else is `prefixGradeFireP`'s bookkeeping one branch over:
the a-side is again a statement-tower slot (`hokA_padded`), the
equality is again `defEqAtP_of_run` — this time on `hdeFld` — and the
...
theorem fieldGradeFireP {m : EnvS2Core V env} {F : Nat}
    ...
    -- the field domains' scope and leaves (`zipFieldTermEqP`'s first
  ...
  have hΔlen : (List.replicate (rP + cnF - N) (AVExpr.sort 0)
  ...
  have hokAll : ∀ i, i ≤ N → i < rP + cnF → ∀ ρ' : Nat → V,
      Sat2 V (List.replicate (rP + cnF - N) (AVExpr.sort 0)
        ++ Γs.drop (rP + cnF - N)) ρ' →
  ...
  have hfire := defEqAtP_of_run (m := m) hclaims (k := (rP + cnF))
    ...
    (Δa := (List.replicate (rP + cnF - N) (AVExpr.sort 0)
