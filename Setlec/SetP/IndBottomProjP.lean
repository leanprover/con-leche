...
`Install/IndBottomProjS.lean` transposed to `denoteP`/`interp2`: the
...
* **the zipper is syntactic.**  `checkProjIota`'s `domsMatchAux` pins
  ...
  so the two *read* contexts are equal (`towerCtxEqDP` + `PiTeleP.det`)
  and the statement's `Sat2` is the constructor fit read at the fired
...
* **the reduct is a β-contraction.**  `checkProjRule` pins the rule to
  ...
  (`towerCtxEqP`) and `lamTowerStepP` delivers the equality *and* the
  truthfulness transport in one descent (`projBodyValueP` names the
  contractum).

**The P tier's own delta is where `pointP`'s `hspMem` comes from.**  The
...
`Γs.getD (K - 1 - q)` and the membership is the frame's own `Sat2`
...
theorem indBottomProjP {μ : CheckMode} {env : Env}
    ...
    -- reduct does not consume: `lamTowerStepP` descends the read tower
  ...
  have hdomsS0 : ∀ (q : Nat) (x : Expr), fvs[q]? = some x →
      denoteP mp.base2.acval env (Level.substFn φ lps us) q
          (Expr.fvarTypeD x)
  ...
  have hCbR : (cvj.type.renameConsts f).looseBVarsBounded 0 = true := by
    ...
    exact hCb
  have hTVjR0 : denoteP mp.base2.acval env (Level.substFn φ lps us) 0
  ...
  have hΓs2 : Γs2 = Γs := by
    have h := htowerS2
    rw [hKeq] at h
    exact (PiTeleP.det htowerS h).1.symm
  have hΓsJ : Γs = Γj := by
    refine (hΓs2 ▸ towerCtxEqDP (acval := mp.base2.acval) (env := env)
  ...
  have hzsFld : ∀ j, j < cnF →
    ...
    rfl
  have hpar : ∀ q, q < cnP →
      interp2 V ρ (ys.getD q default)
        = interp2 V ρ (xs.getD q default) :=
    fun q hq => hparP q hq (by omega)
  have hzsVal : ∀ q, q < rP + cnF →
      interp2 V ρ ((xs.take rP ++ ys.drop cnP).getD q default)
  ...
  have hchainEq : ∀ q, q ≤ rP + cnF →
      chainP V ρ ((xs.take rP ++ ys.drop cnP).take q)
        = chainP V ρ (ys.take q) := by
    intro q hq
    funext q0
    ...
    have htq' : (ys.take q).length = q := by
      ...
      omega
    by_cases hiq : q0 < q
    · rw [chainP_lt (by omega), chainP_lt (by omega), htq, htq']
    ...
    · rw [chainP_ge (by omega), chainP_ge (by omega), htq, htq']
  have hallK : ∀ m, m < rP + cnF →
      interp2 V ρ ((xs.take rP ++ ys.drop cnP).getD m default)
        ∈ˢ interp2 V (chainP V ρ ((xs.take rP ++ ys.drop cnP).take m))
    ...
    exact h1
  have hsat : Sat2 V Γs (chainP V ρ (xs.take rP ++ ys.drop cnP)) :=
    sat2_of_tower htowerS hzslen hallK
  have hfitS : TeleFitPA V ρ Tst (xs.take rP ++ ys.drop cnP)
      (AVExpr.instSeq (xs.take rP ++ ys.drop cnP) (rP + cnF - 1)
        Rbody) :=
  ...
  have hdomsLow : ∀ q, q < rP + cnF →
    ...
    rw [h, hΓsJ]
  have hspMem := projSpineMemP (acval := mp.base2.acval) (env := env)
  ...
  have hΓlamJ : Γlam = Γj :=
    towerCtxEqP (Expr.stripLams_length _ hrhsAstrip)
      (Expr.stripPis_length _ hCstrip) hΓlamlen hΓjlen0 hdomsLam hdomsJ
      hrdomsEq
  have hCval : C = .bvar (cnP + cnF - 1 - (cnP + i)) :=
    projBodyValueP hilt hCden
  have hmemLam : ∀ k, k < cnP + cnF →
      interp2 V ρ ((xs.take rP ++ ys.drop cnP).getD k default)
        ∈ˢ interp2 V (chainP V ρ ((xs.take rP ++ ys.drop cnP).take k))
  ...
  have hvRval : vR1 = .bvar (rP + cnF - 1 - (rP + i)) := by
    refine projRhsValueP (acval := mp.base2.acval) (env := env)
