...
`projFwd_renameOkP` is then `projFwd_renameOkT`'s twin: its first two
clauses are *literally* v1's (`RenameOkP` and `RenameOkT` share them),
...
def ProjPhaseAcvalP (T ctorName : Name) (nF : Nat) (env' : Env)
...
cases *are* `ProjPhaseAcvalP`'s three conjuncts. -/
theorem projFwd_renameOkP {T ctorName : Name} {nF : Nat} {env' : Env}
  ...
  refine ⟨(projFwd_renameOkT hinv).1, (projFwd_renameOkT hinv).2.1, ?_⟩
  intro n ψ
  dsimp only
...
theorem projPhaseAcvalP_cons {T ctorName : Name} {nF : Nat}
  ...
  have hmodelNeP : ∀ j : Nat, projModelName T j ≠ projFnName T i :=
    fun j hh => Setlec.Name.num_ne_str _ _ _ _ hh.symm
  have hstrNeP : ∀ n : Name, n.str "_model" ≠ projFnName T i :=
    fun n hh => Setlec.Name.num_ne_str _ _ _ _ hh.symm
