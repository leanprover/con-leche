import Setlec.SetP.DirectDataP
...
theorem stageFormer (hμ : μ.verified = true) (mp : EnvS2PM V μ env)
  ...
  obtain ⟨hccv, rfl, -⟩ := Setlec.checkDirectInd_shape hind
  ...
  obtain ⟨hwfI, -⟩ := Setlec.direct_ind_wf mp.base2.wf hind
  -- the leaf
  ...
  have hnresI : Setlec.reservedBasisNames.contains
      (ConstantInfo.indInfo cvTa (Setlec.directCaps p)).name = false := by
    show Setlec.reservedBasisNames.contains cvTa.name = false
    rw [hname]; exact hnres
  have hpshapeI : (ConstantInfo.indInfo cvTa (Setlec.directCaps p)).name.isProjFnShape
      = false := by
  ...
  · -- `caps_ok`: the prefix families cross; the block's own family is
    ...
    intro m₂ hac
    refine capsOkP_cons_direct mp (c₀ := .indInfo cvTa (Setlec.directCaps p))
      (A := A) (T := cvTa.name) hfresh
      (ConsCrossEnv.ofNtc fun _ h => nomatch h) hpshapeI
      (Or.inl ⟨cvTa, Setlec.directCaps p, rfl, rfl⟩)
      (fun T' cvT' caps' hf _ hres hcape => hE₀ T' cvT' caps' hf hcape hres)
      m₂ hac ?_
    intro cvT caps hf hres
    have hself := Setlec.Env.find?_cons_self
      (ConstantInfo.indInfo cvTa (Setlec.directCaps p)) env
    obtain ⟨rfl, rfl⟩ :=
    ...
    · -- η-complete would store the constructor, which is fresh
      exfalso
      obtain ⟨-, ⟨cvC, hfC⟩, -⟩ := hfam
      rw [show (Setlec.directCaps p).etaCtor = p.cvC.name from rfl, hCfresh] at hfC
      exact nomatch hfC
    · have hnF : p.nF = 0 := by
        have : (p.nF == 0) = true := hunit
        simpa using this
      have hFD₂ : FormerData m₂ cvTa p.nP p.resSort pps :=
        hFD.cross (c₀ := .indInfo cvTa (Setlec.directCaps p)) (A := A) hfresh
          (ConsCrossAt.ofNtc fun _ h => nomatch h) hcb m₂ hac
      refine directUnitLawP (m := m₂) (T := cvTa.name) (caps := Setlec.directCaps p)
