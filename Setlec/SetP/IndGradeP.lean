...
**The part-4 hinge.**  `ctxOkP_of_openers` (part 3) takes the openers'
...
derivation that carries its own justification while `CtxOkP`'s is an
equation *plus a grading*.  Every one of the surviving stages fires a
recorded run through `defEqAtP_of_run`, and every one of those needs
...
value.  That would have been fatal: `RecRuleLawP`'s interp-equality
...
environment** instead.  `Sat2` already says every context entry's own
...
theorem annotOkP_tower_slot :
  ...
  | succ k ih =>
    ...
    have hsplitA : AnnotOkP V (fun j => ρ' (j + k + 1)) A :=
      ⟨((AnnotOk2_pi V (fun j => ρ' (j + k + 1)) u v A B) ▸ hokT'.1).1,
        ((AnnotValidV_pi V (fun j => ρ' (j + k + 1)) u v A B)
          ▸ hokT'.2).1⟩
...
/-- **`ctxOkP_of_openers`'s `hokA`, discharged at the padded frame** —
...
theorem hokA_padded {K n : Nat} {Tstmt : AVExpr} {Γs : List AVExpr}
    {Rbody : AVExpr} (htower : PiTeleP K Tstmt Γs Rbody)
    (hokT : ∀ σ : Nat → V, AnnotOkP V σ Tstmt) (hn : n ≤ K) :
    ∀ i, i < n → ∀ ρ' : Nat → V,
      Sat2 V (List.replicate (K - n) (.sort 0) ++ Γs.drop (K - n)) ρ' →
      AnnotOkP V (fun j => ρ' (j + (K - 1 - i) + 1))
  ...
  have hpad : ∀ q, K - n ≤ q → q < K →
      (List.replicate (K - n) (AVExpr.sort 0) ++ Γs.drop (K - n))[q]?
    ...
    · rfl
  refine annotOkP_tower_slot K htower (hokT _) (K - 1 - i) (by omega)
    (fun q hq1 hq2 => ?_)
