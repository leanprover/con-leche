...
members of the family, yields the stored family's `UnitLawP`.
...
reading of a `∀` *is* a `.pi`, `TeleFitP` peels `.pi`s under `cons`,
and `interp2` of a `.pi` *is* a `piR`.  So the firing is four moves:

1. the statement's type reads to a `PiTeleP` of length `nP + 2`
   (`stripPis_denotePTele`) and the stored theorem inhabits it
   (`EnvS2PM.acval_memTypeP` — the P twin of `mem_type_step`);
2. the *given* fit is at the family former's type, and the pins'
   domain equalities move it to the statement's (`teleFitP_congr_ext`)
   and continue it at the two member slots, whose domains both
   evaluate to the family at the parameters;
3. `memFoldl_of_teleFitP` applies the theorem's inhabitant along the
   whole spine, landing it in the opened body's reading;
4. that body is the pinned `Eq` spine, which `eq_lawP` computes to
   `eqv x y` — and `eq_of_mem_eqv` reads the equation off.

**Where the type slot's universe membership comes from.**  `EqLawP`,
...
(`teleFitP_okP_residual`, then `AnnotOk2_app` twice) exhibits the `Eq`
leaf in some `piR v A B` with the slot in `A`, `acval_memTypeP` at
`eqName` exhibits it in `piR 1 (univ (χ uN)) _` (the pinned type's
reading, `eqTyP`), and the two domains coincide.  The regime side
...
variable {acval : Name → (Name → Nat) → AVExpr} {φ : Name → Nat}
...
theorem denoteP_openSpine {K : Name} {ci : ConstantInfo}
    ...
    denoteP acval env φ d
      ...
      = some (AVExpr.mkAppN (acval K φ)
          ((List.range nP).map fun q => AVExpr.bvar (d - 1 - (0 + q)))) := by
  refine denoteP_mkAppN (denoteSpineP_openFvars nP 0 d (by omega)) ?_
  rw [denoteP_const hf (by rw [hlps, List.length_map]), hlps,
...
`memberKeyP` proves the two types read the same but returns only the
...
/-- `denoteP_eqA_type` at an arbitrary environment: `Eq`'s type has no
`const` leaf, so its reading does not consult the environment. -/
theorem denoteP_eqA_type_gen (ψ : Name → Nat) :
    denoteP acval env ψ 0 eqA.toConstantVal.type = some (eqTyP ψ) := by
...
rigidity at all. -/
theorem eqSlot_univ (mp : EnvS2PM V μ env)
    (heqfE : env.find? eqName = some eqA) (χ : Name → Nat)
    {σ : Nat → V} {Sa la ra : AVExpr}
    (hok : AnnotOkP V σ
      (.app (.app (.app (mp.base2.acval eqName χ) Sa) la) ra)) :
    interp2 V σ Sa ∈ˢ (univ (χ uN) : V) := by
  -- the statement's own grading exhibits the leaf in *some* product
  have h2 : AnnotOk2 V σ (.app (mp.base2.acval eqName χ) Sa) := by
    have h1 := hok.1
  ...
  have hpin : interp2 V σ (mp.base2.acval eqName χ)
      ∈ˢ piR 1 (univ (χ uN) : V)
        (fun A => piR 1 A fun _ => piR 1 A fun _ => (univ 0 : V)) := by
    have := hmem σ
    rw [show interp2 V σ (eqTyP χ)
        = piR 1 (univ (χ uN) : V)
...
transpose, and the campaign's first `UnitLawP` producer. -/
theorem memberUnitLawP : MemberUnitLawP V := by
  ...
  obtain ⟨Γ₁, Γ₂, M, hΓsplit, hΓ₂len, hΓ₁len, hteleS2, hteleS1⟩ :=
    PiTeleP.split caps.unitParams 2 hteleS
  ...
  have hAx : Ax = AVExpr.mkAppN
      (mp.base2.acval (cvA.name.str "_model") ψ)
      ((List.range caps.unitParams).map fun q =>
        AVExpr.bvar (caps.unitParams - 1 - (0 + q))) := by
  ...
  have hAy : Ay = AVExpr.mkAppN
      (mp.base2.acval (cvA.name.str "_model") ψ)
      ((List.range caps.unitParams).map fun q =>
        AVExpr.bvar (caps.unitParams + 1 - 1 - (0 + q))) := by
  ...
  have hx' : x ∈ˢ interp2 V (consN ts ρ) Ax := by
    ...
    exact hmx
  have hy' : y ∈ˢ interp2 V (cons x (consN ts ρ)) Ay := by
  ...
  have hteleS2' : PiTeleP ts.length ua Γ₂ (.pi ux vx Ax (.pi uy vy Ay Cs)) := by
    rw [hlents]; exact hteleS2
  have hfitFull : TeleFitP V ρ ua (ts ++ [x, y])
      (interp2 V (cons y (cons x (consN ts ρ))) Cs) :=
    teleFitP_congr_ext ts hteleM' hteleS2' hdomEq hfit
      (TeleFitP.cons hx' (TeleFitP.cons hy' TeleFitP.nil))
  have hteleFull : PiTeleP (ts ++ [x, y]).length ua Γs Cs := by
  ...
  have hCs : Cs = .app (.app (.app
      ...
      (AVExpr.mkAppN (mp.base2.acval (cvA.name.str "_model") ψ)
        ((List.range caps.unitParams).map fun q =>
          AVExpr.bvar (caps.unitParams + 2 - 1 - (0 + q)))))
      (.bvar 1)) (.bvar 0) := by
    have h := hbodyS
    ...
    exact (Option.some.inj h).symm
  -- ===== fire =====
  have hokCs : AnnotOkP V (cons y (cons x (consN ts ρ))) Cs := by
    have h := teleFitP_okP_residual (ts ++ [x, y]) hteleFull
      (hokua ρ) hfitFull
    rwa [hconsApp] at h
  have hSval : interp2 V (cons y (cons x (consN ts ρ)))
      (AVExpr.mkAppN (mp.base2.acval (cvA.name.str "_model") ψ)
        ((List.range caps.unitParams).map fun q =>
          AVExpr.bvar (caps.unitParams + 2 - 1 - (0 + q))))
      = ts.foldl SetTheory.app
  ...
  rw [hSval] at hSuniv
  have hlanded := memFoldl_of_teleFitP (ts ++ [x, y]) (hokua ρ)
    (hmemua ρ) hfitFull
  rw [hCs] at hlanded
  have hc1 : (cons y (cons x (consN ts ρ))) 1 = x := rfl
  have hc0 : (cons y (cons x (consN ts ρ))) 0 = y := rfl
  simp only [interp2_app, interp2_bvar, hSval, hc1, hc0] at hlanded
  rw [(mp.eq_lawP heqfE _).1 (cons y (cons x (consN ts ρ))) _ x y
