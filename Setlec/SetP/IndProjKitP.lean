...
* `stripLams_denotePTele` — `stripPis_denotePTele`'s λ twin, move for
  move, with `LamTeleP` (`IndTowerReadP.lean`) in place of v1's
  `lamCtx` constructor;
* `PiTeleP.det` — a read Π-tower is determined by its arity and
  subject;
* `towerCtxEqDP`/`towerCtxEqP` — two canonically-opened towers whose
...
* `projBodyValueP`/`projRhsValueP` — the rule tower's β-contractum and
  the statement's right side are the field's bound/frame variable.

**The new reasoning: `projSpineMemP`.**  The projection statement
records no walks, so `pointP`'s `hspMem` cannot come from a ladder.  It
...
`instPisAt_openerDomsP`, which is `stripPis_denotePTele`'s move at an
`instPisAt` run — and the membership is then the frame's own `Sat2`
...
/-- **A `PiTeleP` is determined by its arity and subject**
(`PiTele.det`). -/
theorem PiTeleP.det : ∀ {k : Nat} {T : AVExpr} {Γ Γ' : List AVExpr}
    {R R' : AVExpr}, PiTeleP k T Γ R → PiTeleP k T Γ' R' →
    Γ = Γ' ∧ R = R' := by
  intro k
  induction k with
  | zero =>
...
is a `LamTeleP` at its domains' readings, with the body and each raw
...
theorem stripLams_denotePTele :
  ...
  | zero =>
    ...
    simp only [Setlec.Expr.stripLams, Option.some.injEq,
      Prod.mk.injEq] at h
    obtain ⟨rfl, rfl⟩ := h
  ...
  | succ k ih =>
    ...
    | .lam nm dom bodyE mb, h =>
      simp only [Setlec.Expr.stripLams] at h
      ...
      have hsI : ((bodyE.instantiate1 (.fvar j Name.anonymous
        ...
        Setlec.Expr.stripLams_instantiate1_isSome k 0 (by rw [hs]; rfl)
      ...
      obtain ⟨hbody', hdoms'⟩ :=
        Setlec.Expr.stripLams_instantiate1_eq k 0 hs hsI2
      obtain ⟨Γ', C, htele, hΓlen, hbody, hdoms⟩ := ih hsI2 hB'
      have hbslen' : bs'.length = k :=
        Setlec.Expr.stripLams_length k hsI2
        ...
        | succ i0 =>
          ...
          have hik : i0 < k := by
            ...
            · rw [List.getElem?_eq_none
                (by rw [Setlec.Expr.stripLams_length k hs]; omega)] at hb
              exact nomatch hb
...
theorem towerCtxEqP {k : Nat} {Γβ Γc : List AVExpr}
    ...
    Γβ = Γc :=
  towerCtxEqDP hrblen hcblen hΓβlen hΓclen hβdoms hcdoms
...
theorem projBodyValueP {cnP cnF i : Nat} (hilt : i < cnF) {Cβ : AVExpr}
    (hCβden : denoteP acval env φ (0 + (cnP + cnF))
  ...
  have h2 := hCβden
  rw [← Option.some.inj hhit, denoteP_fvar] at h2
...
touches the spine is `denoteP_erasedEq`, and the reading is blind to an
opener's name and annotation. -/
theorem instPisAt_openerDomsP :
    ∀ (sp : List Expr) {ty : Expr} {ds : List Expr} {rs : Expr},
      ...
      ∀ {j : Nat} {T : AVExpr},
        ...
        ∀ q, q < sp.length →
          denoteP acval env φ (j + q) (ds.getD q default)
    ...
    | .forallE nmT dom bodyE mb, h =>
      ...
      | succ q =>
        ...
        have hB' : denoteP acval env φ (j + 1)
          ...
          rw [denoteP_erasedEq (Setlec.Expr.ErasedEq.instantiate1
            (Setlec.Expr.ErasedEq.rfl bodyE)
            (show Setlec.Expr.ErasedEq (.fvar (j + 0) nm0 t0)
...
set_option maxHeartbeats 1600000 in
/-- **The projection fire's spine memberships** — `pointP`'s `hspMem`,
...
depth `q`) to the tower slot the frame's `Sat2` already inhabits;
`denoteP_lift` moves both to the frame depth. -/
theorem projSpineMemP
    ...
    (hdomsLow : ∀ q, q < K →
      denoteP acval env φ q (cdoms.getD q default)
        = some (Γs.getD (K - 1 - q) default)) :
    ∀ σ : Nat → V, Sat2 V Γs σ →
      ∀ (q : Nat) (x : Expr), fvs[q]? = some x →
        ∃ w, denoteP acval env φ K x = some w ∧ AnnotOkP V σ w ∧
          ∀ dw, denoteP acval env φ K (cdoms.getD q default) = some dw →
  ...
  have hlift := denoteP_lift (acval := acval) (env := env) (φ := φ) hacl
    (hwsDom q hq) K (by omega)
  rw [hdw, hdomsLow q hq] at hlift
  obtain rfl : dw = AVExpr.liftN (K - q)
