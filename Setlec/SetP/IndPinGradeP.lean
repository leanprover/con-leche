...
theorem annotOkP_instSeq {ρ : Nat → V} :
    ∀ (ws : List AVExpr) {X : AVExpr},
      ...
      AnnotOkP V ρ (Setlec.SetP.AVExpr.instSeq ws (ws.length - 1) X) := by
  intro ws
  induction ws with
  ...
  | cons w ws ih =>
    ...
    show AnnotOkP V ρ (Setlec.SetP.AVExpr.instSeq ws
...
theorem nestedPinGradeP {acval : Name → (Name → Nat) → AVExpr}
    ...
    AnnotOkP V ρ (Setlec.SetP.AVExpr.instRevChain zs vpa) := by
