...
import Setlec.Verify.Direct.DirectBody
...
(`Setlec/Verify/ProjTele.lean`).  Two facts make the reading what the
law needs:
...
theorem denoteP_projTele {acval : Name → (Name → Nat) → AVExpr} {env : Env}
    ...
    ∀ (k d : Nat) (body : Expr) {RA : AVExpr},
      ...
      denoteP acval env φ d (Setlec.projTele k body)
        = some (mkPisAV (List.replicate k (0, 1, .sort 0)) RA)
  | 0, d, body, RA, h => by
    simpa [Setlec.projTele, Expr.instSeq, mkPisAV] using h
  | k + 1, d, body, RA, h => by
    ...
    rw [Setlec.projTele, denoteP_forallE, denoteP_sort, Setlec.projTele_instantiate1,
      Nat.zero_add, ih]
    rfl
...
theorem denoteP_projTele_zero {acval : Name → (Name → Nat) → AVExpr} {env : Env}
    ...
    (h : denoteP acval env φ (nP + 1)
      (Expr.instSpine (Setlec.fvsD nP ++ [Setlec.tfvD nP]) nP body) = some RA) :
    denoteP acval env φ 0 (Setlec.projTele (nP + 1) body)
  ...
  have e : ((List.range (nP + 1)).map fun j => Expr.fvar (0 + j) .anonymous (.sort .zero))
      = Setlec.fvsD nP ++ [Setlec.tfvD nP] := by
    rw [List.range_succ, List.map_append, List.map_cons, List.map_nil]
    simp only [Setlec.fvsD, Setlec.tfvD, Nat.zero_add]
  rw [e]
  exact h
...
theorem bodyFrames {env : Env} (m : EnvS2Core V env)
    ...
    (hcf : Expr.instPisAt (Setlec.fvsD nP ++ Setlec.projArgsD T i nP) cty
    ...
    ∃ fdomA : AVExpr,
      denoteP m.acval env ψ (nP + 1)
        (Expr.instSpine (Setlec.fvsD nP ++ [Setlec.tfvD nP]) nP body) = some fdomA ∧
  ...
  have hlenP : (Setlec.fvsD nP).length = nP := Setlec.fvsD_length nP
  have hfvsDidx : ∀ (k : Nat) (x : Expr), (Setlec.fvsD nP)[k]? = some x →
      ∃ nm ty, x = Expr.fvar k nm ty := by
    intro k x hx
    have hk : k < nP := by
      have := (List.getElem?_eq_some_iff.mp hx).1; rwa [hlenP] at this
    rw [Setlec.fvsD_getElem? nP k hk] at hx
    exact ⟨.anonymous, .sort .zero, (Option.some.inj hx).symm⟩
  have hargs : ∀ a ∈ Setlec.fvsD nP ++ Setlec.projArgsD T i nP,
    ...
    · obtain ⟨j, -, rfl⟩ := List.mem_map.mp ha
      refine ⟨?_, rfl⟩
      simp only [Expr.WScoped, Setlec.tfvD, and_true]
      omega
  ...
  have hsp : DenoteSpineP m.acval env ψ (nP + 1) (Setlec.fvsD nP ++ Setlec.projArgsD T i nP)
