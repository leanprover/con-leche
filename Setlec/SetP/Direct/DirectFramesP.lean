import Setlec.SetP.Direct.DirectTeleP
...
at their own variables (`checkDirectCtor`), and `checkDirectDomsAt`
...
at or above `d`. -/
theorem bvarsBelow_of_reading {m : EnvS2Core V env} {d : Nat} {e : Expr}
    (hw : Expr.WScoped d e) (hb : e.looseBVarsBounded 0 = true)
    {ea : AVExpr} (h : denoteP m.acval env φ d e = some ea) :
    VExpr.bvarsBelow d ea.erase :=
  denote_bvarsBelow m.cval_closed d e hw hb (denoteP_erase m.acval_erase d e h)
...
theorem paramFrames {m : EnvS2Core V env} {F : Nat}
    (hc : ClaimsAtP μ m φ F) {nP nF : Nat}
    {tty cty : Expr} {tfvs cfvs : List Expr} {trest crest : Expr}
    {Γt Γc : List AVExpr} {Rt Rc : AVExpr}
    ...
    (hpin : ∀ i, i < nP → ∃ a b, cfvs[i]? = some a ∧ tfvs[i]? = some b ∧
      Setlec.isDefEqCore μ env F i (Expr.fvarTypeD a) (Expr.fvarTypeD b)
        = .ok true) :
    ∀ i, i ≤ nP →
      (∀ ρ : Nat → V, Sat2 V (Γc.drop (nP + nF - i)) ρ ↔
        Sat2 V (Γt.drop (nP - i)) ρ) ∧
      (i < nP → ∀ ρ : Nat → V, Sat2 V (Γc.drop (nP + nF - i)) ρ →
        interp2 V ρ (Γc.getD (nP + nF - 1 - i) default)
          = interp2 V ρ (Γt.getD (nP - 1 - i) default)) := by
  suffices ∀ i j, j ≤ i → j ≤ nP →
      (∀ ρ : Nat → V, Sat2 V (Γc.drop (nP + nF - j)) ρ ↔
        Sat2 V (Γt.drop (nP - j)) ρ) ∧
      (j < nP → ∀ ρ : Nat → V, Sat2 V (Γc.drop (nP + nF - j)) ρ →
        interp2 V ρ (Γc.getD (nP + nF - 1 - j) default)
          = interp2 V ρ (Γt.getD (nP - 1 - j) default)) from
  ...
  | zero =>
    ...
    · -- the head binder's pin, at the empty frame
      ...
      have hCa : CtxOkP m φ 0 (Γc.drop (nP + nF - 0)) (Expr.fvarTypeD a) :=
        hC.ctx (by omega) hwa hleafa
      have hCb : CtxOkP m φ 0 (Γt.drop (nP - 0)) (Expr.fvarTypeD b) :=
  ...
  | succ i ih =>
    ...
    have hsat : ∀ ρ : Nat → V, Sat2 V (Γc.drop (nP + nF - (i + 1))) ρ ↔
        Sat2 V (Γt.drop (nP - (i + 1))) ρ := by
      intro ρ
      rw [hdc, hdt]
      constructor
      · intro h
        obtain ⟨h0, htl⟩ := Sat2_cons_inv h
        have := Sat2_cons V ((ihsat _).mp htl) (by
          rw [← iheq hlt _ htl]; exact h0)
        rwa [cons_eta] at this
      · intro h
        obtain ⟨h0, htl⟩ := Sat2_cons_inv h
        have htl' := (ihsat _).mpr htl
        have := Sat2_cons V htl' (by rw [iheq hlt _ htl']; exact h0)
        rwa [cons_eta] at this
    ...
    have hCa : CtxOkP m φ (i + 1) (Γc.drop (nP + nF - (i + 1)))
        (Expr.fvarTypeD a) := hC.ctx (by omega) hwa hleafa
    have hCb₀ : CtxOkP m φ (i + 1) (Γt.drop (nP - (i + 1)))
    ...
    have hent : ∀ (p : Nat) (A₁ A₂ : AVExpr),
        (Γt.drop (nP - (i + 1)))[p]? = some A₁ →
        (Γc.drop (nP + nF - (i + 1)))[p]? = some A₂ →
        ∀ ρ : Nat → V, Sat2 V (Γc.drop (nP + nF - (i + 1))) ρ →
          interp2 V (fun j => ρ (j + p + 1)) A₁
            = interp2 V (fun j => ρ (j + p + 1)) A₂ := by
    ...
    have hCb : CtxOkP m φ (i + 1) (Γc.drop (nP + nF - (i + 1)))
        (Expr.fvarTypeD b) :=
