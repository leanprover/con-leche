...
theorem ensureSortCore_of_whnf {F d : Nat} {t : Expr} {u : Level}
    (h : Setlec.whnf μ env F d t = .ok (.sort u)) :
    Setlec.ensureSortCore μ env F d t = .ok u := by
  unfold Setlec.ensureSortCore Setlec.ensureSort
  simp only [Bind.bind, Except.bind, Setlec.whnf_def]
  rw [h]
  exact rfl
    ...
    ∀ (n : Nat) {F d : Nat} {e t : Expr} {fvs : List Expr} {opened : Expr},
      ...
      ∀ (j : Nat) (x : Expr), fvs[j]? = some x →
        ∃ (F' : Nat) (tj : Expr) (u : Level),
          Setlec.inferTypeCore μ env F' (d + j) (Expr.fvarTypeD x) = .ok tj ∧
          Setlec.ensureSortCore μ env F' (d + j) tj = .ok u
    ...
    | .forallE nmT dom bodyE mb, h =>
      ...
      have hB' : denoteP acval env φ (j + 1)
        ...
        rw [denoteP_erasedEq (Setlec.Expr.ErasedEq.instantiate1
          ...
          (show Setlec.Expr.ErasedEq (.fvar (j + 0) nm0 t0) (.fvar j nmT dom) from by
