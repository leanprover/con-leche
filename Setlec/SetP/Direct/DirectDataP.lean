import Setlec.SetP.Direct.DirectLawsP
...
theorem mkPisAV_inj :
  ...
  | d₁ :: pps₁, d₂ :: pps₂, b₁, b₂, hlen, h => by
    simp only [mkPisAV, AVExpr.pi.injEq] at h
...
reversed. -/
theorem stripPisAV_of_piTeleP :
    ∀ {k : Nat} {T : AVExpr} {Γ : List AVExpr} {R : AVExpr},
      PiTeleP k T Γ R →
      ∃ pps : List (Nat × Nat × AVExpr),
...
theorem DomsBelow.drop {k : Nat} :
    ∀ {ds : List (Nat × Nat × AVExpr)} (j : Nat), DomsBelow k ds →
...
structure FormerData {env : Env} (m : EnvS2Core V env) (cvT : ConstantVal)
    (nP : Nat) (resSort : Level)
    (pps : (Name → Nat) → List (Nat × Nat × AVExpr)) : Prop where
  read : ∀ ψ : Name → Nat, denoteP m.acval env ψ 0 cvT.type
    = some (mkPisAV (pps ψ) (.sort (resSort.eval ψ)))
  len : ∀ ψ : Name → Nat, (pps ψ).length = nP
  bits : ∀ (ψ : Name → Nat) (d : Nat × Nat × AVExpr), d ∈ pps ψ → d.2.1 ≠ 0
  okTy : ∀ (ψ : Name → Nat) (ρ : Nat → V),
    AnnotOkP V ρ (mkPisAV (pps ψ) (.sort (resSort.eval ψ)))
...
theorem formerData_of (hμ : μ.verified = true) (mp : EnvS2PM V μ env)
  ...
  have hper : ∀ ψ : Name → Nat, ∃ pps : List (Nat × Nat × AVExpr),
      denoteP mp.base2.acval env ψ 0 type'
        = some (mkPisAV pps (.sort (resSort.eval ψ))) ∧
      pps.length = nP ∧
      (∀ d ∈ pps, d.2.1 ≠ 0) ∧
      (∀ ρ : Nat → V, AnnotOkP V ρ (mkPisAV pps (.sort (resSort.eval ψ)))) ∧
      DomsBelow 0 pps := by
    intro ψ
    ...
    have hokT' : ∀ ρ : Nat → V, AnnotOkP V ρ Ta := fun ρ =>
      hokT ρ (Sat2_nil V ρ)
    obtain ⟨Γ, R, htele, hop'⟩ := openedP_of hop htf' hbt' hTa hokT'
    have hR : R = .sort (resSort.eval ψ) := by
      have := hop'.body
      rw [denoteP_sort] at this
      exact (Option.some.inj this).symm
    subst hR
    obtain ⟨pps, hst', -⟩ := stripPisAV_of_piTeleP htele
