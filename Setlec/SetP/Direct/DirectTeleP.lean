import Setlec.SetP.Direct.DirectRowsP
...
(`OpenedP`) hands out gradings in `Sat2`-of-`drop` form.  This module
bridges the two:

* `PiTeleP.unique`/`piTeleP_of_stripPisAV`: the `.pi` context of a
...
theorem PiTeleP.unique :
    ∀ {k : Nat} {T : AVExpr} {Γ Γ' : List AVExpr} {R R' : AVExpr},
      PiTeleP k T Γ R → PiTeleP k T Γ' R' → Γ = Γ' ∧ R = R' := by
  intro k T Γ Γ' R R' h
...
theorem piTeleP_of_stripPisAV :
    ∀ {k : Nat} {T : AVExpr} {pps : List (Nat × Nat × AVExpr)} {b : AVExpr},
      stripPisAV k T = some (pps, b) →
      PiTeleP k T (pps.map (·.2.2)).reverse b
  | 0, T, pps, b, h => by
  ...
  | k + 1, .pi u v A B, pps, b, h => by
    ...
    obtain ⟨rfl, rfl⟩ := heq
    have := piTeleP_of_stripPisAV hst
    simpa [List.reverse_cons] using PiTeleP.cons (A := A) (u := u) (v := v) this
...
theorem getD_reverse_of_peel {pps : List (Nat × Nat × AVExpr)} {k : Nat}
    (hlen : pps.length = k) {i : Nat} (hi : i < k) {p : Nat × Nat × AVExpr}
    (hp : pps[i]? = some p) :
...
theorem hereditaryWalk {Q : (Nat → V) → List (Nat × Nat × AVExpr) → Prop}
    {n : Nat} {Γ : List AVExpr} {pps : List (Nat × Nat × AVExpr)}
    (hΓ : Γ.length = n) (hlen : pps.length = n)
    (hent : ∀ i, i < n → ∃ p, pps[i]? = some p ∧
      p.2.2 = Γ.getD (n - 1 - i) default)
    (okΓ : ∀ i, i < n → ∀ ρ : Nat → V, Sat2 V (Γ.drop (n - i)) ρ →
      AnnotOkP V ρ (Γ.getD (n - 1 - i) default))
  ...
  suffices ∀ (m i : Nat), n - i = m → i ≤ n → ∀ ρ : Nat → V,
      Sat2 V (Γ.drop (n - i)) ρ → Q ρ (pps.drop i) from
...
def fieldsFrom (Γ : List AVExpr) (k nP nF j : Nat) : List AVExpr :=
  (List.range (nF - j)).map fun t => Γ.getD (k - 1 - (nP + j + t)) default

theorem fieldsFrom_succ {Γ : List AVExpr} {k nP nF j : Nat} (hj : j < nF) :
...
the earlier fields. -/
theorem fieldsOkB_of_frame {w : Nat} {Γ : List AVExpr} {k nP nF : Nat}
    ...
    (hbnd : ∀ j, j < nF → ∀ ρ : Nat → V, Sat2 V (Γ.drop (k - (nP + j))) ρ →
      w ≠ 0 → interp2 V ρ (Γ.getD (k - 1 - (nP + j)) default) ∈ˢ (univ w : V)) :
...
theorem sat2_of_spineFit :
    ∀ {Ds : List AVExpr} {as : List V} {Δ₀ : List AVExpr} {ρ : Nat → V},
      Sat2 V Δ₀ ρ → SpineFit ρ Ds as →
      Sat2 V (Ds.reverse ++ Δ₀) (consList as ρ)
  ...
  | D :: Ds, a :: as, Δ₀, ρ, h, hsp => by
    ...
    exact sat2_of_spineFit (Sat2_cons V h hsp.1) hsp.2

/-- `SpineFit` along the peel's domains from `Sat2` of the peeled
context (the converse, at the context's own valuation). -/
theorem spineFit_of_sat2 :
    ∀ {Ds : List AVExpr} {Δ₀ : List AVExpr} {ρ : Nat → V},
      Sat2 V (Ds.reverse ++ Δ₀) ρ →
