import Setlec.SetP.Direct.DirectEntryKit2P
...
later field uses* (`directUsedLater`: the field's variable occurs in
...
leaf's index (`denoteP_liftN_of_leaf_free`), and interpretation and
grading are invariant under a lift's index (`interp2_congr_lifts`,
...
theorem natLitT2_liftN {za sa : AVExpr} {k : Nat} (hz : za.liftN 1 k = za)
    (hs : sa.liftN 1 k = sa) : ∀ n, (natLitT2 za sa n).liftN 1 k = natLitT2 za sa n
  | 0 => hz
  | n + 1 => by
    show (AVExpr.app sa (natLitT2 za sa n)).liftN 1 k = _
    rw [AVExpr.liftN_app, hs, natLitT2_liftN hz hs n]
    rfl
...
read as. -/
theorem denoteP_liftN_of_leaf_free {env : Env} (m : EnvS2Core V env) {φ : Name → Nat} :
    ∀ (d : Nat) (e : Expr), Expr.WScoped d e →
      ∀ {q : Nat}, q < d → (∀ l ∈ e.fvarLeaves, l.1 ≠ q) →
      ∀ {ea : AVExpr}, denoteP m.acval env φ d e = some ea →
      ∃ X : AVExpr, ea = X.liftN 1 (d - 1 - q) := by
  intro d e
  ...
  | case2 d idx nm ty =>
    ...
    · refine ⟨.bvar (d - 1 - idx), ?_⟩
      rw [AVExpr.liftN_bvar, if_pos (by omega)]
  ...
  | case7 d n ty body mb ihty ihbody =>
    ...
    rw [AVExpr.liftN_lam, show d + 1 - 1 - q = d - 1 - q + 1 from by omega]
  | case8 d f a ihf iha =>
    intro hw q hq hl ea h
  ...
  | case9 d n ty val body ihty ihval ihbody =>
    ...
    rw [AVExpr.liftN_letE, show d + 1 - 1 - q = d - 1 - q + 1 from by omega]
  | case10 d sn i e ihe =>
    intro hw q hq hl ea h
  ...
  | case11 d n hsup =>
    ...
    obtain rfl := Option.some.inj h
    exact ⟨_, (natLitT2_liftN (m.acval_closed _ _ _) (m.acval_closed _ _ _) n).symm⟩
  | case12 d n hsup =>
    intro _ q _ _ ea h
  ...
  | case13 d s hsup =>
    ...
    have h0 : denoteP m.acval env φ 0 (.lit (.strVal s)) = some ea := by
      rw [denoteP, if_pos hsup] at h ⊢; exact h
    have hcl := bvarsBelow_of_reading (m := m) (d := 0) (e := .lit (.strVal s))
      (Expr.WScoped.of_not_hasFvar rfl) rfl h0
    exact ⟨ea, (AVExpr.liftN_eq_self ea (VExpr.bvarsBelow.mono (Nat.zero_le _) hcl) 1).symm⟩
  | case14 d s hsup =>
    intro _ q _ _ ea h
    rw [denoteP, if_neg hsup] at h
    exact nomatch h
...
theorem interp2_congr_lift {e X : AVExpr} {k : Nat} (h : e = X.liftN 1 k) {ρ ρ' : Nat → V}
    (hag : ∀ i, i ≠ k → ρ i = ρ' i) : interp2 V ρ e = interp2 V ρ' e := by
  subst h
  rw [interp2_liftN, interp2_liftN, shiftE_congr_off hag]

theorem annotOkP_congr_lift {e X : AVExpr} {k : Nat} (h : e = X.liftN 1 k) {ρ ρ' : Nat → V}
    (hag : ∀ i, i ≠ k → ρ i = ρ' i) : AnnotOkP V ρ e ↔ AnnotOkP V ρ' e := by
  subst h
  unfold AnnotOkP
  rw [AnnotOk2_liftN, AnnotOk2_liftN, AnnotValidV_liftN, AnnotValidV_liftN, shiftE_congr_off hag]
...
the term alike. -/
theorem interp2_congr_lifts :
    ∀ (N : Nat) {e : AVExpr} {ρ ρ' : Nat → V},
      (∀ i, i < N → ρ i ≠ ρ' i → ∃ X : AVExpr, e = X.liftN 1 i) →
      (∀ i, N ≤ i → ρ i = ρ' i) →
  ...
  | succ N ih =>
    ...
    have h1 : interp2 V ρ e = interp2 V ρ'' e := by
      by_cases hN : ρ N = ρ' N
      · have : ρ'' = ρ := by
          funext i
      ...
      · obtain ⟨X, hX⟩ := hfree N (Nat.lt_succ_self N) hN
        exact interp2_congr_lift hX fun i hi => by
...
theorem annotOkP_congr_lifts :
  ...
  | succ N ih =>
    intro e ρ ρ' hfree hag
    let ρ'' : Nat → V := fun i => if i = N then ρ' N else ρ i
    have h1 : AnnotOkP V ρ e ↔ AnnotOkP V ρ'' e := by
      by_cases hN : ρ N = ρ' N
      · have : ρ'' = ρ := by
          funext i
          show (if i = N then ρ' N else ρ i) = ρ i
        ...
        rw [this]
      · obtain ⟨X, hX⟩ := hfree N (Nat.lt_succ_self N) hN
        exact annotOkP_congr_lift hX fun i hi => by
          show ρ i = (if i = N then ρ' N else ρ i)
          rw [if_neg hi]
    rw [h1]
    refine ih ?_ ?_
...
theorem free_of_diff {nP nF i : Nat} {ds : List (Nat × Nat × AVExpr)} {sorts : List Level}
    ...
    ∀ k, k < i → consList (List.replicate i pt) ρ' k ≠ consList (as'.take i) ρ' k →
      ∃ X : AVExpr, ((ds.drop nP).map (·.2.2)).getD i default = X.liftN 1 k := by
...
theorem spineFit_take_length {Fs : List AVExpr} {ρ : Nat → V} {as : List V}
