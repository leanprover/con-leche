  ...
  | k + 1, d, e, b, r, fvs, o, ea, pds, R, hr, hop, hea, hst => by
    ...
    | .forallE n ty rest m, hr, hop =>
      ...
      · next fvs' o' hop' =>
        ...
        obtain ⟨rfl, rfl⟩ := heq
        have hr'' := Setlec.replacePisPw_instantiate1 (v := .fvar d n ty) k 0 hr'
  ...
  | k + 1, d, e, b, r, fvs, o, ea, pds, R, hr, hop, hea, hst => by
    ...
    | .forallE n ty rest m, hr, hop =>
      ...
      · next fvs' o' hop' =>
        ...
        simp only [Prod.mk.injEq] at heq
        obtain ⟨rfl, rfl⟩ := heq
        have hr'' := Setlec.pisToLamsPw_instantiate1 (v := .fvar d n ty) k 0 hr'
        rw [Nat.zero_add] at hr''
...
theorem ctorResidual {m : EnvS2Core V env} {ψ : Name → Nat} {nP nF : Nat} {cty : Expr}
  ...
  obtain ⟨cdoms, hci⟩ := Setlec.instPisAt_of_stripPis tfvs (by rw [hlenT]; exact hsC)
  rw [hlenT] at hci
...
theorem denoteP_directRecTy {m : EnvS2Core V env} {ψ : Name → Nat} {T C : Name}
    ...
    {tty cty recTy : Expr}
    (hgen : Setlec.directRecTy T lps elim large nP tty [(C, nF, cty)] = some recTy)
    ...
    denoteP m.acval env ψ 0 recTy
      = some (mkPisAV (recDataAV m T C ψ nP nF (Setlec.directElimLevel elim large) pps ds)
          (.app (.bvar 2) (.bvar 0))) := by
  obtain ⟨cbs, crest0, minorTy, hsC, hmin, hrec⟩ := Setlec.directRecTy_single hgen
  ...
  have hfamCl : (Expr.mkAppN (.const T (lps.map .param)) tfvs).looseBVarsBounded 0 = true :=
    Setlec.looseBVarsBounded_mkAppN rfl hclT
  ...
  have e2 : Expr.instSeq tfvs (nP + 1) (Setlec.directFam T lps nP 2)
    ...
    rw [Setlec.directFam_eq, Expr.instSeq_mkAppN,
      ...
      Setlec.map_instSeq_directPsAt tfvs 2 nP hclT (by omega), List.take_of_length_le (by omega)]
  have e3 : Expr.instSeq tfvs (nP + 2) (Expr.app (.bvar 2) (.bvar 0)) = .app (.bvar 2) (.bvar 0) :=
    Setlec.instSeq_eq_self_of_bounded tfvs _ (k := 3) (by simp [Expr.looseBVarsBounded]) (by omega)
  have hbody : Expr.instSeq tfvs (nP - 1)
      (.forallE (.str .anonymous "motive") (Setlec.directMotiveTy T lps nP ℓ)
  ...
  have hmin' := Setlec.replacePisPw_instSeq (tfvs ++ [Expr.fvar nP nmM tyM]) nP (by simp [hlenT]) hmin
  have hres := Setlec.instSeq_minorTele tfvs [Expr.fvar nP nmM tyM] hlenT hclT
  ...
  rw [Setlec.instSeq_minorBody tfvs xFvs (Expr.fvar nP nmM tyM) hlenT hlenX hclT hclM hclX,
  ...
  have hmk : denoteP m.acval env ψ (nP + 1)
      (.forallE (Setlec.Name.lastStr C) (Expr.instSeq (tfvs ++ [Expr.fvar nP nmM tyM]) nP minorTy)
        (.forallE (.str .anonymous "t") (Expr.mkAppN (.const T (lps.map .param)) tfvs)
...
theorem denoteP_directRecRhs {m : EnvS2Core V env} {ψ : Name → Nat} {T C : Name}
    ...
    (hfC : env.find? C = some ciC) (hlpsC : ciC.toConstantVal.levelParams = lps)
    {tty cty rhs : Expr}
    (hgen : Setlec.directRecRhs T lps elim large nP tty [(C, nF, cty)] 0 = some rhs)
    (hTf : tty.hasFvar = false)
    (hCf : cty.hasFvar = false) (hCb : cty.looseBVarsBounded 0 = true)
    (hstripC : (cty.stripPis (nP + nF)).isSome = true)
    {tfvs : List Expr} {trest : Expr} (hopT : openPisAtFvars nP tty 0 = some (tfvs, trest))
    ...
    denoteP m.acval env ψ 0 rhs
      = some (mkLamsAV (ruleDataAV m T C ψ nP nF (Setlec.directElimLevel elim large) pps ds)
          (AVExpr.mkAppN (.bvar nF) (fieldBvars nF))) := by
  obtain ⟨cbs, crest0, minorTy, inner, hsC, hmin, hinner, hr⟩ := Setlec.directRecRhs_single hgen
  obtain ⟨hlenT, hidxT, hclT, hspW⟩ := opening_vars hopT hTf
  ...
  have hmin' := Setlec.replacePisPw_instSeq (tfvs ++ [Expr.fvar nP nmM tyM]) nP (by simp [hlenT]) hmin
  have hres := Setlec.instSeq_minorTele tfvs [Expr.fvar nP nmM tyM] hlenT hclT hcb0
  simp only [List.length_singleton, Nat.add_sub_cancel] at hres
  rw [hres] at hmin'
  obtain ⟨xFvs, xrest, hopX⟩ := openPisAtFvars_of_stripPis_isSome nF (nP + 1) hcstrip
  have hcread1 := ctorResidual_read_lift hcread hcw hlenD 1
  ...
  generalize hmkfv : (Expr.fvar (nP + 1) (Setlec.Name.lastStr C)
  ...
  have hY : ((Expr.instSeq tfvs (nP + 1) inner).instantiate1 (Expr.fvar nP nmM tyM) 1).instantiate1
    ...
    rfl
  have hinner' := Setlec.pisToLamsPw_instSeq (tfvs ++ [Expr.fvar nP nmM tyM, Expr.fvar (nP + 1) nmK tyK])
  ...
  have hres2 : Expr.instSeq (tfvs ++ [Expr.fvar nP nmM tyM, Expr.fvar (nP + 1) nmK tyK]) (nP + 1)
    ...
    Setlec.instSeq_minorTele tfvs [Expr.fvar nP nmM tyM, Expr.fvar (nP + 1) nmK tyK] hlenT hclT hcb0
  rw [hres2] at hinner'
  ...
  rw [Setlec.instSeq_ruleBody tfvs xFvs2 (Expr.fvar nP nmM tyM) (Expr.fvar (nP + 1) nmK tyK)
      hlenT hlenX2 hclT hclM hclK hclX2,
    denoteP_ruleCore hlenX2 hidxX2, Option.map_some] at hinnerR
  -- assembly, bottom-up
  have hmk : denoteP m.acval env ψ (nP + 1)
      (.lam (Setlec.Name.lastStr C) (Expr.instSeq (tfvs ++ [Expr.fvar nP nmM tyM]) nP minorTy)
...
theorem recData_of (hμ : μ.verifiedChecks = true) (mp : EnvS2PM V μ env)
  ...
  · intro ψ d hd
    have hb : d.2.1 = pwBit ψ (Level.zeronessOf (elimLevel p)) := by
      ...
      · rfl
      · rfl
    rw [hb, pwBit_eq_zero_iff, Setlec.PropWhen.zeronessOf_sound, beq_iff_eq]
  · intro ψ ρ
