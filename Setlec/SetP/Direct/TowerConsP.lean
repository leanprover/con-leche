...
theorem noProjEnv_of_fresh (hwf : Setlec.EnvWF env) {T : Name}
  ...
  table tbl hc j hj := by
    ...
    exact Setlec.Expr.noProjAt_of_constsResolve hT _ this.2.2.1
...
structure NoProjHead (c₀ : ConstantInfo) (T : Name) (i : Nat) : Prop where
  ...
  table : ∀ (tbl : Setlec.ProjTable), c₀ = .projInfo tbl →
...
theorem declStepPM_of_tower_cons (mp : EnvS2PM V μ env)
    ...
    (hhead : ∀ i, i < tbl.numFields →
      Setlec.TowerHead ⟨.projInfo tbl :: env.consts⟩ (tbl.entry i))
