import Setlec.SetP.Direct.DirectEntryLawP
...
theorem stageTable (mp : EnvS2PM V μ env)
    ...
    (hTbl : Setlec.checkDirectProjTable (m := Setlec.CheckM) p.cvT.name p.cvC.name
      p.cvT.levelParams p.nP p.nF p.resSort
      (Setlec.directProjGuards cvCa.type p.nP p.nF sorts) cvCa env = .ok envOut)
  ...
  have hguardSem : ∀ k, k < p.nF → ∀ ψ : Name → Nat,
    ...
    intro k hk ψ h0
    rw [Setlec.directProjGuards_getD _ _ _ _ hk,
      eval_foldl_max_if_zero_iff ψ (Setlec.directUsedLater cvCa.type p.nP)
  ...
  have hfree : ∀ (ψ : Name → Nat) (k : Nat), k < p.nF → ∀ (j : Nat), j < k →
    ...
    have hsome : (cvCa.type.stripPis (p.nP + j + 1)).isSome = true :=
      Setlec.stripPis_isSome_of_le (by omega) hstripC
  ...
  have hneC : p.cvC.name ≠ projTableName p.cvT.name := by
    ...
    exact nomatch this
  have hnres : Setlec.reservedBasisNames.contains (projTableName p.cvT.name) = false :=
  ...
  obtain ⟨cds, nmB, bodyB, mbB, hcf⟩ :=
    Setlec.directProjBody_open hbodies hstripC hCb hi
  refine ⟨rfl, rfl, hi, ⟨cvTa, Setlec.directCaps p, hfT₂, hlpsT, ?_, rfl, rfl, rfl⟩,
  ...
  · -- the per-instantiation laws
    ...
    · -- (A)
      ...
      exact entryTypingCore (hCD.len _) (hFD.len _) (by simp) (hpok _) (hiff _) (hbound _)
        (hsortsF _) (used := Setlec.directUsedLater cvCa.type p.nP) hguard' (hfree _ i hi) hi
