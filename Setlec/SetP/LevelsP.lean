...
theorem acvalAt_one (hp : AcvalParamsAt env acval) (nm : Name)
    ...
    (ψ₁ ψ₂ : Name → Nat) :
    acval nm (Level.substFn ψ₁ (Setlec.TTVerify.levelParamsAt env nm) [.zero])
      = acval nm
        (Level.substFn ψ₂ (Setlec.TTVerify.levelParamsAt env nm) [.zero]) := by
  cases hx : env.find? nm with
  | none => rw [hx, hnone] at hfok; exact nomatch hfok
  | some ci =>
    have hlp : Setlec.TTVerify.levelParamsAt env nm
        = ci.toConstantVal.levelParams := by
      simp [Setlec.TTVerify.levelParamsAt, hx]
    rw [hx] at hfok
    rw [hlp]
...
theorem acvalAt_natPair (hp : AcvalParamsAt env acval)
    ...
    acval Setlec.natZeroName ψ₁ = acval Setlec.natZeroName ψ₂ ∧
      acval Setlec.natSuccName ψ₁ = acval Setlec.natSuccName ψ₂ := by
  simp only [Setlec.natLitSupported, Bool.and_eq_true] at hg
  obtain ⟨⟨-, hz⟩, hs⟩ := hg
  refine ⟨acvalAt_scalar hp Setlec.natZeroName Setlec.natZeroOk hz rfl
      ?_ _ _,
    acvalAt_scalar hp Setlec.natSuccName Setlec.natSuccOk hs rfl ?_ _ _⟩
  · intro ci h
    cases ci with
    | ctorInfo cv a b =>
      simp only [Setlec.natZeroOk, Bool.and_eq_true] at h
      simpa [ConstantInfo.toConstantVal] using h.1
    | _ => simp [Setlec.natZeroOk] at h
  · intro ci h
    cases ci with
    | ctorInfo cv a b =>
      simp only [Setlec.natSuccOk, Bool.and_eq_true] at h
      simpa [ConstantInfo.toConstantVal] using h.1
    | _ => simp [Setlec.natSuccOk] at h
...
theorem denoteP_instLevels (hp : AcvalParamsAt env acval)
  ...
  | case13 d s hsup =>
    ...
    obtain ⟨ez, es⟩ := acvalAt_natPair hp h0
      (Level.substFn φ [] []) (Level.substFn (Level.substFn φ ks us) [] [])
    have esol := acvalAt_scalar hp Setlec.stringOfListName
      Setlec.stringOfListTyOk h2 rfl
      (by intro ci hh
          simp only [Setlec.stringOfListTyOk, Bool.and_eq_true] at hh
          exact hh.1)
      (Level.substFn φ [] []) (Level.substFn (Level.substFn φ ks us) [] [])
    have echar := acvalAt_scalar hp Setlec.charName Setlec.charTyOk h6 rfl
      (by intro ci hh
          simp only [Setlec.charTyOk, Bool.and_eq_true] at hh
          exact hh.1)
      (Level.substFn φ [] []) (Level.substFn (Level.substFn φ ks us) [] [])
    have eofn := acvalAt_scalar hp Setlec.charOfNatName
      Setlec.charOfNatTyOk h7 rfl
      (by intro ci hh
          simp only [Setlec.charOfNatTyOk, Bool.and_eq_true] at hh
          exact hh.1)
      (Level.substFn φ [] []) (Level.substFn (Level.substFn φ ks us) [] [])
    have enil := acvalAt_one hp Setlec.listNilName Setlec.listNilTyOk h4
      rfl
      (by intro ci hh
          simp only [Setlec.listNilTyOk] at hh
          split at hh
          · next p hpe => simp [hpe]
          · exact nomatch hh)
      φ (Level.substFn φ ks us)
    have econs := acvalAt_one hp Setlec.listConsName Setlec.listConsTyOk
      h5 rfl
      (by intro ci hh
          simp only [Setlec.listConsTyOk] at hh
          split at hh
          · next p hpe => simp [hpe]
          · exact nomatch hh)
      φ (Level.substFn φ ks us)
