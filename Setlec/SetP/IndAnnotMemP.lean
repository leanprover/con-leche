...
of the `annotS` cluster, and `lamTowerStepP`'s `hmem` input.

* **`annotPFrameEqP`** is the per-position annotation identification:
  ...
  new walk — the two position ladders (`prefixGradeFireP`,
  `fieldGradeFireP`) have already fired the frame walks, so all this
  stage does is compose their equalities with the *renaming* identity
  of the two runs' domains (`instPisAt_renEq` + `RenEqT.denoteP`,
...
* **`annotMemP`** is one more position induction of the now-familiar
  shape: at position `k`, the earlier positions' equalities transport
  `Sat2`'s memberships into the λ-tower's slots, `annotOkP_lamTower_slot`
  ...
  because `CtxOkP`'s per-leaf obligation is semantic and
  `annotPFrameEqP` is exactly the equation it asks for
  (`ctxOkP_of_walked_openers`).
...
theorem annotPFrameEqP {m : EnvS2Core V env}
    ...
    (hdomsX0 : ∀ (i : Nat) (x : Expr), xFvsP[i]? = some x →
      denoteP m.acval env φ (rP + i) (Expr.fvarTypeD x)
    ...
    (hpre : ∀ n, n < rP → ∀ ρ' : Nat → V, Sat2 V Δa ρ' →
      AnnotOkP V (fun j => ρ' (j + (rP + cnF - n)))
        ...
        interp2 V (fun j => ρ' (j + (rP + cnF - n)))
          ...
          = interp2 V (fun j => ρ' (j + (rP + cnF - n)))
              (ΓP.getD (rP - 1 - n) default))
    (hfld : ∀ n, rP ≤ n → n < rP + cnF → ∀ ρ' : Nat → V, Sat2 V Δa ρ' →
      ∀ dw, denoteP m.acval env φ (rP + cnF)
        ...
        AnnotOkP V ρ' dw ∧
          interp2 V (fun j => ρ' (j + (rP + cnF - n)))
              (Γs.getD (rP + cnF - 1 - n) default)
            = interp2 V ρ' dw) :
    ∀ i, i < rP + cnF → ∃ Bi : AVExpr,
      denoteP m.acval env φ (rP + cnF)
        (Expr.fvarTypeD ((fvsPl ++ xFvsP).getD i default)) = some Bi ∧
      ∀ ρ' : Nat → V, Sat2 V Δa ρ' →
        AnnotOkP V ρ' Bi ∧
          interp2 V (fun j => ρ' (j + (rP + cnF - i)))
  ...
  have hbridge : ∀ (j : Nat), j < cnF → ∀ d : Nat,
      denoteP m.acval env φ d (cdoms.getD (cnP + j) default)
    ...
    exact RenEqT.denoteP hroT hrq d
  ...
  · -- prefix: the public opener's annotation lifts the recursor slot
    ...
    have hwsTy : Expr.WScoped i ty := by
      ...
      exact hw.2
    have hdomI : denoteP m.acval env φ i ty
    ...
    · intro ρ' hρ'
      ...
      refine ⟨(AnnotOkP_liftN V (rP + cnF - i) _ 0 ρ').mpr ?_, ?_⟩
      · rw [hshiftEnv]
        exact hokP
      · rw [interp2_liftN, hshiftEnv]
        exact heqP
  · -- fields: the public opener's annotation is the crossed domain
    ...
    have hwsTy : Expr.WScoped (rP + j) ty := by
      ...
      simp only [Expr.WScoped] at hw
      exact hw.2
    have hdomI : denoteP m.acval env φ (rP + j) ty
        = some (Γx.getD (cnF - 1 - j) default) := by
      have h := hdomsX0 j xj hxj
      rw [hx2] at h
      exact h
    have hBiden : denoteP m.acval env φ (rP + cnF)
    ...
    intro ρ' hρ'
    have hcd : denoteP m.acval env φ (rP + cnF)
...
induction: the earlier positions' equalities carry `Sat2`'s
memberships into the λ tower, `annotOkP_lamTower_slot` grades slot `k`
out of `hokRa`, and the recorded lam walk fires at the statement
frame's own context through `ctxOkP_of_walked_openers`. -/
theorem annotMemP {m : EnvS2Core V env} {F : Nat}
    ...
    {Ra : AVExpr} (hokRa : ∀ σ : Nat → V, AnnotOkP V σ Ra)
    {ldomsL : List Expr} {lrest2 : Expr}
    (hinstLam : Expr.instLamsAt pfvs rhsA = some (ldomsL, lrest2))
    {Γlam : List AVExpr} {C : AVExpr} (htowerLam : LamTeleP K Ra Γlam C)
    (hdomsLam : ∀ (i : Nat) (x : Expr), ldomsL[i]? = some x →
      denoteP m.acval env φ i x = some (Γlam.getD (K - 1 - i) default))
    -- the recorded lam walk
    (hdeLam : DefEqListOk μ F env K (pfvs.map Expr.fvarTypeD) ldomsL) :
    ∀ k, k < K → ∀ ρ' : Nat → V, Sat2 V Δa ρ' →
      AnnotOkP V (fun j => ρ' (j + (K - k)))
          (Γlam.getD (K - 1 - k) default) ∧
        interp2 V (fun j => ρ' (j + (K - k)))
  ...
  have hwalk : ∀ (i : Nat) (x : Expr), pfvs[i]? = some x →
      ∃ tya, denoteP m.acval env φ K (Expr.fvarTypeD x) = some tya ∧
        (∀ ρ : Nat → V, Sat2 V Δa ρ →
          interp2 V ρ tya
            = interp2 V (fun j => ρ (j + (K - 1 - i) + 1))
                (Γs.getD (K - 1 - i) default)) ∧
        (∀ ρ : Nat → V, Sat2 V Δa ρ → AnnotOkP V ρ tya) := by
    intro i x hx
    have hiK : i < K := by
  ...
  have hgrall : ∀ ρ0 : Nat → V, Sat2 V Δa ρ0 →
      AnnotOkP V (fun j => ρ0 (j + (K - k)))
        (Γlam.getD (K - 1 - k) default) := by
    intro ρ0 hρ0
    refine cast ?_ (annotOkP_lamTower_slot K htowerLam (ρ' := ρ0)
    ...
    · obtain ⟨-, heq⟩ := ihk (K - 1 - q) (by omega) (by omega) ρ0 hρ0
      have hsatm := hρ0 q (Γs.getD q default) (by
        ...
        exact h)
      show ρ0 q ∈ˢ interp2 V (fun j => ρ0 (j + q + 1))
        (Γlam.getD q default)
  ...
  have hwsLd : Expr.WScoped k ld := by
    ...
    · intro i a ha
      ...
      simp only [Expr.WScoped] at hw ⊢
      exact ⟨by omega, hw.2⟩
  have hLkden : denoteP m.acval env φ K ld
      = some ((Γlam.getD (K - 1 - k) default).liftN (K - k) 0) := by
    rw [denoteP_lift m.acval_closed hwsLd K (by omega),
      hdomsLam k ld hld]
    rfl
  ...
  -- the contexts, walked
  have hctxA : CtxOkP m φ K Δa ty :=
    ctxOkP_of_walked_openers (m := m) hΔalen hPshape hPws hwalk
      (n := K) hleafA (fun l hl => hPlt l (hleafA l hl)) hΔaent
  have hctxB : CtxOkP m φ K Δa ld :=
    ctxOkP_of_walked_openers (m := m) hΔalen hPshape hPws hwalk
      (n := K) hleafB (fun l hl => hPlt l (hleafB l hl)) hΔaent
  have hBk' : denoteP m.acval env φ K ty = some Bk := by
    rw [hgpk] at hBk
    exact hBk
