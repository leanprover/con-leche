...
`Setlec.Name`, the checker's own mirror of `Lean.Name` (cached hash in
a `@[computed_field]`, pointer-and-hash-guarded equality substituted by
`@[csimp]`).  Split out of `Setlec/Kernel/Expr.lean` on 2026-09-06 so
that `Setlec/Kernel/PropWhen.lean` — which needs names and nothing
