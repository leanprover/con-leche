...
`Setlec/Kernel/Basis/Builder.lean`; the annotated forms are computed
from them at elaboration time by `#annotate_pins`
(`Setlec/Kernel/BasisGen.lean`).
-/
...
`Setlec/Kernel/BasisGen.lean`), over the pinned prerequisites the
