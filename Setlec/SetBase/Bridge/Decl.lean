...
`Setlec/SetBase/Bridge/*` bridges the checker's *inference* steps into
the `[set]` relation family; this file bridges its **declarations**.
Each of `checkDecl`'s six branches is inverted into the corresponding
`Decl*R` clause of `Setlec/SetBase/Decl.lean`, and `checkDeclR_of`
assembles them.  Composed with `declStepS` (`SetR/Install/Step.lean`)
that gives the `EnvS`-extension step the collapsed lane's consistency
fold runs; the graded lane composes the same records with its own.
...
**Task #161 S8 — the zero-opener.**  This module moved here from
`Setlec/SetR/Bridge/Decl.lean` (statements byte-unchanged, namespace
`Setlec.SetR` unchanged) together with the rest of the model-free
bridge.  The one declaration that did not come along is
`EnvS.toEnvR`, the collapsed lane's own projection, which stayed
behind in `Setlec/SetR/Bridge/Decl.lean`.
-/
