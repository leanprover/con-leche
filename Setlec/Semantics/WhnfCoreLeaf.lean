...
`Setlec/Verify/*`, which is where proofs about kernel functions belong;
...
then carried the `Setlec.SetR` namespace and a `whnfCoreR_*` name
...
section title below), in `Setlec.Semantics`.
-/
