import Setlec.Semantics.Tower.SumLeaf
import Setlec.Semantics.Tower.TowerMk

/-!
# The sum constructor leaf (task #175 sum-types, stage S3)

Constructor `j` of a direct sum is the constant-bit λ-tower (bit `w`)
over its type reading's binder data with the **injection** body: the
pinned pair constructor applied to the tag domain, the case-split
fibre, the numeral `j` and the constructor's own tupler
(`mkTowerGo`).  It reads to `inj j (mkTower f⃗)` in the graph regime and
to the point at squash (`psigmaMkV2`'s own collapse — `injW`), and its
laws consume `MkPreS`, the structure route's `MkPre` with the
per-constructor chain grading and the constructor's index at the base.
-/

namespace Setlec.Semantics
open Setlec.SetModel

open SetTheory
open Setlec.SetTheory.Tower

universe uv

variable {V : Type uv} [SetTheory V]

/-! ## The injection, spelled at a frame -/

/-- `PSigma'.mk Nat (λ k, case k) tag payload`, spelled `d` binders below
the parameter frame (the tower bodies are scoped there). -/
def sumInjAtAV (w : Nat) (Fss : List (List AVExpr)) (d : Nat) (tag payload : AVExpr) : AVExpr :=
  AVExpr.mkAppN (.const .psigmaMk [w, w])
    [natAV, .lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0)),
      tag, payload]

/-- The semantic injection, both regimes: the point at squash. -/
noncomputable def injW (w i : Nat) (a : V) : V := if w = 0 then pt else inj i a

theorem injW_zero (i : Nat) (a : V) : injW 0 i a = pt := if_pos rfl
theorem injW_pos {w : Nat} (hw : w ≠ 0) (i : Nat) (a : V) : injW w i a = inj i a := if_neg hw

/-- The injection's value lands in the carrier. -/
theorem injW_mem {w : Nat} {f : Nat → V} {i : Nat} {a : V} (ha : a ∈ˢ f i) :
    injW w i a ∈ˢ sumSet w f := by
  by_cases hw : w = 0
  · subst hw; rw [injW_zero]; exact pt_mem_sumSet_zero ha
  · rw [injW_pos hw]; exact inj_mem hw ha

/-- The case-split fibre λ at a frame. -/
theorem sumFibreLam_facts {w : Nat} {ρp σ : Nat → V} {d : Nat} (hsh : shiftE d 0 σ = ρp)
    {Fss : List (List AVExpr)} (hok : SumFieldsOkB w ρp Fss) :
    interp2 V σ (.lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0)))
        = lamR (w + 1) omega (natFibre (sumFibre w ρp Fss)) ∧
      lamR (w + 1) (omega : V) (natFibre (sumFibre w ρp Fss)) ∈ˢ psigmaFibreSpace V w omega ∧
      AnnotOk2 V σ (.lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0))) := by
  refine ⟨?_, ?_, ?_⟩
  · rw [interp2_lam]
    exact lamR_congr fun k hk => (case_fibre_at hsh hok hk).1
  · refine lamR_mem fun k hk => ?_
    rw [← (case_fibre_at hsh hok hk).1]
    exact (case_fibre_at hsh hok hk).2.1
  · rw [AnnotOk2_lam]
    exact ⟨trivial, fun k hk => (case_fibre_at hsh hok hk).2.2, fun _ => (univ w : V),
      fun k hk => (case_fibre_at hsh hok hk).2.1, fun h => absurd h (Nat.succ_ne_zero _)⟩

/-- **The injection reads to `injW`**: the pair at a numeral tag with
a fitting payload, the point at squash. -/
theorem sumInjAtAV_interp {w : Nat} {ρp σ : Nat → V} {d : Nat} (hsh : shiftE d 0 σ = ρp)
    {Fss : List (List AVExpr)} (hok : SumFieldsOkB w ρp Fss) {tag payload : AVExpr} {i : Nat}
    (htag : interp2 V σ tag = vnat i)
    (hpay : w ≠ 0 → interp2 V σ payload ∈ˢ sumFibre w ρp Fss i) :
    interp2 V σ (sumInjAtAV w Fss d tag payload) = injW w i (interp2 V σ payload) := by
  obtain ⟨hBv, hBm, -⟩ := sumFibreLam_facts hsh hok
  show SetTheory.app (SetTheory.app (SetTheory.app (SetTheory.app (bval2 V .psigmaMk [w, w])
    omega) (interp2 V σ (.lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0)))))
    (interp2 V σ tag)) (interp2 V σ payload) = _
  rw [hBv, htag]
  by_cases hw : w = 0
  · subst hw
    show SetTheory.app (SetTheory.app (SetTheory.app (SetTheory.app (psigmaMkV2 V 0 0) _) _) _) _ = _
    rw [psigmaMkV2, show Nat.max 0 0 = 0 from rfl, lamR_zero, app_pt, app_pt, app_pt, app_pt,
      injW_zero]
  · have hpay' : interp2 V σ payload
        ∈ˢ SetTheory.app (lamR (w + 1) omega (natFibre (sumFibre w ρp Fss))) (vnat i) := by
      rw [app_lamR_pos (Nat.succ_ne_zero w) (vnat_mem_omega i), natFibre_vnat]
      exact hpay hw
    show SetTheory.app (SetTheory.app (SetTheory.app (SetTheory.app (psigmaMkV2 V w w) _) _) _) _ = _
    rw [psigmaMkV2_app V (omega_mem_univ_pos hw) hBm (vnat_mem_omega i) hpay',
      show Nat.max w w = w from Nat.max_self w, if_neg hw, injW_pos hw]
    rfl

/-- **The injection is graded**: in the graph regime the four slots
are the pair constructor's product chain, at squash the head is the
point and the slots are trivial. -/
theorem sumInjAtAV_ok2 {w : Nat} {ρp σ : Nat → V} {d : Nat} (hsh : shiftE d 0 σ = ρp)
    {Fss : List (List AVExpr)} (hok : SumFieldsOkB w ρp Fss) {tag payload : AVExpr} {i : Nat}
    (hoktag : AnnotOk2 V σ tag) (htag : interp2 V σ tag = vnat i)
    (hokpay : AnnotOk2 V σ payload)
    (hpay : w ≠ 0 → interp2 V σ payload ∈ˢ sumFibre w ρp Fss i) :
    AnnotOk2 V σ (sumInjAtAV w Fss d tag payload) := by
  obtain ⟨hBv, hBm, hBok⟩ := sumFibreLam_facts hsh hok
  by_cases hw : w = 0
  · subst hw
    refine (mkAppN_ok2_of_pt_head (f := .const .psigmaMk [0, 0]) (σ := σ) trivial ?_ ?_).1
    · show psigmaMkV2 V 0 0 = pt
      rw [psigmaMkV2, show Nat.max 0 0 = 0 from rfl, lamR_zero]
    · intro a ha
      simp only [List.mem_cons, List.not_mem_nil, or_false] at ha
      rcases ha with rfl | rfl | rfl | rfl
      · trivial
      · exact hBok
      · exact hoktag
      · exact hokpay
  · have hbv : interp2 V σ (.const .psigmaMk [w, w]) = psigmaMkV2 V w w := rfl
    have hA : (omega : V) ∈ˢ univ w := omega_mem_univ_pos hw
    have hz1 : w = 0 → ∀ A, A ∈ˢ (univ w : V) →
        piR w (psigmaFibreSpace V w A) (fun B => piR w A fun a =>
          piR w (SetTheory.app B a) fun _ => sigmaSet w A
            fun x => SetTheory.app B x) ∈ˢ (univZero : V) := fun h => absurd h hw
    have hz2 : w = 0 → ∀ B, B ∈ˢ psigmaFibreSpace V w omega →
        piR w omega (fun a => piR w (SetTheory.app B a)
          fun _ => sigmaSet w omega fun x => SetTheory.app B x) ∈ˢ (univZero : V) :=
      fun h => absurd h hw
    have hz3 : w = 0 → ∀ a, a ∈ˢ (omega : V) →
        piR w (SetTheory.app (lamR (w + 1) omega (natFibre (sumFibre w ρp Fss))) a)
          (fun _ => sigmaSet w omega
            fun x => SetTheory.app (lamR (w + 1) omega (natFibre (sumFibre w ρp Fss))) x)
          ∈ˢ (univZero : V) := fun h => absurd h hw
    have hz4 : w = 0 → ∀ x,
        x ∈ˢ SetTheory.app (lamR (w + 1) omega (natFibre (sumFibre w ρp Fss))) (vnat i) →
        sigmaSet w omega
          (fun y => SetTheory.app (lamR (w + 1) omega (natFibre (sumFibre w ρp Fss))) y)
          ∈ˢ (univZero : V) := fun h => absurd h hw
    have hm0 := psigmaMkV2_ww_mem (V := V) w
    have hm1 := app_mem_piR hm0 hA hz1
    have hm2 := app_mem_piR hm1 hBm hz2
    have hm3 := app_mem_piR hm2 (vnat_mem_omega i) hz3
    have hpay' : interp2 V σ payload
        ∈ˢ SetTheory.app (lamR (w + 1) omega (natFibre (sumFibre w ρp Fss))) (vnat i) := by
      rw [app_lamR_pos (Nat.succ_ne_zero w) (vnat_mem_omega i), natFibre_vnat]
      exact hpay hw
    show AnnotOk2 V σ (.app (.app (.app (.app (.const .psigmaMk [w, w]) natAV)
      (.lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0)))) tag) payload)
    rw [AnnotOk2_app]
    refine ⟨?_, hokpay, w, _, _, ?_, hpay', hz4⟩
    · rw [AnnotOk2_app]
      refine ⟨?_, hoktag, w, omega, _, ?_, by rw [htag]; exact vnat_mem_omega i, hz3⟩
      · rw [AnnotOk2_app]
        refine ⟨?_, hBok, w, psigmaFibreSpace V w omega, _, ?_, by rw [hBv]; exact hBm, hz2⟩
        · rw [AnnotOk2_app]
          exact ⟨trivial, trivial, w, univ w, _, hbv ▸ hm0, hA, hz1⟩
        · show SetTheory.app (interp2 V σ (.const .psigmaMk [w, w])) omega ∈ˢ _
          rw [hbv]; exact hm1
      · show SetTheory.app (SetTheory.app (interp2 V σ (.const .psigmaMk [w, w])) omega)
          (interp2 V σ (.lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0))))
          ∈ˢ _
        rw [hbv, hBv]; exact hm2
    · show SetTheory.app (SetTheory.app (SetTheory.app (interp2 V σ (.const .psigmaMk [w, w])) omega)
        (interp2 V σ (.lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0)))))
        (interp2 V σ tag) ∈ˢ _
      rw [hbv, hBv, htag]; exact hm3

/-! ## The constructor leaf -/

/-- **Constructor `j`'s leaf**: the constant-bit λ-tower (bit `w`)
over the constructor type reading's binder data with the injection
of the tupler at the numeral `j`. -/
def directSumMkAV (w j : Nat) (ds : List (Nat × Nat × AVExpr)) (Fs : List AVExpr)
    (Fss : List (List AVExpr)) : AVExpr :=
  mkLamsC w ds (sumInjAtAV w Fss Fs.length (numeralAV j) (mkTowerGo w Fs))

/-- `MkPreS`: the constructor leaf's ONE hereditary premise — each
parameter domain graded, and under every fitting parameter spine every
constructor's chain is graded, this constructor's chain is the `j`-th,
and the type reading's body reads back as the sum carrier. -/
def MkPreS (w j : Nat) (ρ : Nat → V) (Fs : List AVExpr) (Fss : List (List AVExpr))
    (bodyC : AVExpr) : List (Nat × Nat × AVExpr) → Prop
  | [] => SumFieldsOkB w ρ Fss ∧ Fss[j]? = some Fs ∧ ∀ bs, SpineFit ρ Fs bs →
      interp2 V (consList bs ρ) bodyC = sumSet w (sumFibre w ρ Fss)
  | d :: pds => AnnotOk2 V ρ d.2.2 ∧
      ∀ a, a ∈ˢ interp2 V ρ d.2.2 → MkPreS w j (cons a ρ) Fs Fss bodyC pds

/-- The injection body at a fitting field frame: its value, its
grading, its membership in the carrier. -/
theorem sumInj_at_fields {w j : Nat} {ρp : Nat → V} {Fs : List AVExpr}
    {Fss : List (List AVExpr)} {bs : List V}
    (hok : SumFieldsOkB w ρp Fss) (hj : Fss[j]? = some Fs) (hsp : SpineFit ρp Fs bs) :
    interp2 V (consList bs ρp) (sumInjAtAV w Fss Fs.length (numeralAV j) (mkTowerGo w Fs))
        = injW w j (if w = 0 then pt else mkTower bs) ∧
      AnnotOk2 V (consList bs ρp) (sumInjAtAV w Fss Fs.length (numeralAV j) (mkTowerGo w Fs)) ∧
      interp2 V (consList bs ρp) (sumInjAtAV w Fss Fs.length (numeralAV j) (mkTowerGo w Fs))
        ∈ˢ sumSet w (sumFibre w ρp Fss) := by
  have hokF : FieldsOkB w ρp Fs := hok Fs (List.mem_of_getElem? hj)
  have hlen : bs.length = Fs.length := hsp.length_eq
  have hsh : shiftE Fs.length 0 (consList bs ρp) = ρp := by rw [← hlen]; exact shiftE_consList bs ρp
  have hmk := mkTowerGo_interp (w := w) (fun hw => hokF.toBound hw) hsp
  have hpay : w ≠ 0 → interp2 V (consList bs ρp) (mkTowerGo w Fs) ∈ˢ sumFibre w ρp Fss j := by
    intro hw
    rw [hmk, if_neg hw, sumFibre_of_getElem? hj]
    exact mkTower_mem_teleOfFields hw hsp
  have hv := sumInjAtAV_interp hsh hok (interp2_numeralAV j _) hpay
  rw [hmk] at hv
  refine ⟨hv, sumInjAtAV_ok2 hsh hok (numeralAV_ok2 j _) (interp2_numeralAV j _)
    (mkTowerGo_ok2 hokF hsp) hpay, ?_⟩
  rw [hv]
  refine injW_mem (f := sumFibre w ρp Fss) ?_
  rw [sumFibre_of_getElem? hj]
  split
  · next hz => exact hz ▸ pt_mem_tower_teleOfFields hsp
  · next hnz => exact mkTower_mem_teleOfFields hnz hsp

/-- The field phase of the constructor leaf's premise (the walk
carries the prefix spine, as `underTowerOk_fields`). -/
theorem underTowerOkS_fields {w j : Nat} {bodyC : AVExpr} {ρp : Nat → V}
    {Fs : List AVExpr} {Fss : List (List AVExpr)}
    (hok : SumFieldsOkB w ρp Fss) (hj : Fss[j]? = some Fs)
    (hbody : ∀ bs : List V, SpineFit ρp Fs bs →
      interp2 V (consList bs ρp) bodyC = sumSet w (sumFibre w ρp Fss)) :
    ∀ {rest : List (Nat × Nat × AVExpr)} {pre : List AVExpr} {bs : List V},
      Fs = pre ++ rest.map (·.2.2) → SpineFit ρp pre bs →
      UnderTowerOk w (consList bs ρp)
        (sumInjAtAV w Fss Fs.length (numeralAV j) (mkTowerGo w Fs)) bodyC rest
  | [], pre, bs, hsplit, hsp => by
    have hspF : SpineFit ρp Fs bs := by
      rw [hsplit, List.map_nil, List.append_nil]; exact hsp
    obtain ⟨-, hok2, hmem⟩ := sumInj_at_fields hok hj hspF
    refine ⟨hok2, ?_, ?_⟩
    · rw [hbody bs hspF]; exact hmem
    · intro h0
      rw [hbody bs hspF, h0]
      exact sumSet_zero_mem_univZero _
  | d :: rest, pre, bs, hsplit, hsp => by
    have hokF : FieldsOkB w ρp Fs := hok Fs (List.mem_of_getElem? hj)
    have hd : FieldsOkB w (consList bs ρp) (d.2.2 :: rest.map (·.2.2)) :=
      FieldsOkB.drop (hsplit ▸ hokF) hsp
    refine ⟨hd.1, fun a ha => ?_⟩
    have hstep : UnderTowerOk w (consList (bs ++ [a]) ρp)
        (sumInjAtAV w Fss Fs.length (numeralAV j) (mkTowerGo w Fs)) bodyC rest :=
      underTowerOkS_fields hok hj hbody (pre := pre ++ [d.2.2])
        (by rw [hsplit, List.map_cons, List.append_assoc, List.singleton_append])
        (hsp.append ⟨ha, trivial⟩)
    rwa [consList_append, consList_cons, consList_nil] at hstep

/-- The parameter phase: `MkPreS` walks down to the field phase. -/
theorem underTowerOk_of_mkPreS {w j : Nat} {bodyC : AVExpr}
    {Fs : List AVExpr} {Fss : List (List AVExpr)} {fds : List (Nat × Nat × AVExpr)} :
    ∀ {pds : List (Nat × Nat × AVExpr)} {ρ : Nat → V},
      MkPreS w j ρ Fs Fss bodyC pds → Fs = fds.map (·.2.2) →
      UnderTowerOk w ρ (sumInjAtAV w Fss Fs.length (numeralAV j) (mkTowerGo w Fs)) bodyC
        (pds ++ fds)
  | [], ρ, h, hFs =>
    underTowerOkS_fields h.1 h.2.1 h.2.2 (pre := []) (bs := []) (by simpa using hFs) trivial
  | d :: pds, ρ, h, hFs =>
    ⟨h.1, fun a ha => underTowerOk_of_mkPreS (h.2 a ha) hFs⟩

/-- **The constructor leaf inhabits its type's reading.** -/
theorem directSumMkAV_mem {w j : Nat} {bodyC : AVExpr} {ρ : Nat → V}
    {Fss : List (List AVExpr)} {pds fds : List (Nat × Nat × AVExpr)}
    (hz : ∀ d ∈ pds ++ fds, (w = 0 ↔ d.2.1 = 0))
    (hpre : MkPreS w j ρ (fds.map (·.2.2)) Fss bodyC pds) :
    interp2 V ρ (directSumMkAV w j (pds ++ fds) (fds.map (·.2.2)) Fss)
      ∈ˢ interp2 V ρ (mkPisAV (pds ++ fds) bodyC) :=
  mkLamsC_mem hz (underTowerOk_of_mkPreS hpre rfl)

/-- **The constructor leaf is graded.** -/
theorem directSumMkAV_ok2 {w j : Nat} {bodyC : AVExpr} {ρ : Nat → V}
    {Fss : List (List AVExpr)} {pds fds : List (Nat × Nat × AVExpr)}
    (hz : ∀ d ∈ pds ++ fds, (w = 0 ↔ d.2.1 = 0))
    (hpre : MkPreS w j ρ (fds.map (·.2.2)) Fss bodyC pds) :
    AnnotOk2 V ρ (directSumMkAV w j (pds ++ fds) (fds.map (·.2.2)) Fss) :=
  mkLamsC_ok2 hz (underTowerOk_of_mkPreS hpre rfl)

/-- **The constructor leaf's application fold** (graph regime): along
a fitting parameter + field spine the leaf computes the injection of
the tupler. -/
theorem directSumMkAV_fold {w j : Nat} (hw : w ≠ 0)
    {pds fds : List (Nat × Nat × AVExpr)} {Fss : List (List AVExpr)} {ρ : Nat → V}
    {as bs : List V}
    (hsp₁ : SpineFit ρ (pds.map (·.2.2)) as)
    (hsp₂ : SpineFit (consList as ρ) (fds.map (·.2.2)) bs)
    (hok : SumFieldsOkB w (consList as ρ) Fss)
    (hj : Fss[j]? = some (fds.map (·.2.2))) :
    (as ++ bs).foldl SetTheory.app
        (interp2 V ρ (directSumMkAV w j (pds ++ fds) (fds.map (·.2.2)) Fss))
      = inj j (mkTower bs) := by
  have hsp : SpineFit ρ
      (((pds ++ fds).map fun d => (w, d.2.2)).map (·.2)) (as ++ bs) := by
    have h2 : (((pds ++ fds).map fun d => (w, d.2.2)).map (·.2))
        = pds.map (·.2.2) ++ fds.map (·.2.2) := by
      simp [List.map_map, Function.comp_def]
    rw [h2]
    exact hsp₁.append hsp₂
  rw [directSumMkAV, mkLamsC,
    mkLamsAV_fold (fun d hd => by
      obtain ⟨d', -, rfl⟩ := List.mem_map.mp hd
      exact hw) hsp,
    consList_append, (sumInj_at_fields hok hj hsp₂).1, if_neg hw, injW_pos hw]

/-- **The constructor leaf at a squash instantiation is the point.** -/
theorem directSumMkAV_zero {j : Nat} {ds : List (Nat × Nat × AVExpr)} {Fs : List AVExpr}
    {Fss : List (List AVExpr)} {ρ : Nat → V} :
    interp2 V ρ (directSumMkAV 0 j ds Fs Fss) = (pt : V) := by
  match ds with
  | [] =>
    show interp2 V ρ (sumInjAtAV 0 Fss Fs.length (numeralAV j) (mkTowerGo 0 Fs)) = pt
    show SetTheory.app (SetTheory.app (SetTheory.app (SetTheory.app (psigmaMkV2 V 0 0) _) _) _) _ = _
    rw [psigmaMkV2, show Nat.max 0 0 = 0 from rfl, lamR_zero, app_pt, app_pt, app_pt, app_pt]
  | d :: ds => exact mkLamsAV_zero_head d.2.2 _ _ ρ

end Setlec.Semantics
