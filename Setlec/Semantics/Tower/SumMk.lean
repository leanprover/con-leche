import Setlec.Semantics.Tower.IdxEq
...
def sumInjAtAV (w : Nat) (Fss : List (List AVExpr)) (d : Nat) (tag payload : AVExpr) : AVExpr :=
  AVExpr.mkAppN (.const .psigmaMk [w, w])
...
theorem sumFibreLam_facts {w : Nat} {ρp σ : Nat → V} {d : Nat} (hsh : shiftE d 0 σ = ρp)
    {Fss : List (List AVExpr)} (hok : SumFieldsOkB w ρp Fss) :
    interp2 V σ (.lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0)))
        = lamR (w + 1) omega (natFibre (sumFibre w ρp Fss)) ∧
      lamR (w + 1) (omega : V) (natFibre (sumFibre w ρp Fss)) ∈ˢ psigmaFibreSpace V w omega ∧
      AnnotOk2 V σ (.lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0))) := by
  refine ⟨?_, ?_, ?_⟩
  · rw [interp2_lam]
...
theorem sumInjAtAV_interp {w : Nat} {ρp σ : Nat → V} {d : Nat} (hsh : shiftE d 0 σ = ρp)
    {Fss : List (List AVExpr)} (hok : SumFieldsOkB w ρp Fss) {tag payload : AVExpr} {i : Nat}
    (htag : interp2 V σ tag = vnat i)
    (hpay : w ≠ 0 → interp2 V σ payload ∈ˢ sumFibre w ρp Fss i) :
    interp2 V σ (sumInjAtAV w Fss d tag payload) = injW w i (interp2 V σ payload) := by
  obtain ⟨hBv, hBm, -⟩ := sumFibreLam_facts hsh hok
  show SetTheory.app (SetTheory.app (SetTheory.app (SetTheory.app (bval2 V .psigmaMk [w, w])
    omega) (interp2 V σ (.lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0)))))
    (interp2 V σ tag)) (interp2 V σ payload) = _
  rw [hBv, htag]
  by_cases hw : w = 0
  · subst hw
...
theorem sumInjAtAV_ok2 {w : Nat} {ρp σ : Nat → V} {d : Nat} (hsh : shiftE d 0 σ = ρp)
    {Fss : List (List AVExpr)} (hok : SumFieldsOkB w ρp Fss) {tag payload : AVExpr} {i : Nat}
    (hoktag : AnnotOk2 V σ tag) (htag : interp2 V σ tag = vnat i)
    (hokpay : AnnotOk2 V σ payload)
    (hpay : w ≠ 0 → interp2 V σ payload ∈ˢ sumFibre w ρp Fss i) :
    AnnotOk2 V σ (sumInjAtAV w Fss d tag payload) := by
  obtain ⟨hBv, hBm, hBok⟩ := sumFibreLam_facts hsh hok
  by_cases hw : w = 0
  · subst hw
    refine (mkAppN_ok2_of_pt_head (f := .const .psigmaMk [0, 0]) (σ := σ) trivial ?_ ?_).1
    ...
    · intro a ha
      ...
      · exact hoktag
      · exact hokpay
  · have hbv : interp2 V σ (.const .psigmaMk [w, w]) = psigmaMkV2 V w w := rfl
    ...
    have hpay' : interp2 V σ payload
      ...
      rw [app_lamR_pos (Nat.succ_ne_zero w) (vnat_mem_omega i), natFibre_vnat]
      exact hpay hw
    show AnnotOk2 V σ (.app (.app (.app (.app (.const .psigmaMk [w, w]) natAV)
    ...
    · rw [AnnotOk2_app]
      ...
      · rw [AnnotOk2_app]
        ...
        · show SetTheory.app (interp2 V σ (.const .psigmaMk [w, w])) omega ∈ˢ _
          rw [hbv]; exact hm1
      · show SetTheory.app (SetTheory.app (interp2 V σ (.const .psigmaMk [w, w])) omega)
          (interp2 V σ (.lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0))))
          ∈ˢ _
        rw [hbv, hBv]; exact hm2
    · show SetTheory.app (SetTheory.app (SetTheory.app (interp2 V σ (.const .psigmaMk [w, w])) omega)
        (interp2 V σ (.lam (w + 1) natAV (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0)))))
        (interp2 V σ tag) ∈ˢ _
...
def mkTowerGoUPos (w : Nat) (E : AVExpr) : List AVExpr → AVExpr
  | [] =>
...
def mkTowerGoU (w : Nat) (Fs : List AVExpr) (E : AVExpr) : AVExpr :=
  if w = 0 then .const .punitUnit [] else mkTowerGoUPos w E Fs

theorem mkTowerGoU_zero (Fs : List AVExpr) (E : AVExpr) :
    mkTowerGoU 0 Fs E = .const .punitUnit [] := if_pos rfl

theorem mkTowerGoU_pos {w : Nat} (hw : w ≠ 0) (Fs : List AVExpr) (E : AVExpr) :
...
theorem mkTowerGoUPos_interp {w : Nat} (hw : w ≠ 0) {E : AVExpr} :
    ∀ {Fs : List AVExpr} {ρp : Nat → V} {bs : List V},
      FieldsBound w ρp (Fs ++ [E]) → SpineFit ρp Fs bs →
      (pt : V) ∈ˢ interp2 V (consList bs ρp) E →
      interp2 V (consList bs ρp) (mkTowerGoUPos w E Fs) = mkTower (bs ++ [pt])
  | [], ρp, [], hb, _, hpt => by
    have hA : interp2 V ρp E ∈ˢ (univ w : V) := hb.1
    have hB : (lamR (w + 1) (interp2 V ρp E) fun _ => (unitSet : V))
        ∈ˢ psigmaFibreSpace V w (interp2 V ρp E) :=
      lamR_mem fun _ _ => unitSet_mem_univ w
    have hpt' : (pt : V) ∈ˢ interp2 V ρp E := by simpa [consList] using hpt
    have hb' : (pt : V) ∈ˢ SetTheory.app (lamR (w + 1) (interp2 V ρp E) fun _ => (unitSet : V)) pt := by
      rw [app_lamR_pos (Nat.succ_ne_zero w) hpt']
      exact pt_mem_unitSet
    show SetTheory.app (SetTheory.app (SetTheory.app (SetTheory.app (bval2 V .psigmaMk [w, w])
        (interp2 V ρp E)) (lamR (w + 1) (interp2 V ρp E) fun _ => (unitSet : V))) pt) pt
      = mkTower [pt]
  ...
  | F :: Fs, ρp, b :: bs, hb, hsp, hpt => by
    ...
    have hBfun : ∀ x : V,
        interp2 V (cons x (consList bs (cons b ρp)))
          ((towerBodyAV w (Fs ++ [E])).liftN (Fs.length + 1) 1)
        = interp2 V (cons x ρp) (towerBodyAV w (Fs ++ [E])) := fun x => by
    ...
    have hfib : SetTheory.app (lamR (w + 1) (interp2 V ρp F)
          fun x => interp2 V (cons x ρp) (towerBodyAV w (Fs ++ [E]))) b
    ...
    have hbm : mkTower (bs ++ [pt])
        ∈ˢ SetTheory.app (lamR (w + 1) (interp2 V ρp F)
          fun x => interp2 V (cons x ρp) (towerBodyAV w (Fs ++ [E]))) b := by
      rw [hfib]
    ...
    show SetTheory.app (SetTheory.app (SetTheory.app (SetTheory.app
        ...
        (interp2 V (consList bs (cons b ρp)) (mkTowerGoUPos w E Fs))
      = mkTower (b :: bs ++ [pt])
    have hbv : bval2 V .psigmaMk [w, w] = psigmaMkV2 V w w := rfl
    rw [hA, hval, hrec]
    ...
    rfl
where
  spineFit_append_split_mpr {ρ : Nat → V} {Fs Gs : List AVExpr} {as bs : List V}
...
theorem mkTowerGoU_interp {w : Nat} {E : AVExpr} {Fs : List AVExpr} {ρp : Nat → V}
    {bs : List V} (hb : w ≠ 0 → FieldsBound w ρp (Fs ++ [E])) (hsp : SpineFit ρp Fs bs)
    (hpt : (pt : V) ∈ˢ interp2 V (consList bs ρp) E) :
    interp2 V (consList bs ρp) (mkTowerGoU w Fs E)
...
theorem mkTowerGoUPos_ok2 {w : Nat} (hw : w ≠ 0) {E : AVExpr} :
    ∀ {Fs : List AVExpr} {ρp : Nat → V} {bs : List V},
      FieldsOkB w ρp (Fs ++ [E]) → SpineFit ρp Fs bs →
      (pt : V) ∈ˢ interp2 V (consList bs ρp) E →
      AnnotOk2 V (consList bs ρp) (mkTowerGoUPos w E Fs)
  | [], ρp, [], hok, _, hpt => by
    have hokE : AnnotOk2 V ρp E := hok.1
    have hA : interp2 V ρp E ∈ˢ (univ w : V) := hok.2.1 hw
    have hpt' : (pt : V) ∈ˢ interp2 V ρp E := by simpa [consList] using hpt
    have hBm : (lamR (w + 1) (interp2 V ρp E) fun _ => (unitSet : V))
        ∈ˢ psigmaFibreSpace V w (interp2 V ρp E) :=
      lamR_mem fun _ _ => unitSet_mem_univ w
    have hbv : interp2 V ρp (.const .psigmaMk [w, w]) = psigmaMkV2 V w w := rfl
    have hz1 : w = 0 → ∀ A, A ∈ˢ (univ w : V) →
        piR w (psigmaFibreSpace V w A) (fun B => piR w A fun a =>
          piR w (SetTheory.app B a) fun _ => sigmaSet w A
            fun x => SetTheory.app B x) ∈ˢ (univZero : V) := fun h => absurd h hw
    have hz2 : w = 0 → ∀ B, B ∈ˢ psigmaFibreSpace V w (interp2 V ρp E) →
        piR w (interp2 V ρp E) (fun a => piR w (SetTheory.app B a)
          fun _ => sigmaSet w (interp2 V ρp E) fun x => SetTheory.app B x) ∈ˢ (univZero : V) :=
      fun h => absurd h hw
    have hz3 : w = 0 → ∀ a, a ∈ˢ interp2 V ρp E →
        piR w (SetTheory.app (lamR (w + 1) (interp2 V ρp E) fun _ => (unitSet : V)) a)
          (fun _ => sigmaSet w (interp2 V ρp E)
            fun x => SetTheory.app (lamR (w + 1) (interp2 V ρp E) fun _ => (unitSet : V)) x)
          ∈ˢ (univZero : V) := fun h => absurd h hw
    have hz4 : w = 0 → ∀ x,
        x ∈ˢ SetTheory.app (lamR (w + 1) (interp2 V ρp E) fun _ => (unitSet : V)) pt →
        sigmaSet w (interp2 V ρp E)
          (fun y => SetTheory.app (lamR (w + 1) (interp2 V ρp E) fun _ => (unitSet : V)) y)
          ∈ˢ (univZero : V) := fun h => absurd h hw
    have hm0 := psigmaMkV2_ww_mem (V := V) w
    have hm1 := app_mem_piR hm0 hA hz1
    have hm2 := app_mem_piR hm1 hBm hz2
    have hm3 := app_mem_piR hm2 hpt' hz3
    have hlam : interp2 V ρp (.lam (w + 1) E (.const .punit [w + 1]))
        = lamR (w + 1) (interp2 V ρp E) fun _ => (unitSet : V) := rfl
    have hb' : (pt : V) ∈ˢ SetTheory.app (lamR (w + 1) (interp2 V ρp E) fun _ => (unitSet : V)) pt := by
      rw [app_lamR_pos (Nat.succ_ne_zero w) hpt']
      exact pt_mem_unitSet
    show AnnotOk2 V ρp (.app (.app (.app (.app (.const .psigmaMk [w, w]) E)
      (.lam (w + 1) E (.const .punit [w + 1]))) .prf) (.const .punitUnit []))
    rw [AnnotOk2_app]
    refine ⟨?_, trivial, w, _, _, ?_, hb', hz4⟩
    · rw [AnnotOk2_app]
      refine ⟨?_, trivial, w, interp2 V ρp E, _, ?_, hpt', hz3⟩
      · rw [AnnotOk2_app]
        refine ⟨?_, ?_, w, psigmaFibreSpace V w (interp2 V ρp E), _, ?_, ?_, hz2⟩
        · rw [AnnotOk2_app]
        ...
        · show SetTheory.app (interp2 V ρp (.const .psigmaMk [w, w])) (interp2 V ρp E) ∈ˢ _
          rw [hbv]; exact hm1
        · rw [hlam]; exact hBm
      · show SetTheory.app (SetTheory.app (interp2 V ρp (.const .psigmaMk [w, w])) (interp2 V ρp E))
          (interp2 V ρp (.lam (w + 1) E (.const .punit [w + 1]))) ∈ˢ _
        rw [hbv, hlam]; exact hm2
    · show SetTheory.app (SetTheory.app (SetTheory.app (interp2 V ρp (.const .psigmaMk [w, w]))
        (interp2 V ρp E)) (interp2 V ρp (.lam (w + 1) E (.const .punit [w + 1]))))
        (interp2 V ρp .prf) ∈ˢ _
      rw [hbv, hlam, interp2_prf]; exact hm3
  | [], _, _ :: _, _, hsp, _ => hsp.elim
  | _ :: _, _, [], _, hsp, _ => hsp.elim
  | F :: Fs, ρp, b :: bs, hok, hsp, hpt => by
    ...
    have hBv : interp2 V (consList bs (cons b ρp))
        (AVExpr.lam (w + 1) (F.liftN (Fs.length + 1))
          ((towerBodyAV w (Fs ++ [E])).liftN (Fs.length + 1) 1))
        = lamR (w + 1) (interp2 V ρp F)
            (fun x => interp2 V (cons x ρp) (towerBodyAV w (Fs ++ [E]))) := by
      rw [interp2_lam, hA]
    ...
    have hz1 : w = 0 → ∀ A, A ∈ˢ (univ w : V) →
        piR w (psigmaFibreSpace V w A) (fun B => piR w A fun a =>
          piR w (SetTheory.app B a) fun _ => sigmaSet w A
            fun x => SetTheory.app B x) ∈ˢ (univZero : V) := fun h => absurd h hw
    have hz2 : w = 0 → ∀ B,
        B ∈ˢ psigmaFibreSpace V w (interp2 V ρp F) →
        piR w (interp2 V ρp F) (fun a => piR w (SetTheory.app B a)
          fun _ => sigmaSet w (interp2 V ρp F)
            fun x => SetTheory.app B x) ∈ˢ (univZero : V) := fun h => absurd h hw
    have hz3 : w = 0 → ∀ a, a ∈ˢ interp2 V ρp F →
        piR w (SetTheory.app (lamR (w + 1) (interp2 V ρp F)
            fun x => interp2 V (cons x ρp) (towerBodyAV w (Fs ++ [E]))) a)
          (fun _ => sigmaSet w (interp2 V ρp F)
            fun x => SetTheory.app (lamR (w + 1) (interp2 V ρp F)
              fun y => interp2 V (cons y ρp) (towerBodyAV w (Fs ++ [E]))) x)
          ∈ˢ (univZero : V) := fun h => absurd h hw
    have hz4 : w = 0 → ∀ x,
        x ∈ˢ SetTheory.app (lamR (w + 1) (interp2 V ρp F)
          fun y => interp2 V (cons y ρp) (towerBodyAV w (Fs ++ [E]))) b →
        sigmaSet w (interp2 V ρp F)
          (fun y => SetTheory.app (lamR (w + 1) (interp2 V ρp F)
            fun z => interp2 V (cons z ρp) (towerBodyAV w (Fs ++ [E]))) y)
          ∈ˢ (univZero : V) := fun h => absurd h hw
    ...
    have hfib : SetTheory.app (lamR (w + 1) (interp2 V ρp F)
      ...
      rw [app_lamR_pos (Nat.succ_ne_zero w) hsp.1,
        towerBodyAV_interp (fun _ => hb.2 b hsp.1)]
    have hrm : interp2 V (consList bs (cons b ρp)) (mkTowerGoUPos w E Fs)
        ∈ˢ SetTheory.app (lamR (w + 1) (interp2 V ρp F)
          fun x => interp2 V (cons x ρp) (towerBodyAV w (Fs ++ [E]))) b := by
      rw [hrec, hfib]
      exact mkTower_mem_teleOfFields hw (hsp.2.append ⟨hpt, trivial⟩)
    show AnnotOk2 V (consList bs (cons b ρp))
      (.app (.app (.app (.app (.const .psigmaMk [w, w])
          (F.liftN (Fs.length + 1)))
          (.lam (w + 1) (F.liftN (Fs.length + 1))
            ((towerBodyAV w (Fs ++ [E])).liftN (Fs.length + 1) 1)))
        ...
        (mkTowerGoUPos w E Fs))
    rw [AnnotOk2_app]
    refine ⟨?_, mkTowerGoUPos_ok2 hw (hok.2.2 b hsp.1) hsp.2 hpt, ?_⟩
    · rw [AnnotOk2_app]
      refine ⟨?_, trivial, ?_⟩
      · rw [AnnotOk2_app]
        refine ⟨?_, ?_, ?_⟩
        · rw [AnnotOk2_app]
          refine ⟨trivial, ?_, ?_⟩
          · rw [AnnotOk2_liftN, hshift]
            exact hok.1
          · exact ⟨w, univ w, _, hm0, by rw [interp2_liftN, hshift]; exact hAm, hz1⟩
        · rw [AnnotOk2_lam]
          refine ⟨?_, ?_, ?_⟩
          · rw [AnnotOk2_liftN, hshift]
            exact hok.1
          · intro x hx
            rw [hA] at hx
            rw [AnnotOk2_liftN, ← cons_shiftE, hshift]
            exact towerBodyAV_ok2 (hok.2.2 x hx)
        ...
        · refine ⟨w, psigmaFibreSpace V w (interp2 V ρp F), _, ?_, ?_, hz2⟩
          · show SetTheory.app (interp2 V (consList bs (cons b ρp))
                (.const .psigmaMk [w, w]))
              (interp2 V (consList bs (cons b ρp))
                (F.liftN (Fs.length + 1))) ∈ˢ _
            rw [hA]
            exact hm1
          · rw [hBv]
            exact hBm
      · refine ⟨w, interp2 V ρp F, _, ?_, ?_, hz3⟩
        · show SetTheory.app (SetTheory.app
          ...
          rw [hA, hBv]
          exact hm2
        · show consList bs (cons b ρp) (Fs.length) ∈ˢ interp2 V ρp F
          rw [hval]
          exact hsp.1
    · refine ⟨w, SetTheory.app (lamR (w + 1) (interp2 V ρp F)
          fun x => interp2 V (cons x ρp) (towerBodyAV w (Fs ++ [E]))) b, _,
        ?_, hrm, hz4⟩
...
theorem mkTowerGoU_ok2 {w : Nat} {E : AVExpr} {Fs : List AVExpr} {ρp : Nat → V}
    {bs : List V} (hok : FieldsOkB w ρp (Fs ++ [E])) (hsp : SpineFit ρp Fs bs)
    (hpt : (pt : V) ∈ˢ interp2 V (consList bs ρp) E) :
    AnnotOk2 V (consList bs ρp) (mkTowerGoU w Fs E) := by
  by_cases hw : w = 0
  · subst hw; rw [mkTowerGoU_zero]; simp
  · rw [mkTowerGoU_pos hw]; exact mkTowerGoUPos_ok2 hw hok hsp hpt
...
is the point). -/
def uChains (Fss : List (List AVExpr)) : List (List AVExpr) :=
  Fss.map fun Fs => Fs ++ [idxEqAV []]

theorem uChains_getElem? (Fss : List (List AVExpr)) (j : Nat) :
...
theorem pt_mem_idxEqAV_nil (ρ : Nat → V) : (pt : V) ∈ˢ interp2 V ρ (idxEqAV []) :=
...
def directSumMkAV (w j : Nat) (ds : List (Nat × Nat × AVExpr)) (Fs : List AVExpr)
...
theorem sumInj_at_fields {w j : Nat} {ρp : Nat → V} {Fs : List AVExpr}
  ...
  have hpt : (pt : V) ∈ˢ interp2 V (consList bs ρp) (idxEqAV []) := pt_mem_idxEqAV_nil _
  have hmk := mkTowerGoU_interp (w := w) (fun hw => hokF.toBound hw) hsp hpt
  have hpay : w ≠ 0 → interp2 V (consList bs ρp) (mkTowerGoU w Fs (idxEqAV []))
      ∈ˢ sumFibre w ρp Fss j := by
    intro hw
    rw [hmk, if_neg hw, sumFibre_of_getElem? hj]
    exact mkTower_mem_teleOfFields hw (hsp.append ⟨hpt, trivial⟩)
  have hv := sumInjAtAV_interp hsh hok (interp2_numeralAV j _) hpay
  rw [hmk] at hv
  exact ⟨hv, sumInjAtAV_ok2 hsh hok (numeralAV_ok2 j _) (interp2_numeralAV j _)
    (mkTowerGoU_ok2 hokF hsp hpt) hpay⟩

/-- The field phase of the constructor leaf's premise (the walk
carries the prefix spine, as `underTowerOk_fields`). -/
theorem underTowerOkS_fields {w j : Nat} {bodyC : AVExpr} {ρp : Nat → V}
    {Fs : List AVExpr} {Fss : List (List AVExpr)}
    (hok : SumFieldsOkB w ρp Fss) (hj : Fss[j]? = some (Fs ++ [idxEqAV []]))
    (hbody : ∀ bs : List V, SpineFit ρp Fs bs →
      ∃ f : Nat → V, interp2 V (consList bs ρp) bodyC = sumSet w f ∧
        (if w = 0 then (pt : V) else mkTower (bs ++ [pt])) ∈ˢ f j) :
    ∀ {rest : List (Nat × Nat × AVExpr)} {pre : List AVExpr} {bs : List V},
...
theorem underTowerOk_of_mkPreS {w j : Nat} {bodyC : AVExpr}
    {Fs : List AVExpr} {Fss : List (List AVExpr)} {fds : List (Nat × Nat × AVExpr)} :
...
theorem directSumMkAV_mem {w j : Nat} {bodyC : AVExpr} {ρ : Nat → V}
    {Fss : List (List AVExpr)} {pds fds : List (Nat × Nat × AVExpr)}
    (hz : ∀ d ∈ pds ++ fds, (w = 0 ↔ d.2.1 = 0))
    (hpre : MkPreS w j ρ (fds.map (·.2.2)) Fss bodyC pds) :
    interp2 V ρ (directSumMkAV w j (pds ++ fds) (fds.map (·.2.2)) Fss)
      ∈ˢ interp2 V ρ (mkPisAV (pds ++ fds) bodyC) :=
  mkLamsC_mem hz (underTowerOk_of_mkPreS hpre rfl)

/-- **The constructor leaf is graded.** -/
theorem directSumMkAV_ok2 {w j : Nat} {bodyC : AVExpr} {ρ : Nat → V}
    {Fss : List (List AVExpr)} {pds fds : List (Nat × Nat × AVExpr)}
    (hz : ∀ d ∈ pds ++ fds, (w = 0 ↔ d.2.1 = 0))
    (hpre : MkPreS w j ρ (fds.map (·.2.2)) Fss bodyC pds) :
    AnnotOk2 V ρ (directSumMkAV w j (pds ++ fds) (fds.map (·.2.2)) Fss) :=
  mkLamsC_ok2 hz (underTowerOk_of_mkPreS hpre rfl)
...
theorem directSumMkAV_fold {w j : Nat} (hw : w ≠ 0)
    {pds fds : List (Nat × Nat × AVExpr)} {Fss : List (List AVExpr)} {ρ : Nat → V}
    {as bs : List V}
...
theorem directSumMkAV_zero {j : Nat} {ds : List (Nat × Nat × AVExpr)} {Fs : List AVExpr}
    {Fss : List (List AVExpr)} {ρ : Nat → V} :
    interp2 V ρ (directSumMkAV 0 j ds Fs Fss) = (pt : V) := by
  match ds with
  | [] =>
    show interp2 V ρ (sumInjAtAV 0 Fss Fs.length (numeralAV j) (mkTowerGoU 0 Fs (idxEqAV []))) = pt
    show SetTheory.app (SetTheory.app (SetTheory.app (SetTheory.app (psigmaMkV2 V 0 0) _) _) _) _ = _
    rw [psigmaMkV2, show Nat.max 0 0 = 0 from rfl, lamR_zero, app_pt, app_pt, app_pt, app_pt]
