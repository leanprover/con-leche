...
eqs)` (every left side interprets as its right side), `idxEqAV_ok2`
...
def eqChainAV : List (AVExpr × AVExpr) → AVExpr
  | [] => .const .empty [0]
  | (a, b) :: r => .pi 0 0 (.eqE (.sort 0) a b) ((eqChainAV r).liftN 1 0)

/-- **The index equation**: the truth value of every equation holding. -/
def idxEqAV (eqs : List (AVExpr × AVExpr)) : AVExpr := negAV (eqChainAV eqs)

/-- Every equation holds at `ρ`. -/
def EqAll (ρ : Nat → V) (eqs : List (AVExpr × AVExpr)) : Prop :=
  ∀ e ∈ eqs, interp2 V ρ e.1 = interp2 V ρ e.2

theorem EqAll_nil (ρ : Nat → V) : EqAll (V := V) ρ [] := fun _ h => nomatch h

theorem EqAll_cons {ρ : Nat → V} {a b : AVExpr} {r : List (AVExpr × AVExpr)} :
    EqAll ρ ((a, b) :: r) ↔ interp2 V ρ a = interp2 V ρ b ∧ EqAll ρ r := by
  constructor
  · intro h
...
theorem eqChainAV_inhab :
    ∀ (eqs : List (AVExpr × AVExpr)) (ρ : Nat → V),
      (∃ y, y ∈ˢ interp2 V ρ (eqChainAV eqs)) ↔ ¬ EqAll ρ eqs
  | [], ρ => by
    refine ⟨fun ⟨y, hy⟩ => absurd hy (not_mem_empty y), fun h => absurd (EqAll_nil ρ) h⟩
  | (a, b) :: r, ρ => by
    show (∃ y, y ∈ˢ piR 0 (eqv (interp2 V ρ a) (interp2 V ρ b))
      fun x => interp2 V (cons x ρ) ((eqChainAV r).liftN 1 0)) ↔ _
    have heqv : eqv (interp2 V ρ a) (interp2 V ρ b)
        = truthVal (interp2 V ρ a = interp2 V ρ b) := by unfold eqv; rfl
    rw [heqv, piR_zero, exists_mem_truthVal, EqAll_cons]
    have hlift : ∀ x : V, interp2 V (cons x ρ) ((eqChainAV r).liftN 1 0)
        = interp2 V ρ (eqChainAV r) := fun x => by
...
holding.** -/
theorem idxEqAV_interp (eqs : List (AVExpr × AVExpr)) (ρ : Nat → V) :
    interp2 V ρ (idxEqAV eqs) = truthVal (EqAll ρ eqs) := by
  unfold idxEqAV
  rw [interp2_negAV]
...
theorem idxEqAV_mem_univ (eqs : List (AVExpr × AVExpr)) (ρ : Nat → V) (w : Nat) :
    interp2 V ρ (idxEqAV eqs) ∈ˢ (univ w : V) := by
  rw [idxEqAV_interp]; exact truthVal_mem_univ _ w

/-- The point inhabits the index equation exactly when every equation
holds. -/
theorem pt_mem_idxEqAV {eqs : List (AVExpr × AVExpr)} {ρ : Nat → V} :
    (pt : V) ∈ˢ interp2 V ρ (idxEqAV eqs) ↔ EqAll ρ eqs := by
  rw [idxEqAV_interp, mem_truthVal]
  exact ⟨fun h => h.1, fun h => ⟨h, rfl⟩⟩

theorem mem_idxEqAV {eqs : List (AVExpr × AVExpr)} {ρ : Nat → V} {x : V}
    (hx : x ∈ˢ interp2 V ρ (idxEqAV eqs)) : x = pt ∧ EqAll ρ eqs := by
...
def EqsOk2 (ρ : Nat → V) (eqs : List (AVExpr × AVExpr)) : Prop :=
  ∀ e ∈ eqs, AnnotOk2 V ρ e.1 ∧ AnnotOk2 V ρ e.2

theorem eqChainAV_ok2 :
    ∀ {eqs : List (AVExpr × AVExpr)} {ρ : Nat → V}, EqsOk2 ρ eqs →
      AnnotOk2 V ρ (eqChainAV eqs)
  | [], _, _ => trivial
  | (a, b) :: r, ρ, hok => by
    show AnnotOk2 V ρ (.pi 0 0 (.eqE (.sort 0) a b) ((eqChainAV r).liftN 1 0))
    rw [AnnotOk2_pi, AnnotOk2_eqE]
    refine ⟨hok (a, b) List.mem_cons_self, fun x _ => ?_⟩
    rw [AnnotOk2_liftN, show (1 : Nat) = 0 + 1 from rfl, shiftE_succ_cons, shiftE_zero_zero]
    exact eqChainAV_ok2 fun e he => hok e (List.mem_cons_of_mem _ he)

/-- **The index equation is graded** from its sides' gradings. -/
theorem idxEqAV_ok2 {eqs : List (AVExpr × AVExpr)} {ρ : Nat → V} (hok : EqsOk2 ρ eqs) :
    AnnotOk2 V ρ (idxEqAV eqs) := by
  unfold idxEqAV negAV
  rw [AnnotOk2_pi]
  exact ⟨eqChainAV_ok2 hok, fun _ _ => trivial⟩
...
SetP tier's `spineFit_append_inv`, restated here below it). -/
...
fields. -/
theorem spineFit_append_idxEq {Fs : List AVExpr} {eqs : List (AVExpr × AVExpr)}
    {ρ : Nat → V} {as : List V} :
...
theorem restricted_member_elim {w : Nat} (hw : w ≠ 0) {Fs : List AVExpr}
    {eqs : List (AVExpr × AVExpr)} {ρ : Nat → V} {y : V}
...
theorem restricted_member_zero {Fs : List AVExpr} {eqs : List (AVExpr × AVExpr)}
...
theorem restricted_member_intro {w : Nat} {Fs : List AVExpr} {eqs : List (AVExpr × AVExpr)}
...
theorem eqChainAV_below {k : Nat} :
    ∀ {eqs : List (AVExpr × AVExpr)},
      (∀ e ∈ eqs, VExpr.bvarsBelow k e.1.erase ∧ VExpr.bvarsBelow k e.2.erase) →
      VExpr.bvarsBelow k (eqChainAV eqs).erase
  | [], _ => trivial
  | (a, b) :: r, h => by
...
theorem idxEqAV_below {k : Nat} {eqs : List (AVExpr × AVExpr)}
    (h : ∀ e ∈ eqs, VExpr.bvarsBelow k e.1.erase ∧ VExpr.bvarsBelow k e.2.erase) :
    VExpr.bvarsBelow k (idxEqAV eqs).erase :=
...
theorem FieldsBelow_append_idxEq {eqs : List (AVExpr × AVExpr)} :
    ∀ {Fs : List AVExpr} {k : Nat}, FieldsBelow k Fs →
      (∀ e ∈ eqs, VExpr.bvarsBelow (k + Fs.length) e.1.erase ∧
        VExpr.bvarsBelow (k + Fs.length) e.2.erase) →
...
theorem spineFit_liftFields (n : Nat) :
    ∀ {Fs : List AVExpr} {k : Nat} {σ : Nat → V} {as : List V},
      SpineFit σ (liftFields n k Fs) as ↔ SpineFit (shiftE n k σ) Fs as
  | [], _, _, [] => Iff.rfl
  | [], _, _, _ :: _ => Iff.rfl
  | _ :: _, _, _, [] => Iff.rfl
  | F :: Fs, k, σ, a :: as => by
    simp only [liftFields_cons, SpineFit, interp2_liftN]
    rw [cons_shiftE]
...
theorem FieldsOkB_liftFields {w n : Nat} :
  ...
  | [], _, _ => Iff.rfl
  | F :: Fs, k, σ => by
    simp only [liftFields_cons, FieldsOkB, AnnotOk2_liftN, interp2_liftN]
...
under `nF` fields. -/
def idxEqsAt (d nIdx nF : Nat) (Es : List AVExpr) : List (AVExpr × AVExpr) :=
...
def rChain (d nIdx : Nat) (Fs : List AVExpr) (Es : List AVExpr) : List AVExpr :=
  liftFields d 0 Fs ++ [idxEqAV (idxEqsAt d nIdx Fs.length Es)]

/-- The restricted chains of all constructors. -/
def rChains (d nIdx : Nat) (Fss : List (List AVExpr)) (Ess : List (List AVExpr)) :
    List (List AVExpr) :=
  List.zipWith (rChain d nIdx) Fss Ess

theorem rChains_length (d nIdx : Nat) (Fss Ess : List (List AVExpr)) :
    (rChains d nIdx Fss Ess).length = Nat.min Fss.length Ess.length := by
  simp [rChains]

theorem rChains_getElem? (d nIdx : Nat) (Fss Ess : List (List AVExpr)) (j : Nat) :
...
theorem rChain_length (d nIdx : Nat) (Fs Es : List AVExpr) :
...
parameter frame. -/
noncomputable def idxValsAt (ρp : Nat → V) (Es : List AVExpr) (bs : List V) : List V :=
  Es.map (interp2 V (consList bs ρp))
...
theorem getD_mem_of_lt {l : Nat} {Es : List AVExpr} (hl : l < Es.length) :
...
theorem getD_eq_default_of_le {l : Nat} {Es : List AVExpr} (hl : Es.length ≤ l) :
...
frame's index tuple. -/
theorem EqAll_idxEqsAt {d nIdx nF : Nat} {Es : List AVExpr} (hEs : Es.length = nIdx)
  ...
  have hvar : ∀ l, l < nIdx →
      interp2 V (consList bs σ) (.bvar (nF + nIdx - 1 - l)) = σ (nIdx - 1 - l) := by
    intro l hl
    rw [interp2_bvar, show nF + nIdx - 1 - l = (nIdx - 1 - l) + bs.length from by omega,
      consList_apply_add]
  have hlift : ∀ E : AVExpr, interp2 V (consList bs σ) (E.liftN d nF)
      = interp2 V (consList bs (shiftE d 0 σ)) E := by
    intro E
    rw [interp2_liftN, ← hbs, shiftE_consList_len]
  -- the pointwise form of the equation
  have hpt : EqAll (consList bs σ) (idxEqsAt d nIdx nF Es) ↔
      ∀ l, l < nIdx → interp2 V (consList bs (shiftE d 0 σ)) (Es.getD l default)
        = σ (nIdx - 1 - l) := by
...
theorem FieldsOkB_rChain {w d nIdx : Nat} {Fs Es : List AVExpr} {σ : Nat → V}
    (hEs : Es.length = nIdx) (hok : FieldsOkB w (shiftE d 0 σ) Fs)
    (hE : ∀ bs : List V, SpineFit (shiftE d 0 σ) Fs bs →
      ∀ E ∈ Es, AnnotOk2 V (consList bs (shiftE d 0 σ)) E) :
  ...
  rw [AnnotOk2_liftN, ← hsp'.length_eq, shiftE_consList_len]
...
theorem FieldsBelow_rChain {d nIdx : Nat} (hd : nIdx ≤ d) {Fs Es : List AVExpr} {K : Nat}
    (hEs : Es.length = nIdx) (hF : FieldsBelow K Fs)
    (hE : ∀ E ∈ Es, VExpr.bvarsBelow (K + Fs.length) E.erase) :
  ...
  · simp only
    rw [AVExpr.erase_liftN]
    have : VExpr.bvarsBelow (K + Fs.length) (Es.getD l default).erase :=
