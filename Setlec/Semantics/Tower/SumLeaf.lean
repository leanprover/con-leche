import Setlec.Semantics.Tower.SumCase
...
(`Setlec/SetModel/TaggedSum.lean`).  At `w = 0` the `.psigma` spelling
...
The type-former leaf `directSumTyAV` is the λ-tower over the parameter
domains with this body, exactly as `directTyAV`; its laws consume one
...
noncomputable def sumFibre (w : Nat) (ρ : Nat → V) (Fss : List (List AVExpr)) (i : Nat) : V :=
  match Fss[i]? with
  | some Fs => towerSet w (teleOfFields ρ Fs)
  | none => empty

theorem sumFibre_of_getElem? {w : Nat} {ρ : Nat → V} {Fss : List (List AVExpr)} {i : Nat}
    {Fs : List AVExpr} (h : Fss[i]? = some Fs) :
    sumFibre w ρ Fss i = towerSet w (teleOfFields ρ Fs) := by
  unfold sumFibre; rw [h]

theorem sumFibre_of_ge {w : Nat} {ρ : Nat → V} {Fss : List (List AVExpr)} {i : Nat}
...
`FieldsOkB`. -/
def SumFieldsOkB (w : Nat) (ρ : Nat → V) (Fss : List (List AVExpr)) : Prop :=
  ∀ Fs ∈ Fss, FieldsOkB w ρ Fs

theorem SumFieldsOkB.bound {w : Nat} {ρ : Nat → V} {Fss : List (List AVExpr)}
...
theorem selFibre_towers {w : Nat} {ρ : Nat → V} {Fss : List (List AVExpr)}
...
theorem towers_facts {w : Nat} {ρ : Nat → V} {Fss : List (List AVExpr)}
    (hok : SumFieldsOkB w ρ Fss) :
    (∀ T ∈ Fss.map (towerBodyAV w), interp2 V ρ T ∈ˢ (univ w : V)) ∧
    (∀ T ∈ Fss.map (towerBodyAV w), AnnotOk2 V ρ T) := by
  constructor
  · intro T hT
  ...
  · intro T hT
    obtain ⟨Fs, hFs, rfl⟩ := List.mem_map.mp hT
    exact towerBodyAV_ok2 (hok Fs hFs)
...
theorem case_fibre_at {w : Nat} {ρp σ : Nat → V} {d : Nat} (hsh : shiftE d 0 σ = ρp)
    {Fss : List (List AVExpr)} (hok : SumFieldsOkB w ρp Fss) {k : V} (hk : k ∈ˢ (omega : V)) :
    interp2 V (cons k σ) (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0))
        = natFibre (sumFibre w ρp Fss) k ∧
      interp2 V (cons k σ) (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0))
        ∈ˢ (univ w : V) ∧
      AnnotOk2 V (cons k σ) (caseAVAt w (Fss.map (towerBodyAV w)) (d + 1) (.bvar 0)) := by
  ...
  have h := caseAVAt_facts (w := w) (Ts := Fss.map (towerBodyAV w)) (d := d + 1) (k := .bvar 0)
    ...
    (by rw [interp2_bvar]; exact hk)
  rw [hsh'] at h
  refine ⟨?_, h.1, h.2.2⟩
  obtain ⟨i, rfl, hfib⟩ := natFibre_of_mem (sumFibre w ρp Fss) hk
  rw [hfib, h.2.1 i (by rw [interp2_bvar]; rfl), selFibre_towers hok.bound]

/-- The case split at the tag frame reads to the fibre function. -/
theorem case_fibre {w : Nat} {ρ : Nat → V} {Fss : List (List AVExpr)}
    (hok : SumFieldsOkB w ρ Fss) {k : V} (hk : k ∈ˢ (omega : V)) :
    interp2 V (cons k ρ) (caseAVAt w (Fss.map (towerBodyAV w)) 1 (.bvar 0))
        = natFibre (sumFibre w ρ Fss) k ∧
      interp2 V (cons k ρ) (caseAVAt w (Fss.map (towerBodyAV w)) 1 (.bvar 0))
        ∈ˢ (univ w : V) ∧
      AnnotOk2 V (cons k ρ) (caseAVAt w (Fss.map (towerBodyAV w)) 1 (.bvar 0)) :=
  case_fibre_at (d := 0) (shiftE_zero_zero ρ) hok hk

/-! ## The carrier body -/

/-- The carrier body, graph regime. -/
def sumBodyAVPos (w : Nat) (Fss : List (List AVExpr)) : AVExpr :=
...
def sqSumBodyAV (Fss : List (List AVExpr)) : AVExpr :=
  negAV (.pi 1 0 natAV (negAV (caseAVAt 0 (Fss.map (towerBodyAV 0)) 1 (.bvar 0))))

/-- The carrier body, both regimes. -/
def sumBodyAV (w : Nat) (Fss : List (List AVExpr)) : AVExpr :=
  if w = 0 then sqSumBodyAV Fss else sumBodyAVPos w Fss

theorem sumBodyAV_zero (Fss : List (List AVExpr)) : sumBodyAV 0 Fss = sqSumBodyAV Fss := if_pos rfl

theorem sumBodyAV_pos {w : Nat} (hw : w ≠ 0) (Fss : List (List AVExpr)) :
...
theorem sqSumBodyAV_interp {ρ : Nat → V} {Fss : List (List AVExpr)}
    (hok : SumFieldsOkB 0 ρ Fss) :
    interp2 V ρ (sqSumBodyAV Fss) = sumSet 0 (sumFibre 0 ρ Fss) := by
  unfold sqSumBodyAV sumSet
  rw [interp2_negAV, sigmaSet_zero]
  refine truthVal_congr ?_
  rw [interp2_pi, piR_zero, exists_mem_truthVal]
  have hin : ∀ k : V, k ∈ˢ (omega : V) →
      ((∃ y, y ∈ˢ interp2 V (cons k ρ) (negAV (caseAVAt 0 (Fss.map (towerBodyAV 0)) 1 (.bvar 0))))
        ↔ ¬ ∃ z, z ∈ˢ natFibre (sumFibre 0 ρ Fss) k) := by
    intro k hk
    rw [interp2_negAV, (case_fibre hok hk).1, exists_mem_truthVal]
  constructor
  · intro h
...
theorem sumBodyAVPos_interp {w : Nat} (hw : w ≠ 0) {ρ : Nat → V} {Fss : List (List AVExpr)}
    (hok : SumFieldsOkB w ρ Fss) :
    interp2 V ρ (sumBodyAVPos w Fss) = sumSet w (sumFibre w ρ Fss) := by
  ...
  show SetTheory.app (SetTheory.app (bval2 V .psigma [w, w]) omega)
...
theorem sumBodyAV_interp {w : Nat} {ρ : Nat → V} {Fss : List (List AVExpr)}
    (hok : SumFieldsOkB w ρ Fss) :
    interp2 V ρ (sumBodyAV w Fss) = sumSet w (sumFibre w ρ Fss) := by
  by_cases hw : w = 0
...
theorem sumSet_univ_of_okB {w : Nat} {ρ : Nat → V} {Fss : List (List AVExpr)}
    (hok : SumFieldsOkB w ρ Fss) :
...
theorem sqSumBodyAV_ok2 {ρ : Nat → V} {Fss : List (List AVExpr)} (hok : SumFieldsOkB 0 ρ Fss) :
    AnnotOk2 V ρ (sqSumBodyAV Fss) := by
...
theorem sumBodyAVPos_ok2 {w : Nat} (hw : w ≠ 0) {ρ : Nat → V} {Fss : List (List AVExpr)}
    (hok : SumFieldsOkB w ρ Fss) : AnnotOk2 V ρ (sumBodyAVPos w Fss) := by
  ...
  · refine ⟨w + 1, psigmaFibreSpace V w omega, fun _ => (univ w : V), ?_, ?_,
      fun h => absurd h hvac⟩
    · show SetTheory.app (interp2 V ρ (.const .psigma [w, w])) omega ∈ˢ _
      rw [hbv]
...
theorem sumBodyAV_ok2 {w : Nat} {ρ : Nat → V} {Fss : List (List AVExpr)}
    (hok : SumFieldsOkB w ρ Fss) : AnnotOk2 V ρ (sumBodyAV w Fss) := by
  by_cases hw : w = 0
  · subst hw; rw [sumBodyAV_zero]; exact sqSumBodyAV_ok2 hok
  · rw [sumBodyAV_pos hw]; exact sumBodyAVPos_ok2 hw hok
...
def directSumTyAV (w : Nat) (pps : List (Nat × Nat × AVExpr)) (Fss : List (List AVExpr)) :
    AVExpr :=
...
def ParamsOkS (w : Nat) (ρ : Nat → V) (Fss : List (List AVExpr)) :
    List (Nat × Nat × AVExpr) → Prop
  | [] => SumFieldsOkB w ρ Fss
  | d :: pps => d.2.1 ≠ 0 ∧ AnnotOk2 V ρ d.2.2 ∧
      ∀ a, a ∈ˢ interp2 V ρ d.2.2 → ParamsOkS w (cons a ρ) Fss pps

/-- **The leaf inhabits its type's reading.** -/
theorem directSumTyAV_mem {w : Nat} {Fss : List (List AVExpr)} :
    ∀ {pps : List (Nat × Nat × AVExpr)} {ρ : Nat → V},
      ParamsOkS w ρ Fss pps →
      interp2 V ρ (directSumTyAV w pps Fss) ∈ˢ interp2 V ρ (mkPisAV pps (.sort w))
  | [], ρ, h => by
    show interp2 V ρ (sumBodyAV w Fss) ∈ˢ (univ w : V)
  ...
  | d :: pps, ρ, h => by
    ...
    exact lamR_mem_zero_agree (iff_of_false (Nat.succ_ne_zero w) h.1)
      (fun a ha => directSumTyAV_mem (h.2.2 a ha))

/-- **The leaf is graded.** -/
theorem directSumTyAV_ok2 {w : Nat} {Fss : List (List AVExpr)} :
    ∀ {pps : List (Nat × Nat × AVExpr)} {ρ : Nat → V},
      ParamsOkS w ρ Fss pps → AnnotOk2 V ρ (directSumTyAV w pps Fss)
  | [], _, h => sumBodyAV_ok2 h
  | d :: pps, ρ, h => by
    show AnnotOk2 V ρ (.lam (w + 1) d.2.2
      (mkLamsAV (pps.map fun d => (w + 1, d.2.2)) (sumBodyAV w Fss)))
    rw [AnnotOk2_lam]
    exact ⟨h.2.1, fun a ha => directSumTyAV_ok2 (h.2.2 a ha),
      ⟨fun a => interp2 V (cons a ρ) (mkPisAV pps (.sort w)),
       fun a ha => directSumTyAV_mem (h.2.2 a ha),
...
theorem directSumTyAV_fold {w : Nat} {Fss : List (List AVExpr)}
