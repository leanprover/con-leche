...
`EnvWF` for the recursor stage of `checkDirectFix`: the recursor's
cons with its rules, each rule's right-hand side scoped by
`checkDirectFixRules` at the environment holding the recursor's
...
theorem directFixCtors4_length {ctorsA : List (ConstantVal × Nat)}
...
/-- The field-kinds guard pins one kind list per constructor. -/
theorem directFixFieldsOk_length {env₀ : Env} {T : Name} {lps : List Name} {nP nIdx : Nat}
    {ctorsA : List (ConstantVal × Nat)} {kinds : List (List RecFieldKind)}
    (h : directFixFieldsOk env₀ T lps nP nIdx ctorsA kinds = true) :
    ctorsA.length = kinds.length := by
  simp only [directFixFieldsOk, Bool.and_eq_true, beq_iff_eq] at h
  exact h.1
...
(rule-less) cons. -/
theorem checkDirectFixRec_facts {env : Env} {p : DirectFixParts}
    ...
    rhss.length = (directFixCtors4 ctorsA p.kinds).length ∧
  ...
  obtain ⟨cvRi, recTy, sty, u, -, -, hlp, hres, hbv, hfv, -, -, -, hrules, rfl⟩ :=
    checkDirectFixRec_shape h
  obtain ⟨hlen, hall⟩ := checkDirectFixRules_inv hrules
  refine ⟨rfl, rfl, ⟨hfv, hlp, hres, hbv⟩, hlen, ?_⟩
  intro rhs hrhs
  obtain ⟨i, hi⟩ := List.getElem?_of_mem hrhs
  have hi' : i < (directFixCtors4 ctorsA p.kinds).length := by
...
theorem direct_fix_rec_wf {env : Env} (henv : EnvWF env)
    {p : DirectFixParts} {cvTa cvRa : ConstantVal} {ctorsA : List (ConstantVal × Nat)}
    {rhss : List Expr} {F : Nat}
    (h : checkDirectFixRec (fueledOps mode F) env p cvTa ctorsA = .ok (cvRa, rhss)) :
    EnvWF ⟨.recInfo cvRa p.majorIdx p.rulePrefix
      (directSumRules p.nP p.majorIdx p.rulePrefix cvRa.type ctorsA rhss) :: env.consts⟩ := by
  obtain ⟨-, -, ⟨htf, htp, htr, htb⟩, -, hall⟩ := checkDirectFixRec_facts h
  ...
  refine ⟨hrfv, hrlp, Expr.constsResolve_of_find
    (find?_isSome_cons_same (c := .recInfo cvRa p.majorIdx p.rulePrefix [])
      (c' := .recInfo cvRa p.majorIdx p.rulePrefix
        (directSumRules p.nP p.majorIdx p.rulePrefix cvRa.type ctorsA rhss)) rfl) hrres, hrbv, ?_⟩
  intro lvls pins hf
