...
theorem checkDirectSumS_run (hμ : mode.verifiedChecks = true) {env : Env} (henv : EnvWF env)
  ...
  have hF₁p : checkDirectSumInd (fueledOps mode F₁) env p directSumCaps
      = .ok (env₁, cvTa, p') := by
  ...
  obtain ⟨q2, s₂, hct, h⟩ := bindC_ok h
  obtain ⟨hs₂, q2', hP2, F₂, hF₂⟩ :=
    (checkDirectSumCtorsS_sim hμ henv₁ hTf (flushC_csok hs₁.residue)) q2 s₂ hct
  ...
  have g₁ : checkDirectSumInd (fueledOps mode G) env p directSumCaps
      = .ok (env₁, cvTa, p') := by
...
theorem checkDirectFixS_run (hμ : mode.verifiedChecks = true) {env : Env} (henv : EnvWF env)
  ...
  case neg => rw [if_neg hsort] at h; exact absurd h throwC_bind_ok
