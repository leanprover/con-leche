...
open ConLeche (Env Expr Name Level CheckMode ConstantVal ConstantInfo
  ...
  checkDirectSumRec consSumCtors directSumRules directSumCaps directFixCaps EtaFamiliesClosed
...
theorem declDirectSumRun_etaClosed {μ : CheckMode} {F : Nat} {env env₂ : Env}
  ...
  obtain ⟨hnd, cvTa, env₁, p', ctorsA, sortss, cvRa, rhss, hInd, -, hCtors, hRec, rfl⟩ := h
...
theorem checkDirectFixTable_etaClosed {p : ConLeche.DirectFixParts}
    {ctorsA : List (ConstantVal × Nat)} {sortss : List (List Level)} {env env₂ : Env}
    (h : ConLeche.checkDirectFixTable (m := ConLeche.CheckM) p ctorsA sortss env = .ok env₂)
    (hE : EtaFamiliesClosed env) : EtaFamiliesClosed env₂ := by
  unfold ConLeche.checkDirectFixTable at h
  split at h
  · split at h
    · exact checkDirectProjTable_etaClosed h hE
