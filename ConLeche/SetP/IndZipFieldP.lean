...
`Expr.WScoped (rP + j)`, because `denoteP_lift` — the step that turns
...
`TVj`'s closedness is the **lifting equation**, not `VExpr.Closed`:
...
theorem zipFieldTermEqP
    ...
    {TVj : AVExpr}
    (hTVjK : denoteP acval env φ (rP + cnF) ctyR = some TVj)
    ...
    {zs : List AVExpr} (hzslen : zs.length = rP + cnF)
    {mix : List AVExpr} (hmixlen : mix.length = cnP + cnF)
    (hmixsp : ∀ (q : Nat) (x : Expr), sp[q]? = some x →
      ∃ w0, denoteP acval env φ (rP + cnF) x = some w0 ∧
    ...
    ∃ vdomLow,
      denoteP acval env φ (rP + j) (cdoms.getD (cnP + j) default)
        = some vdomLow ∧
      AVExpr.instSeq (zs.take (rP + j)) (rP + j - 1) vdomLow
  ...
  have hspFacts : ∀ (q : Nat) (x : Expr), sp[q]? = some x →
      (∃ w, denoteP acval env φ (rP + cnF) x = some w) ∧
  ...
  have hspTake : ∀ (q : Nat) (x : Expr),
    ...
    exact ⟨hx, hqm⟩
  obtain ⟨vMid, hvMid⟩ := instPisAt_denoteP_defined hacl hainst _ htr
    (D := rP + cnF)
    (fun q x hx => hspFacts q x (hspTake q x hx).1)
    hfbCty hCbR hTVjK
  -- read the pi apart: the domain reads at depth `K`
  rw [denoteP_forallE] at hvMid
  rcases hvd : denoteP acval env φ (rP + cnF) domJ with _ | vDomK
  · rw [hvd] at hvMid
    exact nomatch hvMid
  rw [hvd] at hvMid
  ...
  have hlow := denoteP_lift (acval := acval) (env := env) (φ := φ) hacl
    (p := rP + j) (e := domJ) hwsDom (rP + cnF) (by omega)
  rw [hvd] at hlow
  rcases hvl : denoteP acval env φ (rP + j) domJ with _ | vdomLow
  · rw [hvl] at hlow
    exact nomatch hlow
  rw [hvl] at hlow
  have hVK : vDomK = AVExpr.liftN (rP + cnF - (rP + j)) vdomLow 0 :=
  ...
  have hlenTake : (sp.take (cnP + j)).length = cnP + j := by
    ...
    omega
  have htow : PiTeleP (sp.take (cnP + j)).length
      (AVExpr.instSeq zs (rP + cnF - 1) TVj)
      (Γj.drop (cnP + cnF - (cnP + j))) mid' := by
    rw [hlenTake, instSeqP_eq_self_of_closed hTVjcl]
    exact hpre
  ...
  have hvMid' : denoteP acval env φ (rP + cnF)
    ...
    rfl
  have hcross := instPisAt_denoteP_cross hacl hainst _ htr
  ...
  obtain ⟨uN, vN, Anext, Bnext, rfl, hAnext⟩ :
    ...
    rw [show cnP + cnF - 1 - (cnP + j) = cnF - j - 1 from by omega,
      ← h2, hg, hΓt, ← hΓ''len, h1]
  rw [instSeqP_pi _ _ _ _ _ _ (by rw [hzslen]; omega),
    instSeqP_pi _ _ _ _ _ _ (by
      rw [List.length_take, hmixlen]
      omega)] at hcross
  have hdomEq : AVExpr.instSeq zs (rP + cnF - 1) vDomK
      = AVExpr.instSeq (mix.take (cnP + j)) (cnP + j - 1) Anext := by
    injection hcross with _ _ h3 _
    rw [show (List.take (cnP + j) mix).length = cnP + j from by
      rw [List.length_take, hmixlen]
      omega] at h3
    exact h3
  have habs : AVExpr.instSeq zs (rP + cnF - 1)
      (AVExpr.liftN (rP + cnF - (rP + j)) vdomLow 0)
      = AVExpr.instSeq (zs.take (rP + j)) (rP + j - 1) vdomLow :=
    instSeqP_absorb_left hzslen (by omega)
  calc AVExpr.instSeq (zs.take (rP + j)) (rP + j - 1) vdomLow
      = AVExpr.instSeq zs (rP + cnF - 1)
          (AVExpr.liftN (rP + cnF - (rP + j)) vdomLow 0) := habs.symm
    _ = AVExpr.instSeq zs (rP + cnF - 1) vDomK := by rw [← hVK]
    _ = AVExpr.instSeq (mix.take (cnP + j)) (cnP + j - 1) Anext :=
        hdomEq
    _ = AVExpr.instSeq (mix.take (cnP + j)) (cnP + j - 1)
          (Γj.getD (cnP + cnF - 1 - (cnP + j)) default) := by
        rw [← hAnext]
