...
(`checkDirectFieldSortsI` at the former's opened telescope), joined
...
theorem idxOk_of (hμ : μ.verifiedChecks = true) (mp : EnvS2PM V μ env)
  ...
  have hbnd : ∀ j, j < nIdx → ∀ ρ : Nat → V,
      Sat2 V ((((ppsAll ψ).map (·.2.2)).reverse).drop (nP + nIdx - (nP + j))) ρ →
      interp2 V ρ ((((ppsAll ψ).map (·.2.2)).reverse).getD (nP + nIdx - 1 - (nP + j)) default)
...
theorem DenoteSpineP.mem_inv {acval : Name → (Name → Nat) → AVExpr} {φ : Name → Nat} {d : Nat} :
    ∀ {as : List Expr} {vs : List AVExpr}, DenoteSpineP acval env φ d as vs →
      ∀ v ∈ vs, ∃ a ∈ as, denoteP acval env φ d a = some v
