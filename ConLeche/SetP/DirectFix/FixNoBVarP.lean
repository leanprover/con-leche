...
(`interp2_congr_noBVar`, `AnnotOk2_congr_noBVar`).  The recursive
...
theorem NoBVar_congr {P P' : Nat → Prop} (h : ∀ i, P i ↔ P' i) :
    ∀ (e : AVExpr), NoBVar P e → NoBVar P' e := by
  intro e
...
theorem NoBVar_projAV {P : Nat → Prop} :
    ∀ (i : Nat) (e : AVExpr), NoBVar P e → NoBVar P (projAV i e)
  | 0, _, h => h
  | i + 1, e, h => NoBVar_projAV i (.proj 1 e) h

/-- **A leaf-free reading mentions none of the excluded slots.** -/
theorem noBVar_of_leaf_free {env : Env} (m : EnvS2Core V env) {φ : Name → Nat} :
  ...
  | case11 d n hsup =>
    intro _ Q _ _ ea h
    have h0 : denoteP m.acval env φ 0 (.lit (.natVal n)) = some ea := by
