...
The three clauses of `TowerEntryLawP` at a STRUCTURE-LIKE block on the
...
* **(B) the iota law** (`fixEntryIotaCore`/`fixEntryIotaCoreZero`): the
  ...
  (`directSumMkAV_fold`: the application folds to the tagged tuple);
...
eliminations, `annotOk2_proj1_sum`/`annotOk2_proj1_pt` grade the tag
projection.
-/
...
theorem fixFibre_elim {w : Nat} (hw : w ≠ 0) {ρ' : Nat → V} {Fs : List AVExpr} {x : V}
...
fitting field spine. -/
theorem fixFibre_zero_elim {ρ' : Nat → V} {Fs : List AVExpr} {x : V}
...
unrestricted one. -/
theorem fieldsBound_append_idxEq {w : Nat} {ρ' : Nat → V} {Fs : List AVExpr} (hw : w ≠ 0)
...
theorem annotOk2_proj1_sum {w : Nat} (hw : w ≠ 0) {ρ' ρ : Nat → V} {Fs : List AVExpr}
    {e : AVExpr} (hb : FieldsBound w ρ' (Fs ++ [idxEqAV []]))
    (hok : AnnotOk2 V ρ e)
    (hval : interp2 V ρ e ∈ˢ sumSet w (sumFibre w ρ' [Fs ++ [idxEqAV []]])) :
    AnnotOk2 V ρ (.proj 1 e) := by
  rw [AnnotOk2_proj]
...
theorem annotOk2_proj1_pt {ρ : Nat → V} {e : AVExpr} (hok : AnnotOk2 V ρ e)
    (hpt : interp2 V ρ e = (pt : V)) : AnnotOk2 V ρ (.proj 1 e) := by
  rw [AnnotOk2_proj]
...
theorem annotOk2_projAV_succ_fibre {w i : Nat} {ρ' ρ : Nat → V} {Fs : List AVExpr}
    {e : AVExpr} (hokB : w ≠ 0 → FieldsOkB w ρ' Fs)
    (hok : AnnotOk2 V ρ e)
    (hval : interp2 V ρ e ∈ˢ sumSet w (sumFibre w ρ' [Fs ++ [idxEqAV []]]))
    (hi : i < Fs.length) : AnnotOk2 V ρ (projAV (i + 1) e) := by
  show AnnotOk2 V ρ (projAV i (.proj 1 e))
  by_cases hw : w = 0
  · subst hw
    obtain ⟨hpt, -⟩ := fixFibre_zero_elim hval
    refine annotOk2_projAV_pt (annotOk2_proj1_pt hok hpt) ?_
  ...
  · obtain ⟨fs, heq, -, hmem⟩ := fixFibre_elim hw hval
    ...
    refine annotOk2_projAV_tower hb hmem (annotOk2_proj1_sum hw hb hok hval) ?_
      (by rw [List.length_append, List.length_singleton]; omega)
    rw [interp2_proj, if_neg (by decide), heq]
    exact ssnd_spair _ _

/-! ## (A) the typing law -/

theorem fixEntryTypingCore {u w nP nF i : Nat} {pps ds eds : List (Nat × Nat × AVExpr)}
    ...
    (hres : ∀ ρ : Nat → V,
      ...
      interp2 V ρ R
        = interp2 V (consList (projList i (dropS 1 (ρ 0))) (fun j => ρ (j + 1)))
    ...
    ∀ (ρ : Nat → V) (vs : List AVExpr) (x rest : AVExpr),
      vs.length = nP →
      AnnotOkP V ρ (AVExpr.mkAppN (directFixTyAVI u w pps [] rss tlss eiss Fss₀ Ess) vs) →
      AnnotOkP V ρ x →
      interp2 V ρ x ∈ˢ interp2 V ρ
        (AVExpr.mkAppN (directFixTyAVI u w pps [] rss tlss eiss Fss₀ Ess) vs) →
      ConLeche.SetP.AVExpr.peelPis (mkPisAV eds R) (vs ++ [x]) = some rest →
      AnnotOkP V ρ (projAV (i + 1) x) ∧ AnnotOkP V ρ rest ∧
        interp2 V ρ (projAV (i + 1) x) ∈ˢ interp2 V ρ rest := by
  ...
  have hx : interp2 V ρ x ∈ˢ sumSet w
      (sumFibre w (consList (vs.map (interp2 V ρ)) ρ) [((ds.drop nP).map (·.2.2)) ++ [idxEqAV []]]) := by
    rw [interp2_mkAppN_foldl, hfold ρ _ hsp] at hmem
    exact hmem
  ...
  have hchain : chainP V ρ (vs ++ [x]) = cons (interp2 V ρ x) (consList (vs.map (interp2 V ρ)) ρ) := by
    ...
    rfl
  have hframeX : (cons (interp2 V ρ x) (consList (vs.map (interp2 V ρ)) ρ)) 0 ∈ˢ sumSet w
      (sumFibre w (fun j => (cons (interp2 V ρ x) (consList (vs.map (interp2 V ρ)) ρ)) (j + 1))
  ...
  have hinterpRest : interp2 V ρ rest
      = interp2 V (consList (projList i (dropS 1 (interp2 V ρ x))) (consList (vs.map (interp2 V ρ)) ρ))
          (((ds.drop nP).map (·.2.2)).getD i default) := by
    rw [hrest, hlen', interp2_instSeq, hchain, hres _ hframeX hframeS]
    rfl
  refine ⟨?_, ?_, ?_⟩
  · -- the projection's grading
    exact ⟨annotOk2_projAV_succ_fibre (fun hw => hokB _ hsatC) hokx.1 hx (by rw [hlenFs]; exact hi),
...
theorem fixEntryIotaCore {w nP nF i : Nat} {ds : List (Nat × Nat × AVExpr)} {ρ : Nat → V}
    (hw : w ≠ 0) (hlenDs : ds.length = nP + nF) (hi : i < nF)
    (hokB : ∀ ρ : Nat → V, Sat2 V ((ds.take nP).map (·.2.2)).reverse ρ →
      FieldsOkB w ρ ((ds.drop nP).map (·.2.2)))
    (ys : List AVExpr) (hlen : ys.length = nP + nF)
    (hok : AnnotOkP V ρ (AVExpr.mkAppN
      (directSumMkAV w 0 ds ((ds.drop nP).map (·.2.2)) (uChains [(ds.drop nP).map (·.2.2)])) ys)) :
    interp2 V ρ (projAV (i + 1) (AVExpr.mkAppN
        (directSumMkAV w 0 ds ((ds.drop nP).map (·.2.2)) (uChains [(ds.drop nP).map (·.2.2)])) ys))
  ...
  have hfold := directSumMkAV_fold (pds := ds.take nP) (fds := ds.drop nP) (j := 0) hw hsp₁ hsp₂
    hokU rfl
  rw [List.take_append_drop] at hfold
  rw [projAV_interp, interp2_mkAppN_foldl, heq, hfold, projS_succ_inj,
...
theorem fixEntryIotaCoreZero {nP nF i : Nat} {ds : List (Nat × Nat × AVExpr)} {ρ : Nat → V}
    ...
    interp2 V ρ (projAV (i + 1) (AVExpr.mkAppN
        (directSumMkAV 0 0 ds ((ds.drop nP).map (·.2.2)) (uChains [(ds.drop nP).map (·.2.2)])) ys))
      = interp2 V ρ (ys.getD (nP + i) default) := by
  rw [projAV_interp, interp2_mkAppN_foldl, directSumMkAV_zero, foldl_app_pt, projS_pt]
  rw [show ds.map (·.2.2) = (ds.take nP).map (·.2.2) ++ (ds.drop nP).map (·.2.2) from by
    rw [← List.map_append, List.take_append_drop]] at hsp
  obtain ⟨as, bs, heq, hsp₁, hsp₂⟩ := spineFit_append_inv hsp
  have hlenAs : as.length = nP := by rw [hsp₁.length_eq]; simp [hlenDs]
...
theorem fixEntryEtaCore {u w nP nF : Nat} {pps ds : List (Nat × Nat × AVExpr)}
    {rss : List (List Bool)} {tlss : List (List (List (Nat × Nat × AVExpr)))}
    {eiss : List (List (List AVExpr))} {Fss₀ Ess : List (List AVExpr)} {ρ : Nat → V}
    ...
    (hx : x ∈ˢ ts.foldl SetTheory.app
      (interp2 V ρ (directFixTyAVI u w pps [] rss tlss eiss Fss₀ Ess))) :
    x = (ts ++ (List.range nF).map fun j => projS (j + 1) x).foldl SetTheory.app
      (interp2 V ρ (directSumMkAV w 0 ds ((ds.drop nP).map (·.2.2))
  ...
  · subst hw
    ...
    rw [hpt, directSumMkAV_zero, foldl_app_pt]
  · obtain ⟨fs, heq, hspF, -⟩ := fixFibre_elim hw hx
    ...
    have hokU : SumFieldsOkB w (consList ts ρ) (uChains [(ds.drop nP).map (·.2.2)]) := by
      ...
      subst hFs'
      exact FieldsOkB_append_idxEq (hokB _ hsat) fun _ _ e he => absurd he List.not_mem_nil
    have hfold' := directSumMkAV_fold (pds := ds.take nP) (fds := ds.drop nP) (j := 0) hw hsp₁ hspF
      hokU rfl
    rw [List.take_append_drop] at hfold'
