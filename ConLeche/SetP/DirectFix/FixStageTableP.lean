import ConLeche.SetP.DirectFix.FixEntryLawP
import ConLeche.SetP.DirectFix.FixAssemblyKitP
import ConLeche.SetP.Direct.DirectStageTableP
...
index; `checkDirectFixTable`).  It is the structure route's table
stage (`stageTable`, `ConLeche/SetP/Direct/DirectStageTableP.lean`)
...
`declDirectFixTable` is the assembly-facing wrapper: the case split
on `checkDirectFixTable` (nothing consed at a block that is not
...
open ConLeche (Env Expr Name Level ConstantInfo ConstantVal IndCaps DirectSumParts
  DirectFixParts BinderMeta ProjEntry ProjTable RecRule RecFieldKind projTableName)
...
theorem liftFields_zero : ∀ (k : Nat) (Fs : List AVExpr), liftFields 0 k Fs = Fs
  | _, [] => rfl
  | k, F :: Fs => by rw [liftFields_cons, AVExpr.liftN_zero, liftFields_zero (k + 1) Fs]

omit [SetTheory V] in
/-- The one constructor's restricted chain at no index is its
unrestricted chain closed by the trivial index equation. -/
theorem rChains_single_nil (Fs : List AVExpr) :
...
theorem fixTableFamFree {p : DirectFixParts} {ctorsA : List (ConstantVal × Nat)}
...
theorem stageFixTable (mp : EnvS2PM V μ env)
    {p : DirectFixParts} {cvTa cvCa : ConstantVal} {nF : Nat} {sorts : List Level}
    {envOut : Env} {caps : IndCaps}
    (hTbl : ConLeche.checkDirectProjTable (m := ConLeche.CheckM) p.cvT.name cvCa.name
    ...
    {pps ds : (Name → Nat) → List (Nat × Nat × AVExpr)} {Es : (Name → Nat) → List AVExpr}
    (hFD : FormerData mp.base2 cvTa p.nP p.resSort pps)
    (hCDread : ∀ ψ, denoteP mp.base2.acval env ψ 0 cvCa.type
    ...
    (hleafT : ∀ ψ, mp.base2.acval p.cvT.name ψ
      = directFixTyAVI (u ψ) (p.resSort.eval ψ) (pps ψ) [] rss (tlss ψ) (eiss ψ) (Fss₀ ψ) (Ess ψ))
    (hleafC : ∀ ψ, mp.base2.acval cvCa.name ψ
      = directSumMkAV (p.resSort.eval ψ) 0 (ds ψ) (((ds ψ).drop p.nP).map (·.2.2))
  ...
  let tbl : ProjTable := ⟨p.cvT.name, p.cvT.levelParams, p.nP, cvCa.name, nF, p.resSort,
    bodies, ConLeche.directProjGuards cvCa.type p.nP nF sorts, 1⟩
  ...
  have hguardSem : ∀ k, k < nF → ∀ ψ : Name → Nat,
      ((ConLeche.directProjGuards cvCa.type p.nP nF sorts).getD k .zero).eval ψ = 0 →
  ...
  have hguardOf : ∀ k, k < nF → ∀ ψ : Name → Nat,
      ...
      ((ConLeche.directProjGuards cvCa.type p.nP nF sorts).getD k .zero).eval ψ = 0 := by
    intro k hk ψ hall
    rw [ConLeche.directProjGuards_getD _ _ _ _ hk,
      eval_foldl_max_if_zero_iff ψ (ConLeche.directUsedLater cvCa.type p.nP)
  ...
  have hfree : ∀ (ψ : Name → Nat) (k : Nat), k < nF → ∀ (j : Nat), j < k →
    ...
    obtain ⟨pps', b, hstA, -, -, hbind⟩ := denoteP_openPis (p.nP + nF) hopAll (hCDread ψ)
  ...
  have hCDread₂ : ∀ ψ, denoteP m₂.acval ⟨.projInfo tbl :: env.consts⟩ ψ 0 cvCa.type
    ...
    rw [hbody, hac]
    exact denoteP_cons_mono hfresh hcrossC ψ 0 hcbC (hCDread ψ)
  ...
  · -- the per-instantiation laws
    ...
    obtain ⟨fdomA, hfdA, hokFd, hresFd⟩ := bodyFrames m₂ (off := 1) hcf hCf hCb
      ...
      (fun ρ hx hsat j hj => by
        refine annotOk2_projAV_succ_fibre (fun _ => hokB _ _ hsat) trivial ?_ ?_
    ...
    · -- (A)
      ...
      have hacT' : m₂.acval tbl.structName (Level.substFn φ (tbl.entry i).levelParams us)
          = directFixTyAVI (u (Level.substFn φ p.cvT.levelParams us))
    ...
    · -- (B): the constructor type's reading at the instantiation,
      ...
      · intro hguardAt ρ ys rest hlen hok hfit
        have hacC' : m₂.acval (tbl.entry i).ctor (Level.substFn φ (tbl.entry i).levelParams us)
            = directSumMkAV (p.resSort.eval (Level.substFn φ p.cvT.levelParams us)) 0
          ...
          rw [hacC, hleafC]
        rw [hacC'] at hok ⊢
        show interp2 V ρ (projAV (i + 1) _) = _
        ...
        · -- squash: the certified fit pins the selected field to a
          ...
          have hsp : SpineFit ρ ((ds (Level.substFn φ p.cvT.levelParams us)).map (·.2.2))
              (ys.map (interp2 V ρ)) :=
            spineFit_of_teleFitP (by simp only [List.length_map, hlen, hCDlen]; rfl) hfit
          rw [hw]
...
theorem declDirectFixTable {F : Nat} {env env₁ envC env₂ : Env} {block : List ConstantInfo} {p : DirectFixParts}
    ...
    (hTbl : ConLeche.checkDirectFixTable (m := ConLeche.CheckM) p ctorsA sortss
      ⟨.recInfo cvRa p.majorIdx p.rulePrefix
        ...
        :: envC.consts⟩ = .ok env₂)
    (mpC : EnvS2PM V μ envC)
    (mp₃ : EnvS2PM V μ ⟨.recInfo cvRa p.majorIdx p.rulePrefix
        (ConLeche.directSumRules p.nP p.majorIdx p.rulePrefix cvRa.type ctorsA rhss)
        :: envC.consts⟩)
    {sAV : (Name → Nat) → Nat} {ppsAll : (Name → Nat) → List (Nat × Nat × AVExpr)}
    ...
    (hXR : ∀ (ψ : Name → Nat) (ρp : Nat → V),
      ...
      ChainsRealI (fixFamI (uAV ψ) (p.resSort.eval ψ) ρp (((ppsAll ψ).drop p.nP).map (·.2.2)) p.nIdx
        ...
        (essOfR (fixCtorDataList dsF esF ksF eissF tssF ψ ctorsA 0)))
    (hRec : ConLeche.checkDirectFixRec (ConLeche.fueledOps μ F) envC p cvTa ctorsA = .ok (cvRa, rhss)) :
    Nonempty (EnvS2PM V μ env₂) := by
  -- the case split: only a structure-like block conses a table
  unfold ConLeche.checkDirectFixTable at hTbl
  split at hTbl
  · next cA sorts =>
    split at hTbl
    · next hnIdx =>
      ...
      obtain ⟨-, -, -, -, hfreshTbl₃, -⟩ := ConLeche.checkDirectProjTable_inv hTbl
      ...
      have hnpC4 : ∀ j, ∀ c' ∈ ConLeche.directFixCtors4 [cA] p.kinds,
          Expr.NoProjAt p.cvT.name j c'.2.2.1 := by
        intro j c'
        unfold ConLeche.directFixCtors4
        cases p.kinds with
      ...
      have hnp₃ : ∀ j, NoProjEnv ⟨.recInfo cvRa p.majorIdx p.rulePrefix
        ...
        · show Expr.NoProjAt p.cvT.name j cvRa.type
          rw [hRtype]
          exact ConLeche.Expr.NoProjAt.directRecTyR hgenR (hnpT j) (hnpC4 j)
        · intro cv mI rP rules heq r hr
          injection heq with _ _ _ hrules'
          subst hrules'
          obtain ⟨hlenR, hallR⟩ := ConLeche.checkDirectFixRules_inv hrules
          cases rhss with
          | nil => exact absurd hr List.not_mem_nil
          | cons rhs rest =>
            simp only [ConLeche.directSumRules, List.mem_cons, List.not_mem_nil, or_false] at hr
            subst hr
            obtain ⟨rhs', hget, hgen, -, -, -, -⟩ := hallR 0 (by
              rw [← hlenR]; exact Nat.succ_pos _)
            obtain rfl : rhs = rhs' := by simpa using hget
            refine ⟨ConLeche.Expr.NoProjAt.directRecRhsR hgen (hnpT j) (hnpC4 j), ?_⟩
      ...
      have hleafT₃ : ∀ ψ, mp₃.base2.acval p.cvT.name ψ
          = directFixTyAVI (uAV ψ) (p.resSort.eval ψ) (ppsAll ψ) [] (rssOfK ksF [cA].length)
      ...
      have hleafC₃ : ∀ ψ, mp₃.base2.acval cA.1.name ψ
          = directSumMkAV (p.resSort.eval ψ) 0 (dsF 0 ψ) (((dsF 0 ψ).drop p.nP).map (·.2.2))
      ...
      have hfoldAt : ∀ (ψ : Name → Nat) (ρ : Nat → V) (ts : List V),
        ...
        intro ψ ρ ts hsp
        have hρ' : Sat2 V (((ppsAll ψ).take p.nP).map (·.2.2)).reverse (consList ts ρ) := by
          rw [hTake]
