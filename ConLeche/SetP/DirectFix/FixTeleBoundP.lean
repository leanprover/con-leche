...
(`checkDirectFieldSortsI`: `imax` of the domains' sorts and the
...
the earlier binders. -/
theorem teleBound_walk (hμ : μ.verifiedChecks = true) (mp : EnvS2PM V μ env) (ψ : Name → Nat)
    {w : Nat} :
  ...
  | n + 1, d, e, fvs, o, Δ, tl, hop, hlen, hC, hws, hb, hL, hread, hinf, k, hk, ρ, hρ => by
    ...
    | .forallE ty body mb, hop, hws, hb =>
      ...
      · next fvs' o' hop' =>
        ...
        have hread0 : denoteP mp.base2.acval env ψ d ty = some t0.2.2 := by
          have := hread 0 _ rfl
        ...
        have hrow := (claimsAtP_of hμ mp ψ F).sortRow hi hens hws2.1 hbty hLty hCty hread0
        cases k with
        | zero =>
        ...
        | succ k =>
          ...
          have hC' : CtxOkP mp.base2 ψ (d + 1) (t0.2.2 :: Δ) (body.instantiate1 (.fvar d ty)) :=
            CtxOkP.open hC.forallE_body hCty hread0 fun ρ hρ => (hrow ρ hρ).1
          obtain ⟨hws', hb', hL'⟩ := frame_open2 hws2.1 hbty hws2.2 hbb hLty hLbody
          have hread' : ∀ k a, fvs'[k]? = some a →
              denoteP mp.base2.acval env ψ (d + 1 + k) a.fvarTypeD = some (tl'.getD k default).2.2 := by
            intro k a hk
          ...
          have hρ' : Sat2 V (((tl'.take k).map (·.2.2)).reverse ++ (t0.2.2 :: Δ)) ρ := by
...
theorem fixTeleBound_of (hμ : μ.verifiedChecks = true) (mp : EnvS2PM V μ env)
    ...
    (hρp' : Sat2 V (((ds ψ).take nP).map (·.2.2)).reverse ρp) {as' : List V}
    ...
    ∀ k, k < ((tss ψ).getD i []).length → ∀ bs : List V,
      ...
      AnnotOk2 V (consList bs (consList as' ρp)) (((tss ψ).getD i []).getD k default).2.2 ∧
  ...
  have hO : OpenedP mp.base2 ψ (nP + nF) cvCa.type (fvsP ++ xFvs) xrest
    ...
    openedP_of_peel hopAll hcf hcb (hD.read ψ) (hD.len ψ) (hD.okTy ψ)
  ...
  have hC : CtxOkP mp.base2 ψ (nP + i)
  ...
  have hρ := sat2_of_spineFit hsat hbs
