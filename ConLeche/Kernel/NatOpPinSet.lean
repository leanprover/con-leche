...
variants in that order and enables the operation's literal fast path
on the first whose guards pass, whose pin is definitionally equal to
the stream's stored value and whose certificates check.
