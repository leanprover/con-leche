...
The same core bodies (`ConLeche.Kernel.Core`) tied together at a state
...
depth invariance theorems (`ConLeche/Verify/Deep.lean`): every entry
...
* The scoping half of depth invariance — every key consulted or
  ...
  (`ConLeche/Verify/Disc.lean`): the checker's entry points are only
...
* The environment half (`EnvWF`) is a hypothesis of the bridge,
  ...
  `ConLeche/Verify/Bridge.lean` / `ConLeche/Verify/BridgeWFDecl.lean` —
  the checker only ever calls the core on environments it built
  itself.

This instance is what the checker executes; the pure knot
(`ConLeche.Kernel.TypeChecker`) is the verified specification and the
...
check: the call discipline (`ConLeche/Verify/Disc.lean`) proves every
