...
if par:
    ...
    A("measurement, and it is taken in the one-thread lane.  The")
    A("instruction count barely moves with the worker count (12.85 T at one")
    A("worker against 12.10 T at four, on `mathlib-full`), so what the extra")
    A("threads buy is overlapped memory stalls, not divided work.")
    A("")
