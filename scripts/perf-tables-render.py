...
A("  representation moved.  It now prints the parsed `DeclC` count: one")
A("  per accepted stream declaration record.  `LECH_VERBOSE=1` still")
...
A("  preprocessor is itself a Mathlib-scale process).  Raw cells (with")
