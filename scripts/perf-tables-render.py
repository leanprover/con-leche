...
A("")
A("> **KNOWN STALENESS — the `--no-model --core=cached-parsed` column.**")
A("> If the binary-provenance commit above predates the cached")
A("> parity-lane landing")
A("> (`agent/cached-parity-lane`), that column is **not** a cert-free")
A("> engine measurement: with no cert-skipping twin under")
A("> `Setlec/Cached/` it is the certified cached driver with two checks")
A("> gated off, which is caveat 1 below.  When the parity lane lands the")
A("> column changes meaning — it becomes a real cert-free lane — and")
A("> this file must be regenerated before any of its numbers are quoted")
A("> again.  In particular today's headline oddity, `NM/cached` being")
A("> the *most expensive* cell on `init-full`, is that confound and not")
A("> a property of the cached core.")
A("")
...
A("   `--set-model --core=cached-parsed` is therefore **exactly those")
A("   two gated checks and nothing else** — bounded by construction, and")
A("   not a verification tax whatever it measures.  (It is a few percent")
A("   on the declaration-heavy streams and materially more on `app-lam`,")
A("   where the annotation validation bites; see the derived table")
A("   above, and do not read that column as a certificate cost.)  This")
A("   is caveat 5 of the task-#161 canonical table; the cached NC twin")
A("   does not exist **as of the binary measured above** — see the")
A("   staleness note in the header: `agent/cached-parity-lane` builds")
A("   one, and this table must be regenerated once it lands.  Until")
A("   then the parity lane is `--no-model --core=production` and only")
A("   that.")
