...
(`Lech/TT/Semantics/Consistency.lean`): no side condition about a
satisfiable context.

**The layer is an upper bound on the checker, not a characterization.**
Lech deliberately never aliases `T := T._model`, so streams cannot see
...
inductive HasType : List VExpr → VExpr → VExpr → Prop where
  ...
  `Lech/TT/Examples.lean`, `eqRec_derivable`). -/
  ...
  **The two sides need not inhabit the *same* `Prop`.**  Soundness
  ...
  `Lech/Kernel/Core.lean`'s `proofIrrel` checks that *each* side's
