...
This document records the design of `Lech/TT/*`.  It should be folded
...
theory lech actually implements.  It is a separate module hierarchy
...
| module | may import |
|---|---|
| `Lech/TT/Syntax.lean` | nothing |
| `Lech/TT/Subst.lean` | `TT/Syntax` |
| `Lech/TT/Const.lean` | `TT/Subst` |
| `Lech/TT/Judgment.lean` | `TT/Const` |
| `Lech/TT/Examples.lean` | `TT/Judgment` |
| `Lech/TT/Semantics/*` | the above **plus `Lech/SetTheory/*` only** |

Nothing in `Lech/Kernel/*`, `Lech/Model/*` or `Lech/Verify/*` is
imported anywhere in the hierarchy — not even by the semantics, which
restates the basis values (`Lech/TT/Semantics/Value.lean`) directly
over the `SetTheory` interface rather than reusing
`Lech/Model/BasisVal.lean`.  Conversely nothing in the existing tree
imports `Lech/TT/*` yet; the layer needs its own `lean_lib`
(`LechTT`, rooted at `Lech/TT.lean`) to be built at all.
...
convenience: *every constant lech accepts either (a) has a value, or
...
context" side condition: `Lech/TT/Semantics/Consistency.lean` proves
outright that no closed term has type `Empty`.

**The layer is an UPPER BOUND on the checker, not a characterization.**
Lech deliberately never aliases `T := T._model` (standing ruling:
...
`Nat` equality.  `imax` is a computed function (`Lech.TT.imax`), so
...
(task #119, `Lech/TTVerify/DESIGN.md` §10.2):
...
(`Q` in `Lech/TT/Examples.lean`), the same trade the inert `eqE`
slot already makes.
...
* **`proj i e`, a former whose type arguments live in the premise**
  ...
  *The original design said "no `proj`; projections denote to
  ...
  (`Lech/TTVerify/DESIGN.md` §3) — the kind of thing the design
...
* `CtxOk` — the bridge's context correspondence
  (`Lech/TTVerify/Claims.lean`) — was discovered to be incomplete by
...
> `Lech/TT/Examples.lean` for the layer or in the first lemma that
...
| checker capability (`Lech/Kernel/Core.lean`) | in the layer |
|---|---|
...
| Nat literal ↔ constructor, `reduceNat` (12 GMP ops), String-literal expansion | **derived** — the certified recurrences are discharged into the numeral families of `Lech/TT/Nat/*`; see §7 |
...
`interp V ρ : VExpr → V` (`Lech/TT/Semantics/Interp.lean`) with
...
Two places in `Lech/TT/Semantics/Value.lean`:
...
on numerals.  `Lech/TT/Nat/*` does this; `BConst`, `VExpr` and the
...
layer never mentions a particular one, so `Lech/TT/Nat/*` imports
only `Lech/TT/*` and stays checker-free.

Two shape decisions, both about pushing work away from the bridge:

* **the hypotheses mirror the certified recurrences.**  They are
  `Lech/Kernel/Core.lean`'s `natOpEquations` and
  `Lech/Model/Interp.lean`'s `DivModClauses` — the very equations the
  checker certifies at install (`certifyNatEqs`, and the pinned
  certificates of `Lech/PinGen/Certs.lean`) — so denoting a
...
The induction skeletons are `Lech/Model/NatOps.lean` transposed from
...
for Lean's own `Nat.land`/`Nat.lor`/`Nat.xor`.  `Lech/PinGen/Certs.lean`
...
on `LechPinCerts` would tie `Lech/TT/*` to the checker's build
...
`Lech/TT/Nat/WfOps.lean` derives them in three lines each from the
...
`Lech/TT/Nat/Numeral.lean`: `Deq` and its calculus, `ap2`, `numeral`,
`hasType_numeral`.
`Lech/TT/Nat/Ops.lean`: `numeral_pred`, `numeral_add`, `numeral_sub`,
`numeral_mul`, `numeral_pow`, `numeral_beq`, `numeral_ble`.
`Lech/TT/Nat/WfOps.lean`: `numeral_div`, `numeral_mod`, `numeral_gcd`,
`numeral_shiftLeft`, `numeral_shiftRight`, `numeral_log2`,
`numeral_land`, `numeral_lor`, `numeral_xor`.
`Lech/TT/Nat/Examples.lean`: the instantiations, including
`f 12345 67890 ≡ 80235`.
