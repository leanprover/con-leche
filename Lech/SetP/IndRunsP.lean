...
tier hands the *run* to `DefEqClaims2P` at the same context, built by
`ctxOkP_of_openers`.  One conversion, no soundness theorem in
between.
...
* the two sides' syntactic guards (`WScoped`/`looseBVarsBounded`/
  `LeavesBounded`), which `DefEqClaims2P` requires and which a
...
for each side from the same opener data; `DefEqClaims2P` turns the
run into the `interp2` equality at any satisfying valuation.  Note
...
theorem defEqAtP_of_run {m : EnvS2Core V env} {F : Nat}
    ...
    (hleafA : ∀ l ∈ a.fvarLeaves, Expr.fvar l.1 l.2.2 ∈ fvs)
    (hltA : ∀ l ∈ a.fvarLeaves, l.1 < n)
    (hleafB : ∀ l ∈ b.fvarLeaves, Expr.fvar l.1 l.2.2 ∈ fvs)
    (hltB : ∀ l ∈ b.fvarLeaves, l.1 < n)
    {aa ba : AVExpr}
    (haa : denoteP m.acval env φ k a = some aa)
    (hbaR : denoteP m.acval env φ k b = some ba)
    (hga : ∀ ρ : Nat → V, Sat2 V Δa ρ → AnnotOkP V ρ aa)
    (hgb : ∀ ρ : Nat → V, Sat2 V Δa ρ → AnnotOkP V ρ ba)
    {ρ : Nat → V} (hsat : Sat2 V Δa ρ) :
    interp2 V ρ aa = interp2 V ρ ba :=
  hclaims hrun hwa hba hLa hwb hbb hLb
    (ctxOkP_of_openers m.acval_closed hΔlen hshape hws hdoms hleafA
      hltA hent hokA)
    (ctxOkP_of_openers m.acval_closed hΔlen hshape hws hdoms hleafB
      hltB hent hokA)
...
theorem defEqListP_of_runs {m : EnvS2Core V env} {F : Nat}
    ...
    (hbaR : denoteP m.acval env φ k (bs.getD i default) = some ba)
    (hga : ∀ ρ : Nat → V, Sat2 V Δa ρ → AnnotOkP V ρ aa)
    (hgb : ∀ ρ : Nat → V, Sat2 V Δa ρ → AnnotOkP V ρ ba)
    {ρ : Nat → V} (hsat : Sat2 V Δa ρ) :
    interp2 V ρ aa = interp2 V ρ ba :=
  defEqAtP_of_run hclaims hΔlen hshape hws hdoms hent hokA
