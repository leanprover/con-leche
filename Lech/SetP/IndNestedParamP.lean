...
`plainParamSupplyP`'s sibling, and the stage part 7 declined to write
before its consumer existed.  `zipperP`/`fieldGradeFireP`/`pointP` all
...
own openers through `paramGradeFireP`, and the `.nested` fire supplies
...
conversion is the one place in the ind tier where `InferClaims2P`
does the work `DefEqClaims2P` does everywhere else:

```
   TypedListOk … pinsP cdomsP        (inferTypeCore pinsP_q = ok ty,
                                      isDefEqCore ty cdomsP_q = ok true)
     ⇒ InferReadsP                    ty reads
     ⇒ InferClaims2P                  pinsP_q graded, ty graded,
                                      ⟦pinsP_q⟧ ∈ˢ ⟦ty⟧
     ⇒ DefEqClaims2P (defEqAtP_of_run) ⟦ty⟧ = ⟦cdomsP_q⟧
```

Three things are worth naming:

* **the ladder is still an induction, and for `paramGradeFireP`'s
  reason**: the *b-side* grading (`AnnotOkP ρ' dw` for the run's `q`-th
  domain) is `instPisAt_domsP_graded`, which walks the constructor
  ...
  from — the inference claim, not the frame's own `Sat2` slot;
* **a run carries no reading** (part 4's lesson): the pins' readings
  ...
  `ErasedEq` — the bottom reads them off `denoteP_mkAppN_inv` and
  hands them down;
* **the context is the recursor frame's** (`Δb`), exactly as in
  `paramGradeFireP`: the pins mention only the public frame's openers,
...
`paramGradeFireP`'s `.nested` twin, on the `TypedListOk` row.
...
theorem nestedPinFireP {m : EnvS2Core V env} {F : Nat}
    ...
    {TVjP : AVExpr} (hTVjP : denoteP m.acval env φ K ctyP = some TVjP)
    ...
    (hpinRead : ∀ q, q < cnP → ∃ w, denoteP m.acval env φ K
      (Expr.instSpine (fvsP.take rP) (rP - 1) (pins.getD q default))
      = some w)
    -- the recursor-frame context
    {Δb : List AVExpr} (hΔblen : Δb.length = K)
    (hΔbent : ∀ i, i < rP →
      Δb[K - 1 - i]? = some (ΓP.getD (rP - 1 - i) default)) :
    ∀ q, q < cnP → ∀ ρ' : Nat → V, Sat2 V Δb ρ' →
      ∃ w, denoteP m.acval env φ K
          (Expr.instSpine (fvsP.take rP) (rP - 1) (pins.getD q default))
          = some w ∧ AnnotOkP V ρ' w ∧
        ∀ dw, denoteP m.acval env φ K (cdomsP.getD q default) = some dw →
  ...
  have hopenerLeafP : ∀ (q0 : Nat) (a : Expr), (fvsP.take rP)[q0]? = some a →
      ∀ l ∈ a.fvarLeaves,
        Expr.fvar l.1 l.2.2 ∈ fvsP ∧ l.1 < rP := by
    intro q0 a ha l hl
    have hq0lt : q0 < rP := by
  ...
  have hctxPin : ∀ q, q < cnP →
      CtxOkP m φ K Δb
        (Expr.instSpine (fvsP.take rP) (rP - 1) (pins.getD q default)) := by
    intro q hq
    exact ctxOkP_of_openers m.acval_closed hΔblen hshapeP hwsFvsPK hdomsP0
  ...
  have hleafTy : ∀ l ∈ ty.fvarLeaves,
      Expr.fvar l.1 l.2.2 ∈ fvsP ∧ l.1 < rP := fun l hl =>
  ...
  have hgdw : ∀ ρ0 : Nat → V, Sat2 V Δb ρ0 → AnnotOkP V ρ0 dw := by
    intro ρ0 hρ0
    refine instPisAt_domsP_graded (ρ' := ρ0) m.acval_closed
...
theorem nestedParamSupplyP {m : EnvS2Core V env} {F : Nat}
    ...
    (hpinRead : ∀ q, q < cnP → ∃ w,
      denoteP m.acval env φ (rP + cnF) (sp.getD q default) = some w) :
  ...
  have hsubj : ∀ q, q < cnP →
      denoteP m.acval env φ (rP + cnF) (sp.getD q default)
        = denoteP m.acval env φ (rP + cnF)
            (Expr.instSpine (fvsP.take rP) (rP - 1)
              (pins.getD q default)) := by
    intro q hq
    rw [hspgetD q hq]
    exact RenEqT.denoteP hro (hpsRen q hq) (rP + cnF)
  ...
  have hsatB : ∀ ρ' : Nat → V, Sat2 V Δa ρ' →
    ...
    · have hp : (rP + cnF) - 1 - ((rP + cnF) - 1 - i) = i := by omega
      ...
      exact hsatA
  -- the ladder at the recursor frame
  have hladder := nestedPinFireP (m := m) hclaims hinfC hreads
