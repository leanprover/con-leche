...
unblocked.  `prefixGradeFireP` (part 4) walks the *statement* frame's
...
context at all — `CtxOkP` identifies a leaf's *annotation* with the
...
`prefixGradeFireP` has already proved the two contexts' slots
interp-equal.

The payoff of the separate context is that the ladder needs **no
external input**: `Sat2 V Δb` alone grades every `ΓP` slot
(`annotOkP_tower_slot` at the recursor tower, whose memberships *are*
...
theorem paramGradeFireP {m : EnvS2Core V env} {F : Nat}
    ...
    ∀ q, q < cnP → ∀ ρ' : Nat → V, Sat2 V Δb ρ' →
      ∀ dw : AVExpr,
        denoteP m.acval env φ K (cdomsP.getD q default) = some dw →
        AnnotOkP V ρ' dw ∧ ρ' (K - 1 - q) ∈ˢ interp2 V ρ' dw := by
  ...
  have hAv : denoteP m.acval env φ K tyP
      = some ((ΓP.getD (rP - 1 - q) default).liftN (K - q) 0) := by
    rw [denoteP_lift m.acval_closed hwsTyP K (by omega), hdomTyP]
    rfl
  ...
  have hgbAll : ∀ ρ0 : Nat → V, Sat2 V Δb ρ0 → ∀ dw : AVExpr,
      denoteP m.acval env φ K (cdomsP.getD q default) = some dw →
      AnnotOkP V ρ0 dw := by
    intro ρ0 hρ0 dw hdw
  ...
  have hfire := defEqAtP_of_run (m := m) hclaims (k := K) (fvs := fvsP)
    ...
    (fun ρ0 hρ0 => (AnnotOkP_liftN V (K - q) _ 0 ρ0).mpr (by
      rw [hshiftEnv ρ0]
