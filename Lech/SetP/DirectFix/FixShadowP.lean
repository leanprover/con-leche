...
recursive variable (`directFixOpenedOk`), so the claims' context can
...
the point at those slots — which `CtxOkP` accepts because it
constrains a context only at the leaves of the term read.  The rows
(`ClaimsAtP.sortRow`) at the shadow context grade every entry, and the
...
def shadowCtx (nP : Nat) (ks : List RecFieldKind) (k : Nat) (Γ : List AVExpr) : List AVExpr :=
...
variable {nP k : Nat} {ks : List RecFieldKind} {Γ : List AVExpr} {fvs : List Expr}
...
/-- `CtxOkP` at the shadow context for a term whose leaves avoid the
recursive variables, from the grading of the non-recursive entries at
the shadow context below it. -/
theorem shadowCtxOk {m : EnvS2Core V env} {ψ : Name → Nat} {k : Nat} {e : Expr}
    {fvs : List Expr} {o : Expr} {Γ : List AVExpr} {R : AVExpr}
  ...
  have hgetD : ∀ j, (hj : j < k) → fvs.getD j default = fvs[j]'(by rw [hlenF]; exact hj) := by
    ...
    rfl
  refine ctxOkP_of_openers m.acval_closed (fvs := (shadowFvs nP ks k fvs).take b)
...
theorem fixShadowGrading (hμ : μ.verifiedChecks = true) (mp : EnvS2PM V μ env)
    ...
    (∀ b, b < nP + nF → ∀ ρ : Nat → V,
      ...
      AnnotOkP V ρ ((((ds ψ).map (·.2.2)).reverse).getD (nP + nF - 1 - b) default) ∧
      (nP ≤ b → ¬ recAt nP ks b → resSort.eval ψ ≠ 0 →
        interp2 V ρ ((((ds ψ).map (·.2.2)).reverse).getD (nP + nF - 1 - b) default)
  ...
  have hO : OpenedP mp.base2 ψ (nP + nF) cvCa.type (fvsP ++ xFvs) xrest
    ...
    openedP_of_peel hopAll htf' hbt' (hD.read ψ) (hD.len ψ) (hD.okTy ψ)
  ...
  have hrow := (claimsAtP_of hμ mp ψ F').sortRow hib hensb hO.bodyScoped.1 hO.bodyScoped.2.1
