...
theorem domsWalk_of_prefixOk :
    ∀ (rds : List (Nat × Nat × AVExpr)) (Δ₀ : List AVExpr) (ρ : Nat → V), Sat2 V Δ₀ ρ →
      (∀ k d, rds[k]? = some d → ∀ σ : Nat → V,
        Sat2 V (((rds.take k).map (·.2.2)).reverse ++ Δ₀) σ → AnnotOk2 V σ d.2.2) →
      DomsWalk ρ rds
  | [], _, _, _, _ => trivial
  | d :: ds, Δ₀, ρ, hρ, hok => by
    refine ⟨hok 0 d rfl ρ (by simpa using hρ), fun a ha => ?_⟩
    refine domsWalk_of_prefixOk ds (d.2.2 :: Δ₀) (cons a ρ) (Sat2_cons V hρ ha) ?_
    intro k d' hk σ hσ
...
theorem drop_reverse_map {rds : List (Nat × Nat × AVExpr)} {n k : Nat} (hlen : rds.length = n)
    (hk : k ≤ n) :
...
gradings. -/
theorem prefixOk_of_okΓ {rds : List (Nat × Nat × AVExpr)} {n : Nat} (hlen : rds.length = n)
    (okΓ : ∀ i, i < n → ∀ ρ : Nat → V,
      Sat2 V ((((rds.map (·.2.2)).reverse)).drop (n - i)) ρ →
      AnnotOkP V ρ ((((rds.map (·.2.2)).reverse)).getD (n - 1 - i) default)) :
    ∀ k d, rds[k]? = some d → ∀ σ : Nat → V,
      Sat2 V (((rds.take k).map (·.2.2)).reverse ++ []) σ → AnnotOkP V σ d.2.2 := by
  intro k d hk σ hσ
...
theorem domsBelow_getElem? :
    ∀ {ds : List (Nat × Nat × AVExpr)} {k j : Nat} {d : Nat × Nat × AVExpr},
      DomsBelow k ds → ds[j]? = some d → VExpr.bvarsBelow (k + j) d.2.2.erase
  | [], _, _, _, _, h => nomatch h
  | d' :: ds, k, 0, d, hb, h => by
...
theorem spineFit_liftDoms_iff (n : Nat) :
  ...
  | d :: ds, k, σ, a :: as => by
    ...
    rw [interp2_liftN, spineFit_liftDoms_iff n ds (k + 1) (cons a σ) as, shiftE_succ_cons']

/-! ## Spines of the recursor's binder data -/

/-- A spine fitting a single domain. -/
theorem spineFit_singleton {σ : Nat → V} {D : AVExpr} :
    ∀ {xs : List V}, SpineFit σ [D] xs → ∃ x, xs = [x] ∧ x ∈ˢ interp2 V σ D
  | [], h => h.elim
  | [x], h => ⟨x, rfl, h.1⟩
  | _ :: _ :: _, h => h.2.elim
...
theorem fixPre_of {m : EnvS2Core V env} {ψ : Name → Nat} {T : Name} {elimL : Level}
    ...
    (hokTy : ∀ ρ : Nat → V,
      AnnotOk2 V ρ (mkPisAV (fixRecDataAV m T ψ nP nIdx elimL pps ips cds) (recConcAV n nIdx)))
    (hunivTy : ∀ ρ : Nat → V,
  ...
  · -- `hdoms`
    intro ρb
    exact domsWalk_of_prefixOk rds [] ρb (Sat2_nil V ρb)
