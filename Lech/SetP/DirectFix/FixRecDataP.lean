...
`denoteP_directRecTyR`), its universe (the kernel's own sort
...
def fixRdsAV {env : Env} (m : EnvS2Core V env) (p : DirectFixParts)
...
theorem fixRecData_of (hμ : μ.verifiedChecks = true) (mp : EnvS2PM V μ env)
  ...
  · have hc := claimsAtP_of hμ mp ψ F
    exact (hc.sortRow hsty hens hw hbt hL (CtxOkP.nil hnil) (hread ψ) ρ (Sat2_nil V ρ)).2
