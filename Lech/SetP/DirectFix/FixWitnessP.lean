...
`directUsedLater` guard) — its POSITIONS are the recursive fields'
...
theorem shadowFs_okB {u w nP nF : Nat} (hw : w ≠ 0) {ρp : Nat → V} {Ids : List AVExpr}
    {ks : List RecFieldKind} {tls : List (List (Nat × Nat × AVExpr))} {Fs : List AVExpr}
    {Eis : List (List AVExpr)} {Es : List AVExpr}
...
def shadowFss (nP : Nat) (ksF : Nat → List RecFieldKind) (Fss : List (List AVExpr)) :
    List (List AVExpr) :=
  (List.range Fss.length).map fun j => shadowFs nP (ksF j) (Fss.getD j []).length (Fss.getD j [])

theorem shadowFss_length (nP : Nat) (ksF : Nat → List RecFieldKind) (Fss : List (List AVExpr)) :
    (shadowFss nP ksF Fss).length = Fss.length := by simp [shadowFss]

theorem shadowFss_getElem? (nP : Nat) (ksF : Nat → List RecFieldKind) (Fss : List (List AVExpr))
...
noncomputable def shapeSet (u w nP : Nat) (ρp : Nat → V) (Ids : List AVExpr)
    (ksF : Nat → List RecFieldKind) (Fss Ess : List (List AVExpr)) (t : V) : V :=
...
the tuple's. -/
theorem mem_shapeSet {u w nP : Nat} (hw : w ≠ 0) {ρp : Nat → V} {Ids : List AVExpr}
    {ksF : Nat → List RecFieldKind} {Fss Ess : List (List AVExpr)} {t a : V} :
  ...
  · rintro ⟨j, as', hj, rfl, hfit, hidx⟩
    ...
    show (pt : V) ∈ˢ interp2 V (consList as' ρp) (idxEqAV []) ∧ True
...
theorem shapeSet_mem {u w nP : Nat} (hw : w ≠ 0) {ρp : Nat → V} {Ids : List AVExpr}
    {ksF : Nat → List RecFieldKind} {tlss : List (List (List (Nat × Nat × AVExpr)))}
...
theorem lamTower_mem_univ {w : Nat} (hw : w ≠ 0) {g : (Nat → V) → V} :
    ∀ {tl : List (Nat × Nat × AVExpr)} {ρ : Nat → V},
  ...
  | d :: tl, ρ, hF, hg => by
    have hU := univ_isTGUniverse (V := V) hw
    show lamR w (interp2 V ρ d.2.2) (fun a => lamTower w (cons a ρ) tl g) ∈ˢ _
...
theorem lamTower_congr_exclP {Q : Nat → Prop} {m : Nat} {g : (Nat → V) → V} :
    ∀ (tl : List (Nat × Nat × AVExpr)) {d : Nat}, (∀ q, Q q → q < d) → ∀ {σ σ' : Nat → V},
  ...
  | dd :: tl, d, hQ, σ, σ', hag, hnb, hg => by
    show lamR m (interp2 V σ dd.2.2) (fun a => lamTower m (cons a σ) tl g)
      = lamR m (interp2 V σ' dd.2.2) (fun a => lamTower m (cons a σ') tl g)
    have hnb0 : NoBVar (exclP Q d) dd.2.2 := by simpa using hnb 0 dd rfl
    rw [← interp2_congr_noBVar dd.2.2 hnb0 hag]
...
theorem piTele_eta {w : Nat} (hw : w ≠ 0) {B : List V → V} :
    ∀ {tl : List (Nat × Nat × AVExpr)} {ρ : Nat → V} {acc : List V} {f : V},
  ...
  | d :: tl, ρ, acc, f, hf => by
    ...
    show lamR w (interp2 V ρ d.2.2) (fun a => lamTower w (cons a ρ) tl
...
theorem spineFit_take_prefix {Fs : List AVExpr} {ρ : Nat → V} {as : List V}
...
noncomputable def spineSet (w : Nat) (ρp : Nat → V) (tl : List (Nat × Nat × AVExpr)) (as : List V) :
    V :=
...
noncomputable def posSet (w : Nat) (ρp : Nat → V) (rss : List (List Bool))
    (tlss : List (List (List (Nat × Nat × AVExpr)))) (Fss : List (List AVExpr)) (a : V) : V :=
...
noncomputable def posTgt (u : Nat) (ρp : Nat → V) (tlss : List (List (List (Nat × Nat × AVExpr))))
    (Eiss : List (List (List AVExpr))) (Fss : List (List AVExpr)) (a p : V) : V :=
  tupW u (((Eiss.getD (shapeTag a) []).getD (natIdx (sfst p)) []).map
    (interp2 V (consList (projList ((tlss.getD (shapeTag a) []).getD (natIdx (sfst p)) []).length (ssnd p))
...
noncomputable def mkShape (w : Nat) (ρp : Nat → V) (rss : List (List Bool))
    (tlss : List (List (List (Nat × Nat × AVExpr)))) (Fss : List (List AVExpr)) (a g : V) : V :=
...
variable {u w nP : Nat} (hw : w ≠ 0) {ρp : Nat → V} {Ids : List AVExpr} (hI : IdxOk u ρp Ids)
...
theorem shadowOf_fits {j : Nat} (hj : j < Fss.length) {X t : V} :
  ...
  | F :: Fs', i, as, bs, hi, hF, h => by
    ...
    have hget : (shadowFs nP (ksF j) (Fss.getD j []).length (Fss.getD j []))[as.length]
        = if recAt nP (ksF j) (nP + as.length) then AVExpr.sort 0
    ...
    · by_cases hr : recAt nP (ksF j) (nP + as.length)
      · rw [if_pos hr, if_pos hr, interp2_sort]
        exact shadowVal_mem
      · rw [if_neg hr, if_neg hr]
        ...
        rw [← hFgetD] at hnb
        rw [← hFgetD, ← interp2_congr_noBVar F hnb (agreeOff_shadow (shadowRel_shadowOf nP (ksF j) as) ρp)]
        exact hb
...
theorem idxValsAt_shadow {j : Nat} (hj : j < Fss.length) {fs : List V}
  ...
  rw [← hlen] at hnb
  exact (interp2_congr_noBVar E hnb (agreeOff_shadow (shadowRel_shadowOf nP (ksF j) fs) ρp)).symm
...
theorem fixStep_elim_container {X : V} (hX : X ∈ˢ lfpFamSpace V w (idxSet u ρp Ids)) {t : V}
  ...
  have hEmap : ∀ i ∈ recIdx (rss.getD j []) (Fss.getD j []).length, ∀ bs : List V,
      ...
      ((Eiss.getD j []).getD i []).map (interp2 V (consList bs (consList ((shadowOf nP (ksF j) fs).take i) ρp)))
        = ((Eiss.getD j []).getD i []).map (interp2 V (consList bs (consList (fs.take i) ρp))) := by
    intro i hi bs hbs
    apply List.map_congr_left
    intro E hE
    have hnb := (hC j hj).nbE i (mem_recIdx.mp hi).1 (hrecAt i hi) E hE
    rw [← hbs] at hnb
    exact (interp2_congr_noBVar E hnb (agreeOff_exclP_consList bs (hQ i) (hag i hi))).symm
