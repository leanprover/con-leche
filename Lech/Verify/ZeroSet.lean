...
`Lech/Kernel/ZeroSet.lean` defines the canonical representation
...
(`Lech/Verify/PropWhen.lean`) — against the **same** semantics
...
open Lech.ZPropWhen Lech.ZeroSet
...
theorem _root_.Lech.ZPropWhen.holds_eq_of_equiv {p q : ZPropWhen}
...
@[simp] theorem holds_ofFree (φ : Name → Nat) : ∀ p : PropWhen,
  ...
  | never => rfl
  | ifAllZero ps =>
    rw [ofFree_ifAllZero, Lech.PropWhen.holds_ifAllZero]
...
theorem toFree_ofFree_equiv (p : PropWhen) :
  ...
  (Lech.PropWhen.equiv_iff_holds _ _).mpr fun φ => by simp
...
theorem ofFree_eq_iff_equiv (p q : PropWhen) :
  ...
  rw [eq_iff_holds, Lech.PropWhen.equiv_iff_holds]
  simp

theorem eq_iff_equiv_toFree (a b : ZPropWhen) :
    a = b ↔ PropWhen.equiv a.toFree b.toFree = true := by
  rw [Lech.PropWhen.equiv_iff_holds, eq_iff_holds]
  simp
...
theorem ofFree_zeronessOf (l : Level) :
  ...
  eq_of_holds fun φ => by
    rw [holds_ofFree, Lech.PropWhen.zeronessOf_sound,
...
private theorem holds_substPW_free (φ : Name → Nat) (ks : List Name)
  ...
  Lech.Level.holds_substPW φ ks vs
