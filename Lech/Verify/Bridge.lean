...
scope check** (`memoE`/`memoB` in `Lech/Kernel/TypeCheckerC.lean`):
a cache entry must therefore be consumable at *every* depth at which
its key is well-scoped — `CacheOK` (`Lech/Verify/Scoped.lean`) backs
...
(`Lech/Verify/Deep.lean`, requiring `EnvWF`).

What makes the missing runtime check sound is the **call discipline**
(`Lech/Verify/Disc.lean`): the conditional simulation (`ScopedSim`)
