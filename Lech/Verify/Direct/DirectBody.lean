...
.proj T j (bvar 0)]` (`directProjBodies`, the domain of field `i` of
the constructor telescope peeled at the loose parameter variables and
the earlier projections of the subject — `directProjResidP`).  The
...
(`directProjBody_open`).  It is `instSeq_instSeqLift` — the collapse
...
theorem directProjBodiesGo_spec (T : Name) :
...
first `i` subject projections (`directProjResidP`), and there is one
per field. -/
theorem directProjBodies_spec {T : Name} {nP nF : Nat} {cty : Expr}
  ...
  | some r =>
    ...
    induction i with
    | zero => simp [directProjResidP, hr]
    | succ i ih =>
      rw [directProjResidP, ih (by omega), List.range_succ, List.foldl_append,
...
theorem directProjArgs_bounded (T : Name) (nP i : Nat) :
    ∀ a ∈ directProjPs nP ++ (List.range i).map (directProjArgP T),
...
projections. -/
theorem directProjArgs_instSeq (T : Name) (nP i : Nat) :
    (directProjPs nP ++ (List.range i).map (directProjArgP T)).map
  ...
  · show ((List.range i).map (directProjArgP T)).map (instSeq (fvsD nP ++ [tfvD nP]) nP)
      = (List.range i).map (fun j => Expr.proj T j (tfvD nP))
    rw [List.map_map]
    apply List.map_congr_left
    intro j _
...
theorem directProjBody_open {T : Name} {nP nF : Nat} {cty : Expr}
    {bodies : Array Expr} (h : directProjBodies T nP nF cty = some bodies)
