...
(`interp2_substProjAt`, `ihIdxM_interp`, `lamTower_ihTeleAt`,
...
below them. -/
theorem interp2_substProjAt (σ : Nat → V) (y : V) (bs : List V) :
    ∀ (i : Nat) (e : AVExpr),
      interp2 V (consList bs (cons y σ)) (substProjAt bs.length i e)
        = interp2 V (consList bs (consList (projList i y) (cons y σ))) e
  | 0, _ => rfl
  | i + 1, e => by
    show interp2 V (consList bs (cons y σ)) (substProjAt bs.length i (e.inst (projAV i (.bvar i)) bs.length)) = _
    rw [interp2_substProjAt σ y bs i, interp2_inst, shiftE_consList, projAV_interp, interp2_bvar]
...
theorem AnnotOk2_substProjAt (σ : Nat → V) (y : V) (bs : List V) :
    ∀ (i : Nat) (e : AVExpr),
      (∀ m, m < i → AnnotOk2 V (consList (projList m y) (cons y σ)) (projAV m (.bvar m))) →
      (AnnotOk2 V (consList bs (cons y σ)) (substProjAt bs.length i e) ↔
        AnnotOk2 V (consList bs (consList (projList i y) (cons y σ))) e)
  | 0, _, _ => Iff.rfl
  | i + 1, e, hp => by
    show AnnotOk2 V (consList bs (cons y σ)) (substProjAt bs.length i (e.inst (projAV i (.bvar i)) bs.length)) ↔ _
    rw [AnnotOk2_substProjAt σ y bs i _ (fun m hm => hp m (by omega)),
      AnnotOk2_inst V _ _ _ _ (by rw [shiftE_consList]; exact hp i (by omega)), shiftE_consList,
...
def ihArgBody (nP n nIdx D i m : Nat) (Eis : List AVExpr) : AVExpr :=
...
omit [SetTheory V] in
theorem ihArgAV_eq (ℓ nP n nIdx D i : Nat) (tl : List (Nat × Nat × AVExpr)) (Eis : List AVExpr) :
...
theorem ihIdxM_interp (hfr : RecFrameS D ρ₀ σ) (y : V) (i : Nat) (bs : List V) (E : AVExpr) :
    interp2 V (consList bs (cons y σ))
        (substProjAt bs.length i (E.liftN (D + Ids.length + Fss.length + 2) (i + bs.length)))
      = interp2 V (consList bs (consList (projList i y) (frP Fss.length Ids.length ρ₀))) E := by
  rw [interp2_substProjAt, interp2_liftN, ← consList_append,
...
theorem ihIdxM_ok2 (hfr : RecFrameS D ρ₀ σ) (y : V) (i : Nat) (bs : List V) (E : AVExpr)
    (hp : ∀ m, m < i → AnnotOk2 V (consList (projList m y) (cons y σ)) (projAV m (.bvar m)))
    (hE : AnnotOk2 V (consList bs (consList (projList i y) (frP Fss.length Ids.length ρ₀))) E) :
    AnnotOk2 V (consList bs (cons y σ))
      (substProjAt bs.length i (E.liftN (D + Ids.length + Fss.length + 2) (i + bs.length))) := by
  rw [AnnotOk2_substProjAt σ y bs i _ hp, AnnotOk2_liftN, ← consList_append,
    show i + bs.length = (projList i y ++ bs).length from by rw [List.length_append, projList_length],
    shiftE_consList_len, shiftE_payload hfr, consList_append]
  exact hE
...
remaining telescope. -/
def ihTeleAtGoP (nIdx n D i : Nat) : Nat → List (Nat × Nat × AVExpr) → List (Nat × Nat × AVExpr)
  | _, [] => []
...
omit [SetTheory V] in
theorem ihTeleAt_eq_go (nIdx n D i : Nat) (tl : List (Nat × Nat × AVExpr)) :
...
theorem lamTower_ihTeleAtGoP (hfr : RecFrameS D ρ₀ σ) (y : V) (i : Nat) {g₁ g₂ : (Nat → V) → V} :
  ...
  | d :: tl, as, hg => by
    show lamR ℓ (interp2 V _ (substProjAt as.length i (d.2.2.liftN (D + Ids.length + Fss.length + 2) (i + as.length))))
        (fun a => lamTower ℓ (cons a _) (ihTeleAtGoP Ids.length Fss.length D i (as.length + 1) tl) g₁)
      = lamR ℓ (interp2 V _ d.2.2) (fun a => lamTower ℓ (cons a _) tl g₂)
...
theorem domsWalk_ihTeleAtGoP (hfr : RecFrameS D ρ₀ σ) (y : V) (i : Nat)
  ...
  | [], _, _ => trivial
  | d :: tl, as, h => by
    refine ⟨ihIdxM_ok2 hfr y i as d.2.2 hp h.1, fun a ha => ?_⟩
...
theorem spineFit_ihTeleAtGoP (hfr : RecFrameS D ρ₀ σ) (y : V) (i : Nat) :
  ...
  | d :: tl, as, b :: bs => by
    show (b ∈ˢ interp2 V _ (substProjAt as.length i (d.2.2.liftN (D + Ids.length + Fss.length + 2) (i + as.length)))
        ∧ SpineFit (cons b _) ((ihTeleAtGoP Ids.length Fss.length D i (as.length + 1) tl).map (·.2.2)) bs)
      ↔ (b ∈ˢ interp2 V _ d.2.2 ∧ SpineFit (cons b _) (tl.map (·.2.2)) bs)
    rw [ihIdxM_interp hfr y i as d.2.2]
    have := spineFit_ihTeleAtGoP hfr y i tl (as ++ [b]) bs
    rw [length_snoc', ← consList_snoc', ← consList_snoc'] at this
    exact and_congr_right fun _ => this

theorem spineFit_ihTeleAt (hfr : RecFrameS D ρ₀ σ) (y : V) (i : Nat) (tl : List (Nat × Nat × AVExpr))
    (bs : List V) :
...
theorem ihArg_head_interp (hfr : RecFrameS D ρ₀ σ) (y : V) (bs : List V) :
    interp2 V (consList bs (cons y σ)) (.bvar (D + 1 + Ids.length + Fss.length + 1 + nP + bs.length))
      = frR nP Fss.length Ids.length ρ₀ := by
  rw [interp2_bvar, consList_apply_add bs (cons y σ)]
  have := rAt_interp (nP := nP) (Fss := Fss) (Ids := Ids) hfr y
  rwa [interp2_bvar] at this
...
theorem ihArg_args_interp (hfr : RecFrameS D ρ₀ σ) (y : V) (i : Nat) (bs : List V) (Eis : List AVExpr) :
    (idxVarsAV (nP + 1 + Fss.length) (D + 1 + Ids.length + bs.length) ++
        ...
        [AVExpr.mkAppN (projAV i (.bvar bs.length)) (teleVarsAV bs.length)]).map
      (interp2 V (consList bs (cons y σ)))
      = frKSpine nP Fss.length Ids.length ρ₀ ++
          Eis.map (interp2 V (consList bs (consList (projList i y) (frP Fss.length Ids.length ρ₀)))) ++
  ...
  rw [List.map_append, List.map_append, map_idxVarsAV_interp (recFrameS_tele hfr y bs), List.map_map,
    List.map_cons, List.map_nil, interp2_mkAppN, ← List.foldl_map (f := interp2 V _) (g := SetTheory.app),
    map_teleVarsAV_interp, projAV_interp, interp2_bvar, hy]
  congr 2
  apply List.map_congr_left
  intro E _
...
theorem ihArg_leaf_interp (hfr : RecFrameS D ρ₀ σ) (y : V) (i : Nat) (bs : List V) (Eis : List AVExpr) :
    interp2 V (consList bs (cons y σ)) (ihArgBody nP Fss.length Ids.length D i bs.length Eis)
      = (frKSpine nP Fss.length Ids.length ρ₀ ++
          Eis.map (interp2 V (consList bs (consList (projList i y) (frP Fss.length Ids.length ρ₀)))) ++
          [bs.foldl SetTheory.app (projS i y)]).foldl SetTheory.app (frR nP Fss.length Ids.length ρ₀) := by
  unfold ihArgBody
  rw [interp2_mkAppN, ← List.foldl_map (f := interp2 V _) (g := SetTheory.app), ihArg_args_interp hfr y i bs,
...
theorem ihArgAV_interp (hfr : RecFrameS D ρ₀ σ) (y : V) (i : Nat) (tl : List (Nat × Nat × AVExpr))
    (Eis : List AVExpr) :
    interp2 V (cons y σ) (ihArgAV ℓ nP Fss.length Ids.length D i tl Eis)
      = lamTower ℓ (consList (projList i y) (frP Fss.length Ids.length ρ₀)) tl fun σ' =>
          (frKSpine nP Fss.length Ids.length ρ₀ ++ Eis.map (interp2 V σ') ++
            [(frameIdx tl.length σ').foldl SetTheory.app (projS i y)]).foldl SetTheory.app
            (frR nP Fss.length Ids.length ρ₀) := by
  rw [ihArgAV_eq, interp2_mkLamsC]
...
theorem ihArgsOk_tele (h : FixKI ℓ w u nP ρ₀ Fss Ess Fss₀ Ids rss tlss Eiss rds) (hw : w ≠ 0)
  ...
  have hleaf : ∀ i ∈ recIdx (rss.getD j []) (Fss.getD j []).length, ∀ bs : List V,
      ...
      SpineFit (frP Fss.length Ids.length ρ₀) Ids
        (((Eiss.getD j []).getD i []).map (interp2 V (consList bs (consList (projList i y) (frP Fss.length Ids.length ρ₀))))) ∧
      (∀ E ∈ (Eiss.getD j []).getD i [],
        AnnotOk2 V (consList bs (consList (projList i y) (frP Fss.length Ids.length ρ₀))) E) ∧
      bs.foldl SetTheory.app (projS i y) ∈ˢ SetTheory.app
        ...
        (tupW u (((Eiss.getD j []).getD i []).map
          (interp2 V (consList bs (consList (projList i y) (frP Fss.length Ids.length ρ₀)))))) := by
    intro i hi bs hbs
  ...
  · -- the readings
    ...
    apply List.map_congr_left
    intro i _
    show interp2 V (cons y σ) (ihArgAV ℓ nP Fss.length Ids.length D i _ _) = _
  ...
  · -- the gradings and the memberships
    ...
    have hgetD : (((recIdx (rss.getD j []) (Fss.getD j []).length)).map fun i =>
        piTele ℓ (teleOfFields (consList ((projList (Fss.getD j []).length y).take i) (frP Fss.length Ids.length ρ₀))
          ...
          (fun as => SetTheory.app
            ((((Eiss.getD j []).getD i []).map
              (interp2 V (consList as (consList ((projList (Fss.getD j []).length y).take i)
        ...
        = piTele ℓ (teleOfFields (consList (projList i y) (frP Fss.length Ids.length ρ₀))
            ...
            (fun as => SetTheory.app
              ((((Eiss.getD j []).getD i []).map
                (interp2 V (consList as (consList (projList i y) (frP Fss.length Ids.length ρ₀))))).foldl
    ...
    · rw [spineFit_ihTeleAt hfr y i] at hbs
      ...
      have hargs : ∀ a ∈ idxVarsAV (nP + 1 + Fss.length) (D + 1 + Ids.length + bs.length) ++
          ...
          [AVExpr.mkAppN (projAV i (.bvar bs.length)) (teleVarsAV bs.length)],
          AnnotOk2 V (consList bs (cons y σ)) a := by
        intro a ha
        ...
        · obtain ⟨_, -, rfl⟩ := List.mem_map.mp ha
          simp
        · exact ihIdxM_ok2 hfr y i bs E (hp i (Nat.le_of_lt hik)) (hEok E hE)
        · -- the field applied to the telescope's variables
          have hy : interp2 V (consList bs (cons y σ)) (.bvar bs.length) = y := by
            rw [interp2_bvar]
            have := consList_apply_add bs (cons y σ) 0
            rwa [Nat.zero_add] at this
          refine (mkAppN_ok2_of_chain (hproj i hik _ _ hy) (fun a ha => teleVarsAV_ok2 ha) ?_).1
      ...
      have hchain : AppChainOk
          ...
          ((idxVarsAV (nP + 1 + Fss.length) (D + 1 + Ids.length + bs.length) ++
            ((Eiss.getD j []).getD i []).map
              (fun E => substProjAt bs.length i (E.liftN (D + Ids.length + Fss.length + 2) (i + bs.length))) ++
            [AVExpr.mkAppN (projAV i (.bvar bs.length)) (teleVarsAV bs.length)]).map
              (interp2 V (consList bs (cons y σ)))) := by
        rw [ihArg_args_interp hfr y i bs, ihArg_head_interp hfr y bs]
        exact h.app_chain hvsp hfmem
      refine ⟨(mkAppN_ok2_of_chain (by simp) hargs hchain).1, ?_, fun h0 => ?_⟩
