...
/-- The step: the element's minor at its fields and the ih values. -/
noncomputable def elemSt (ℓ u : Nat) (ρp : Nat → V) (rss : List (List Bool))
    (tlss : List (List (List (Nat × Nat × AVExpr)))) (Eiss : List (List (List AVExpr)))
    (Fss : List (List AVExpr)) (ms : Nat → V) (e g : V) : V :=
  (elemFields Fss e ++
    elemIhs ℓ u ρp (rss.getD (elemTag e) []) (tlss.getD (elemTag e) []) (Eiss.getD (elemTag e) [])
      (Fss.getD (elemTag e) []).length (elemFields Fss e) g).foldl SetTheory.app (ms (elemTag e))
...
variable {ℓ u w : Nat} {ρp : Nat → V} {M : V} {ms : Nat → V} {Fss Ess Fss₀ : List (List AVExpr)}
...
theorem elemGraph_singleton (hX : XChainsOk u w ρp Ids rss tlss Eiss Fss₀ Ess)
    ...
    (hst : ∀ e, e ∈ˢ elemSet u ρp Ids (fixFamI u w ρp Ids Ids.length rss tlss Eiss Fss₀ Ess) → ∀ g,
      g ∈ˢ piSet (elemPred u ρp Ids rss tlss Eiss Fss (fixFamI u w ρp Ids Ids.length rss tlss Eiss Fss₀ Ess) e)
        (fun e' => SetTheory.app
          (elemGraph ℓ u ρp M Ids rss tlss Eiss Fss ms (fixFamI u w ρp Ids Ids.length rss tlss Eiss Fss₀ Ess)) e') →
      elemSt ℓ u ρp rss tlss Eiss Fss ms e g ∈ˢ elemB u Ids.length M e) :
    ∀ t, t ∈ˢ idxSet u ρp Ids → ∀ x,
      x ∈ˢ SetTheory.app (fixFamI u w ρp Ids Ids.length rss tlss Eiss Fss₀ Ess) t →
      (∃ v, v ∈ˢ SetTheory.app
        (elemGraph ℓ u ρp M Ids rss tlss Eiss Fss ms (fixFamI u w ρp Ids Ids.length rss tlss Eiss Fss₀ Ess))
        (kpair t x)) ∧
  ...
  let G := elemGraph ℓ u ρp M Ids rss tlss Eiss Fss ms μ
...
theorem elemGraph_sub_B (h : FixKI₀ ℓ w u K Fss Ess Fss₀ Ids rss tlss Eiss) {e v : V}
    ...
    (hv : v ∈ˢ SetTheory.app (elemGraph ℓ u (frP Fss.length Ids.length K) (frM Fss.length Ids.length K) Ids rss
      tlss Eiss Fss (frMs Fss.length Ids.length K) (famK u w K Fss Ess Fss₀ Ids rss tlss Eiss)) e) :
    ...
    ∀ e, e ∈ˢ elemSet u (frP Fss.length Ids.length K) Ids (famK u w K Fss Ess Fss₀ Ids rss tlss Eiss) → ∀ g,
      g ∈ˢ piSet (elemPred u (frP Fss.length Ids.length K) Ids rss tlss Eiss Fss
        ...
        (fun e' => SetTheory.app (elemGraph ℓ u (frP Fss.length Ids.length K) (frM Fss.length Ids.length K) Ids
          rss tlss Eiss Fss (frMs Fss.length Ids.length K) (famK u w K Fss Ess Fss₀ Ids rss tlss Eiss)) e') →
      elemSt ℓ u (frP Fss.length Ids.length K) rss tlss Eiss Fss (frMs Fss.length Ids.length K) e g
...
theorem elemK_facts (h : FixKI₀ ℓ w u K Fss Ess Fss₀ Ids rss tlss Eiss) (hw : w ≠ 0) (hℓ : ℓ ≠ 0)
    ...
    recSel (elemGraph ℓ u (frP Fss.length Ids.length K) (frM Fss.length Ids.length K) Ids rss tlss Eiss Fss
        (frMs Fss.length Ids.length K) (famK u w K Fss Ess Fss₀ Ids rss tlss Eiss)) (kpair (tupW u is) x)
      ∈ˢ SetTheory.app (is.foldl SetTheory.app (frM Fss.length Ids.length K)) x ∧
    recSel (elemGraph ℓ u (frP Fss.length Ids.length K) (frM Fss.length Ids.length K) Ids rss tlss Eiss Fss
        (frMs Fss.length Ids.length K) (famK u w K Fss Ess Fss₀ Ids rss tlss Eiss)) (kpair (tupW u is) x)
      = elemSt ℓ u (frP Fss.length Ids.length K) rss tlss Eiss Fss (frMs Fss.length Ids.length K)
          (kpair (tupW u is) x)
  ...
  have hsing := elemGraph_singleton (M := frM Fss.length Ids.length K) (ms := frMs Fss.length Ids.length K)
