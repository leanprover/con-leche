...
structure FixPre (V : Type uv) [SetTheory V] (ℓ w u nP : Nat) (Fss Ess Fss₀ : List (List AVExpr)) (Ids : List AVExpr)
  ...
  recursion over the ω-iterate), or the block is a `Prop` (task #202:
  the inhabitation is by lfp induction; at a large eliminator the
  recursion equation's unique solution, `hsq` in the K-frame package) -/
