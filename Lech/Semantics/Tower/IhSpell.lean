...
The `AVExpr` spellings shared by the P tier's readings of the kernel's
...
def prefixVarsAV (nP n nF m : Nat) : List AVExpr :=
  ((List.range nP).map fun k => AVExpr.bvar (nP + nF + n + 1 + m - 1 - k)) ++ [.bvar (nF + n + m)] ++
...
def ihAppAVb (b : Nat) (Rm : Nat → AVExpr) (nP n nF e i : Nat) (tl : List (Nat × Nat × AVExpr))
    (Eis : List AVExpr) : AVExpr :=
  mkLamsC b (ihTeleAtR nF (n + 1 + e) i 0 tl)
    (AVExpr.mkAppN (Rm tl.length) (prefixVarsAV nP n nF (tl.length + e) ++
...
def srcOfEs (Es : List AVExpr) (nF j : Nat) : Option Nat :=
...
def srcList (Es : List AVExpr) (nF : Nat) : List (Option Nat) :=
...
def fieldTeleAt (o : Nat) (Fs : List AVExpr) : List (Nat × Nat × AVExpr) :=
...
index variables. -/
def sqFixBodyAV (ℓ nP n nIdx : Nat) (Fs Es : List AVExpr) (rs : List Bool)
    (tls : List (List (Nat × Nat × AVExpr))) (Eis : List (List AVExpr)) : AVExpr :=
  AVExpr.mkAppN
    (mkLamsC ℓ (fieldTeleAt (nIdx + n + 2) Fs)
      (AVExpr.mkAppN (.bvar (Fs.length + 1 + nIdx + n - 1))
