...
theorem throw_ne_ok {α : Type} {e : Lech.CheckError} {a : α}
    (h : (throw e : Lech.CheckM α) = .ok a) : False := by
...
theorem checkDirectInd_inv {μ : CheckMode} {F : Nat} {env envI : Env}
    ...
    (h : checkDirectInd (m := Lech.CheckM) (fueledOps μ F) env p
      = .ok (envI, cvTa)) :
    ∃ cv : ConstantVal,
      checkConstantVal (m := Lech.CheckM) (fueledOps μ F) env p.cvT = .ok cv ∧
      cvTa = cv ∧ envI = ⟨.indInfo cv (directCaps p) :: env.consts⟩ := by
  unfold checkDirectInd at h
  repeat' first
    | (obtain ⟨_, _, h⟩ := Lech.exceptBind_ok h)
    | split at h
  all_goals first
    | (try dsimp only at h
...
theorem checkDirectCtor_inv {μ : CheckMode} {F : Nat} {env₀ env envC : Env}
    ...
    (h : checkDirectCtor (m := Lech.CheckM) (fueledOps μ F) env₀ env p cvTa
      = .ok (envC, cvCa, sorts)) :
    ∃ cv : ConstantVal,
      checkConstantVal (m := Lech.CheckM) (fueledOps μ F) env p.cvC = .ok cv ∧
...
theorem checkDirectRec_inv {μ : CheckMode} {F : Nat} {env : Env}
    ...
    (h : checkDirectRec (m := Lech.CheckM) (fueledOps μ F) env p cvTa cvCa
      = .ok (cvRa, rhsA)) :
    ∃ cv : ConstantVal,
      checkConstantVal (m := Lech.CheckM) (fueledOps μ F) env p.cvR = .ok cv ∧
      cvRa.name = p.cvR.name ∧ cvRa.levelParams = p.cvR.levelParams := by
  unfold checkDirectRec at h
  obtain ⟨cvRi, hcv, h⟩ := Lech.exceptBind_ok h
  repeat' first
    | (obtain ⟨_, _, h⟩ := Lech.exceptBind_ok h)
    | split at h
    | (dsimp only at h)
...
theorem checkDirectProjTable_shape {T C : Name}
    ...
    (h : checkDirectProjTable (m := Lech.CheckM) T C lps nP nF
      resSort guards cvCa env = .ok env') :
    ∃ tbl : Lech.ProjTable, env.find? (projTableName T) = none ∧
      tbl.structName = T ∧ env' = ⟨.projInfo tbl :: env.consts⟩ := by
  obtain ⟨bodies, -, -, -, hfresh, rfl⟩ := Lech.checkDirectProjTable_inv h
...
theorem declDirectRun_etaClosed {μ : CheckMode} {F : Nat} {env env₂ : Env}
  ...
  obtain ⟨hfC, -, -, -, -, -, _, _, _, -, -, -, -, -, hCeq⟩ :=
    Lech.checkConstantVal_inv hcvC
  obtain ⟨cvRi, hCV, hnR, -⟩ := checkDirectRec_inv hRec
  obtain ⟨hfR, -, -, -, -, -, _, _, _, -, -, -, -, -, -⟩ :=
    Lech.checkConstantVal_inv hCV
