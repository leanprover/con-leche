...
> removed").  `Lech/TTVerify/*.lean` — 50 files, 33,808 lines — is
> gone, together with the `--tt-model` mode it was stated at and the
> `LechTTV` library target.  The replacement surface is the fourteen
> `*_R` theorems of `Lech/SetR/Main.lean`.**
>
...
This document records the design of `Lech/TTVerify/*`.  It lives here
while the work is on its own branch (the same reason
`Lech/TT/DESIGN.md` does), and folds into the top-level `DESIGN.md`
...
1. **`denote_mono`** replaced the entire `Lech/Model/Extend/Transport`
...
codebase's measured base rate (`Lech/TT/DESIGN.md` §3.1: three for
...
`Lech/TTVerify/*` is therefore:

```
rm -f .lake/build/lib/lean/Lech/TTVerify/*.olean
...
The rule composes with `Lech/TT/DESIGN.md` §3.1's house rule
...
declarative type theory of `Lech/TT/*` and conclude with a `HasType`
derivation.  Both paths coexist; nothing in `Lech/Model/*` is
...
Layering: `Lech/TTVerify/*` may import `Lech/TT/*`,
`Lech/Kernel/*` and `Lech/Verify/*`.  The checker never imports it,
and `Lech/TT/*` stays checker-free — every adaptation lives on this
side.  Its own `lean_lib` target is `LechTTV`.

The subject is the **pure knot** (`Lech.Kernel.TypeChecker` at
`CheckM`): plain `Expr`, no arena, no caches, fuel-bounded open
recursion.  The descent from the executing checker (interned twins,
memoized knot) already exists in `Lech/Verify/*` and is inherited;
...
which is `Lech/Model/Interp.lean`'s `interpExpr` with the
...
`denote_delta_step` (`Lech/TTVerify/WhnfCoreStep.lean`) concludes
...
`thmInfo`.  There is no delta rule in `Lech/TT/Judgment.lean` at all.
...
* **`denote` is structural — every clause maps a constructor to a
  ...
  syntactic-substitution swamp `Lech/TT/DESIGN.md` §6 is proud of
...
* **level comparison is trivial**, as `Lech/TT/DESIGN.md` §2.2
...
`Lech/TT/DESIGN.md` §3's "projections denote to applications of
...
* `no_constant_of_Empty_TT` and `no_proof_of_Empty_TT` — the
  consistency corollary, through `Lech.TT.no_proof_of_empty`;
* `checkSoundTT` — the mutual fuel induction, with its **fuel-zero**
  case proved outright and its step named (`Lech/TTVerify/Claims.lean`);
...
* `denote_mono` — **denotations survive environment extension**
  (`Lech/TTVerify/Extend.lean`).  This is the workhorse every install
...
* `CheckStepTT` — the `succ` case of the fuel induction, the
  clause-by-clause transpose of `Lech/Model/Core/*`.
...
**`CheckStepTT` is proved** (`checkStepTT`, `Lech/TTVerify/MajorStep.lean`),
...
The dispatch is proved (`checkDeclTT_of`, `Lech/TTVerify/DeclStep.lean`):
...
(`Lech/TTVerify/Inst.lean`), the transpose of `interp_erasedEq`.
...
**`NatOpPinTT` is discharged** (`Lech/TTVerify/NatOpPin.lean`).  Its
...
**`DivModPinTT` is discharged** (`Lech/TTVerify/DivModPin.lean`), and
...
`Lech/TTVerify/HasTypeSubst.lean` says the substitution stack is
for.
...
**`DeclAxiomTT`'s guard chain is proved** (`Lech/TTVerify/DeclAxiom.lean`),
...
* **`PropextKeyTT` is discharged, and §8.4's prediction is
  ...
  `Lech/TTVerify/StdAxiomKey.lean` never mentions `rec_rules`.
...
* **`OfReduceKeyTT` is discharged** (`Lech/TTVerify/OfReduceKey.lean`),
...
* **`propext` and `Classical.choice` need no inhabitation argument.**
  ...
  (`Lech/TT/Const.lean`, `Lech/TT/Semantics/ConstOk.lean`), so
...
**`ReducePinTT` is discharged** (`Lech/TTVerify/ReducePin.lean`), and
...
Two facts in `Lech/TTVerify/Claims.lean` are properties of the
...
* **The `DeqC` middle layer: REJECTED.**  A conversion relation sitting
  ...
  core-generic-over-env value (`Lech/Model/DESIGN.md`, "no basis
...
1. **The set model is the consistency proof of the real checker in its
   *default* configuration** — the shipped binary with direct install,
   `LECH_INFER_ONLY` **off**.  Nothing in task #119 weakens, replaces
...
2. **The TT bridge covers the *certified* configuration** —
   certificates on (`LECH_INFER_ONLY` off) and
   ...
   **There are now three conditionalities in this project, not two**,
   ...
   `LECH_INFER_ONLY` is a third, and it is the one neither
...
3. **The layer's unconditional returns** are not conditional on
   anything: `Lech/TT/*`'s own absolute consistency, its two-lemma
...
(`Lech/Kernel/Direct.lean`, and the top-level `DESIGN.md` section
...
denotes, as `Lech/Model/DirectTower.lean` already interprets it), or
...
* **Nat literals** (`Lech/TT/Nat/*`, task #119's other half).
  ...
  **LANDED — the correspondence between meta-level `Nat` and the
  ...
  (`Lech/TTVerify/WhnfCoreStep.lean`):
  ...
  *values*, so its transpose is a `Deq` in context `[natT, natT]`,
  while `Lech/TT/Nat/*` wants it closed at numerals.  The recipe:
  ...
  **LANDED, and stated for an arbitrary domain.**  `Deq.close2` and
  `Deq.close1` (`Lech/TTVerify/Weaken.lean`) are the recipe
  mechanized:
...
> (`Lech/TT/DESIGN.md` §2.4), so an equation carries no typing
...
`Lech/Kernel/CoreI.lean:1534` (the certificate at 1545–1547 and its
post-whnf twin at 1554–1556); the spec-side twin is the `.app` clause
of `inferBody`, `Lech/Kernel/Core.lean:1495`.  `--yolo` is a literal
alias for `LECH_NO_PROOF_CERTS`.
...
> infer-only mode: LECH_INFER_ONLY (task #134)", for the
> decomposition.
>
...
(`Lech/TTVerify/Certs.lean`) is the transpose of `certs_fit`, and it
builds a `TeleTyped` from a successful `iotaCerts` run.

`iotaCerts` (`Lech/Kernel/Core.lean`) walks a telescope one argument
...
lemma**: `certs_fit` (`Lech/Model/Core/Certs.lean`) builds `TeleFitI`
...
live in `Lech/Verify/*`, so they are importable from here.  The
...
The `.proj` clause of `Lech/TTVerify/WhnfCore.lean` stays unproved
until it lands.

Note carefully what D is **not**: it is not the threading withdrawn in
`Lech/TTVerify/Claims.lean`.  That withdrawal was about the
...
`Lech/Model/Core/Claims.lean`.  An earlier revision threaded a typing
...
4. **A global argument**: the declaration was checked once in checking
   ...
   **This is the official kernel's actual justification, and it is
   ...
   premise-free equations of `Lech/TT/DESIGN.md` §2.4 (see the
   retraction above).
...
lemmas instead of 123 (`Lech/TT/DESIGN.md` §6) and what makes
...
annotation (`Lech/TTVerify/Inversion.lean`).  The premise is not
...
`interp_substFvarAt` (`Lech/Model/Subst.lean`), and the `Expr`-side
machinery both lean on — `substFvarAt`, `shiftFrom`,
`fvarsBelow_instantiate1` and friends — is in
`Lech/Verify/{Shift,Subst}.lean`, which this hierarchy may import.

### Landed: the shift lemma (`Lech/TTVerify/Shift.lean`)
...
### Landed: instantiation commutes (`Lech/TTVerify/Inst.lean`)
...
`Lech/TTVerify/{SubstAlgebra,HasTypeSubst}.lean` (landed separately)
...
`Lech/TT/DESIGN.md` §6 makes this accounting for the *layer*:
...
* `CtxOk.open` (`Lech/TTVerify/Claims.lean`) — opening a binder
  extends the context correspondence, which every binder clause needs.
* `denote_beta_step` (`Lech/TTVerify/Inst.lean`) — the beta
...
`iotaRec_inv` (`Lech/Verify/InferLemmas.lean`) is **importable** and
...
1. **Assembly — done** (`rec_rules_fire`,
   `Lech/TTVerify/Iota.lean`).  `certs_typed` twice, once per
...
2. **`majorToCtor` soundness** — its prediction settled
   ...
   (`eta_rescue`, `Lech/TTVerify/Iota.lean`), which is the branch the
   ...
   counterpart (`Lech/Model/Core/MajorToCtor.lean`) is 550 lines.
...
`whnfCoreBody` (`Lech/Kernel/Core.lean`) has, beyond the leaf cases
(which are `Deq.refl`) and beta (done):

* **iota** — `iotaRec`, whose set-model counterpart
  `Lech/Model/Core/Iota.lean` is 1326 lines, plus `NestedFire` and
...
model spends ~7500 lines on `Lech/Model/Core/*` for what
...
(`Lech/TTVerify/Tele.lean`): the transpose of `TeleFitI`
(`Lech/Model/Interp.lean`) with membership replaced by `HasType`, one
...
Its transport (`CapsOk.cons`, `Lech/Model/Extend/Sibs.lean`) then runs
...
(`Lech/TTVerify/WhnfCoreStep.lean`), modulo two named obligations.
...
needed: `iotaRec_WScoped` (`Lech/Verify/Deep.lean`, made public for
...
That is not an oversight in `Lech/Verify/*`: the set model does not
use standalone lemmas either.  Its `iota_sound`
(`Lech/Model/Core/Iota.lean`) *returns* all four frame conditions
...
(`Lech/TTVerify/WhnfCoreStep.lean`), modulo one named obligation.
...
literal *is* a numeral, `Lech/TT/Nat/*`'s meta-induction lemmas apply
...
`Lech/TTVerify/NatOpsStep.lean` closes the chain for `Nat.add`, and
...
`Lech/TT/Nat/WfOps.lean` instead, with the same shape.
...
* **Deliberate forward provision** — a clause written for a consumer
  ...
  (`Lech/Verify/EnvGuards.lean`) is documented as existing "to keep
...
in `Lech/Model/*`.  If that holds to the end, the relocation task
...
`Lech/TT/Semantics/Soundness.lean`'s `letE` case uses only the third
...
that type's head* (`Lech/Kernel/Core.lean`, `annotateBody`).
...
(`Lech/TTVerify/Iota.lean`), needed **precisely** the freedom the
...
3. **Precedent** — it is how the layer already relates a former to the
   ...
   the `proj` former and left `BConst` (`Lech/TT/DESIGN.md` §3).
...
* `Eq.rec` — **nothing new needed**: `eqRec_derivable`
  (`Lech/TT/Examples.lean`) already exists, from the `congrEq` work.
...
(`Lech/TTVerify/DeclBasis.lean`) shows the reason that actually
...
The premise-free discipline in `Lech/TT/DESIGN.md` §2.4 governs the
...
Every `defeq` call in `Lech/Kernel/Core.lean`, by what the caller
...
carried for the denotation and never checked — `Lech/TT/Syntax.lean`).
...
the directive sanctions adding it and `Lech/TTVerify/Typable.lean` is
...
`Lech/TTVerify/Typable.lean`: the predicate and the inversion family
...
back.  The count-to-four alarm (`Lech/TT/DESIGN.md` §3.1) is
...
(`whnfCore_WScoped` and friends, all in `Lech/Verify/*`), and the
...
**`ReduceNatStepTT` is discharged** (`Lech/TTVerify/NatOpsStep.lean`),
...
set model spends `Lech/Model/Core/MajorToCtor.lean` (550 lines) plus
...
(`Lech/TTVerify/InferStep.lean`), modulo two clause-granularity
...
(`Lech/TT/Judgment.lean`) each carry three premises:
...
concluding: `Lech/TT/Semantics/Soundness.lean`'s `projFst` case
...
`IotaStepTT` is proved (`Lech/TTVerify/IotaStep.lean`), modulo the
...
discharged by `pairEtaCert_stepTT` (`Lech/TTVerify/PairEtaStep.lean`).
...
They are not decoration.  `psigmaEta_law` (`Lech/TT/Semantics/Value.lean`)
...
`pairEta_sound` (`Lech/Model/Core/PairEta.lean`) gets the two
...
(`Lech/TTVerify/ProjStep.lean`, proved) turns a successful
...
> strongest case for relocation to `Lech/Verify/*`.

The bridge cannot import `Lech/Model/*` (both paths must stand
...
> `Lech/Verify/Extend/` holds `Decl`, `Ind`, `Inversions`, `Iota`,
> `Modeled`, `Proj`, `Recs`, `Sibs` and `Transport` — **3 934 lines,
> `V`-free** (two files mention `V` once each, in prose) — and the
> bridge already imports from it (`Lech/TTVerify/DeclValue.lean`
> imports `Lech.Verify.Extend.Inversions`).  `Lech/Model/Extend/`
> keeps the 5 591 lines that genuinely carry `V`.  So `EtaPins`
> (`Lech/Verify/Extend/Iota.lean:917`) and its siblings are
...
(`Lech/Model/Basis/Quot/Consistency.lean:23`, 479 lines) exists.  The
other five blocks' drivers are *open-coded inline* in
`Lech/Model/Consistency.lean:777–1841` — 1 057 lines of driver, of
...
* `hheadEta`'s second wants an eta capability whose constructor is the
  ...
  documented in `Lech/Verify/EnvGuards.lean` as existing "to keep the
...
> guards on the level relation**.  `Lech/Kernel/Core.lean:1281` fires
...
* **The inversion already had the fact.**  `iotaRec_inv`
  (`Lech/Verify/InferLemmas.lean:711`) exposes the `isEquivList`
  ...
  (`Lech/Model/Core/Iota.lean:455`).  The bridge had simply
...
model never needed it.**  `ruleLhsAux` (`Lech/Model/Interp.lean:319`)
...
* **The block's iota will be β, not a computation rule.**  `Eq.rec`'s
  ...
  *derivable* — `eqRec_derivable` in `Lech/TT/Examples.lean`,
  established in the layer's first week (`Lech/TT/DESIGN.md`, "Four
...
scattered.**  `Lech/TT/DESIGN.md` §2.2 makes universe levels ground
...
`Lech/TTVerify/DeclInd.lean`'s header states the fold headers'
...
1. **Relocate tier A** (or the ~1 500 lines of it the bridge needs) out
   of `Lech/Model/Extend/*`.  Blocking; see §14.1.
...
1. **Done** — `Lech/Verify/Extend/*`, 3 934 `V`-free lines; see the
   gate note in §14.1.
...
Gate procedure, mandatory for anything touching `Lech/TTVerify/*`:
...
re-deriving: every change was confined to `Lech/TTVerify/*`, which
...
touches `Lech/Kernel/*` or `Lech/Verify/*`, that argument lapses.
...
`denote_beta` (`Lech/TTVerify/Inst.lean:186`) to see that the
...
(`Lech/Verify/Extend/Iota.lean:917`), obtained via
...
* `RenEq` lives in `Model/TeleElim.lean` (blocked) but unfolds to
  `ErasedEq` from `Lech/Verify/Subst.lean` (usable) — the bridge
...
bites.**  `Lech/Kernel/Basis/Quot.lean`'s raw `quotBasis` says
...
already existed** in `Lech/TTVerify/Extend.lean` (`ValParams`,
...
(`Lech/TTVerify/Inversion.lean`).  A "stored types are types"
...
`structEtaCertWithI` (`Lech/Kernel/CoreI.lean:1063`) runs
...
```
-- Lech/Kernel/CoreI.lean, majorToCtorI's eta branch (~1246)
...
inverts it**: `Lech/TTVerify/MajorStep.lean`'s `fab_reduct` takes
...
(`Lech/Kernel/Checker.lean:125`, `cbody == directFam …`, plus the
...
**The reading.**  `checkMemberVal` (`Lech/Kernel/Modeled.lean:361`)
...
(`Lech/Kernel/Checker.lean:125`) requires the constructor's residual
...
`Lech/TTVerify/DeclInd.lean` now carries
...
(`Lech/TTVerify/StructEtaCertStep.lean`) carries `hbT : HasType Δ vb W`
...
`iotaRec` (`Lech/Kernel/Core.lean`, ~1299), after both `iotaCerts`
...
* **A `RecRulesTT` re-signing** — one premise, in the fired form's
  ...
  (`Lech/TTVerify/IotaStep.lean`) must supply it, and the guard above
...
* `iotaRec_inv` (`Lech/Verify/InferLemmas.lean` ~726) already returns
...
`Lech/Model/*` and `Lech/Verify/*`.  Same value, read back from the
...
(`Lech/Verify/Extend/Iota.lean:34`) — already `V`-free and already in
...
(`Lech/Verify/Extend/Iota.lean:34`) is already `V`-free and already
...
* `Lech/TTVerify/Tele.lean` — `TeleTyped.{toV,appN,spine,rest_eq}`,
...
* `Lech/TTVerify/Rename.lean` — `denote_renameConsts`, `RenEqT.*`,
  `PiDomsRenEqT.*`;
* `Lech/Verify/` — `defEqList_inv` (**returns length *and*
...
(`Lech/TTVerify/IndBottomPlain.lean`, sorry-free, master `ea9fcea`).
...
(`Lech/Model/Extend/Recs.lean`) is what supplies it.  The bridge
...
accessor, `Lech/Kernel/CheckerBase.lean`).  `PlainChecked`,
`NestedChecked` (`Lech/Verify/Extend/Iota.lean`) and
`checkProjIota_inv` (`Lech/Verify/Extend/Proj.lean`) carry
...
**The ask.**  `checkIotaSidesTy` (`Lech/Kernel/Modeled.lean`) — which
...
The carrier `IotaSlotSorted` (`Lech/TTVerify/IndBottom.lean`) is that
...
`Lech/TTVerify/IndBottomNested.lean` (untracked WIP, header-marked,
...
§19.1's prescription, first half.  `Lech/TTVerify/IndBottomStages.lean`
...
* **Fit**: build the mixed fit exactly as the plain Stage E does
  ...
  (`instSeq_erasedEq_args`, `Lech/Verify/Subst.lean`).
...
* **Reduct** (the one genuinely new piece): the rule's rhs is the
  ...
  from `Lech/Verify/Subst.lean`) — sketched, not landed.
...
(`Lech/Model/Extend/`), with what the TT side already has:
...
The phase's final theorem is landed.  `Lech/TTVerify/Main.lean`:
...
(`Lech/TTVerify/DeclFamilies.lean`), proved with no valuations or
