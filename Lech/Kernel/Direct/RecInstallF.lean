...
def directFixFieldsOkF (fe₀ : FEnv) (T : Name) (lps : List Name) (nP : Nat)
...
def checkDirectFixRulesF (feR : FEnv) (rlps : List Name) (T : Name) (lps : List Name)
  ...
  | k + 1, j => do
    ...
    let rest ← checkDirectFixRulesF feR rlps T lps elim large nP tty ctors recC rlvls k (j + 1)
    pure (rhs :: rest)
...
def checkDirectFixRecF (ops : CheckerOps m) (fe : FEnv) (p : DirectFixParts)
  ...
  let rhss ← checkDirectFixRulesF feR p.cvR.levelParams T lps p.elim p.large p.nP cvTa.type
