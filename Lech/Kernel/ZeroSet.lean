...
annotation datum that `Lech.PropWhen` (`Kernel/Expr.lean`) holds in
...
lemmas in `Lech/Verify/ZeroSet.lean`) lets the swap be staged.

Layering: this file is implementation tier (it imports only
`Lech.Kernel.Level`) and carries exactly the invariant-preservation
proofs the definitions need; the law battery lives in
`Lech/Verify/ZeroSet.lean`, which states it against the *same*
...
There is no order on `Lech.Name` in the tree (the `NNode` arena keys
...
`ZPropWhen` mirrors `Lech.PropWhen` constructor for constructor; the
