...
def main : IO Unit := do
  ...
  for k in blocks do
    for ci in k.decls do
      ...
      match annotateCore .setModelP env checkFuel 0 cv.type with
      | .error e =>
      ...
      | .ok ty' =>
        ...
        let annotateRules (rules : List RecRule) (self : ConstantInfo) :
          ...
          for r in rules do
            match annotateCore .setModelP env' checkFuel 0 r.rhs with
  ...
  for ci in iffFamily ++ nonemptyFamily do
    let cv := ci.toConstantVal
    match annotateCore .setModelP envS checkFuel 0 cv.type with
    | .error e =>
      IO.println s!"ERROR annotating {cv.name}: {e}"
      return
    | .ok ty' =>
  ...
  for cv in [reduceOpRaw reduceNatName, reduceOpRaw reduceBoolName] do
    match annotateCore .setModelP envT checkFuel 0 cv.type with
    | .error e =>
      IO.println s!"ERROR annotating {cv.name}: {e}"
      return
    | .ok ty' =>
